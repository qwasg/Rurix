//! RenderGraph 帧调度骨架声明(G6.3 uc08;照 uc06 十五 pass 一字不改——物理
//! 不加 pass,MV 供给是 host 数据喂入(physics → GpuScene → 各 pass 同源消费);
//! RFC-0016 章 A + RFC-0017 §4.B)——全帧管线以声明式 pass 描述,**帧内零手写
//! 屏障**:barrier/fence/transient 别名全部由 `graph_compile` 四趟产出;AO/GI
//! 滤波 pass 标 `QueueClass::AsyncCompute`;历史颜色/深度、VSM 页表/物理池、
//! GI 探针历史经 `import`(跨帧外部资源纪律 §4.0-3)。
//!
...
pub fn build_frame_graph(
    ...
    let compiled = g
        ...
        .expect("uc08 帧图必须编译通过(声明完整性由图编译器兜底)");

    // 结构性锚定(失败 = 声明面回归,panic 早于静默):
