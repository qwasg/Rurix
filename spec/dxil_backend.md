...
### 2.1 图形=B 待落条款(RFC-0004,计划映射)

> **RXS-0159 / 0161 / 0162 已落条款体(下文 §2)**;本小节仅登记**尚未落地**的 **RXS-0160** 计划映射(非裸条款头,零 `### RXS-####` 三级标题,trace_matrix 不计未锚条款)。承 [RFC-0004](../rfcs/0004-spirv-dxil-graphics-backend.md) §5 下游条款计划表;条款体随其实现接缝就绪后的实现 PR 同落(条款 PR 先于实现 PR,硬规则 7)。feature gate 复用 `dxil-backend`(Q-Gate-B);错误码归 **6xxx 段**(只追加)。

- **RXS-0160(待落,按 B)— 阶段间接口 → 阶段链接一致性核对**:RXS-0155 阶段间接口契约(vertex out ↔ fragment in)经语义名 / 类型 / 插值匹配核对,错链即显式 6xxx(RFC-0004 §4.4 校验门,映射 RX6011)。**未落地原因**:链接核对需 vertex+fragment **多阶段联编点**,本片 [`dxil_codegen`](../src/rurixc/src/dxil_codegen.rs) 为单着色阶段编译,尚无该接缝;[`signature_gate`](../src/rurixc/src/dxil_sig_gate.rs) 已具语义名等价 / 系统值匹配基件,联编点就绪后落 `check_stage_link` + 条款体 + 测试锚定(承后续里程碑)。测试锚定计划:dxil-sig accept(链接一致)+ reject(错链 → RX6011)+ golden。
...
- IR3(SM 下限登记):上表 shader model 下限随阶段登记(光栅/compute SM6.0、mesh/amp SM6.5、RT SM6.3);本片落地阶段(SM6.0)的着色器类型映射经各自后端实测(compute=A 路;vertex/fragment=B 路 dxc,见 RXS-0161/0162),SM6.5/6.3 阶段(mesh/task/RT)为 deferred 阶段的映射登记值,无 passing 测试(RD-012);DXIL golden 经 dxc validator 验证、bless 归 owner pin 环境。
...
- IR1(系统值 / 语义名映射):`#[builtin]` → DXIL 系统值(`position`→SV_Position、`vertex_index`→SV_VertexID、`instance_index`→SV_InstanceID、`frag_coord`→SV_Position、`frag_depth`/`depth`→SV_Depth 等,按阶段 + 方向);用户命名 varying / interpolate → **by-construction `UserSemantic` 保名**(`POSITION` / `NORMAL` 不退化为通用 `TEXCOORD#`,RFC-0004 §4.4 机制①)。映射在 [`dxil_spirv`](../src/rurixc/src/dxil_spirv.rs) emit。
...
- IR5(golden 形态):仅 **DXIL 文本反汇编**入 golden(Q-Golden-B);本机产物标 `NOT BLESSED (local)`,由 owner 在 pin 环境重 bless(硬规则 2/3,device 真跑 / run URL / evidence 归 owner)。
