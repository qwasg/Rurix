...
| v1.4 | 2026-06-25 | **PR-C2 分片3 RXS-0159 签名映射真达产物补证(仅追加修订记录,RXS-0159 条款体/语义 0-byte 不变)**:分片3 落地时 dxc validation 因 dev 环境缺 patched llc 而 SKIP,本轮在 RD-011 patched llc 环境(RURIX_LLC,LLVM 23.0.0git assertions,dxil target;dxc/dxv 1.9.2602.24)补齐真验证。**(B)真发现**:vs_io/ps_io 既有 `.dxil-ll`(含 `!rurix.dxil.sig.*`)经 patched llc `-filetype=obj` 产 DXContainer,IDxcValidator ×25 = 25/25 accept(`{0x0:25}`)+ dxv.exe ×20 = 20/20 `Validation succeeded.`,**但产物 ISG1/OSG1 签名 part `elemcount=0`(无 SV_Position/SV_Target/SV_VertexID)**——validator accept 仅因 `void` 入口空签名结构合法,**非** SV 映射真达。根因:LLVM DirectX 后端 `addSignature()`(DXContainerGlobals.cpp)对图形着色器**无条件写空签名**(`// FIXME: support graphics shader`,上游 issue #90504),`!rurix.dxil.sig.*` 自有元数据被忽略;且 `Signature::addParam` 填元素强制要 Register/Mask/ExclusiveMask = 二进制 ABI 布局,经检索后端无图形签名填充路径——**任何元数据形态(rurix.*/hlsl.*/标准 dx.entryPoints)当前均无 lower 路径**,『改 lowering 为后端可消费形态』对当前后端不成立。让 SV 真达产物唯一途径 = patch 后端填 Register/Mask 二进制布局 = RFC-0003 §4.6/§9 Q-Builtin 🔒 FFI ABI 禁区,**越出 RXS-0159 类型面** → 硬规则 5 **停手标需人工升档**(需 owner 独立 Full RFC〔承 RD-013 backfill 已声明的签名元素二进制布局由 owner 后续 Full RFC 定义〕+ 上游 #90504 图形签名支持)。处置:**不录 `.dxil-disasm` golden**(签名空,录入即伪造)、**不 patch LLVM 后端**(禁区)、**不改 RXS-0159 语义/不动 PTX/committed D-205 pin**,IR2 类型面签名元数据描述如实(元数据确 emit 入 IR,IR3 已声明 disasm/validator 为带工具链环境关卡)。RD-011 history 追加补证留痕、bless_log 追加(disasm 关卡由 SKIP 收紧为 measured『accept 但签名空』)、新增 evidence/dxil_slice3_rxs0159_sig_disasm_round8.md(签名 part dump + accept ×N + 根因 + patched llc digest)。`ci/trace_matrix.py --check` 维持 **159/159**(无新增条款/无悬空)。档位 **Full RFC**(RFC-0003),无体例变更 | **Full RFC**（RFC-0003） |
