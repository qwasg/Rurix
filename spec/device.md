...
> **M5.2 范围裁决(shared+barrier 一致性数据流 + scoped atomics 类型契约)**:RXS-0079 把 M4.1 的 barrier uniform 可达保守骨架(RXS-0068,HIR 层)**完整化为 `shared let` 读写与 `block.sync()` barrier 的一致性数据流判定**(06 §2.2 barrier 可达性 D-123;05 §5 `shared` addrspace 3);RXS-0080 条款化 **scoped atomics `Atomic<T, Scope>` 类型契约 + PTX `atom.{order}.{scope}` 映射层**(06 §4 同步三层 D-123)。本批为已锁定决策(D-123 / 07 §4 保守先行)的条款化(档位 Direct);判定取**保守上界**——能证经 barrier 同步才放行,证不出保守拒绝(可经 `unsafe` 逃生),避免假阴性(漏报竞争)。**D-406 禁区**:scoped atomics 的 PTX `atom.{order}.{scope}` 内存序/作用域映射语义由自主落笔(AI 仅条款化语义 + 挂测试,不擅自实现映射);unsafe 弱序协议(`fence`/volatile/自定义自旋)及其竞争 UB 边界**不在本批**(UB 节仅人类经 Full RFC 落笔,[README.md](README.md) §2 / 10 §7.5),本批只覆盖 safe scoped atomics 类型契约。错误码 `RX3009`~`RX3010` 为 3xxx 着色/地址空间段位续接(接 M5.1 的 `RX3008`),**spec 先行引用,正式分配于 M5.2 实现 WP**(沿用 3xxx/4xxx/5xxx/6xxx 在实现 PR 落 registry 的节奏,registry revision_log 留痕,编号不复用)。libdevice 链接 / gpu 并行基元 / 完整弱序协议条款随 M5.3+(07 §4 / 06 §7),本批不覆盖。

### RXS-0079 shared let 读写与 barrier 一致性(数据流判定)

**Syntax**(`shared` 块作用域存储,05 §5 / RXS-0067·RXS-0071 已建模 addrspace 3):

```
SharedDecl ::= "shared" "let" Ident (":" Type)? ";"   // block 作用域,addrspace 3
BarrierCall ::= "block" "." "sync" "(" ")"            // block 级 barrier(RXS-0072 codegen 为 llvm.nvvm.barrier0)
```

`shared let` 声明的存储为 block 内全部线程共享(addrspace 3,RXS-0071);其读写经 view 收窄(M5 device 借用扩展)或下标访问;`block.sync()`(block barrier)是 block 内全部线程的同步点。

**Legality**(shared+barrier 一致性,MIR 借用检查 device 扩展层数据流判定,07 §4——完整化 RXS-0068 的 HIR 保守骨架):

- **一致性规则**:对同一 `shared` 位置,某 lane 的写入对**其它 lane** 的读取可见**当且仅当**该写入之后、该读取之前在两 lane 的控制流上存在一次 block 级 barrier(`block.sync()`)同步;否则构成 shared 数据竞争(读到陈旧/未定义的他 lane 写入)。
- **写后未同步读拒绝**:某 lane 写 `shared` 位置后,未经 `block.sync()` 即读取**他 lane 写入**(或对该位置发起跨 lane 的读写交错)→ 一致性违例 `RX3009`。**保守上界(07 §4)**:能在数据流上证明写读之间隔有 barrier 才放行;证不出(barrier 可达性不确定 / 跨 lane 别名不可判)保守拒绝(误拒边界情形,措辞容许粗糙,可经 `unsafe` 逃生)。
- **barrier uniform 可达延续**:`block.sync()` 调用点本身的 block-uniform 可达性延续 RXS-0068(thread-id 依赖分支内调 barrier → `RX3003`);本条款在其之上加数据流层的"写读跨 barrier 同步"判定。
- **本 lane 自读自写**:同一 lane 对 `shared` 位置的写后自读无须 barrier(数据流内即可见),不触发 `RX3009`。

**诊断要求**(3xxx 着色/地址空间段位续接,接 M5.1 的 `RX3008`):

- **shared+barrier 一致性违例** `RX3009`:`shared` 位置写后未过 `block.sync()` 即读他 lane 写入,或证不出经 barrier 同步的保守拒绝(措辞标注"无法证明 shared 写读经 barrier 同步")。

**Implementation Requirements**:

- shared+barrier 一致性作为 **MIR 借用检查的 device 扩展 pass** 的数据流分析(07 §4),与 views 不相交证明(RXS-0078)同层,在 host 借用检查之后、消费 RXS-0066 着色 / RXS-0067 地址空间(`shared`→addrspace 3)边界信息之上运行;barrier 的 block-uniform 可达延续 RXS-0068。
- `block.sync()` 为编译器已知 device 方法(resolve/typeck 兜底识别,用户同名定义优先遮蔽,沿用 RXS-0067 `View` 族兜底纪律);`shared.*` message-key 随 `RX3009` 在 M5.2 实现 WP 落 registry(只追加)。
- host 回归网(hello-world 冒烟 + MIR golden + SAXPY 回归)持续绿(本扩展不退化 host 借用检查)。

> 锚定测试:`conformance/shared/accept/*.rx`(正例 0 诊断:`shared` 写 → `block.sync()` → 读他 lane 写入的合法模式);reject 语料(写后未同步读)+ `tests/ui/shared/`(`RX3009` 黄金路径 5 子集 snapshot)随 M5.2 实现 WP;shared+barrier 数据流单测。

### RXS-0080 scoped atomics 类型契约与 PTX 映射(D-406 映射自主落笔)

**Syntax**(scoped 原子,06 §4.1 D-123 同步三层之 safe 受限层):

```
AtomicTy   ::= "Atomic" "<" Type "," Scope ">"
AtomicView ::= "AtomicView" "<" AddrMark "," Type ("," Shape)? ">"
Scope      ::= "Scope" "::" ("Block" | "Gpu" | "System")
Order      ::= "Ordering" "::" ("Relaxed" | "Acquire" | "Release" | "AcqRel")
AtomicOp   ::= Receiver "." ("fetch_add" | "fetch_max" | "fetch_min" | "fetch_and" | "fetch_or" | "exchange" | "compare_exchange") "(" Args ")"
```

`Atomic<T, Scope>`(原子标量)与 `AtomicView<space, T, Shape>`(原子视图,元素为 `Atomic<T, _>`)提供原子读改写方法;`Scope ∈ {Block, Gpu, System}`、`order ∈ {Relaxed, Acquire, Release, AcqRel}`(对齐 06 §4.1;形态与既有 `conformance/syntax/atomics_sync.rx`、`conformance/syntax/kernel_atomics_scoped.rx` 一致)。`Atomic`/`AtomicView`/`Scope`/`Ordering` 为编译器已知类型(resolve/typeck 兜底识别,用户同名定义优先遮蔽,沿用 RXS-0067 `View` 族兜底纪律)。

**Legality**(scoped atomics 类型契约,编译期裁决):

- **scope/order 枚举契约**:`Scope` 与 `order` 为编译器已知封闭枚举;原子方法的 `Scope`/`order` 实参必须为合法枚举值且与方法语义相容(如 `compare_exchange` 须给定 success/failure order)。
- **类型一致(避开 mixed-size)**:safe 层对同一原子位置的访问类型一致(由类型系统保证,避开 PTX 公理未覆盖的 mixed-size 冲突,06 §4.2);`AtomicView` 元素类型 `T` 与原子方法操作数类型须合一。
- **scope 误用拒绝**:scoped atomics 的 `Scope` 误用(越权使用未授予的作用域 / 与目标地址空间不相容的 scope/不支持的 scope·order 组合)→ `RX3010`(保守拒绝,措辞容许粗糙,07 §4)。

**Dynamic Semantics**(scope/order → PTX 映射,06 §4.2 映射层规范;**非 UB 节**):

- `Atomic<T, Scope>` 的原子操作下译为满足 **morally strong** 条件的 PTX `atom.{order}.{scope}` 指令(同 proxy、scope 双向包含、完全重叠——r5 / PTX ISA 公理前提),从而获得原子性与 SC-per-location。
- safe 层全部走 generic proxy(tex/generic proxy 差异 MVP 不暴露,纹理路径 G2 再扩展)。
- `System` scope 原子在 WDDM 下的跨设备/跨进程一致性按官方文档**保守声明**,不做承诺(登记 G1+ 验证项)。
- **映射实现禁区(D-406)**:`atom.{order}.{scope}` 的内存序/作用域映射**语义实现由自主落笔**;本条款只规范类型契约与映射目标语义,AI 不实现 PTX 映射代码。

**诊断要求**(3xxx 着色/地址空间段位续接,接 RXS-0079 的 `RX3009`):

- **scoped atomics scope 误用** `RX3010`:scoped 原子操作的 `Scope`(或 scope·order 组合)误用——越权作用域 / 与地址空间不相容 / 不支持的组合。

**Implementation Requirements**:

- scoped atomics 类型契约检查在 typeck / HIR 层裁决(`Scope`/`order` 枚举合法性 + 类型一致),不依赖数据流;`atomics.*` message-key 随 `RX3010` 在 M5.2 实现 WP 落 registry(只追加)。
- **PTX `atom.{order}.{scope}` 映射层为 D-406 禁区,由自主落笔**(本条款先行,AI 仅条款化 + 挂测试骨架);映射 PR 必须引用本条款号 + 经 Compute Sanitizer racecheck 运行期背书(M5 契约 G-M5-4)。
- Err 容忍不级联:参与类型为容忍区 `Err`(RXS-0047)或 scope/order 不可判定时不触发 `RX3010`(防一错多报,与 RXS-0069/0075 同口径)。

> 锚定测试:`conformance/atomics/accept/*.rx`(正例 0 诊断:合法 `Scope`/`order` 的原子操作);reject 语料(scope 误用)+ `tests/ui/atomics/`(`RX3010` 黄金路径 5 子集 snapshot)+ PTX `atom` 映射真跑随 M5.2 实现 WP(映射自主落笔);scoped atomics 类型契约单测。
