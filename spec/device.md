...
### RXS-0080 scoped atomics 类型契约与 PTX 映射(D-406 映射人工落笔)
...
- **映射实现禁区(D-406)**:`atom.{order}.{scope}` 的内存序/作用域映射**语义实现由人工落笔**;本条款只规范类型契约与映射目标语义,AI 不实现 PTX 映射代码。
...
- **PTX `atom.{order}.{scope}` 映射层为 D-406 禁区,由人工落笔**(本条款先行,AI 仅条款化 + 挂测试骨架);映射 PR 必须引用本条款号 + 经 Compute Sanitizer racecheck 运行期背书(M5 契约 G-M5-4)。
- Err 容忍不级联:参与类型为容忍区 `Err`(RXS-0047)或 scope/order 不可判定时不触发 `RX3010`(防一错多报,与 RXS-0069/0075 同口径)。

> 锚定测试:`conformance/atomics/accept/*.rx`(正例 0 诊断:合法 `Scope`/`order` 的原子操作);reject 语料(scope 误用)+ `tests/ui/atomics/`(`RX3010` 黄金路径 5 子集 snapshot)+ PTX `atom` 映射真跑随 M5.2 实现 WP(映射人工落笔);scoped atomics 类型契约单测。
