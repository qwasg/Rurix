...
- **CRT + 跨堆所有权红线**：`--emit=dll` 用 libcmt（静态）；subset v1 **无任何堆/资源所有权跨 ABI 边界**（调用方
  ...
  评审 F2）。
- **GPU-using 导出面链接线**（EI1.4 落地）：导出根可达 MIR 含 `rxrt_*` 调用时（= 导出体使用 gpu 宿主 API，判定表达式
  与 EXE 段**同一份**，driver.rs `uses_gpu`），`--emit=dll` 链接段追加 `rurix_rt_cabi.lib`（crt-static 构建，与本段
  基础集 libcmt 系一致）+ Rust staticlib 系统库固定集（`ntdll` / `userenv` / `ws2_32` / `dbghelp`），与 EXE 段
  **逐参数同源**；定位/构建失败 → **RX7021**（§RXS-0195 既有码复用，零新码）。device 产物（PTX + 可选预编 cubin）
  的嵌入由上游 `build_gpu_artifacts` 于 codegen 前完成，EXE / DLL **两路共用同一段**，无第二事实源。缺此段则
  GPU 导出面在 link 期报 `rxrt_*` 未解析外部符号——EI1.2 首落时按「subset v1 无 GPU」实现，EI1.4 嵌入面据实扩展。
...
| v1.1 | 2026-07-20 | EI1.4 引擎嵌入落地：**`--emit=dll` GPU-using 导出面链接线扩展**（§RXS-0252 Implementation Requirements 新增一条）。EI1.2 首落时 `emit_dll` 按「subset v1 无 GPU」实现——链接段只有 CRT 基础集 + `/EXPORT:` 序列，导出体一旦使用 gpu 宿主 API（`Context::create` / RHI 图 / kernel 派发）即在 link 期报 `rxrt_*` 未解析外部符号。本次据实扩展：driver 把 EXE 段既有的 `uses_gpu` 判定（导出根可达 MIR 含 `rxrt_*` 调用，**同一份表达式**）透传进 `emit_dll`，为真时追加 `rurix_rt_cabi.lib`（crt-static，与基础集 libcmt 系一致）+ Rust staticlib 系统库固定集（ntdll / userenv / ws2_32 / dbghelp），与 EXE 段逐参数同源；定位/构建失败复用 **RX7021**（§RXS-0195），**零新 RX 码**。device 产物（PTX + 可选预编 cubin）嵌入由上游 `build_gpu_artifacts` 于 codegen 前完成，EXE / DLL 两路本已共用同一段，本次不动。条款文本外零语义变更（`/EXPORT:` 发射、内建头生成、空导出集 RX6032、subset v1 签名与 panic 面判据全部 0-byte）。见证：`apps/uc05-rhi/src/embed.rx`（全 .rx 导出面）→ `rurix_rhi.dll` + import lib + 生成头 → C++/D3D12 宿主 engine_host v2 真跑（步骤 74 `ci/uc05_engine_embed_smoke.py`，三方数值对照）。 | **Full RFC**（RFC-0014 / §4.A / EI1.4） |
