...
- L2(不可映射,strict-only):unbounded / bindless descriptor array([`ResourceCount::Unbounded`])→ **RD-018** defer → **RX6013** `codegen.dxil_unmappable`(复用既有不可映射码,owner 已裁不新开;G2.3 PR-E2b-2 落码 + emit 接线);非建模资源种类同理。无 fallback,不发明 descriptor heap 编码。
...
- **错误码策略**:绑定布局推导失败归 6xxx codegen 段,按实现时 registry 实际最高空号续;**PR-E2a 不预留、不预造、不落码**(条款体占位「6xxx」)。**G2.3 PR-E2b-2 已落码**(owner 裁定方案 B):不可映射资源**复用 RX6013** `codegen.dxil_unmappable`(bindless/unbounded RD-018,owner 已裁不新开);register/layout 冲突 → **RX6015** `codegen.dxil_register_conflict`、超 64 DWORD → **RX6016** `codegen.dxil_root_signature_too_large`、PSV0 mismatch → **RX6017** `codegen.dxil_psv0_mismatch`(新真实可达类别,自实际最高空号 RX6013 后顺延 RX6015/6016/6017——**RX6014 经 owner 方案 B 归 RXS-0160 阶段间接口错链**,绑定码避让顺延)。emit 接线在 [`dxil_codegen`](../src/rurixc/src/dxil_codegen.rs) 边界(`DxilBError::Binding` → `emit_b_error` 按变体分派);golden/bless + device 真跑仍归 PR-E2b owner 闸门(G-G2-3)。
...
| v1.2 | 2026-06-28 | **PR-E2b-2:绑定布局推导失败占位「6xxx」换真实专属错误码 + 生产 emit 接线**（owner 裁定 **方案 B**:RXS-0160 阶段间接口错链占 **RX6014**，绑定布局码顺延）。`registry/error_codes.json` append-only 追加 **RX6015**（`codegen.dxil_register_conflict`，register/layout 冲突，RXS-0164 L2）/ **RX6016**（`codegen.dxil_root_signature_too_large`，超 64 DWORD，RXS-0165 L2）/ **RX6017**（`codegen.dxil_psv0_mismatch`，PSV0 反射失配，RXS-0166 L2）+ en/zh message-key 齐全（`ci/bilingual_coverage.py` PASS）；**不可映射 / bindless·unbounded（RD-018）复用既有 RX6013**（owner 已裁不新开，既有 RX6001~6013 字段 0-byte 不动）。§2 reject 条款体（RXS-0163~0166 Legality L2/L3 + IR3/IR4 emit 注记）占位「6xxx」换实码；emit 接线落 `dxil_codegen` 边界（`serialize_root_signature` → `DxilBError::Binding(BindingInferError)` → `emit_b_error` 按变体分派 RX6013/6015/6016/6017，替换 E2b-1 经 RX6013 一律透传的 interim），`binding_layout.rs` `BindingInferError` 变体 doc 同步实码（host 推导仍纯 safe、不直接发码）。reject 真实红绿:`binding_layout` host 单测断言失败变体 + `dxil_codegen::emit_b_error_binding_routes_to_dedicated_codes` 断言 emit 实码。**🔒 禁区不变**:错误码 message 只描述失败类别，不落 register/space/packing 物理布局值（06 §4.2 / RFC-0004 §4.6(a)）；golden/bless + device 真跑/run URL 仍归 PR-E2b owner 闸门（G-G2-3）。deferred.json / spike_gating.json 不动，bindless 维持 RD-018 deferred；`ci/trace_matrix.py --check` 全锚定维持（无新增/删除 RXS）。 | **Full RFC**（RFC-0005 / PR-E2b-2） |
