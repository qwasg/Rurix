...
| v1.4 | 2026-07-19 | **RXS-0168/RXS-0169 加性修订行(G3.5 render graph D3D12 执行器 + D6 手动复核门引用,零改条款体):** [render_graph.md](render_graph.md) **`### RXS-0240`**(双后端执行器,RFC-0013 §4.D5a)的 D3D12 执行器经 uc04 shim 消费 render_graph.md RXS-0238 `graph.rs` 推导产物——pass 数组 + 逐边界 barrier 数组经 shim ABI **数值透传下发**(枚举数值即 D3D12 原生常量,C++ 侧零状态映射逻辑防第二事实源),执行 = 逐 pass set RT/DSV → draw → 重放边界 barrier 数组,与步骤 48 offscreen 路径同判据(G-G3-5);shim ABI 增量走**新增独立入口**(pass/barrier 数组下发,聚合版顺延,§6.3),既有 `rx_uc04_offscreen_run`/`rx_uc04_present_run` 入口字节 0-byte(步骤 48 0-byte 守卫)。**D6 手动复核门**(render_graph.md RXS-0241 / §4.D6):uc04 `### RXS-0169` 资源状态+barrier 编排锚点(`barrier::plan_barriers` / RX6021)**永续保留、条款 0-byte 不动**——转为 `graph.rs` 纯 host 自动推导产物的**独立复核 oracle**(`graph.rs` 禁 import barrier.rs;deferred 三 pass 图两实现 barrier 集合相等双向互证,恒跑纯 host 单测 `src/uc04-demo/tests/d6_crosscheck.rs`)。**RXS-0168/RXS-0169 条款体字节 0-byte**;零新条款号 / 零新 RX 码 / 零新 RD / 零新 RFC。D3D12 shim C++ 改动大时诚实标注留后续(Vulkan run_graph + host 推导 + D6 互证为本面核心,device 段真跑归主循环活驱动)。档位 **Full RFC**(RFC-0013 §4.D / render_graph.md RXS-0240/0241)。无体例变更 | **Full RFC**（RFC-0013 / §4.D） |
