...
- L4(present defer):swapchain / 窗口 present(平台 surface `VK_KHR_win32_surface`/`VK_KHR_android_surface` + `VK_KHR_swapchain` + `vkAcquireNextImageKHR`/`vkQueuePresentKHR` + semaphore/fence)→ honest-defer **RD-032**(不进必要条款,尾门 DoD:真窗口目视 present + win32/android surface)。窗口/present 无法 headless 数值校验、引入平台 surface 分叉,offscreen-first 对齐 uc04 先例。
...
- L1(OS 加载缝,per-OS 唯一确定):`cfg(windows)` → 库名 `vulkan-1.dll` + 加载原语 `LoadLibraryA`/`GetProcAddress`(Win32 kernel32);`cfg(not(windows))` → 库名 `libvulkan.so` + `dlopen(RTLD_NOW)`/`dlsym`(POSIX;Android/Linux libc 直接提供)。库名与加载原语由 OS 唯一确定;缺库/缺 `vkGetInstanceProcAddr` → 确定性 `Err` 非 panic(承 RXS-0207 L2,P-01)。
...
- IR1(cfg-gated loader,Windows 零漂移):`mod loader` cfg 分叉,`cfg(windows)` 分支逐调用等价现行 `load_vulkan_loader`(`open`=`LoadLibraryA` / `sym`=`GetProcAddress` / `VULKAN_LIB`=`vulkan-1.dll`),NVIDIA/桌面路径行为字节不变。
- IR2(交叉 build 绿):`cargo build -p rurix-rt --features vulkan --target aarch64-linux-android`(lib + `vk_saxpy` bin)链接无未定义符号。
