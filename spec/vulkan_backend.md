...
### RXS-0248 🔒 Vulkan mesh/RT 运行时执行语义（mesh pipeline + BLAS/TLAS + SBT + TraceRays，RFC-0013 §4.E7/E8）

#### Syntax

无用户面语法（运行时 FFI;消费 RXS-0246/0247 编码腿产物 SPIR-V + RXS-0245 RT 类型面契约）。

#### Legality

- **mesh 管线（E7）**:device 扩展 `VK_EXT_mesh_shader` + feature `VkPhysicalDeviceMeshShaderFeaturesEXT.meshShader`（task 时并 `taskShader`）探测——**缺失 → 确定性 `Err`**（非 panic、不占 RX 码、无 fallback,RXS-0210 L3 纪律）。管线 **无 vertex input / input assembly state**,stage 集 =（task?）+ mesh + fragment。
- **RT 运行时（E8,🔒）**:扩展集 `VK_KHR_acceleration_structure` + `VK_KHR_ray_tracing_pipeline` + `VK_KHR_deferred_host_operations` + `bufferDeviceAddress`（core 1.2 feature）——探测缺失 → 确定性 `Err`（[`negotiate_device_extensions`](../src/rurix-rt/src/vk.rs) 纯 host 协商 + feature bit 探测,全列缺失名）。
- **descriptor 布局**:沿 §4.B7 Vk-native set-per-class 形态——TLAS 归 SRV 轴、storage image 归 UAV 轴,分属各自类别 set（§4.0-4）。
- **validation fail-closed**:`VK_LAYER_KHRONOS_validation` ERROR 级校验消息经 messenger 翻 `Err`（承 RXS-0210,`RURIX_VK_VALIDATION=1`;VVL/驱动崩溃以退出码区分,反 grep）。
...
- **IR1**（mesh unsafe 折叠 U27）:mesh 管线 FFI 在 graphics 边界内,`// SAFETY:` 折叠进 U27 扩注（0 新 U 号,§6.4/E-2;沿 U26/U27 折叠约定）。
- **IR2**（🔒 AS/SBT/device-address 切 U30）:全期唯一新细审计 U 号——裸 GPU 指针算术 + SBT 对齐铺设;`#[repr(C)]` VkStruct（含 `AccelerationStructureInstanceKHR` bitfield 手工打包 / `AccelerationStructureGeometryKHR` union）逐字节对齐;句柄线性配对 create/destroy（AS handle 先于其 storage buffer 逆序销毁）。
- **IR3**（纯 host 单测）:SBT 三 region 对齐律不变式 + 扩展协商缺失确定性 `Err` + FFI 布局逐字节锚([`vk::tests`](../src/rurix-rt/src/vk.rs) `plan_sbt_*`/`align_up_*`/`negotiate_device_extensions_*`/`mesh_rt_ffi_layout_anchors`/`accel_instance_bitfield_packing`)。
- **IR4**（device 见证 harness,SKIP 三态）:[`bin/vk_mesh`](../src/rurix-rt/src/bin/vk_mesh.rs)（mesh 管线出图 covered 计数 + 篡改 SetMeshOutputs 顶点数 RED,步骤 66）+ [`bin/vk_rt`](../src/rurix-rt/src/bin/vk_rt.rs)（三件套 TLAS 命中/miss 双色 + 移动顶点命中区移动 RED,步骤 67）;无 Vulkan/无 GPU/feature 缺失 → SKIP dev-env degrade（非 fake pass;`RURIX_REQUIRE_REAL=1` 翻硬红）。**mesh/RT device 见证语料(coverage-producing mesh + 写 storage image 的 raygen)+ 像素判据阈值 = owner 本机 RTX 4070 Ti 迭代校准 TODO**;`g3.counter.mesh_task_rt_stages`（阶段去重基数 ≥3:mesh/raygen/closesthit）= evidence/meshrt_*.json 中 stages_ok 去重数（ci/budget_eval.py）。**AMD 真卡见证 = G-MB1-6 硬件尾门独立存续**（本机 RTX 4070 Ti measured 不充作 AMD）。
- **IR5**（NVIDIA/CUDA 零回归 + 既有面 0-byte）:gate feature `vulkan` 默认关闭;既有 vk.rs 全部入口（present / 采样 v2 / graph run_graph）**0-byte 纯加性**;codegen 腿 0-byte（运行时只消费 SPIR-V 产物）。

> 锚定测试:[`vk::tests`](../src/rurix-rt/src/vk.rs)（`plan_sbt_alignment_invariants` + `plan_sbt_nvidia_typical_exact` + `align_up_rounds_to_power_of_two` + `negotiate_device_extensions_missing_deterministic_err` + `mesh_rt_ffi_layout_anchors` + `accel_instance_bitfield_packing`）+ device harness [`bin/vk_mesh`](../src/rurix-rt/src/bin/vk_mesh.rs) / [`bin/vk_rt`](../src/rurix-rt/src/bin/vk_rt.rs)（步骤 66/67,`ci/meshrt_device_smoke.py`;SKIP 三态,device 真跑归 owner 本机活驱动）。

### RXS-0290 device 描述表 v2 blob 布局（artifacts v2：v1 前缀兼容 + SPIR-V 入口表，RFC-0015 §4.C1，RD-031 兑现）

#### Syntax

无用户面语法（host 产物内部格式;消费 RXS-0192 单源嵌入纪律与 RXS-0209 host 产物模型泛化）。

#### Legality

- **v2 blob = 64B packed 小端布局**:`version:u32 = 2`(@0)/ `reserved:u32`(@4)/ `ptx_ptr:u64`(@8)/ `ptx_len:u64`(@16)/ `cubin_ptr:u64`(@24)/ `cubin_len:u64`(@32)/ `sm_key:[u8;8]`(@40)/ **`spirv_count:u64`(@48)/ `spirv_entries_ptr:u64`(@56)**。v1 48B 字段原位不变(v1 前缀兼容)。
- **入口表项 = 40B packed**:`name_ptr:u64`(@0)/ `name_len:u64`(@8)/ `stage_tag:u32`(@16)/ `reserved:u32`(@20)/ `spv_ptr:u64`(@24)/ `spv_len:u64`(@32);表 = 连续 `spirv_count` 项。
- **`stage_tag` 映射 `ShaderStage` 枚举声明序(单一事实源)**:0=Vertex / 1=Fragment / 2=Compute / 3=Mesh / 4=Task / 5=RayGen / 6=ClosestHit / 7=AnyHit / 8=Miss / 9=Intersection / 10=Callable;越界值 → 畸形拒。
- **版本分支**:`version == 1` → v1 解析路径 **0-byte**;`version == 2` → v2 解析;其余 → 拒绝(RXS-0193 确定性诊断,不占 RX 码)。
- **畸形判据(全部确定性拒)**:`spirv_count > 0` 而 `spirv_entries_ptr` 为空;表项 `name_ptr`/`spv_ptr` 空而对应 `len > 0`;`stage_tag` 越界;入口名重复;`spirv_count` 与表指针组合越出合理上界(防越界读)。`spirv_count == 0` 合法(feature on 但本单元无 SPIR-V 产物)。
- **feature 门控**:cargo feature `vulkan-backend` **off → 发射 v1 blob,产物与此前逐字节一致**(「默认构建零依赖绿」承诺不变);**on → 发射 v2 blob**。「单源 Vulkan RHI 通道」可用性 = 工具链构建面,非默认构建可达。

#### Dynamic Semantics

- **NVIDIA PTX/cubin 装载路径逐字节不变**:v1 blob 产物、cubin/PTX 装载协商、PTX JIT ladder **0-byte**(RXS-0209 IR1 纪律);v2 的 PTX/cubin 字段语义与 v1 同。
- **指针字段重定位**:blob 内全部指针 = 常量段全局符号的 link-time 重定位(v1 `ptx_ptr` 同构先例;DLL 形态经 EI1.2 `--emit=dll` device 真跑验证——ASLR/基址重定位对常量段绝对地址成立),EXE/DLL 双形态均可解析。
- **装载语义**:v2 解析后 SPIR-V 入口表按**入口名索引**供 Vulkan 通道消费(RXS-0292/0293);SPIR-V 槽缺席不影响 CUDA 腿装载(v1 字段完整时 CUDA 路径与今日一致)。

#### Implementation Requirements

[`rurixc`](../src/rurixc/src/codegen.rs) `emit_gpu_artifact_globals` 组装 v2 + [`rurix-rt-cabi`](../src/rurix-rt-cabi/src/artifacts.rs) 版本分支解析;布局 golden(字段偏移/字节序/64B 总长)+ feature-off v1 产物零漂移断言 + 解析红绿(v1/v2/各畸形类)。

> 锚定测试:[`codegen::tests`](../src/rurixc/src/codegen.rs)(v2 blob 布局 golden + feature-off v1 零漂移)+ [`artifacts::tests`](../src/rurix-rt-cabi/src/artifacts.rs)(v1/v2/畸形解析红绿)。

### RXS-0291 `@__rx_gpu_spirv` 段发射与多入口 SPIR-V 收集（同一编译单源，每入口独立模块，RFC-0015 §4.C2）

#### Syntax

无用户面语法(codegen/driver 内部);消费 MIR 全部 GPU 入口(compute kernel 与阶段着色函数,RXS-0200~0204/0242~0246)。

#### Legality

- **收集 = driver 侧入口迭代器**([`build_gpu_artifacts`](../src/rurixc/src/driver.rs) 扩展):枚举 MIR 全部 GPU 入口,**逐入口调用与 `--target vulkan` 相同的 lowering 入口**——compute kernel → [`vulkan_codegen::lower_compute`](../src/rurixc/src/vulkan_codegen.rs);vertex/fragment → [`dxil_spirv::emit_spirv_body_vulkan`](../src/rurixc/src/dxil_spirv.rs);mesh/task → RXS-0275 lowering(承接期落地前,该阶段入口缺席 = 下条 note 口径)。两路径共用 lowering 函数 = **单一事实源**;`build_and_emit_vulkan` 单入口形态与 `--target vulkan` 单文件产出面 **0-byte 不变**。
- **每入口一独立 SPIR-V 模块**——无 SPIR-V 合并、无链接器;`@__rx_gpu_spirv_<i>` 每入口一全局(模块字节)+ 名字节全局 + 入口表全局(RXS-0290 表项),经 `emit_gpu_artifact_globals` 同函数加性发射。
- **入口名 = 既有 PTX launch 路径同名内核标识**(单一事实源;同名映射保证 RHI/launch 面按名索引 SPIR-V 模块与 CUDA 模块一致)。
- **逐入口降级失败 → 编译期 note**(诊断输出,非静默)+ 该入口缺席于 SPIR-V 表;**经 Vulkan 通道要求缺席入口 → 运行期确定性 `Err`**(RXS-0193 口径,不占 RX 码);lowering 不可映射维持 `RX6026` 既有族(--target vulkan 路径)。PTX 腿不受 SPIR-V 缺席影响。
- **feature 门控**:`vulkan-backend` off → 不产 SPIR-V 段、发射 v1 blob(逐字节不变,RXS-0290 Legality);on → 收集并发射 v2。

#### Dynamic Semantics

- **同一编译单源防漂移**:PTX 与 SPIR-V 由同一次编译、同一份 MIR 产出(非两趟独立编译);PTX 产物与今日逐字节一致。
- **sentinel 纪律**:工具链缺(vulkan-backend off / 无 kernel)按既有 sentinel 口径(v1 blob 或空表 v2),不伪造产物。

#### Implementation Requirements

[`driver`](../src/rurixc/src/driver.rs) 入口迭代器 + [`codegen`](../src/rurixc/src/codegen.rs) `GpuArtifacts.spirv_modules` 与 v2 全局发射;codegen 单测(多入口表内容/名字/阶段号锚定)+ feature-off 零漂移 + spirv-val 三态(RXS-0212 gate)对产物模块校验。

> 锚定测试:[`driver::tests`](../src/rurixc/src/driver.rs)/[`codegen::tests`](../src/rurixc/src/codegen.rs)(多入口收集:两 kernel + vertex/fragment 四入口表项、名字、stage_tag)+ v1 零漂移断言。

### RXS-0292 rxrt artifacts v2 解析与 `DeviceArtifactSet` SPIR-V 填充（RFC-0015 §4.C3）

#### Syntax

无用户面语法(运行时解析与产物模型;消费 RXS-0290 blob v2)。

#### Legality

- **解析版本分支**([`rurix-rt-cabi/src/artifacts.rs`](../src/rurix-rt-cabi/src/artifacts.rs)):`version == 1` → 既有解析 **0-byte**;`version == 2` → v1 字段 + SPIR-V 入口表逐项核 RXS-0290 畸形判据;其余 → 拒绝(RXS-0193 确定性诊断 + 句柄 0,不占 RX 码)。
- **`DeviceArtifactSet` 加性扩展**([`fatbin.rs`](../src/rurix-rt/src/fatbin.rs)):新增**按名索引的 SPIR-V 入口集合**(`with_spirv_entries` builder + 访问器);既有 `spirv_fallback` 槽与全部既有访问器 **0-byte**;`ArchKey`/`ArtifactKind`/`LoadChoice` 语义不变。
- **装载面**:CUDA 装载协商(cubin → PTX JIT ladder)0-byte;SPIR-V 入口集合仅被 Vulkan 通道消费(RXS-0293),CUDA-only 进程不触碰。

#### Dynamic Semantics

- **解析失败语义**:任何畸形(版本/表项/指针/阶段号/重名)→ 确定性诊断(op + 上下文)+ 句柄 0(RXS-0193 同族);无部分解析、无静默截断。
- **EXE/DLL 双形态回环**:v2 blob 在 EXE 与 DLL(export(c) 产物)两种 host 形态均可被解析(常量段重定位成立,RXS-0290 Dynamic Semantics)。

#### Implementation Requirements

新增指针走查(表项 name/spv 指针解引)登记 **U31 起**(unsafe-audit,单块单操作,沿 artifacts.rs U25 族审计模式);解析单测(v1/v2/各畸形类/双形态);`DeviceArtifactSet` 填充单测(按名索引、重名拒、空表)。

> 锚定测试:[`artifacts::tests`](../src/rurix-rt-cabi/src/artifacts.rs)(v2 双形态回环 + 畸形类逐条)+ [`fatbin::tests`](../src/rurix-rt/src/fatbin.rs)(按名索引填充 + 既有槽 0-byte)。

### RXS-0275 MIR→SPIR-V mesh/task 编码兑现深化（mesh body 类型面新建 + MIR lowering，G4.2，RFC-0015 §4.A5，RXS-0246 兑现深化）

#### Syntax

mesh body 用户面(**零新文法产生式**;消费 RXS-0243 mesh/task 入口契约 `#[numthreads]` + `#[outputs]`):

```
mesh fn ms(#[numthreads(32, 1, 1)] #[outputs(topology = "triangles", max_vertices = 64, max_primitives = 42)]) {
    // per-vertex 输出:内建输出数组(位置为 builtin position,余为 interpolate varying,沿 RXS-0159 io_sig 形态)
    mesh_vertex[i].position = …; mesh_vertex[i].color = …;     // 已知内建输出面(i < max_vertices,越界编译期拒)
    mesh_triangle[p].indices = (a, b, c);                       // per-primitive 索引输出(p < max_primitives)
    mesh_set_outputs(vertex_count, primitive_count);            // 已知函数:输出计数终结子(≤ 声明上限,编译期核)
}
```

#### Legality

- **mesh body 类型面(新建)**:`mesh_vertex` / `mesh_triangle` 为 mesh 阶段**已知内建输出面**(per-vertex / per-primitive
  输出数组,元素类型自 `#[outputs]` 声明面的 io_sig 推导,沿 RXS-0159);`mesh_set_outputs` 为 mesh 阶段**已知函数**
  (终结输出计数);`#[outputs]` 标注对接 io_sig 扩展(per-vertex `Position` builtin + varying Location 数组 /
  per-primitive `PrimitiveTriangleIndicesEXT` uvec3 数组)。越界写(索引 ≥ `max_vertices` / `max_primitives` 编译期
  可知形态)→ 编译期拒(复用既有 const/索引诊断族,**零新码**);着色违例(输出面用于非 mesh 阶段)→ **RX3012 /
  RX3013 / RX3017** 族复用。
- **MIR lowering**:mesh/task 执行模型经 **MeshEXT / TaskEXT** 编码——capability `MeshShadingEXT` + `OpExtension
  "SPV_EXT_mesh_shader"` + execution modes(`LocalSize` 承 `#[numthreads]` / `OutputVertices` / `OutputPrimitivesEXT` /
  `OutputTrianglesEXT` 承 `#[outputs]`)+ per-vertex 输出 Block(`Position` builtin 成员 + varying Location)+
  `PrimitiveTriangleIndicesEXT` 输出数组 + `mesh_set_outputs` → `OpSetMeshOutputsEXT`;task payload(`TaskPayloadWorkgroupEXT`
  + `OpEmitMeshTasksEXT`)为条件臂(RXS-0270 RT/task 条件臂同评估窗,首期 mesh-only)。
- **SPIR-V 版本**:mesh 入口 emit **1.4** + interface 全量(RXS-0247 per-entry 分叉既有机制);既有 compute / vertex /
  fragment 维持 1.0 字节零漂移。
- 子集外(非 triangle 拓扑 / 动态输出计数上限不可知 / task payload 条件臂外)→ **RX6026** 扩类别(strict-only,不发明编码)。

#### Dynamic Semantics

- mesh 语义本体 = 阶段类型面既有条款(RXS-0243/0244)与 G3.6 编码框架(RXS-0246)的**兑现深化**:本条把「固定最小合规
  模块(witness 手工构造)」升为「真实 `.rx` MIR 体的 SetMeshOutputs / 输出写 intrinsic 降级」——同一 `.rx` mesh fn
  经本 lowering 产 SPIR-V 模块,与 witness 发射器(`emit_mesh_min`)产物**语义等价**(golden 参照)。
- 全部产物过 `spirv-val --target-env vulkan1.2` / `spv1.4` 三态 gate(RXS-0212);**DXIL 腿**维持 B 链 probe 既有判据
  0-byte(本库面首期 device 门锚 Vulkan 腿,不扩面)。

#### Implementation Requirements

- 编码落 [`vulkan_codegen`](../src/rurixc/src/vulkan_codegen.rs)(复用 GLCompute/LocalSize workgroup 基建,RXS-0246
  E-4 钉死)与 [`dxil_spirv`](../src/rurixc/src/dxil_spirv.rs) 装饰/I/O 机制(单一事实源分工,不重复实现);
  mir_build mesh body 内建输出面与 `mesh_set_outputs` 已知函数接线;witness 发射器(`emit_mesh_min` 等)**0-byte 保留**
  (golden 参照);同场景 MIR lowering 产物 vs witness 模块结构对照单测 + spirv-val 双口径;device 端到端(mesh pass 经
  RHI 库面出图)归 RXS-0272 执行面与步骤 76。

> 锚定测试:[`vulkan_codegen::tests`](../src/rurixc/src/vulkan_codegen.rs)(mesh lowering:MeshEXT 执行模型 /
...
### RXS-0300 MIR→SPIR-V compute RayQuery 编码 + SPIR-V 1.4 per-entry 升版（RayQueryKHR/SPV_KHR_ray_query 按需声明；W1/W2 1.0 零漂移，RFC-0018 §4 章 B）

> **编号续号说明**:本条 RXS-0300 由 **RFC-0018**(G7 生产帧闭环期伞形章 B,Agent Approved 2026-08-01)新增,与章 A(spec/shader_stages.md RXS-0297~0299)共享 **RXS-0297 起**顺位(先合先得、后合校准),兑现 number_ledger `reserved_in_flight[G7]` claim。与既有 RT 条款关系:**只增量不矛盾**——RT pipeline 六执行模型(RXS-0247/0248)维持既有面不动,compute inline ray query 是**新增**供给面。

#### Syntax

无用户面语法(codegen 内部);消费 spec/shader_stages.md RXS-0297~0299 compute RayQuery 类型面/状态机/动态语义。

#### Legality

- **per-entry 升版判定(并集钉死,RFC-0018 §9.1 V-1)**:**使用 RayQuery(MIR 体存在 `RayQuery` local 类型/ray query intrinsic)或 compute 签名含 `AccelStruct` 形参的 compute entry → 该入口模块升 SPIR-V 1.4**(`SPIRV_VERSION_1_4` header + `OpEntryPoint` interface **全量枚举**全部被引用全局变量,与 mesh/RT 同律,RXS-0247)。二者任一即触发(具体检测点实现期核实并随实现 PR 冻结);`AccelStruct` 形参 → descriptor 变量(UniformConstant)→ `OpTypeAccelerationStructureKHR`,其 capability 承载 = `RayQueryKHR`——仅看 RayQuery local 会把「AS 形参在、RayQuery 不在」的 kernel 留在 1.0 且不声明 capability,致 `OpTypeAccelerationStructureKHR` 无 capability 承载、spirv-val 必拒;并集判定封闭该情形(W1/W2 五 kernel 无 AS 形参,零漂移面不受并集影响)。同一 kernel 的判定结果必须确定性(同 MIR 同版本轴)。
- **capability/extension 按需声明**:`RayQueryKHR` capability + `OpExtension "SPV_KHR_ray_query"` **当且仅当模块含 `OpTypeRayQueryKHR` 或 `OpTypeAccelerationStructureKHR` 时声明**(与判定点并集同源);均不含的 compute entry 维持 1.0 且**零新 capability**(承 Int64/Int64Atomics「capability 只按真实使用声明」既有先例)。`OpTypeAccelerationStructureKHR` 在 compute(非 RT 阶段)模块中的 capability 承载 = **`RayQueryKHR`**(SPV_KHR_ray_query rev 17:该指令列于 RayQueryKHR capability 下;RT 阶段模块另有 RayTracingKHR/RayTracingPipelineKHR 承载路径,compute 面唯一);shader 面**不引入** `SPV_KHR_physical_storage_buffer`(BDA 为 host/运行时面,首期 kernel 不使用物理地址缓冲指令)。
- **RayQuery 指令面编码约束**:`OpTypeRayQueryKHR`(**Function 存储类自觉**,RXS-0297 Function-only 收窄)+ `OpRayQueryInitializeKHR`(flags 恒 Opaque,mask 恒 0xFF,承 RXS-0298)+ `OpRayQueryProceedKHR` + `OpRayQueryTerminateKHR` + committed 查询族 `OpRayQueryGetIntersection{TKHR,BarycentricsKHR,InstanceIdKHR,PrimitiveIndexKHR,GeometryIndexKHR}`(按真实使用);`has_committed` 映射 `OpRayQueryGetIntersectionTypeKHR`(Committed)或等价(实现期核实)。`OpStore`/`OpLoad`/`OpCopyMemory`/`OpCopyMemorySized` 对 RayQuery 类型指针禁用——非 Copy 纪律 by-construction 成立(RXS-0297)。
- 子集外构造 / SPIR-V emit 失败 / spirv-val 拒 → **RX6026 类别扩充或 RX6034 起新类别**(只追加、不预造,RFC-0018 §7.4;RX6026 类别扩充先例承 RXS-0246)。

#### Dynamic Semantics

- **升版依据(如实归因,RFC-0018 §9.1 评审精确化)**:SPV_KHR_ray_query 扩展自身 **requires SPIR-V 1.0**(rev 17 Dependencies)——1.4 并非 SPV 扩展强制;升版依据 = ① Vulkan 侧依赖链(VK_KHR_ray_query 依赖 VK_KHR_spirv_1_4 **或** Vulkan 1.2 核心,+ VK_KHR_acceleration_structure)+ 调研报告 §1.3(使用 ray query 的 SPIR-V 模块版本不得低于 1.4,Vulkan API 有效性规则);② RXS-0247 同源口径(RT 腿硬性 1.4)与 interface 全量枚举同律——rurix **自觉沿 1.4 per-entry 口径冻结**(非 SPV 扩展强制)。升版与回归分波(W3a 独立波次,调研报告 §1.3「版本升级单独一波先回归」)。
- **W1/W2 零漂移门**:既有 compute/vertex/fragment 入口维持 1.0 emit,**产物字节零漂移**(RXS-0247 零回归门延伸):五 W1/W2 kernel golden(cull/classify_resolve/vsm_page_mark/taa/visbuffer_sw_u64)+ 全部既有 vulkan golden **字节 diff 空,不重 bless**;DXIL B 路消费的 SPIR-V 字节不变;dxil 套件恒定。既有 `assemble`(1.0)**0-byte 不动**;RayQuery compute 走**新增发射路径**(新增 assemble 变体或 `lower_compute` 内分叉,形态实现期定),分叉落**发射函数级**(RXS-0247 既有机制)。
- **校验轴**:合规判定以 `spirv-val` **退出码**为准,**不以驱动宽容度为准**(承 RXS-0247);`--target-env vulkan1.2` 与 `--target-env spv1.4` **双口径皆 accept**(承 RXS-0212 三态 gate)。
- **反汇编 golden 锚定(最小集,G-G7-4 逐字)**:`OpTypeRayQueryKHR`、`OpRayQueryInitializeKHR`、`OpRayQueryProceedKHR`、`OpRayQueryTerminateKHR`、committed 查询族(按真实使用)+ 1.4 header + interface 全量 + `RayQueryKHR`/`SPV_KHR_ray_query` 声明行;golden 随实现 PR 落 SPIR-V golden 套件,**W1/W2 golden 0-byte**。

#### Implementation Requirements

落 [`vulkan_codegen`](../src/rurixc/src/vulkan_codegen.rs) **新增发射路径**(`lower_compute` 内分叉或新 assemble 变体,形态实现期定);RayQuery local → Function `OpVariable`(`OpTypeRayQueryKHR`)+ initialize/proceed/terminate/committed 族指令降级;`AccelStruct` 形参 → UniformConstant descriptor(set/binding 由绑定布局推导产出,不手写钉死,RFC-0018 §5.C2);spirv-val 双口径三态(RXS-0212);device 段最小 hit/miss/属性查询 kernel 真跑归 CI 步骤 93(`RURIX_REQUIRE_REAL=1`,缺 provisioning SKIP = dev-env degrade 不充绿)。**本期 = 条款先行(spec-first,G7.1 PR-1,硬规则 7)**;编码实现、golden 锚定与 RED 语料转正归 G7.2(W3a)。
