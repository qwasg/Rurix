...
> 每条按需分 Syntax / Legality / Dynamic Semantics / Implementation Requirements 节,**严禁 UB 节**(UB 为人类经 Full RFC 落笔的禁区,10 §7.5)。Legality 违例只**引用**错误码(§3 引用汇总),不在此定义其含义。cublas 绑定边界的设备指针生命周期 / 所有权语义以 **affine 所有权 + 确定性诊断(RX 错误码)** 定义,不以 UB 表述。
...
- cublas runtime DLL 按需附带仅限 **Attachment A 白名单最小集**(`cublas64_*.dll` / `cublasLt64_*.dll`,运行期库);完整 CUDA Toolkit / 驱动 / Nsight / 静态库 / headers / libdevice **永不捆绑**(许可红线 r6)。触及即停下标注「需人工升档」(§4)。
- NVIDIA EULA 法律签署维持 `pending-human-review`(**AI 不代签**,对齐 M5 redistribution_audit 先例)。
