...
> 规范先行(AGENTS.md 硬规则第 7 条):**条款 commit 先于实现 commit**(MS1.2 单 PR 内,G-MS1-1);`ci/trace_matrix.py --check` 要求每条 `### RXS-####` ≥1 测试锚定(`//@ spec: RXS-####`)——本文件条款的锚定测试(conformance/host_orch/* 语料 + rurixc/cabi 单测 + ci/host_orch_smoke.py)随各波实现 commit 同 PR 落,trace_matrix 至 184→192(MS1.2)→195(MS1.2b)全锚定;stable 快照因条款计数增长同 PR 重 bless(RXS-0180 L2 加性演进)。
...
- **present 宿主 typestate 面**(RXS-0197,MS1.2b):`Present` / `Ready` / `Acquired` / `Presentable` affine 消费式帧状态机;错序 = 编译期 move 违例(复用 RXS-0054,零新借用码);fence 偶/奇协议引用 RXS-0142 不重述;窗/泵/交换链维持 C++ shim(D-130 0-byte)。
- **present backbuffer 借用缓冲与 blit 契约**(RXS-0198,MS1.2b):仅 Acquired 态可得的借用句柄 `Buffer<C, f32>`(drop 不释放);内容布局对齐 RXS-0143 / RXS-0121;可作 launch 实参。
- **宿主图像落盘桥**(RXS-0199,MS1.2b):`write_ppm` 编译器已知自由函数 → `rxio_write_ppm`;RXS-0114~0117 确定性 PPM 序列化语义 0-byte 复用;尺寸失配 = 运行期确定性失败。

不在本文件落语义本体的范围:窗/泵/交换链/固定 present pass(维持 C++ shim,D-130 0-byte——RXS-0197 只落 `.rx` 面 typestate 与 cabi 状态断言转发,不把窗口/输入/事件循环带进语言,RD-027);std::gpu 首期子集外(RD-026);真 NVIDIA fatbinary 容器;跨包 `.rx` 依赖 / `rx new` 脚手架;`#[export(c)]`(RD-009,本面为**导入**方向,消费手写 extern "C",RXS-0125 口径 0-byte)。

**编号区间**:本文件条款自 **RXS-0189** 起续号(全 spec 唯一、分配制递增、永不复用,见 [README.md](README.md) §1;最高现存 RXS-0188 @ [release.md](release.md);RXS-0181~0184 已被 GRX showcase 分支 claim,跳号避撞维持)。MS1.2 落 **RXS-0189 ~ RXS-0196**,MS1.2b 落 **RXS-0197 ~ RXS-0199**(条款体均已落地)。区间登记于 [README.md](README.md) §4 文件清单。

## 2. 条款(RXS-0189 ~ RXS-0199)
...
### RXS-0197 present 宿主 typestate 面

**Syntax**(present 帧状态机类型与方法集,lang items;RFC-0009 §4.6):

```
Present / Ready / Acquired / Presentable                                 // 非 Copy affine 帧状态句柄
Present::create(&ctx, rw: u32, rh: u32, ww: u32, wh: u32) -> Present     // 建链(render / window 宽高)
sess.ready() -> Ready               // 消费 self;纯类型面转移(不落运行时符号)
ready.wait() -> Acquired            // 消费 self;→ rxp_wait(fence acquire 步)
acq.backbuffer() -> Buffer<C, f32>  // 借用句柄(仅 Acquired 态,RXS-0198)
acq.signal() -> Presentable         // 消费 self;→ rxp_signal(fence cuda_done 步)
pres.pump() -> bool                 // 非消费;→ rxp_pump(true = 关闭请求)
pres.present() -> Ready             // 消费 self;→ rxp_present(帧 +1)
```

**Legality**:

- 四类型为编译器 lang items(用户同名定义优先遮蔽、语义不变,兜底纪律沿 RXS-0189);全部**非 Copy affine**。消费式方法(`ready` / `wait` / `signal` / `present`)接收者按值消费——**错序 = 编译期 move 违例**:跳态重用 / 重复转移复用 RXS-0054 既有裁决(RX4001;经引用消费 → RX4003),非本态方法不存在走既有方法查找诊断(RX2004),**零新借用码、零新 RX 码**。
- 方法签名为编译器已知(RXS-0190 口径):元数不符 → RX2003、实参类型不符 → RX2001。`Present::create` 首实参为 `&Context`(RXS-0189 lang item;单 brand 方案沿 RXS-0189 实现要求),`rw` / `rh` / `ww` / `wh` 为 `u32`。
- **宿主 API 着色合法性**:present 系构造与方法与 std::gpu 同一识别面——`kernel` / `device fn` 体内出现 → **RX3015**(RXS-0189 同点位)。

**Dynamic Semantics**:

- `Present::create` 求值 = 调用 `rxp_create(ctx, rw, rh, ww, wh)`(RXS-0194 延伸符号)建 present 会话(D3D12 窗口 / 交换链 / 共享资源经既有 shim 建链);句柄 0 = 失败 → 确定性诊断 + 终止(RXS-0193)。
- `wait` / `signal` / `present` 分别对应共享 fence 协议的 acquire / cuda_done / d3d_done 步——**偶/奇值协议引用 RXS-0142,本条不重述**(单一事实源留 interop 帧机);`pump` 泵送窗口消息并返回「关闭请求」布尔。任一 `rxp_*` 负值返回 / 运行期状态错序(cabi 状态断言拒)→ 确定性诊断 + 终止(RXS-0193 口径),**不产生 UB、无静默降级**。
- `sess.ready()` 为纯类型面转移(`Present` → `Ready` 同句柄再定名),无运行期效果。

**Implementation Requirements**:

- 窗/泵/交换链/固定 present pass 全在 C++ shim **不动**(D-130;RXS-0140~0143 语义面 0-byte);cabi `rxp_*` 只做句柄表状态断言 + 转发(RXS-0194 纪律)。
- 消费式转移的 lowering 以 MIR move 表达(接收者按值 move 进转移结果值),move/init 数据流(RXS-0054)天然拦截编译期错序;方法集经 typeck 编译器已知签名分支表达(RXS-0190 先例)。

> 测试锚定:conformance/host_orch/accept/present_loop(完整帧循环,IR 断言 declare rxp_*)+ reject/present_out_of_order(错序 = RX4001)+ reject/present_in_kernel(RX3015)。

### RXS-0198 present backbuffer 借用缓冲与 blit 契约

**Legality**:

- `backbuffer()` 仅 `Acquired` 态可得(`Ready` / `Presentable` 无此方法,走既有 RX2004);产**借用句柄** `Buffer<C, f32>`——类型面即 RXS-0189 `Buffer`(元素定型 f32,不经 RXS-0190 使用点推断),**可作 launch 实参**(`ViewMut` / `View` 位,RXS-0191 首期子集内)。
- 接收者非消费(借用读取);同帧可重复取得,均指同一底层共享缓冲。

**Dynamic Semantics**:

- 借用句柄语义 = owned=false:其 drop **不释放**底层共享资源(D3D12 COM owner 与 CUDA import 生命周期均留运行时 / shim 侧,对齐 RXS-0140 Drop 纪律与 RXS-0124 借用缓冲先例)。
- 句柄有效窗口 = 本帧 `wait` 后至 `present` 返回前;窗口外经其发起的 gpu 操作由 cabi 状态断言确定性拒(RXS-0193)。
- 内容布局 = **行主序紧密共享 f32 RGB、分量 0…255**(读取约定引用 **RXS-0143**,量化语义对齐 **RXS-0121** `sr_tonemap` 输出口径);blit kernel 经 `ViewMut<global, f32>` 位对其写入即写共享 backbuffer 等价缓冲,写入排他窗口由 fence 协议(RXS-0142)保证。

**Implementation Requirements**:

- `rxp_backbuffer(h) -> u64` 返回借用缓冲句柄(0 = 失败 → 终止,RXS-0193);句柄注册进 cabi 缓冲表(与 `rxrt_buf_*` 同表消费,销毁对借用句柄为 no-op)。

> 测试锚定:conformance/host_orch/accept/present_loop(backbuffer 作 launch 实参 + IR 断言 rxp_backbuffer)。

### RXS-0199 宿主图像落盘桥

**Syntax**(编译器已知自由函数;RFC-0009 §4.7 形态定案 = 自由函数面):

```
write_ppm(path: &str, w: u32, h: u32, data: &PinnedBuffer<C, f32>)
```

**Legality**:

- `write_ppm` 为**编译器已知自由函数**(值位置兜底解析,用户同名定义优先遮蔽;形态定案:自由函数面、不引入 `Image` 容器 lang item——最小侵入,解析锚点镜像 `Context::create`,RFC-0009 §4.7「实现 PR 定形」就此收口);签名编译器已知(RXS-0190 口径:元数不符 → RX2003、实参类型不符 → RX2001)。
- `data` 为 `&PinnedBuffer<C, f32>` 锁页缓冲共享借用;该形参位为元素类型使用点约束(RXS-0190):未定元素在此定型为 f32,定型冲突走 RX2001。
- host-only:`kernel` / `device fn` 体内出现 → **RX3015**(RXS-0189 同点位)。

**Dynamic Semantics**:

- 求值 = 调用 `rxio_write_ppm(path, w, h, data_ptr, n)`(RXS-0194 延伸符号):`path` 为 NUL 终止字符串,`data_ptr` / `n` 自锁页句柄物化(n = 锁页元素数)。
- 序列化语义 = **RXS-0114 ~ RXS-0117 确定性 PPM 语义 0-byte 复用**(P6 header 规范化 / 行主序 / 通道序 R,G,B / f32→u8 确定量化 = RXS-0116;运行时桥接既有 image-io 实现,**不复制第二份编码器**):同输入两次落盘逐字节一致。
- **n ≠ w·h·3 → 运行期确定性失败**(尺寸失配诊断 + 终止,RXS-0193);路径不可写 / IO 失败同口径。成功无返回值。

**Implementation Requirements**:

- lowering 经与 RXS-0191 同机制:`rxrt_pinned_ptr` / `rxrt_pinned_len` 物化指针与元素数 → `rxio_write_ppm`,负值返回 → 终止(RXS-0193)。

> 测试锚定:conformance/host_orch/accept/imageio_write(pinned 填充 + write_ppm,IR 断言 rxio_write_ppm;cabi 落位后可真跑:退出 0 + PPM 字节非空)。

### RXS-0225 宿主 `SamplerDesc` 采样器状态面（RFC-0013 §4.B2 形态 b）

> **编号续号说明**:本条 RXS-0225 由 **RFC-0013**(§4.B2 形态 b)新增,超出 MS1.2/MS1.2b 的 RXS-0189~0199 区间(独立 RFC 续号)。定义宿主运行时 sampler 状态对象 `SamplerDesc`——与着色阶段静态属性 `#[sampler(...)]`(spec/shader_stages.md RXS-0224)**同一状态空间**的运行期形态,供 rurix-rt/cabi + uc04 shim 消费(wrap-vs-clamp 像素对照等 device 判据走本形态)。宿主面无新语法。

#### Syntax

本条不新增语法。`SamplerDesc` 为宿主运行时库类型(`std::gpu` / rurix-rt),经既有构造 / 字段赋值 API 使用,不引入着色语言文法。

#### Legality

- L1(同一状态空间,单一事实源):`SamplerDesc` 承载与 RXS-0224 **同一枚举集**——`filter ∈ {nearest, linear}`(min/mag/mip 三合一)、`address ∈ {clamp, wrap, mirror, border}`、`max_anisotropy: u32`、`lod_bias`(钳 [-16,16))、`min_lod`/`max_lod`;`SamplerCmp` 附 `compare ∈ {less, less_equal, greater, greater_equal}`。状态空间与静态属性共用同一单一事实源(不发明第二套枚举)。
- L2(aniso 探测确定性 Err):`max_anisotropy > 1` 时 Vulkan 侧探测 `samplerAnisotropy` device feature,缺失 → **运行期确定性 Err**(RFC-0011 §4.11 纪律,**不占 RX 码**,对齐 RXS-0193 运行期失败口径)。
- L3(无新语法 / 无新码):宿主面为运行期对象,零新着色语法、零新编译期 RX 码;非法状态组合走库层 `Result`(运行期确定性失败,RXS-0193)。

#### Dynamic Semantics

`SamplerDesc` 为运行期状态对象,经 rurix-rt/cabi 降级为 Vulkan `VkSamplerCreateInfo`(RXS-0230 vk descriptor 建面消费)/ D3D12 `D3D12_SAMPLER_DESC`。给定状态字段,采样运行期过滤 / 寻址行为由该状态决定(与 RXS-0176/RXS-0227 语义一致)。宿主构造为确定性:同一状态字段产同一 device sampler 配置。

#### Implementation Requirements

- IR1(rurix-rt 状态对象):`SamplerDesc`(+ `SamplerCmpDesc` 的 `compare` 扩展)为 `rurix-rt` 宿主类型,字段 = L1 枚举集;经 cabi 降级 `VkSamplerCreateInfo`(filter→`VkFilter`/`VkSamplerMipmapMode`、address→`VkSamplerAddressMode`、compare→`VkCompareOp` + `compareEnable`)。
- IR2(uc04 shim 消费):uc04 shim 经**新增独立 sampler 入口**(SC-6/§6.3;非扩 `rx_uc04_offscreen_run` 参数面)承载 SamplerDesc 下发;wrap-vs-clamp 像素对照(步骤 63 模式⑤)走本形态。
- IR3(测试锚定):≥1 `//@ spec: RXS-0225` 覆盖 cabi/shim 单测(SamplerDesc → VkSamplerCreateInfo 映射 + aniso 缺失确定性 Err)+ 步骤 63 wrap-vs-clamp 对照。

### RXS-0235 std::gpu `TextureTable` 宿主注册面与运行时 feature chain（RFC-0013 §4.C4）

`TextureTable<C>` 为 std::gpu 宿主无界纹理表句柄:非 Copy **affine**(纪律与 brand 契约全量复用 RXS-0189,零新借用码);`ctx.texture_table() -> TextureTable<C>`、`table.register(tex) -> u32`(**注册序即索引,稳定单调**)、`table.len() -> u32`。宿主经 `rxrt_table_*` C ABI(只追加,RXS-0194)与 Vulkan descriptor pool/set-layout(UPDATE_AFTER_BIND + PARTIALLY_BOUND + feature chain 探测)承载;shader 侧无界表(RXS-0231)动态索引经其已注册段消费(codegen clamp 表长 RXS-0234)。

#### Syntax

`TextureTable<C>` 与 `ctx.texture_table()`/`register`/`len` 为编译器已知签名(lang-item 宿主类型 + 已知方法,复用 RXS-0189/0190 分支,零新文法产生式)。

#### Legality

- L1(affine 承 RXS-0189):`TextureTable<C>` 非 Copy affine 句柄,move/borrow 违例复用 RXS-0054/0057~0061(零新借用码);`C` = Context brand(单 brand 方案,RFC-0009 §9 Q-Brand)。
- L2(宿主 API 着色):`texture_table()`/`register`/`len` 为宿主编排 API——kernel/device 体内出现 → **RX3015**(承 RXS-0189,coloring 层同点位)。
- L3(格式擦除,Q-B-TableFormat):`TextureTable<C>` **不带元素类型参数**;host↔shader 元素类型错配 = **运行期确定性 Err**(非 fake、非静默),本条显式声明该边界。

#### Dynamic Semantics

- `register(tex)` 按调用序返回稳定单调索引(0,1,2,…);`len()` = 已注册计数;注册写入仅发生在提交前,in-flight 期间不更新(§4,触 GPU 时间线并发语义,结构性拒)。
- **Vulkan feature chain(RXS-0193 封口)**:descriptor pool/set-layout 带 `UPDATE_AFTER_BIND` + `PARTIALLY_BOUND` binding flags;探测 `VkPhysicalDeviceDescriptorIndexingFeatures` 四 bit(`shaderSampledImageArrayNonUniformIndexing` / `descriptorBindingSampledImageUpdateAfterBind` / `descriptorBindingPartiallyBound` / `runtimeDescriptorArray`)——**任一缺失 → 确定性 Err**(运行期不占 RX 段位,无静默降级 P-01)。无界表独占 set4+ 使有界 set0~3 完全不触 feature chain(零回归最强形)。

#### Implementation Requirements

- IR1(std::gpu lang item):`TextureTable` 注册于 [`resolve`](../src/rurixc/src/resolve.rs)(追加于既有 lang items 之后,DefId 编号稳定);`texture_table()`/`register`/`len` 经 `gpu_host_method` 已知签名分支([`typeck`](../src/rurixc/src/typeck.rs)),降级为 `rxrt_table_*` 字面符号(mir_build,镜像 `rxrt_buf_*`)。
- IR2(cabi 只追加,🔒 FFI):`rxrt_table_create`/`register`/`len`/`destroy` 只追加([`rurix-rt-cabi`](../src/rurix-rt-cabi/src/lib.rs)),`rxrt_launch` 及既有符号面字节不变(RXS-0194);u64 句柄表 / handle-0 = 失败 / poisoned 传播跨后端不变式维持;新 unsafe 折叠 U27 扩注(0 新号)。
- IR3(Vulkan 运行时):descriptor pool/set-layout UPDATE_AFTER_BIND + PARTIALLY_BOUND + `VkPhysicalDeviceFeatures2`/`pNext` 探测四 bit,缺失确定性 Err([`vk`](../src/rurix-rt/src/vk.rs),折叠 U27);无界表独占 set4;注册写入仅提交前。
- IR4(测试锚定):≥1 `//@ spec: RXS-0235`——cabi 单测(`rxrt_table_*` 符号面 + 注册序即索引 + handle-0 失败路)+ feature-probe host 单测(mock 缺 bit → Err)+ 步骤 64 device 段(bin/bindless_modes)。
...
launch 既有契约码(RX3004/RX3005/RX3006/RX2001)复用 RXS-0074/0075,本文件零重复分配;move/borrow 违例复用 RXS-0054/0057~0061 既有裁决(零新借用码)。**RXS-0197 ~ RXS-0199(MS1.2b)零新码**:present 错序 = move 违例(RX4001 / RX4003,RXS-0054)、非本态方法 / 元数 / 类型违例走既有 RX2004 / RX2003 / RX2001、宿主 API 着色违例复用 **RX3015**(RXS-0189 同点位,present 系与 `write_ppm` 同属识别面);运行期错序 / 失败走 RXS-0193 确定性诊断 + 终止,不占 RX 码。**RXS-0225(G3.3 宿主 SamplerDesc)零新码**:宿主运行期状态对象,非法状态组合 / aniso feature 缺失走库层 `Result`(运行期确定性失败,RXS-0193),不占 RX 段位。
...
- **窗口 / 输入 / 事件循环进语言(D-130 / RD-027)**:present 面(RXS-0197~0199,MS1.2b 已落体)维持窗/泵/交换链在 C++ shim(D-130 0-byte)——`.rx` 面只落 typestate 与 cabi 状态断言转发,fence 协议单一事实源留 interop 帧机(RXS-0142 引用不重述);交互模式 → RD-027;「窗口/UI 框架进语言」或「通用异步宿主运行时」扩张诱惑 → 登记 SG-010 gating 而非提案。
...
| v1.3 | 2026-07-19 | **RFC-0013 §4.C4 宿主 TextureTable 注册面 + RXS-0235 条款体(spec-first,G3.4 条款先行)**。承 RFC-0013(Agent Approved 2026-07-18)。新增 `### RXS-0235`(std::gpu `TextureTable` 宿主注册面与运行时 feature chain,超 RXS-0189~0199 区间,自 RFC-0013 伞形分配):`TextureTable<C>` 非 Copy affine 句柄(纪律与 brand 契约全量复用 RXS-0189,零新借用码);`ctx.texture_table()`/`register(tex)->u32`(注册序即索引稳定单调)/`len()->u32`;**格式擦除**(无元素类型参数,host↔shader 元素错配 = 运行期确定性 Err,Q-B-TableFormat);kernel 体内 → RX3015(承 RXS-0189)。cabi `rxrt_table_*`(create/register/len/destroy)**只追加**(`rxrt_launch` 及既有符号面字节不变,RXS-0194);Vulkan descriptor pool/set-layout UPDATE_AFTER_BIND + PARTIALLY_BOUND + `VkPhysicalDeviceDescriptorIndexingFeatures` 四 bit 探测(缺失确定性 Err,RXS-0193 封口,无静默降级);无界表独占 set4+ 使有界 set0~3 不触 feature chain(零回归)。新 unsafe 折叠 U27 扩注(0 新号)。FLS 分节,**严禁 UB 节**;零新语法、零新 RX 码。≥1 `//@ spec: RXS-0235` 锚定(cabi 单测 + feature-probe host 单测 + 步骤 64 device 段)。类型面见 spec/shader_stages.md RXS-0231/0232,codegen clamp 表长见 spec/dxil_backend.md RXS-0234。 | **Full RFC**(RFC-0013) |
| v1.2 | 2026-07-18 | **RFC-0013 §4.B2 宿主 sampler 状态面 + RXS-0225 条款体(spec-first,G3.3 条款先行)**。承 RFC-0013(Agent Approved 2026-07-18)。新增 `### RXS-0225`(宿主 `SamplerDesc` 采样器状态面,超 RXS-0189~0199 区间独立 RFC 续号):与着色阶段静态属性 `#[sampler(...)]`(spec/shader_stages.md RXS-0224)**同一状态空间**的宿主运行期形态——filter{nearest,linear}/address{clamp,wrap,mirror,border}/max_anisotropy/lod_bias/min_lod/max_lod + SamplerCmp 的 compare{less,less_equal,greater,greater_equal};单一事实源(不发明第二套枚举);aniso>1 探测 `samplerAnisotropy` 缺失 → 运行期确定性 Err(RXS-0193,不占 RX 码);经 cabi 降级 `VkSamplerCreateInfo`(RXS-0230 消费)/ D3D12 `D3D12_SAMPLER_DESC`;uc04 shim 经新增独立 sampler 入口承载(SC-6);wrap-vs-clamp 像素对照走本形态。FLS 分节,**严禁 UB 节**;零新语法、零新 RX 码(§3 补零新码说明)。每条 ≥1 `//@ spec` 测试锚定随实现 commit 同落 | **Full RFC**(RFC-0013) |
| v1.1 | 2026-07-15 | 兑现 MS1.2b 预留区间,落带编号条款体 `### RXS-0197 ~ ### RXS-0199`(FLS 体例,严禁 UB 节;承 [RFC-0009](../rfcs/0009-host-gpu-orchestration.md) §4.6/§4.7 与 §9 Q-Present 裁决):RXS-0197 present 宿主 typestate 面(`Present` / `Ready` / `Acquired` / `Presentable` 非 Copy affine lang items + `Present::create(&ctx, rw, rh, ww, wh)` / `ready()` / `wait()` / `backbuffer()` / `signal()` / `pump()` / `present()` 编译器已知方法集;消费式转移 self 按值 move,错序 = 编译期 move 违例复用 RXS-0054(RX4001/RX4003)零新码;`wait`/`signal`/`present` 对应共享 fence acquire/cuda_done/d3d_done 步,偶/奇值协议引用 RXS-0142 不重述;`ready()` 纯类型面转移不落运行时符号;运行期错序/失败 = 确定性诊断 + 终止 RXS-0193 口径;窗/泵/交换链维持 C++ shim D-130 0-byte,cabi rxp_* 只做状态断言转发)/ RXS-0198 present backbuffer 借用缓冲与 blit 契约(仅 Acquired 态可得,产借用句柄 `Buffer<C, f32>` 元素定型 f32,drop 不释放底层共享资源对齐 RXS-0140/RXS-0124;可作 launch 实参 RXS-0191 子集内;内容布局 = 行主序紧密共享 f32 RGB 分量 0…255 引用 RXS-0143 / 对齐 RXS-0121)/ RXS-0199 宿主图像落盘桥(`write_ppm(path: &str, w: u32, h: u32, data: &PinnedBuffer<C, f32>)` **形态定案 = 编译器已知自由函数**(值位置兜底可遮蔽,解析锚点镜像 `Context::create`,不引入 Image 容器 lang item);→ `rxio_write_ppm`,RXS-0114~0117 确定性 PPM 序列化 0-byte 复用、量化 = RXS-0116;n ≠ w·h·3 / IO 失败 = 运行期确定性失败 RXS-0193;data 形参位为元素类型使用点约束 RXS-0190)。§1 范围三条兑现 + 依据补 interop_d3d12/imageio/softraster 引用;§3 追加 MS1.2b 零新码说明(present 系与 write_ppm 复用 RX3015 识别面);§4 D-130 行更新为已落体。每条 ≥1 测试锚定(conformance/host_orch/accept/present_loop·imageio_write + reject/present_out_of_order·present_in_kernel)随 MS1.2b 实现 commit 同 PR 落(trace_matrix 192→195 全锚定;stable 快照同 PR 重 bless,RXS-0180 L2) | **Full RFC**(RFC-0009) |
