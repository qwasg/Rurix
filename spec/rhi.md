...
```
g.pass(kernel) -> Pass<C>                      // 逐 pass builder;声明序 = 提交序
pass.reads(&Res<C>) -> Pass<C>                 // read 访问声明
pass.writes(&Res<C>) -> Pass<C>                // write 访问声明
```
...
> 测试锚定(**I4 诚实收窄**):conformance/uc05/accept/pass_declared.rx(pass 声明面 0 诊断)+ I4
> 未声明访问核验机制(`PassSpec::with_reflection` 声明-反射双向精确相等)由 **rurix-rt rhi.rs 库单测
> `rejects_reflection_mismatch_i4` / `accepts_reflection_exact_match_i4` 兑现**(纯 host)。**I4 的 `.rx`
> 编译器反射喂入(pass 绑 kernel)与 kernel 绑定 / compute dispatch 天然耦合,随 EI1.4(device 真跑)
> 落地——EI1.3 不宣称 I4 `.rx` 路 ci_checked**,亦不锚不存在的 reject/pass_undeclared_read.rx(证据级
> = `lib_tested(EI1.3) / .rx_wiring:EI1.4`,矩阵 RXS-0263 I4 行同口径)。
...
- **诚实收窄(EI1.3 落地口径)**:EI1.3 兑现面 = **host 侧执行语义**——`submit()` → 装配核验 → 声明全序 hazard 推导
  → 顺序调度计划(rhi.rs 纯函数,同图逐字节相同)。in-EXE demo(`apps/uc05-rhi/src/demo.rx`)于 GPU 上建 `Context`
  (std::gpu 底座)+ 声明式三 pass 图 + submit 装配核验通过(见证行 `UC05_RHI_OK`);**pass 绑 kernel + 每 pass
  compute dispatch(经 RXS-0191 launch lowering)+ device 数值对照(I9,GPU vs host checksum)归 EI1.4**——EI1.3 demo
  **不 launch kernel、不作数值断言**(host 图安全见证)。声明全序执行序由 rhi.rs `derive_syncs` 确定性(库单测
  `accepts_linear_graph_derives_raw_syncs` / `derivation_is_deterministic` 纯 host 见证)。RURIX_REQUIRE_REAL 纪律下
  demo EXE 真跑(GPU Context)不许 SKIP 充绿(步骤 72 device 段);无 GPU / link 工具链 → SKIP dev-env-degrade。

> 测试锚定:apps/uc05-rhi/src/demo.rx(host 侧三 pass 图 submit 装配核验通过 device 真跑,见证 `UC05_RHI_OK`;步骤 72
> device 段 GREEN)+ rhi.rs `accepts_linear_graph_derives_raw_syncs` / `derivation_is_deterministic`(声明全序执行序纯
> host 确定性)+ cabi `rhi_symbols_failure_path_and_assembly`(装配 → 声明全序 RAW 同步);pass 绑 kernel compute
> dispatch + 数值对照(I9)EI1.4。
...
| **I4** | 未声明访问(触碰未声明 `Res`) | lib_tested | 声明-反射精确相等(编译器喂反射集,`with_reflection`) | RXS-0257 / 库层状态 Err(镜像 RX6030 口径,零新码) | rhi.rs `rejects_reflection_mismatch_i4`(`.rx` 反射喂入 EI1.4) | 库层确定性 Err | **lib_tested(EI1.3) / .rx_wiring:EI1.4** |
...
| **I9** | compute pass 数值正确性(GPU 输出 vs host 参考) | report_only | 运行期 device 数值对照(本质动态,**不可静态全证**;device EI1.4) | RXS-0263 报告项 / 无诊断码 | evidence(device 数值对照) | GPU vs host checksum 一致 | measured_local;Python 侧 = **无数字的定性历史陈述**(上一项目代码 / 交接档不在仓库,EI1_PLAN R3;非可复跑、零杜撰数字) |
| **I10** | transient 资源执行期峰值 / 生命周期(并发存活 vs 声明容量) | report_only | 运行期观测(**不可静态全证实际峰值**;host 侧容量记账 EI1.3 兑现,device 峰值 EI1.4) | RXS-0263 报告项 / 无诊断码 | rhi.rs `transient_resource_capacity_accounting`(host 记账)+ evidence(device 峰值) | 实际峰值 ≤ 声明容量 | measured_local;Python 侧 = 无数字的定性历史陈述(同 I9,非可复跑、零杜撰数字) |
