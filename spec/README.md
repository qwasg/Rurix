...
| [interop.md](interop.md) | 互操作语义面(`rx build --emit=pyd` PYD 产出约定 / `__cuda_array_interface__` v3 + DLPack 双协议零拷贝接入 PyTorch/CuPy / C ABI 边界;C ABI·PYD 唯一通道·永不 Python 原生嵌入·PTX-only·affine 所有权不设 UB) | RXS-0122 起续号预留（M8.1：`rx build --emit=pyd` PYD 产出 / `__cuda_array_interface__` v3 / DLPack 双协议零拷贝 + 设备指针所有权 / C ABI 边界）。M8.1 开工脚手架仅登记新文件名 + 预留区间，**不落裸条款头**——条款体与测试锚定随 M8.1 实现 PR 同落（条款 PR 先于实现 PR，trace_matrix 维持全锚定） | M8.1 |
## 5. 修订记录
