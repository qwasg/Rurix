...
> agent ruling:选项① HLSL 文本边界保名改写;**否决③**(不放宽 `signature_gate`)。

## 1. 命题

验证 agent 裁定的选项①是否兑现四个验收点(对**输出 varying** 与 **fragment 输入
...
- (c) ABI 中立:改写只动 HLSL struct field 的 semantic token,不触 register / mask /
  packing / byte layout / Location 数值(agent 边界 B);
- (d) 确定性:同输入 ×N 改写字节一致。
...
判定:**不构成** agent 边界 B 的物理 ABI 触碰,理由——
...
- 本机 `spirv-cross` 为 1.3.290,agent pin 环境留痕为 1.3.296。改写机制依赖
...
- **签名 validator(`dxv.exe`/`dxil.dll`)未参与**:本机无签名 validator,只测 `dxc`
  结构接受 + 签名名存活。完整签名验证、golden bless、device 真跑归 **agent pin 环境
  (G-G2-4)**,本 spike 自主签署、不翻 RD-017 状态、不收口。
...
**open** 至 agent golden/device bless。
