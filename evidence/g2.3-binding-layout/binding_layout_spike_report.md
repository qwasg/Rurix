...
> 承接：RFC-0005「绑定布局推导」的 Design 与 spike 留痕（Draft / Awaiting Owner）。
...
→ **推论（measured 支撑，裁决留 owner）**：register/space 分配可由编译器**确定性导出**，但「索引轴」有两种自洽策略：(A) 单一全局 binding 轴（spirv-cross 默认，t/s 索引跨种类连号）；(B) 按种类各自从 0 计数（需 Rurix 在 emit SPIR-V binding 时按 (种类, 种类内序) 分配，或译后用 register 覆盖）。两者都确定性、都能产合规 DXIL。**策略选择 = Q-Space，落 §9，AI 不自填默认**。
...
以下属 RFC-0005 实现侧待建面或待 owner 裁决，本 spike **明确不实测、不声称已打通**：
...
RXS-0156 资源使用             [assumed: Rurix 侧待建，结构上当前不可达]
  (Texture2D<F>/Sampler/
   cbuffer/structured buffer)
        ...
        ▼                       [Q-Space：(set,binding) 分配轴策略待 owner]
  ...
  HLSL register(x#, spaceN)     [measured：4/4 语料]
        ...
        ▼                       [Q-RootShape / Q-Sampler：形态策略待 owner；推导=Rurix 职责（measured：工具链不自动合成）]
  dxc → DXIL 容器
...
- 不裁 §9 任何路径抉择（Q-Space / Q-RootShape / Q-Sampler / Q-Bindless / Q-Gate / Q-Err / Q-File / Q-Inference-vs-Explicit）——均留 owner（RFC-0005 §9）。
