...
3. **崩溃栈未符号化**(2026-07-17 实测已锐化原因,仍 pending):六个 layer 帧仅有 PC + BuildId;「use-after-free / MTE tag 错配」是从信号签名(`SEGV_ACCERR` + `0xb400…` tagged-pointer 地址 + `tagged_addr_ctrl PR_TAGGED_ADDR_ENABLE`)推断的项目侧诊断,非符号级证明。本轮取得官方 1.4.350.1 arm64 `.so`,`readelf -n` 证 BuildId 与 tombstone **逐字节匹配**(崩溃二进制已在手),**但该发行二进制被 strip**(仅 `.dynsym`,无 `.symtab`/`.debug_*`;六帧 PC 全在导出符号之外),llvm-symbolizer 六帧皆 `??` → 精确 Vulkan 入口(vkCreateShaderModule vs vkCreateGraphicsPipelines)**仍未定**,须未 strip/debug build 方可解;草稿保持 `<PENDING>`,绝不硬凑(见 [symbolication_log_20260717.md](symbolication_log_20260717.md))。
...
- **诚实边界**:桌面 VVL **1.3.296** vs 设备 **1.4.350.1**(版本差);桌面 VUID `pCode-08737` vs Android
  ...
  重放。`spirv-val` 拒该损坏模块(word-5 OpAtomicSMax 解码越界)。逐字命令 + 输出已入库:
  [repro_log_desktop_control_20260717.md](repro_log_desktop_control_20260717.md)(good/tampered/no-vvl
  三腿 + 3×重复,2026-07-17 同日两次独立执行逐字一致;`.spv` 工件本体不入库,再生配方在 log 内)。
...
| 4 | Describe the issue | 崩溃瞬间精确 Vulkan 入口点 | **pending(原因已锐化,2026-07-17 实测)**:BuildId `13204c6e…` 经 `readelf -n` 与官方 1.4.350.1 arm64 `.so` **逐字节匹配**(崩溃二进制已在手),**但该 redistributable 被 strip**(仅 `.dynsym` ~140 导出,聚于 `0xf1ec88–0xf1edd4`,无 `.symtab`/`.debug_*`);六帧 PC(`0xb72f08`/`0x1283494`/`0x128a064`/`0x12a291c`/`0x129a0e8`/`0x12df32c`)全落在导出符号之外 → llvm-symbolizer 六帧皆 `??`。**绝不硬凑**;须**未 strip / debug(.sym)build 的 VVL 1.4.350.1** 方能符号化。桌面对照旁证入口在 shader-module 创建路径(pCode-08737 VUID)。逐字命令+输出:[symbolication_log_20260717.md](symbolication_log_20260717.md) |
