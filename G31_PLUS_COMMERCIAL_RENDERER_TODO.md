...
| 90 | **FIF 与动态/蒙皮共存** `[遗漏]` | FIF 拒 `tlas_update`/`blas_refit`；动态/蒙皮被迫顺序提交。**G38 登记**:判档 GPU 双 PASS(`g31_fif_dyn_probe` 三臂等价门 rebuild/refit,evidence = artifacts/day_0830_delivery/w3_deep/fif_dyn/)→ **L2a 已登记 RFC-0030 v1.1(G38)**;加性入口 `submit_with_frame_update_slot_as` 在树,内存预算门条目 g31.fif_dyn.slot_as_group_mem_bytes 占位,g14_3 生产接线计划 = artifacts/day_0830_g38/t2_fifdyn/WIRING_PLAN.md | 每槽实例缓冲 / BLAS 顶点,动态角色也能 N 帧在飞 | P1 | #2+#4+#10 约束面,勿另开「再做 FIF」 |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.2.3 | 2026-09-02 | **G40 ReSTIR 画质补窗 + device cut P2 + G39 两留窗收尾战役交付登记**(artifacts/day_0901_g40;**主 agent 串行承担侦察/实施/验收三层〔本执行环境无子 agent 调用工具,偏差如实登记 CAMPAIGN_LOG L5〕**,GPU 锁内三批 + 收役门 + soak 串行,入役 = G39 收官 HEAD 1478859a **全程零重锚**,不 commit 归 owner,交接 HANDOVER.md):①**T1 ReSTIR 画质补窗三件合一**(消解 G39 HANDOVER §D-2/§D-4;g31_realism.rx +91/−31 + g31_window_present.rs +895/−19 + 新 SPV g31_realism_restir.spv sha 8ac52dc4〔G39 件 5571e875 退役留档;既有 9 下位工件 sha 全等〕;g28_restir.rx/gi/rt 冻结面 0-byte 由收役 restir_wiring 门 g27/g28 双面 0-byte 机器证明)——承载面 reservoir stride 4→12 f32/px(y/w_sum/m/phat + 命中位 + 法线,零新增 binding)+ **T1a host 镜像对拍臂** `--lamp-restir-verify N`(verify 面双份回读 + host 逐像素零射线复算 WRS/重投影/merge 链;16f×33.2M 像素命中 100%、merge 路 90.9%;y 位级硬门 + W/w_sum p100 登记 + phase/resv 双 red-arm 必红;对拍不可常开,墙钟 5-10×)+ **T1b disocclusion 两拒** `--lamp-restir-depth-rej/-nrm-rej`(缺省 0.10/0.80 进链,折入 merge 计数门,阈 0 ⇒ 恒 1 逐字复现 G39 v1 语言形)+ **T1c per-pixel W·phat 钳** `--lamp-restir-clamp`(缺省 0=off;只钳输出消费不动写回四元组,正交性由 clamp4 镜像统计位级不变实测自证);params 扩面 76→80 仅 on 门内,off 面恒 72/56;B3 验收(三跑谱系如实:run1 宿主终端关闭 stdout OSError 中断〔编排脚本 log() 兜底修复〕/ run2 verify 三跑连红主 agent ABORT / run3 六 GPU 段 + judge 段补跑全绿,B3_SUMMARY 覆盖面仅 judge 段、六段绿事实源 = b3_log.jsonl):off==双锚 MATCH(三跑批第三次复现,四代二进制同值)+ on/onmin r1==r2 位级(跨三跑两个二进制恒值,on 臂 digest 换代无锚如实登记)+ verify 六判 + ladder 八臂 k12_off==full19 锚 + dolly 240f digest+242 项 digest_seq 双跑位级 + storm×restir 三并联(G39 D-4 末句消解)+ **judge k26_on render_wall p50 7.841 ≤ 11.11(余量 +3.27;G39 判档 A 钉死的 26 簇交付组合终态树复现;on 臂三档 7.84-8.24 恒平,off 9.79→14.21 随簇数近线性)**;**首红裁决 = 判读器 f32 边界口径过严、产物零缺陷**(px=(1768,1044) 帧 8 host_y=7 dev_y=13,w_sum/m 位级同值,`margin=0.0` 判定量恰为零,Vulkan FDiv ≤2.5ULP 非正确舍入),修正 = 判据两级化〔全等 ∨ 单判定翻转 (y,m) 位级复现 device ∧ |margin| ≤ 32ULP ⇒ y_attributed;未归因 100% fail-closed;ATTRIB_CAP 4096 令红臂不入归因路——收紧非放宽〕+ 代码审查同时暴露 phase 红臂常量 0.618034 与 kernel 黄金比 f32 同值**红臂空转**(改 0.62,resv 篡改改 y+1/m+1,修后双红臂 rc=1 真实必红)+ schema verify_stats v2(y_unattributed const 0 钉进 schema;二号补丁 _patch_g31_window_evidence_schemas_g40.py 幂等 properties-only 不进 required,check_schemas PASS);**两条负面如实不进全绿叙述**:画质收益为负(dark ROI p95 on 臂 2.1-2.7×,k12 −108%/k26 −164%/k38 −167%,G39 D-3 方差税未消解继承留窗)+ T1b 边缘改善 ≈0(rej vs norej 四 ROI ±1% 内低于机态地板,命题未证实);T1c 缺省维持 0(clamp4 本场景几乎不触发;mcap4 −0.41ms 换 +41.9% 噪声**不推荐作画质手段**),缺省裁决归 owner ②**#77 device cut P2 生产 dispatch**(frame_cut_arm +495/−69 + probe +12/−7;kernel g31_cluster_cull.rx 冻结/rt/lane_body/窗口 bin/schema 0-byte;缺省 `--cut-source host` 0-byte)——同旗标 device 语义升级 P1 对拍臂→P2 决策码为源:独立常驻 cull 会话(10 资源 kernel 布局字面继承,表 device_local 驻留 staging 一次上传;每帧仅 params 256B 上传 + 决策码 ~493KB 回读)+ host 由 d==4 构造 cut 集(d∈{2,4} 闭集断言)→ verify_cut_coverage 影子核直跑回读集 fail-closed 逐字保持 → min-level 提升/差集/上传/refit 施加链 0 改;cut_ms 拆 select/verify/promote 三分项 + device_cut_dispatch_gpu_ms(DESIGN §4-2 登记义务;P1 device_cut_probe_ms 随 run_compute 路退役,决策码 sha 保留审计面);selftest ⑧ 段新增;B1 六判:dev 16f digest==host 位级 + 臂内建/跨进程双跑 + ×ml1 + incr==full + **跨 G38 t3_incr/G39 t5_dev 双锚 MATCH**(免重锚推导链 DESIGN §3.1 落地)+ red-arm 篡改⇒决策翻转⇒覆盖性必破必红(报文形态变化如实)+ 缺省 host 回归 + 窗口臂 5540ecae(B1 首验 W0 二进制,B2 附录新树复验闭合时序缺口);帧时 measured 登记不判红:**cull dispatch GPU 0.113ms**(P1 run_compute 82.7ms 上界→三个量级降)/ ml0 device cut_ms 8.179 vs host 7.874 **无净收益**(fence+回读税抵消,DESIGN §4-1「下沉不解预算」维持)/ **ml1×device 组合墙钟 16.095ms 落 DESIGN §4-2 预期带 15-19ms**(G38 ml1 host 23.7-27.3ms 对照;UPDATE build 地板仍在不冒充进预算);**P2 收口,P3(verify device 化/直写竞技场)留窗** ③**skin/dyn 生产规模 AS 副本内存 evidence 补登**(G39 D-7 前半;lane_body +99/−1,禁改 render_exec* 0-byte)——session telemetry allocation ledger 过滤式(probe slot_as_mem_from_ledger 同源)collect 半程首帧采集 + bench receipt `slot_as_mem` 空串注入位(非 slot_as 臂 receipt 字节 0 漂,inflight=1 负控无键);B2:dyn ×2/×3 = 240,614,400/360,921,600B、skin ×2/×3 = 240,621,312/360,931,968B(120.3MB/槽,skin +3,456B/槽 = 角色 updatable BLAS 面差,「数百 MB 级」预告兑现)+ flip-trace 六臂对 G39 B1/B2 在案件位级全等 + skin_verify all_pass;预算门条目 g31.fif_dyn.slot_as_group_mem_bytes probe 口径 44,544B **0 动不混**(差 ~2,700× 分口径必要性自证) ④**bridge_ext×FIF rt 平行入口评估**(G39 D-7 后半;纯文档 t4_bridgeext/EVAL_BRIDGEEXT_FIF.md 零代码/零 GPU)——形态案 = 加性第四入口 submit_with_frame_update_slot_as_bridge_ext(None 路逐字等价既有 slot_as 入口;G39 T3 折叠先例 ~+80-150 行 ≤0.5 窗)+ 决策语义与 WP §4 NO-GO 结构相反(copy_regions host 当帧纯函数产出,零回读反馈环)+ src 写面竞争为真约束两解(per-slot staging/×S 副本 或 GPU 写 src)+ 判据推导链与 T2 §3.1 同构零重锚 + 诚实风险「收益面窄」(skin 脏区≈全量 incr≈full、frame_cut 为顺序单槽非 FIF 车道 ⇒ 当前无「稀疏 dirty×FIF」真实消费者);**建议机制 GO / 开窗条件 DEFER(条件 = 首个稀疏 dirty×FIF 真实消费者成立),判档段 §6 留 owner 回填不代填** ⑤收役:W_GATES PASS(profiling **1020s N=5 中位十轮全 identity_ok 第二次真跑绿**〔g31 中位 1.370/g14 0.109,重标定预案未启用〕/ wp_hlod 870s / g36_geo 1388s / restir_wiring 47s〔G28 门维持 y 锚 20000/20000 + g27/g28 双面 0-byte〕四门 + framecut probe host 四臂 + **device 臂终态树** incr==full 16f 位级+双跑+dev==host+跨 G38 MATCH + fif probe 双臂 gates 全 true trimmed_mean 44,544B + calibrate --check + CPU 守卫 7/7〔check_schemas 含 v2 补丁 / budget_eval 330 pass 0 skip〕)+ soak **1930.6s≥1800s**(终态树 17:21 新建 exe;32f 锚 a8204b3b 钉死 **6/6 恒值** + 前置 all-off/full19 双锚 + it4 Stage A 探针 MATCH〔it9 未及如实登记〕+ VUID=0 fails=0);**soak frame_ms_max 12.393 > 11.111 记账如实不判红**(verdict 不消费该字段;digest 8/8 恒值;同锚语义面在案对照 W0 11.125 / run3 10.824 / ladder k12_off 9.996〔p50 9.785 进预算〕/ G39 soak 10.798,散布 9.8-12.4ms 即机态噪声;soak 口径 32f 短跑 GPU 自 P8 爬坡 + 宿主并发桌面负载未控制变量;帧时验收法定口径仍为阶梯 render_wall p50);构建面事实登记:同源三次构建 exe sha16 互异(cc39f6dc/66e94de5/7c77870a)非位级可重现,渲染语义以 digest 门为准;t1_restir2 ab 逐帧 p.raw ~2.0GB gitignore 排除(G38/G39 同律);工作树无兄弟任务在途面,入役即在树的 G39 两件判档登记改动由 owner 一并择入。既有行字面 0-byte 不回写 |
| v1.2.2 | 2026-09-01 | **G39 ReSTIR 生产接线 + G38 四留窗收尾战役交付登记**(artifacts/day_0831_g39;主agent+两层子agent编排,GPU 锁内五批+收役串行,入役=收官 HEAD 82a59ae3 **全程零重锚**,不 commit 归 owner,交接 HANDOVER.md):①**#7/M100 车道集成窗兑现**——窗口 bin `--lamp-restir off\|on`(+`--lamp-restir-mcap` 默认 8 闭集 [1,64])高档时域 reservoir 提灯臂:g31_realism.rx 第 9 链位 gate 化时域段(M=8 闭式 WRS〔R2/R3 黄金比共轭相位,零跨帧 RNG 状态〕+ prev-VP 重投影〔g14_mv 同式〕+ m_cap 截断时域 merge〔host merge 同义〕+ 1 条验证射线 + W=w_sum/(m·phat) 三算术门形)+ 新 SPV 第 10 工件 g31_realism_restir.spv(sha 5571e875,字节隔离律:off 各臂恒载既有锚定字节,既有 9 工件 sha 全等)+ reservoir ×2 parity 双缓冲(TSR 同律,初值零读门)+ prev_vp 64B 小 SSBO 逐帧上传 + params [72..76) 纯追加(56 槽锚面不扩);依赖集 fail-closed(须随 lamp-lights+smooth+tex;--fg 互斥〔FG 静态下标族撞位新增卫兵〕;--gi2 非依赖按 kernel 实况;不进 full dup 表 ⇒ full19 锚零漂);B3 验收:off==双锚 MATCH + on r1/r2 双跑位级 + dolly 240f digest_seq 位级 + VUID=0 + 阶梯 12/26/38 簇(grid 标定 0.10→38)×on/off——**26 簇档 off 11.546ms 超线 → on 7.526ms 进 11.11 预算(+3.58 余量);on 臂帧时与簇数解耦 7.4-7.5ms 恒平,「提灯数须 ReSTIR」机制兑现**;方差如实登记 dark ROI p95 噪声 ~1.5-2.6×(1-spp 换选代价,TSR 部分吸收,不判红)+ 软阴影半影让位/玻璃硬影两新税 + host 镜像对拍臂留窗;首红裁决 = prev_vp 误带 device_local 帧 0 fail-closed(接线 bug 非判读器,两处 true→false 修复 off 路径零触碰);**lamp-k 提档提案判 GO 只登记留 owner**(t1_restir/LAMP_K_PROPOSAL.md:A 维持+B 留窗〔重锚合批〕,26 簇进预算显式组合已钉死可交付)②**#90 skin 批次 B 收口**——WIRING_PLAN §1-A6+§2-B2 兑现(lane_body +704/−120):skin_rec_from_output 提取/prepare_update_skin scene_as_override 推 (1,b)〔skin scene pass=1,新字段 skin_scene_bindings〕/submit_frame_skin_slot_as(blas_refit base+slot 逐帧换槽,副本组逐表项 updatable_blas=[1])/PendingSkinFrame+collect/核验闭包凭帧号复算 palette;CLI fail 块解除(WIRING_PLAN 锚过期以在树字面为准登记);B1 验收:skin 1\|2\|3 flip-trace **x1≡x2≡x3 逐字节 + 各臂双跑位级 ⇒ skin refit 实测纯函数免 L2a 降档** + skin_verify all_pass + 窗级真动门 + VUID=0 + G38 旧轨迹跨役字节对照 MATCH;生产规模 AS 副本内存 receipt notes 登记面与 probe 预算口径不混 ③**slot_as 单源折叠**(fif_dyn REPORT §7-3 治理项)——submit_pipelined_frame 加 Option<&SlotAsGroup> 末参吸收三插入一换向(render_exec.rs +125/−14),复制体 g37_submit_pipelined_frame_slot_as 连 doc 整删 378 行,公共入口/SlotAsGroup/校验器 0-byte;B2 零语义门:静态 FIF/dyn rebuild/refit/skin ×1\|2\|3 折叠后轨迹逐字节==折叠前基线 + fif probe 双臂 digest 序列==B1 ④**profiling 门 identity 判据多轮中位鲁棒化**(G37 W6 以来唯一诚实红处置)——单轮→N=5 轮中位消费(identity_ok 规则与容差 [−0.10,2.00] 字面 0 动,四面同源 0-byte git 证明;--rounds 闭集 [1,9]),fact ⑤ 加固 off 锚×on 全 N 轮位级只加严,evidence 可选块 identity_rounds 纯追加(_patch 幂等,存量 evidence 免疫);B4 真跑 **GATE PASS**(863.9s):g14 第 4 轮 residual −0.2499 单轮越界被中位 +0.0913 吸收,g31 五轮全绿——**「单轮抖动非系统性偏移」定性实证成立,鲁棒化机制当场兑现**;若再红重标定预案在档(REPORT §5,归 budget 程序窗禁凑绿)⑤**#77 device cut**——段 1 DESIGN.md 判 GO(host cut=相机纯函数零回读反馈环,与 WP §4 NO-GO 结构相反;诚实边界 = UPDATE build ~21ms 地板主导,收益须与 --min-level 组合)+ 段 2 P1 probe-only `--cut-source device` 决策码回读对拍臂(三关超集 kernel 0-byte 消费,params/数据域中和判定码收缩 {2,4} 闭集,rurixc 现编 SPV 运行时装载,frame_cut_arm +594/−12);首红裁决 = 对拍臂域检误拒根簇合法 f32::MAX 编码(判读器口径错产物零缺陷,撤域检回归 harness 上传律,消费谓词两侧同向饱和判定码逐位不变);B5:C1 判定码逐项全等×16f×双跑+decisions 闭集/C2 dev==host digest+跨 G38 t3_incr 锚 MATCH/C3 red-arm 期望码驱动构造性必红/C4 ×ml1/C5 缺省 0-byte 全 PASS——**P1 等价门收口,P2/P3 留窗**(device_cut_probe_ms 82.7ms 上界参考)⑥收役:W_GATES PASS(profiling/wp_hlod/g36/restir_wiring 四门 + framecut incr==full 16f 位级+双跑+ml1+跨 G38 MATCH + CPU 守卫 7/7〔budget_eval 330 pass 0 skip〕;fif probe 由 B2 终态树承载登记)+ soak **1936.2s≥1800s**(32f 锚 a8204b3b 钉死 6/6 恒值 + 前置 all-off/full19 双锚 + it4 Stage A 探针 MATCH〔it9 未及如实登记〕+ frame_ms_max 10.798≤11.111 + VUID=0);t1 ab 逐帧 p.raw ~1.2GB gitignore 排除(G38 同律);工作树兄弟任务在途面(g35 三件+day_0831_site)不混登记。既有行字面 0-byte 不回写 |
| v1.2.1 | 2026-08-30 | **G38 五任务推进战役交付登记**(artifacts/day_0830_g38;主agent+9 子agent编排,GPU 锁内三批串行):①**#90 收口**——RFC-0030 v1.1 L2a 正式登记(草案转正+§9.2 修订行+G14PLUS_RECORD §7)+ g14_3 dyn 臂 slot_as 生产接线(inflight 1/2/3 × rebuild/refit 双硬门逐帧 digest 逐字节等+双跑位级;refit 实测纯函数免降档;skin 批次 B 留窗 WIRING_PLAN 在案)+ 每槽 AS 内存预算门(probe v2 evidence slot_as_mem 账+budget 条目 measured=44,544B threshold=66,816 程序产,budget_eval 330 pass)②**frame_cut 增量 refit**(#77/#89 向)——桥接 copy 多 region 化(75MB→脏槽差集,copy GPU 4.348→0.022ms)+ 独立时戳拆解(UPDATE build 地板 21.6ms@2.08M tri 主导项 measured)+ --min-level 降档臂(1 档 arena 减半 build 8.78ms/exec 9.54ms ≤ 11.11ms 预算,90fps 进预算交付形态);incr==full 16 帧位级+跨进程双跑+窗口臂命中 5540ecae 锚(presented 零漂铁证)③**#96 消费面收口**——RXCS/RXCP/RXHL 三格式 v2(UV 加性,v1 路径 fc /b 逐字节不变自证)+bake 双变体 --attrs(双构建字节相等)+运行时 gather_tri_uv_attrs/geo_patch_proxy_tritex_v2(v2 资产代理带真 corner UV 走图集采样,tritex=−1 强制回退退役 patched=0,kernel 0 动);g34 三验收锚 PASS+wp_hlod/g36 两门全量回归 PASS ④**RIS/NEE 画质定量**——四臂无 AE A/B(全双跑位级):方差收缩 presented u8+TSR 后口径 nee dark p95 +2.43%/mean +5.94%、both +2.09%/+4.88%、ris 单开噪声级(口径诚实登记:dark ROI std 0.25~0.6 LSB 贴量化地板+TSR 已吸收输入噪声差);帧时反直觉 measured:ris −1.67ms/nee −1.31/both −0.65 更快;lamp-k 阶梯六档(RURIX_G31_LAMP_GRID_M 旋钮+散臂):16 簇贴线(同 digest 档帧时差 1.3ms 机态噪声≥余量)、26 簇确定超线 −1.9ms ⇒ **默认不提档(维持 12/0.6),ReSTIR 前置条件 measured 登记**(#6/G21/G28 开窗证据) ⑤**法线 v2 消费切换+整批重锚**——normal_dir 默认字面切 baked_normals_bin_v2(slot14 修复消费面兑现);G38_ANCHORS.json:full19 `a5521e4708a814e364fd3bf95b18f0ab69b6646efd039246e8686533c30e4fb1`(谱系 7636f72f 作废)/RD-045 `066395b0…`(双二进制同值,谱系 ef2b5b19 作废,blocked_probes L68 回写+门 PASS);负控 all-off/bench/StageA 探针全 MATCH+ris_nee/transparency 单开不动抽验 MATCH(v2 只动 nrm 面机器证明);day_0829 nrm 历史锚(dca78cbe 等)谱系作废不回写。三判读器口径修正如实登记(B6 桥窗字节/B7 s09 口径/transparency 臂形,均产物无缺陷) |
| v1.2.0 | 2026-08-30 | **G37 商业化交付收官战役交付登记**(day_0830_delivery;默认翻转获批执行 + 剩余零散模块接线 + 深水区判档 + 已知问题可修面清零 + SDK bundle 候选;主agent + 23 子agent两层编排,全程 GPU 锁内,战役日志 artifacts/day_0830_delivery/CAMPAIGN_LOG.md,交接 w6_final/HANDOVER.md):①**默认翻转执行**(DEFAULT_FLIP_PLAN 六步兑现)——窗口 --quality 缺省 off→full **十九臂**(十六臂 + transparency/gi2-ris/gi2-nee 并入),off 升显式回退档,CI 调用面 A 类 18 点补 --quality off(B 类 5 点语义随翻转升级登记);**锚整批重收割**:full 新锚 `7636f72f`(96f 双跑位级,帧时 9.75/10.59ms ≤ 11.11ms 预算)/all-off `55e4a92d` 与 bench `c1d28ad7` 不变/RD-045 P02 新锚 `ef2b5b19`(release+target-night 双二进制同值,blocked_probes 字面回写门复跑 PASS)/tex 臂 `ac2e5ff5` 登记(判读器"不消费固定锚"定为长期形态);②**七组新臂接线**(off==锚+on 双跑位级+VUID=0 全验):--transparency 玻璃 ray 穿透真解(≤8 层透射+点光阴影衰减,mat7 130,792 tris 精确命中,day_0829 §H 雾状楔形缺陷收口)/--lut 色彩分级(#79 五级链收口,LUT 内嵌 enc_params 零新绑定,neutral==off 位级恒等证明)/PSO 变体账本(#82/#113:era0 precache+era≥1 告警,full×storm3 实测 pso_runtime_creates=0)/--visbuffer 生产证据臂(#74/#111 档 2:窗口会话真轨迹×真 RXCP device 真跑机制链,presented 0-byte)/--gi2-ris+--gi2-nee(#6 修复:反弹 RIS 选灯 M=6 + 44k 灯片 CDF 面光 NEE,EVAL_RESTIR §9.3 路径)/--cluster-per-frame-cut(#77/#89 合流最小面:BLAS 顶点 refit 竞技场 2M tri,逐帧 cut 顶点切换,16 帧双跑位级+单调+refit 27ms measured 登记)/FG×full 两点式组合(G34 out-of-scope 行兑现:comp parity 双缓冲,fg off 生产路径 0-byte,验收八判据 PASS);③**G36 五留窗全处置**:FIF×动态判 GO(#90:每槽 AS 副本 rt 加性入口+probe 三臂 rebuild/refit 双 PASS B/C≡A 逐字节;RFC-0030 §4.3 L2a 修订草案)/HZB×蒙皮同车道 GO(g34_unified_primary_skin 新 kernel+render_exec blas_refit_b 加性,ACCEPT 七 facts:剔除像素中性 74 帧位级+单开不降级)/FG 组合 GO/#96 属性保持简化 crate 面(TriMeshAttrs+QEM 线段插值,45/45 测试,m90 golden 零漂,消费面分界登记)/逐帧 cut→AS GO;④**异步三件套 M59 判档 = 维持 no-go + 新鲜 measured**(D3-Q7 从证据零升级为在案低于阈值):digest 等价硬前置全过(single==dual==CPU 参照逐字节,机制正确性证明)+重叠率中位 48.54%<50% 阈值;timeline 提交器/(2v-1,2v) 值域合法化层+validator/compute-only family 探测入库,RXS-0239 修订草案留档;⑤**已知问题修复十件**:G12 PT 卫兵基线升 058f8e68(必红→绿)/gpu_device_lock 解锁重试+自述容错+stale 回收(selftest 2红1绿)/em+AE override 错位(_EM 分支,day_0829 §G.1)/11 断言 debug→assert(§G.2)/rurixc「if 包 while」codegen(structured_merge 回边裁剪,526+108 测试绿,98 kernel pre/post 90/90 位级零漂,绕行未摘)/slot14 法线损坏件 v2 烘焙(69 张逐字节相等+平坦替换,L1 范数检测律;消费切换留下一重锚窗)/g34 fx-fy 三面同源修(kernel 6 处+host 镜像 2 处+B4 探针原形态 SPV,同错抵消面消除,g34×3+g36 门复跑 PASS)/encode 共享件收编 v2(43b0c255→e7291c79,RD-045"旧二进制"实已被 Phase F 事故覆盖之侦察修正)/encode parity 硬门转正(g31.g37w1.encode_parity PASS,ACES 转置防复发红臂)/纹理判读器 heap 同步(77 臂 selftest,6fab598c 字面清理);⑥**商业化面**:GAP-01~03 许可义务闭合(THIRD_PARTY_NOTICES+CycloneDX 内嵌视图+许可字面一致化,门 7/7 PASS;#53 附带义务闭)/SDK bundle 候选 sdk-1.1.0(组件 16→24,schema v2 版本化+路由序机核,selftest 67 臂,2.26MiB digest 闭环+双 SBOM+install 四级校验,签名如实降级登记;#52 候选面)/docs 5 篇刷新+PRODUCT_NOTES 新增(#49 同步)/release notes/RD-036 判档维持 open;⑦**判档如实登记**:VSM 页管线 no-go/maintain-defer(#104/#106:ray 车道无消费面,g31_vsm_device_probe 三腿 GPU PASS 补 dirty_depth 轴)/tsrq clamp K 档关死(#30:K∈[1.5,3] 近惰性,降噪投资转第 1 级);⑧不可为项 12+3 行如实登记(w6_final/IMPOSSIBLE_ITEMS.md);W6 门矩阵+soak @新默认见 W6_GATES.json。既有行字面 0-byte 不回写 |
| v1.1.9 | 2026-08-28 | **双仓对账：外部宿主采纳事实回溯登记**（新增 15_EXTERNAL_ADOPTION_REGISTER.md 登记件；动机 = 引擎仓 RurixForge（D:\游戏引擎）单侧登记消费事实，本仓治理面此前零记录）：①**#48 DoD 工程事实面首次获得真实承载件**——RurixForge 以 path 依赖消费 rurix-rt(vulkan) render_exec / rurix-physics / soft-raster / rurix-asset / rurix-geom-build / rurix-pkg 与 rx/rurixc CLI（视口实渲染 + VK→D3D12 零拷贝帧通道 + 物理固定步 + gltf→RXGB .rxmesh 构建链），引擎仓 evidence/f1-w2-viewport-smoke-20260819-164157.log PASS（RTX 4070 Ti，两帧逐字节一致 + presenter presented=3/3 + OS 截屏锚点逐字节一致）+ F1 四波全验收；宿主为 Rust、同作者跨仓——#48 行字面是否翻转留 owner 治理程序，不预支。②**#56 维持未宣称（0-byte 不动）**：RurixForge 为同作者维护，不满足「非作者维护」字面，本登记不改变其状态。③**render_exec R4–R7 上游补面回流 provenance 回溯登记**（15_EXTERNAL_ADOPTION_REGISTER.md §A2）：VK_KHR_external_memory(+win32) 启用 + new_with_imported_d3d12_textures + pNext 注入 + vkBindImageMemory 返回值守卫（~L12401）源于引擎 F1 wave.3（2026-08-17）当日补面（补丁脚本 _f1w3_upstream_patch4~7.ps1 已随引擎仓落账清理），此前本仓治理面无来源标注；图像探针/堆腿随后随共享体改线性 buffer 在引擎侧退役（引擎 f1_zerocopy.rs:264 留痕）。既有行字面 0-byte 不回写 |
| v1.1.8 | 2026-08-28 | **生产渲染画质战役交付登记**（曝光/死黑/异常色块/噪点四问题生产修复；夜巡五臂 + 六相 A1/A2/A2b/B/C/D + E1 终局合流；战役日志 artifacts/day_0828/CAMPAIGN_LOG.md，总验收 artifacts/day_0828/e_final/E_ACCEPTANCE_SUMMARY.json；独立评审「有条件合入」条件已全闭环——C-1 EOL 六件主线归一 + C-2 RD-045 腿复跑绿）：①**九臂加性画质面**（默认全 off = 冻结面 0-byte，Stage A 18 格锚 18/18 零漂移终验在案）——`--lamp-lights`（44k emissive 灯片确定性聚类→≤K 代表点光 I=Φ·gain/4π + 逐灯 radius 阴影截断，scene_gpu +1.93ms）/`--auto-exposure`（log-luma 两微 pass EMA，增益走 encode params[133] 预留槽**零新增绑定**）/`--textures`（texel heap 单 SSBO 全 70 材质 100% 三角 + DDS 源 mip 直搬 282.7MiB + fx/fy 双线性 bug 自有面 5 处修复；色块 26 块/13.62%→1 块/0.29%）/`--gi2`（R2 帧旋转 1 反弹间接光+单灯 NEE+firefly clamp，暗区噪点 std_p95 −77~−90%，scene ×1.65）/`--tsr-quality`（Karis 反亮度混合 + 稳态 α 档〔母版 min_alpha 地板构造性不可达，缺陷已登记〕+ 深度验证 3×3 膨胀区间化，gi2 微光点 −87~−97%，帧时 ~0）+ 夜巡 `--dither/--smooth-normals/--ggx/--bloom`/env 环境光；②**ACES 1.3 样条转置 bug 修复**（g31_display_encode.rx 12 处 b1/b2——「蓝扇叶/灯罩环带/暗部系统性提亮」根因；v2 SPV 路线隔离共享 m_c 件 0-byte；device-vs-host parity 探针 2,073,600px exact 99.9891%/p100=1LSB/>1LSB=0——encode_parity_probe.py 转正 CI 门候选）；③**`--quality off\|full` 预设**（窗口 = 九臂一键展开〔含 --lamp-gain 4/--gi2-clamp 0.01〕，bench = 质量腿子集〔textures/bloom/dither/AE 无 bench 车道如实注释〕；解析层展开先于全部校验/换载 ⇒ 与显式旗标**位级等价**〔窗口 9e5f6300 ×4 跑 / bench 1c12b7fd ×3 跑，含预设 ambient 自注入 == 显式 env 双证〕；展开面旗标重叠 fail-closed；RURIX_G18_AMBIENT 缺席自注入 0.004 显式 env 优先〔lane_body OnceLock 槽，forbid(unsafe_code) 下 set_var 不可用〕；storm×textures 互斥解除并风暴验收 rc=0/resize_eras=1/validation 静默，svt×storm 转显式 fail-closed）；④**E1 回归/soak 终验**：Stage A 全 18 格锚检 18/18（6 tsr 隔离先行 + 12 vendor 批跑，dlss 格各 1 次 VUID 计数留观察）、窗口 all-off==55e4a92d、bench 默认==c1d28ad7、snrm==778f1dfc、snrm+tsrq==05532d5e 全复验、soak ≥1800s（13 迭代 ×32f + 4 次 Stage A 探针，1895s 零失败 digest 恒值）、g31_blocked_probes_smoke 门 rc=0（RD-045 数字腿 060e69a8 复跑闭环）；⑤**治理登记**：窗口纹理合流臂 presented 锚 = **二进制绑定锚**（D 相终态重建一次性 ULP 漂移经三二进制 A/B 归因在案；6bd3af63/8b1c12f3 作废 → 9e5f6300/d89848b9 多跑重收割，e_final/e2_reanchor_registry.json）；默认翻转治理方案只案不执行（e_final/DEFAULT_FLIP_PLAN.md：RD-045 P02 腿 + 三处源码-锚字节 divergence 收编步骤 + 作废锚清单 + 回滚）；交接清单 e_final/HANDOVER.md（g34 三 kernel fx/fy 同源 bug/纹理判读器同步/SVT×heap 互斥/rurixc「if 包 while」缺陷/gpu_device_lock PermissionError/母版 min_alpha 不可达/R2 f32 frame_idx 精度/Karis HDR 偏/地板尾噪/quad NEE 缺口等 21 项）；hero 对照 e_final/hero_campaign_before_after.png + 四问题特写 closeup_*.png；磁盘清理 59.37GiB（逐帧 EXR，converged+receipt 全保）。既有行字面 0-byte 不回写 |
