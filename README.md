...
> **状态勘误(2026-08-25,只追加不回写;五期串行战役第四期行)**:上方 G28 勘误行状态止于 G28 收口——**G29(材质 device 集成期,五期串行战役第四期,`milestones/g29/`)已于 2026-08-25 收口**(`status: closed`,close-out flip 见 [`milestones/g29/G29_CONTRACT.md`](milestones/g29/G29_CONTRACT.md) §8.7;tag `g29-closed`):五 P0 全绿(M-a slab device kernel **implemented**——`kernels/g29_slab.rx` 真跑:对拍 p100=1.192e-7〔恰 f32@1.0 一 ULP〕≤ 冻结容差 2.384e-7〔程序产 measured×2.0〕+ 角点 rc=ab=1 device **位级 1.0**〔RFC-0046 blocker F2 修法 A:删算术门 + max(denom,1e-30) 分母安全化,在真 GPU 兑现〕+ 输出有限性一等断言〔F3 先于聚合硬 FAIL〕+ 白炉行 dev=1.19e-7 如实登记〔OpFDiv ≤2.5 ULP 来源注明〕+ 能量上界 + 双跑位级 + RED 臂 + material/ 整目录 0-byte 机核,RD-041-slab「device kernel」件兑现;M-b 侧表供参加性臂 **implemented**——16 槽 bin-local 独立 SSBO〔MaterialClosure 32B 与 reserved 拓扑位零触碰 + 生产侧表 side_table.rs 零挂接防混淆〕+ 逐槽对拍 p100=3.68e-8 + 逐槽白炉互核双端登记,RD-041-slab「侧表集成」件兑现〔生产集成 out-of-scope 维持〕;M-c SVT 四行 + KTX2 三行重判**全维持 defer**——常量 pattern 表 + 锚派生关键词映射〔F6 三件〕七行零实现树内实测 + g22 两表 0-byte + RD-041 history G29.3 只追加断档口径注明;M-d WG capability 复测 **not-available 维持**——VK_AMDX_shader_enqueue 新鲜 vulkaninfo absent〔与 G22 在案零漂移〕+ DGC 三扩展全 true 互核 + FSR 3.1.5 maintain 盘点;M-e 旧门零降级)+ P2 穷举 **7 行零空行**(closed-go 7)+ soak(53 迭代 1919.2s ≥1800s 零失败,八车道探针扩容穿插 10 次〔四实现件 + framegen/hzb/restir/slab 四 device 快车道〕)+ close-out 终审(**八 facts VERDICT=READY**);RFC-0046 经 D-409 对抗评审 11 findings〔含 1 blocker〕全 disposition(v0.2 修法批)Agent Approved;RD 八条 open 维持;cargo test 486 passed 维持。各门终态以 [`milestones/g29/G29_CONTRACT.md`](milestones/g29/G29_CONTRACT.md) §8.7、`G29_P2_DECISIONS.md` 与 `registry/` 为准,旧行按只追加纪律不回写。

> **状态勘误(2026-08-25,只追加不回写;五期串行战役收官期行)**:上方 G29 勘误行状态止于 G29 收口——**G30(商用终审收官期,G26-G30 五期串行战役收官,`milestones/g30/`)已于 2026-08-25 收口**(`status: closed`,close-out flip 见 [`milestones/g30/G30_CONTRACT.md`](milestones/g30/G30_CONTRACT.md) §8.7;tag `g30-closed`,**五期战役收官**):五 P0 全绿(M-a 尾锚六件重判闭集**全维持**——九组常量 pattern 表〔F6 三件 + F10 门态映射 + F8 锚源钉死〕:M125 maintain-5.3〔三类需求证据全空 1/3 + sys56 cargo check 绿 + A/B latest 盘点〕/ M127 研究子轨〔corpus 四目录 + 消费方 token 两半 0/2〕/ M114 card/mesh〔外部盘根不可达 SKIP 三态兜底 F15〕/ M118 maintain-SDR〔vulkaninfo 新鲜三 token absent〕/ G10-N6 双场景闭集〔三工具 PATH 缺 + 源资产 0 命中〕/ SAFE-GPU defer 改锚 defer-to-G31+,RD-042/043/044 同批零命中 history 只追加幂等〔F9〕;M-b 三面商用终审——画质十项 vs g25-closed 0-byte + 两层零接线〔F2:四 token 沿用 + 战役九件名字面〕⇒ G18 达标终态维持,性能焦点格 canonical 160 帧新鲜真跑 frame_ms=3.5767ms 新鲜 ratio=**0.960479 < 1.00** + 两半锚 6 pattern 零命中〔F3〕⇒ **G17-MD-F1 = 17/18 诚实红终判定盘**〔G26 carry 兑现,合法收官态零冒充〕,确定性 Stage A 18/18 + 四 device 双跑位级绿件 + RD-045 复核〔F14〕;M-c 全链零回归——11/11 tag + verify g29 链 + budget --strict 301 pass 0 skip 禁 --allow-pending〔F18〕;M-d 承接归档——`g30_campaign_handover_registry.json` 9 期行 + rd 八条 + tail 六行分 section 字段闭集〔F5〕= **G31+ 唯一法定输入面**;M-e 旧门零降级)+ P2 穷举 **12 行零空行**(closed-go 12)+ soak(67 迭代 1817.7s ≥1800s 零失败,八车道探针穿插 13 次)+ close-out 终审(**八 facts VERDICT=READY**);RFC-0047 经 D-409 对抗评审 18 findings〔major 5〕全 disposition(v0.2 修法批)Agent Approved;RD 八条 open 维持;**战役总结**:五期 80 CI 步骤(445~524)全数接线、四 device kernel + 侧表臂 implemented、五期 soak 累计 315 迭代零失败、RFC-0043~0047 共 63 findings 全 disposition、诚实终态零冒充(总结全文见 G30_CONTRACT §8.7 ④)。各门终态以 [`milestones/g30/G30_CONTRACT.md`](milestones/g30/G30_CONTRACT.md) §8.7、`G30_P2_DECISIONS.md` 与 `registry/` 为准,旧行按只追加纪律不回写。
