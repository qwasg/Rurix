#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# G31+ 波 C Task C3：check_schemas.py 三处纯追加注册（io.open 补丁法——
...
'''
LOAD_ADD = '''    # G31+ 波 C Task C3 设备兼容矩阵与能力降级链门前缀纯追加（重放幂等面；与既有
...
'''
VALIDATOR_ADD = '''    g31_capability_fallback_validator = (
...
def main() -> int:
    ...
    for name, anchor, add, token, before in [
        ("load", LOAD_ANCHOR, LOAD_ADD, "g31_capability_fallback_schema = load(", False),
        ("validator", VALIDATOR_ANCHOR, VALIDATOR_ADD, "g31_capability_fallback_validator = (", False),
        ("route", ROUTE_ANCHOR, ROUTE_ADD, 'f.name.startswith("g31_capability_fallback_")', True),
    ]:
        n = src.count(token)
        if n == 1:
            print(f"[patch_g31_capability] {name} 已驻留（幂等跳过插入）")
            continue
        if n != 0:
            print(f"[patch_g31_capability] FAIL: {name} token 驻留数 {n}（≠0≠1 拒改）",
                  file=sys.stderr)
            return 1
        if src.count(anchor) != 1:
            print(f"[patch_g31_capability] FAIL: {name} 锚出现 {src.count(anchor)} 次（≠1 拒改）",
                  file=sys.stderr)
            return 1
        src = src.replace(anchor, (add + anchor) if before else (anchor + add), 1)
        print(f"[patch_g31_capability] {name} 插入完成")

    # 驻留核验（各 1）+ gpu fallthrough 单例 + 语法编译。
    misses = []
    for name, token in [
        ("load", "g31_capability_fallback_schema = load("),
        ("validator", "g31_capability_fallback_validator = ("),
        ("route", 'f.name.startswith("g31_capability_fallback_")'),
    ]:
        n = src.count(token)
        if n != 1:
    ...
    if misses:
        for m in misses:
            print(f"[patch_g31_capability] FAIL {m}", file=sys.stderr)
        return 1
    ...
    for token in [
        ...
        if token not in back:
            print(f"[patch_g31_capability] FAIL: 重读未驻留 {token!r}", file=sys.stderr)
            return 1
    import py_compile
    py_compile.compile(P, doraise=True)
    print("[patch_g31_capability] PASS：三处纯追加驻留（load/validator/route）+ 重读核验 + py_compile 绿")
    return 0
