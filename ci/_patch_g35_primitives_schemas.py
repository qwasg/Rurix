...
# G35 GPU 粒子系统 G35-1：check_schemas.py 三处纯追加注册（io.open 补丁法——
...
LOAD_ANCHOR = '''    g34_skin_unified_gate_schema = load(
        ROOT / "milestones/g34/g34_skin_unified_gate_evidence_schema.json"
    )
'''
LOAD_ADD = '''    # G35 GPU 粒子系统 G35-1 基元库门前缀纯追加（重放幂等面；仅门裁决件
...
PROBE = "g35_primitives_gate_schema = load"
...
def main() -> int:
    ...
    if PROBE in s:
        ...
        print(f"[patch_g35_primitives] 已驻留，核验 {'PASS' if ok else 'FAIL'}")
    ...
    if missing:
        print(f"[patch_g35_primitives] 锚缺失: {missing}", file=sys.stderr)
        return 1
    ...
    print(f"[patch_g35_primitives] 应用完成，重读核验 {'PASS' if ok else 'FAIL'}")
    return 0 if ok else 1
