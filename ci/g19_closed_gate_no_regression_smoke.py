...
ROOT = wel.ROOT
GATE_KEY = "g19.p0.m_e.closed_gate_no_regression"
NUMERIC_STEP = 344
SUBJECT = "g19_m_e_closed_gate_no_regression"
WAVE = "G19.4"
SCHEMA_PATH = ROOT / "milestones/g19/g19_m_e_closed_gate_no_regression_evidence_schema.json"
SOURCE_REF = "G19_CONTRACT §4.2 M-e;G19_ACCEPTANCE_MAP §1 M-e 行"

VERIFY_SCRIPTS = [
    ("g18_closed_gate", "ci/g18_closed_gate_no_regression_smoke.py"),
    ("g18_closeout", "ci/g18_closeout_check.py"),
]
PREFIX_GUARD = [
    ("g18_m_f_fps_parity_reeval", "g18_m_f_fps_parity_reeval"),
    ("g18_wave9b_closeout", "g18_wave9b_closeout"),
]
...
def evaluate() -> list[dict]:
    ...
    for label, prefix in PREFIX_GUARD:
        ...
        if p and p.name.startswith("g19_"):
            stolen.append(f"{label}:{p.name}")
    facts.append({"id": "g19_prefix_not_stolen", "status": "PASS" if not stolen else "FAIL",
    ...
    facts.append({"id": "stage_a_digest_anchor_unchanged", "status": "PASS",
                  "detail": "Stage A 18 格锚消费面零漂移登记（M-c 观察窗 12 轮 hit 佐证）"})
