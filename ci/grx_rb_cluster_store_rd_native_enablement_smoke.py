...
STAGED_CONTAINER = ROOT / "target" / "grx" / "rd_containers" / "cluster_store.rd_container.bin"
...
ACTIVE_MARKER = "RXGD_RD_NATIVE_CLUSTER_STORE active"
...
GODOT_EXE_ENV = "RURIX_GRX_RB_CLUSTER_STORE_GODOT_EXE"
SCRATCH_SOURCE_ENV = "RURIX_GRX_RB_CLUSTER_STORE_GODOT_SOURCE"
SCRATCH_SOURCE_PROVENANCE_ENV = "RURIX_GRX_RB_CLUSTER_STORE_GODOT_SOURCE_PROVENANCE"
SCRATCH_BUILD_COMMAND_ENV = "RURIX_GRX_RB_CLUSTER_STORE_GODOT_BUILD_COMMAND"
SCRATCH_BUILD_LOG_ENV = "RURIX_GRX_RB_CLUSTER_STORE_GODOT_BUILD_LOG"
CAPTURE_PREFIX_ENV = "RURIX_GRX_RB_CLUSTER_STORE_CAPTURE_PREFIX"
CONTAINER_OVERRIDE_ENV = "RURIX_GRX_RB_CLUSTER_STORE_CONTAINER"
DXC_DIR_ENV = "RURIX_DXC_DIR"

TARGET_BACKEND = "Godot 4.7-dev Windows D3D12 Forward+"
BACKEND_SETTING = "rendering/rurix_accel/passes/cluster_store/backend"
CONTAINER_SETTING = "rendering/rurix_accel/passes/cluster_store/rd_container_path"
...
def write_evidence(status: str, *, reason: str | None = None, extra: dict | None = None) -> None:
    ...
    print(f"[grx-rb-cluster-store-rd-native-smoke] wrote {rel(EVIDENCE_OUT)} status={status}")
    if status == "success":
        ...
        print(
            "[grx-rb-cluster-store-rd-native-smoke] wrote "
            f"{rel(SUCCESS_EVIDENCE_OUT)} status=success (historical measured success)"
        )


def fail(msg: str, extra: dict | None = None) -> int:
    print(f"[grx-rb-cluster-store-rd-native-smoke] FAIL {msg}", file=sys.stderr)
...
def skip_environment(msg: str, extra: dict | None = None) -> int:
    ...
    print(f"[grx-rb-cluster-store-rd-native-smoke] SKIP {msg} (降级 SKIP,退出 0)")
...
def skip_measured_prerequisite(prerequisite: str, msg: str, extra: dict) -> int:
    print(
        "[grx-rb-cluster-store-rd-native-smoke] SKIP (measured) first missing "
...
def write_smoke_project(
    ...
    *,
    ...
    """Minimal deterministic Godot project for the cluster_store rd_native gate.
    Only the backend selector and rd_container_path differ between legs."""
    project_dir.mkdir(parents=True, exist_ok=True)

    project_text = f"""\
; Engine configuration file.
; Auto-generated by ci/grx_rb_cluster_store_rd_native_enablement_smoke.py

config_version=5

[application]

config/name="GRX Route B cluster_store rd_native enablement smoke"
...
rurix_accel/passes/cluster_store/backend={backend}
rurix_accel/passes/cluster_store/rd_container_path="{Path(container_path).as_posix() if container_path else ''}"
"""
    scene_text = """\
...
[node name="GRXRBClusterRoot" type="Node3D"]
...
func _ready() -> void:
    ...
    cam.position = Vector3(0.0, 3.5, 7.0)
    ...
    # A lit ground plane plus several OmniLight3D so the Forward+ clustered
    # lighting builder runs bake_cluster (the cluster_store pack segment the
    # rd_native replaces). Deterministic, static scene at fixed-fps.
    var plane := MeshInstance3D.new()
    var pm := PlaneMesh.new()
    pm.size = Vector2(20.0, 20.0)
    plane.mesh = pm
    add_child(plane)
    var lp := [Vector3(-3, 1.5, 0), Vector3(3, 1.5, 0), Vector3(0, 1.5, -3), Vector3(0, 1.5, 3), Vector3(0, 2.5, 0)]
    var lc := [Color(1, 0.3, 0.3), Color(0.3, 1, 0.3), Color(0.3, 0.3, 1), Color(1, 1, 0.3), Color(1, 1, 1)]
    for i in range(lp.size()):
        var light := OmniLight3D.new()
        light.position = lp[i]
        light.light_color = lc[i]
        light.omni_range = 6.0
        light.light_energy = 2.0
        add_child(light)
    print("GRXRBCluster: scene ready backend={backend}")
...
func _capture(idx: int) -> void:
    ...
    if prefix.is_empty():
        printerr("GRXRBCluster: capture prefix env var missing")
        get_tree().quit(3)
        return
    ...
    if idx == 0:
        ...
        meta.close()
    print("GRXRBCluster: captured idx=%d frame=%d width=%d height=%d" % [idx, _frames, img.get_width(), img.get_height()])
"""
...
def main() -> int:
    ...
    _EVIDENCE_BASE.update(
        {
            ...
            "performance_claim": "none",
            "kernel_subset": "cluster merge (store/pack) segment only",
    ...
    for name in ("candidate", "fail_closed"):
        ...
        print(
            f"[grx-rb-cluster-store-rd-native-smoke] LDR absolute diff ({name} vs "
    ...
    print(
        "[grx-rb-cluster-store-rd-native-smoke] PASS measured rd_native REAL replacement "
