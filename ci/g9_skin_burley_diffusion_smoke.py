#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G9.5 M115 皮肤 Burley 扩散门冒烟(g9.p1.m115.skin_burley_diffusion;
RFC-0025 §4.F;spec/display_pipeline.md RXS-0373;G9_ACCEPTANCE_MAP §3 M115;
G9_CONTRACT §8.1 裁决① P1 全进;触 MaterialClosure 32B 经 RFC-0025 §4.L 🔒
显式修订行——资产化侧表扩展通道,32B 布局 0-byte,禁静默扩)。
...
实记 device_name=host-only/validation=not_applicable)。三段判据:
  ...
  host 段:rurix-render display::skin 4 单测逐名锚定(全零衰减退化纯漫反射
    RED/扩散半径随 falloff 单调/缺省侧表逐位不变基线/LUT 回退档+档差)+
    conformance display_pipeline M115 双件语料锚定(//@ spec: RXS-0373 +
    门 key/脚本名留痕)+ 冻结带 milestones/g9/g9_m115_skin_band.json
    provenance 机器核验(含 closure_32b_layout_digest 32B 布局冻结 digest)。
  harness 段:持锁(gpu_device_lock)真跑 g9_m115_skin --evidence(直出件落
    .tmp 工作区不覆盖 evidence/ harness 直出件;schema/spec_anchor/
    assertion_id/status==pass + 13 判据闭集全真)+ --red-arm
    zero-falloff-not-diffuse/slot-overreach 子模式独立复跑抽检。

用法:
  py -3 ci/g9_skin_burley_diffusion_smoke.py --gate g9.p1.m115.skin_burley_diffusion
  py -3 ci/g9_skin_burley_diffusion_smoke.py --selftest
"""
...
SCHEMA_PATH = ROOT / "milestones" / "g9" / "g9_m115_skin_burley_diffusion_evidence_schema.json"
BAND_PATH = ROOT / "milestones" / "g9" / "g9_m115_skin_band.json"
CORPUS_DIR = ROOT / "conformance" / "display_pipeline"
WORK_DIR = ROOT / ".tmp" / "g95_gates" / "m115"
HARNESS_EVIDENCE = WORK_DIR / "harness_evidence.json"

sys.path.insert(0, str(ROOT / "ci"))
from gpu_device_lock import gpu_device_lock  # noqa: E402

GATE_KEY = "g9.p1.m115.skin_burley_diffusion"
NUMERIC_STEP = 160
SOURCE_REF = "RFC-0025 §4.F;spec/display_pipeline.md RXS-0373;G9_ACCEPTANCE_MAP §3 M115"
TAG = "g9_m115"
SUBJECT = "g9_m115_skin_burley_diffusion"
MATRIX_ROW = "M115"

MODULE_TESTS = {
    "display::skin": [
        "zero_falloff_degrades_to_pure_diffuse",
        "diffusion_radius_monotonic_in_falloff",
        "default_side_table_bit_equal_baseline",
        "lut_fallback_and_tier_diff",
    ],
}
CORPUS_FILES = [
...
BAND_SCHEMA = "rurix.g9m115.skin_band.v1"
BAND_SPEC_ANCHOR = "RXS-0373"
BAND_DIGEST_FIELDS = ["sss_digest", "closure_32b_layout_digest"]
HARNESS_BIN = "g9_m115_skin"
HARNESS_SCHEMA = "rurix.g9m115.skin.v1"
HARNESS_ASSERTION = "g9.p1.m115.skin_burley_diffusion"
HARNESS_TAG = "G9_M115_SKIN"
HARNESS_CHECKS = [
    "conformance_corpus_anchored",
    "closure_32b_size_and_face_untouched",
    "closure_32b_layout_digest_frozen_equal",
    "side_table_asset_roundtrip",
    "burley_single_pass_dual_kernel",
    "diffusion_radius_response_golden",
    "lut_fallback_tier_quality_diff",
    "default_side_table_bit_invariant",
    "zero_falloff_degrades_pure_diffuse",
    "double_run_bit_equal",
    "golden_frozen_equal",
    "red_arm_zero_falloff_and_default_table",
    "red_arm_slot_overreach_and_reserved_bits",
]
RED_ARMS = ["zero-falloff-not-diffuse", "slot-overreach"]
...
def main() -> int:
    ...
    with gpu_device_lock(purpose="g9_m115 skin harness 腿"):
        exe = build_harness()
        if exe:
            ...
            note("harness:Burley 屏单 pass 双 kernel golden + profile 资产化 + 32B 布局 0-byte 机核 + RED 双臂子模式复跑")
