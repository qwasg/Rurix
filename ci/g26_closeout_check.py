#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G26.6 close-out）
"""G26.6 close-out 终审门（g26.wave.6b.closeout，步骤 460）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g26.wave.6b.closeout"
NUMERIC_STEP = 460
SUBJECT = "g26_wave6b_closeout"
WAVE = "G26.6"
SCHEMA_PATH = ROOT / "milestones/g26/g26_wave6b_closeout_evidence_schema.json"
P0_SUBJECTS = [
    ("m_a", "g26_m_a_framegen_device_kernel"),
    ("m_b", "g26_m_b_framegen_device_bench_accounting"),
    ("m_c", "g26_m_c_rd045_backfill_rejudgment"),
    ("m_d", "g26_m_d_g17_md_f1_rejudgment_window"),
    ("m_e", "g26_m_e_closed_gate_no_regression"),
]
...
def evaluate() -> tuple[list[dict], str]:
    ...
    p2 = wel.load_latest_evidence("g26_p2_decisions_check")
    ...
    ma = wel.load_latest_evidence("g26_m_a_framegen_device_kernel")
    ma_ok = ma is not None and wel.load_json(ma).get("host_section_pass") is True
    facts.append(fact("handover_ledger_chain", ma_ok,
                      f"{ma.name}（device kernel 主锚绿件）" if ma else "missing"))
    hits = list((ROOT / "rfcs").glob("0043-*.md"))
    facts.append(fact("rfc_0043_archived", bool(hits), hits[0].name if hits else "RFC-0043 missing"))
    me_ok, me_d = _latest_green("g26_m_e_closed_gate_no_regression")
    ...
    soak = wel.load_latest_evidence("g26_stabilization_soak")
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes=f"G26.6 closeout 八 facts VERDICT={verdict}",
        host_section_pass=ok,
    )
    print(f"[g26_closeout] VERDICT={verdict}")
