#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# G31+ 波 C Task C5：check_schemas.py 三处纯追加注册（io.open 补丁法——
# newline="" 字节面保全，既有路由 0-byte；打补丁后立即重读验证驻留）。
# 本脚本幂等：已驻留即只做核验（多 agent 并发改该文件——锚唯一性机核，
# 锚不唯一/缺失即 FAIL 拒改不猜）。先例 ci/_patch_g31_renderer_sdk_schemas.py。
import io
import sys

P = r"h:\rurix\ci\check_schemas.py"

LOAD_ANCHOR = '''    g31_renderer_sdk_schema = load(
        ROOT / "milestones/g31/g31_renderer_sdk_evidence_schema.json"
    )
'''
LOAD_ADD = '''    # G31+ 波 C Task C5 渲染器 SDK 分发打包门前缀纯追加（重放幂等面；与既有
...
def main() -> int:
    ...
    if not already:
        ...
        for name, anchor in [
            ...
            n = src.count(anchor)
            if n != 1:
                print(f"[patch_g31_sdk_dist] FAIL: {name} 锚出现 {n} 次（≠1 拒改）",
                      file=sys.stderr)
                return 1
        ...
        print("[patch_g31_sdk_dist] 三处纯追加插入（load/validator/route 各 1）")

    # 驻留核验（每 token 恰 1）。
    misses = []
    for name, token in [
        ("load", "g31_sdk_dist_schema = load("),
        ("validator", "g31_sdk_dist_validator = ("),
        ("route", 'f.name.startswith("g31_sdk_dist_")'),
    ]:
        n = src.count(token)
        if n != 1:
    ...
    if misses:
        for m in misses:
            print(f"[patch_g31_sdk_dist] FAIL {m}", file=sys.stderr)
        return 1
    ...
    print("[patch_g31_sdk_dist] PASS：三处纯追加驻留（load/validator/route 各 1）+ py_compile 绿")
    return 0
