#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G24.6 close-out）
"""G24.6 close-out 终审门（g24.wave.6b.closeout，步骤 428）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g24.wave.6b.closeout"
NUMERIC_STEP = 428
SUBJECT = "g24_wave6b_closeout"
WAVE = "G24.6"
SCHEMA_PATH = ROOT / "milestones/g24/g24_wave6b_closeout_evidence_schema.json"
P0_SUBJECTS = [
    ("m_a", "g24_m_a_hair_strand_oit_rejudgment"),
    ("m_b", "g24_m_b_hdr_calibration_rejudgment"),
    ("m_c", "g24_m_c_bistro_exterior_conversion_rejudgment"),
    ("m_d", "g24_m_d_safe_gpu_and_legacy_rd_disposition"),
    ("m_e", "g24_m_e_closed_gate_no_regression"),
]
...
def evaluate() -> tuple[list[dict], str]:
    ...
    p2 = wel.load_latest_evidence("g24_p2_decisions_check")
    ...
    hzb = wel.load_latest_evidence("g24_m_a_hair_strand_oit_rejudgment")
    hzb_ok = hzb is not None and wel.load_json(hzb).get("host_section_pass") is True
    facts.append(fact("legacy_ledger_chain", hzb_ok, hzb.name if hzb else "missing"))
    hits = list((ROOT / "rfcs").glob("0041-*.md"))
    facts.append(fact("rfc_0041_archived", bool(hits), hits[0].name if hits else "RFC-0041 missing"))
    me_ok, me_d = _latest_green("g24_m_e_closed_gate_no_regression")
    ...
    soak = wel.load_latest_evidence("g24_stabilization_soak")
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes=f"G24.6 closeout 八 facts VERDICT={verdict}",
        host_section_pass=ok,
    )
    print(f"[g24_closeout] VERDICT={verdict}")
