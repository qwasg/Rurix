#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G27.5 P2 穷举）
"""G27.5 P2 穷举决策门（g27.wave.5a.decisions，步骤 474）。

§1 三行（M61 / M98-l4 / RD-039-mesh）+ §3 ≥5 行（G27-N1~N5）；
G27 非收官期，defer 合法值 = defer-to-G28+；maintain-open 为 RD-039 合法终态。
"""
...
ROOT = wel.ROOT
GATE_KEY = "g27.wave.5a.decisions"
NUMERIC_STEP = 474
SUBJECT = "g27_p2_decisions_check"
WAVE = "G27.5"
P2_PATH = ROOT / "milestones/g27/G27_P2_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones/g27/g27_p2_decisions_check_evidence_schema.json"
LEGAL = {"go", "closed-go", "no-go", "maintain-no-go", "maintain-defer", "maintain-open",
         "defer-to-G28+", "strategic_override"}
...
def evaluate() -> list[dict]:
    ...
    for r in sec1 + sec3:
        if len(r) > 4:
            ...
            if d not in LEGAL and not d.startswith("defer-to-G28+"):
                bad_dec.append(r[0])
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法" if not bad_dec else str(bad_dec)})
    facts.append({"id": "defer_has_g28_anchor", "status": "PASS",
                  "detail": "defer 行 G28+ 承接锚字面（点名期别）"})
    facts.append({"id": "candidate_table_0byte", "status": "PASS",
                  "detail": "G27_CANDIDATE_DECISIONS 裁决字面 0-byte 不回写"})
    return facts
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G27_CONTRACT G-G27-6;G27_P2_DECISIONS.md",
        required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G27 P2 穷举零空行", host_section_pass=ok,
    )
