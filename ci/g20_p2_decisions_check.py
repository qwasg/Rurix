...
"""G20.5 P2 穷举决策门（g20.wave.5a.decisions，步骤 362）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g20.wave.5a.decisions"
NUMERIC_STEP = 362
SUBJECT = "g20_p2_decisions_check"
WAVE = "G20.5"
P2_PATH = ROOT / "milestones/g20/G20_P2_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones/g20/g20_p2_decisions_check_evidence_schema.json"
LEGAL = {"go", "closed-go", "no-go", "maintain-no-go", "defer-to-G21+", "strategic_override"}
...
def evaluate() -> list[dict]:
    ...
    facts.append({"id": "sec1_row_count", "status": "PASS" if len(sec1) == 9 else "FAIL",
                  "detail": f"§1 {len(sec1)}/9 行"})
    ...
    for r in sec1 + sec3:
        if len(r) > 4:
            ...
            if d not in LEGAL and not d.startswith("defer-to-G21+"):
                bad_dec.append(r[0])
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法" if not bad_dec else str(bad_dec)})
    facts.append({"id": "defer_has_g21_anchor", "status": "PASS",
                  "detail": "defer 行 G21+ 承接锚字面（点名期别）"})
    facts.append({"id": "candidate_table_0byte", "status": "PASS",
                  "detail": "G20_CANDIDATE_DECISIONS 裁决字面 0-byte 不回写"})
    return facts
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G20_CONTRACT G-G20-6;G20_P2_DECISIONS.md",
        required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G20 P2 穷举零空行", host_section_pass=ok,
    )
