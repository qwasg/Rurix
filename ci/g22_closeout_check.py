#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G22.6 close-out）
"""G22.6 close-out 终审门（g22.wave.6b.closeout，步骤 396）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g22.wave.6b.closeout"
NUMERIC_STEP = 396
SUBJECT = "g22_wave6b_closeout"
WAVE = "G22.6"
SCHEMA_PATH = ROOT / "milestones/g22/g22_wave6b_closeout_evidence_schema.json"
P0_SUBJECTS = [
    ("m_a", "g22_m_a_slab_material_host_realization"),
    ("m_b", "g22_m_b_svt_disposition"),
    ("m_c", "g22_m_c_ktx2_basisu_disposition"),
    ("m_d", "g22_m_d_work_graphs_fsr_reeval_disposition"),
    ("m_e", "g22_m_e_closed_gate_no_regression"),
]
...
def evaluate() -> tuple[list[dict], str]:
    ...
    p2 = wel.load_latest_evidence("g22_p2_decisions_check")
    ...
    hzb = wel.load_latest_evidence("g22_slab_probe")
    hzb_ok = hzb is not None and wel.load_json(hzb).get("white_furnace_identity") is True
    facts.append(fact("slab_realization_chain", hzb_ok, hzb.name if hzb else "missing"))
    hits = list((ROOT / "rfcs").glob("0039-*.md"))
    facts.append(fact("rfc_0039_archived", bool(hits), hits[0].name if hits else "RFC-0039 missing"))
    me_ok, me_d = _latest_green("g22_m_e_closed_gate_no_regression")
    ...
    soak = wel.load_latest_evidence("g22_stabilization_soak")
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G22_CONTRACT G-G22-6", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes=f"G22.6 closeout 八 facts VERDICT={verdict}",
        host_section_pass=ok,
    )
    print(f"[g22_closeout] VERDICT={verdict}")
