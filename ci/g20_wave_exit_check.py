#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G20 波聚合门）
"""G20 波聚合门（参数化 g20.wave.{2..6}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g20/g20_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g20.wave.2.exit": ("G20.2", "g20_m_a_hzb_occlusion_host_realization", 353),
    "g20.wave.3.exit": ("G20.3", "g20_m_b_cluster_streaming_p4_disposition", 355),
    "g20.wave.4.exit": ("G20.4", "g20_m_c_mesh_shader_rejudgment", 357),
    "g20.wave.5.exit": ("G20.5", "g20_m_d_far_field_l4_disposition", 359),
    "g20.wave.6.exit": ("G20.6", "g20_m_e_closed_gate_no_regression", 361),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g20_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G20_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g20_wave{n}_exit",
        notes=f"G20 wave {n} exit", host_section_pass=overall,
    )
