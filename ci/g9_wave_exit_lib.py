#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G9 波次聚合门共享库(milestones/g9/CI_GATES.md §5;同构 ci/g8_wave_exit_lib.py)。

只读汇总独立门 evidence,不重跑 smoke、不代绿。任一 required gate 缺失 /
非 PASS / SKIP / DEV_ENV_DEGRADE → 聚合红。供 G9.2~G9.8b 薄壳复用。

与 g8 版差异:docstring 对照说明、selftest 临时目录前缀、示例注释 subject 前缀;
...
# UTC stamp in evidence filenames: g9_<slug>_YYYYMMDDTHHMMSSZ.json
...
def load_latest_evidence(subject_prefix: str, evidence_dir: Path | None = None) -> Path | None:
    ...
    subject_prefix 例如 ``g9_m90_cluster_dag_deepening``(匹配
    ``g9_m90_cluster_dag_deepening_<UTC>.json``)。
    """
...
def run_selftest_missing_gate(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="g9_wave_exit_selftest_") as td:
        empty = Path(td)
