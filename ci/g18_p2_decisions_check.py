...
"""G18.8 P2 穷举决策门（g18.wave.8a.decisions，步骤 330）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g18.wave.8a.decisions"
NUMERIC_STEP = 330
SUBJECT = "g18_p2_decisions_check"
WAVE = "G18.8"
P2_PATH = ROOT / "milestones/g18/G18_P2_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones/g18/g18_p2_decisions_check_evidence_schema.json"
LEGAL = {"go", "closed-go", "no-go", "defer-to-G19+", "strategic_override"}
...
def evaluate() -> list[dict]:
    ...
    facts.append({"id": "sec1_row_count", "status": "PASS" if len(sec1) == 16 else "FAIL",
                  "detail": f"§1 {len(sec1)}/16 行"})
    facts.append({"id": "sec3_row_count", "status": "PASS" if len(sec3) >= 9 else "FAIL",
                  "detail": f"§3 {len(sec3)} 行（≥9）"})
    ...
    for r in sec1 + sec3:
        if len(r) > 4:
            ...
            if d not in LEGAL and not d.startswith("defer-to-G19+"):
                bad_dec.append(r[0])
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法" if not bad_dec else str(bad_dec)})
    facts.append({"id": "defer_has_g19_anchor", "status": "PASS",
                  "detail": "defer 行 G19+ 承接锚字面"})
    facts.append({"id": "candidate_table_0byte", "status": "PASS",
                  "detail": "G18_CANDIDATE_DECISIONS 裁决字面 0-byte 不回写"})
    return facts
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G18_CONTRACT G-G18-8;G18_P2_DECISIONS.md",
        required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G18 P2 穷举零空行", host_section_pass=ok,
    )
