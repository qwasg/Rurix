#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G29.5 soak 波）
"""G29.5 稳定 soak（g29.wave.5a.soak，步骤 507）。

管线 bench 四组合轮转 + 探针轮换扩容至八车道（G29-N5）：每第 5 迭代按
...
ROOT = wel.ROOT
GATE_KEY = "g29.wave.5a.soak"
NUMERIC_STEP = 507
SUBJECT = "g29_stabilization_soak"
WAVE = "G29.5"
SCHEMA_PATH = ROOT / "milestones/g29/g29_stabilization_soak_evidence_schema.json"
...
def run_gate() -> int:
    facts: list[dict] = []
    md = wel.load_latest_evidence("g29_m_d_wg_dgc_capability_recheck")
    ...
    if not md_ok or not BIN.is_file() or not lane_ok:
        ...
        while time.perf_counter() - t0 < MIN_SECONDS:
            ...
            if iters % 5 == 4:
                lane = probe_iters % 8
                out_dir = ROOT / ".tmp" / "g29_soak_probes"
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G29 soak ≥1800s（管线四组合 + 八车道探针轮换扩容穿插，含 framegen/hzb/restir/slab 四 device 车道穿插（八车道轮换））",
