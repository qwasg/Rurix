#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GRX-009 segment 4c: real Windows D3D12 dispatch smoke for the luminance pass.

This harness proves the *offline* GRX-009 luminance_reduction artifacts (the
rurixc-emitted DXIL container, the RTS0 root signature, and the descriptor
layout) can complete **one minimal compute dispatch on a real D3D12 device and
command queue**. It produces measured smoke evidence only. It does NOT:
...
Discipline (mirrors ci/dxil_binding_device_smoke.py):

  * The device/command queue are always real: fake/null handles are never
    accepted. If there is no hardware D3D12 adapter, no D3D12 runtime, or the
    device lacks the 64-bit integer shader capability the pass requires, the
    harness records ``status=skip`` with a concrete reason. SKIP never advances
    the ready gate.
  * The existing tracked DXIL / root signature / descriptor layout artifacts
    are used as-is. Their SHA-256 digests must match the tracked segment 3a
    offline compile evidence, and the descriptor layout must match the current
    resource mapping (2 resources src_luminance=t0 SRV + dst_luminance=u0 UAV,
    a 28-byte b0 root-constant block, and the 64-bit integer requirement).
    Any mismatch is ``status=fail`` with the mismatch recorded.
  ...
  * A ``status=success`` run records adapter/device info, artifact hashes,
    dispatch dimensions, fence completion, and a readback checksum of the
    dst_luminance UAV. Any PSO / root signature / descriptor / resource /
    readback failure is ``status=fail`` with the D3D12 error code and log kept.
...
WORK = ROOT / "target" / "grx009_d3d12_dispatch_smoke"

SUBJECT = "grx009_luminance_real_d3d12_dispatch_smoke"
...
def locate_dxcapi_include(dxc_dir: Path | None) -> Path | None:
    """Find the directory containing dxcapi.h (for compiling the signing path)."""
    if dxc_dir is None:
        return None
...
def descriptor_layout_matches_resource_mapping(layout: dict) -> str | None:
    ...
    mapping (segment 3b), otherwise a human-readable mismatch reason.

    The bindings are derived from this layout; a mismatch means the harness
    would be guessing resource shapes, which is forbidden.
    """
...
def fail(msg: str, extra: dict | None = None) -> int:
    print(f"[grx009-d3d12-dispatch-smoke] FAIL {msg}", file=sys.stderr)
...
def skip(msg: str, extra: dict | None = None) -> int:
    ...
    print(f"[grx009-d3d12-dispatch-smoke] SKIP {msg}(降级 SKIP,退出 0)")
...
def write_evidence(status: str, *, reason: str | None = None, extra: dict | None = None) -> None:
    ...
    print(f"[grx009-d3d12-dispatch-smoke] wrote {EVIDENCE_OUT.relative_to(ROOT)} status={status}")
...
#
# argv: <dxil> <rts0>
# Exit codes: 0 = success, 1 = fail (device operation error), 2 = skip
# (no hardware adapter / no D3D12 runtime / device missing 64-bit int caps).
#
# Root signature is created DIRECTLY from the Rurix RTS0 bytes (device-parse),
# the compute PSO is built from the Rurix DXIL container, and the SRV(t0) /
# UAV(u0) / b0 root constants are bound per the descriptor layout:
...
// Sign the DXIL container in place with the DXIL validator (dxil.dll). A
// compiler-emitted container that is only dxv-validated still lacks the
// validation hash the D3D12 runtime requires (without Developer Mode). Signing
// does not change shader semantics; it appends the hash so the same container
// can create a compute PSO on a normal device. Returns true on success.
...
int wmain(int argc, wchar_t** argv) {
    ...
    // Best-effort: enable experimental shader models so a compiler-emitted DXIL
    // container without a Microsoft-signed hash can still load. Requires Windows
    // Developer Mode; failure is recorded but not fatal (a signed container will
    ...
    // Sign an in-memory copy of the container so it can create a compute PSO on a
    // normal (non-Developer-Mode) device. The tracked artifact is untouched.
    std::string sign_err;
    ...
    // The luminance pass requires the 64-bit integer shader capability; a device
    // without it cannot run this package, so SKIP (not fail) with a clear reason.
    D3D12_FEATURE_DATA_D3D12_OPTIONS1 opt1 = {};
    device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS1, &opt1, sizeof(opt1));
    std::printf("RXGD_DISPATCH: int64_shader_ops=%d\n", opt1.Int64ShaderOps ? 1 : 0);
...
HARNESS_CPP += r"""
    ...
    // Minimal source: 8x8 R32_FLOAT src_luminance (t0), filled with 1.0.
    const UINT src_w = 8, src_h = 8;
    const UINT dst_w = (src_w + 7) / 8 > 0 ? (src_w + 7) / 8 : 1;  // max(src/8,1) level-0 reduce shape
    const UINT dst_h = (src_h + 7) / 8 > 0 ? (src_h + 7) / 8 : 1;
    ...
    // dst_luminance (u0): R32_FLOAT UAV, level-0 reduce shape.
...
HARNESS_CPP += r"""
    ...
    // Copy src texels up, transition to non-pixel-shader resource for compute SRV.
    ...
    // Derived layout: root param 0 = 7-dword b0 root constants, root param 1 =
    // descriptor table [SRV t0, UAV u0].
    ...
    std::memcpy(&rc[4], &maxl, sizeof(float));    // max_luminance
    std::memcpy(&rc[5], &minl, sizeof(float));    // min_luminance
    std::memcpy(&rc[6], &expo, sizeof(float));    // exposure_adjust
    ...
    const UINT gx = 1, gy = 1, gz = 1;  // 8x8 block over an 8x8 source = 1 group
    ...
    for (UINT y = 0; y < dst_h; ++y) {
        ...
        for (UINT x = 0; x < dst_w; ++x) {
            ...
            if (!got_first) { std::memcpy(&first, px, 4); got_first = true; }
            for (int b = 0; b < 4; ++b) { checksum ^= px[b]; checksum *= 16777619u; }
        }
    }
    ...
    std::printf("RXGD_DISPATCH: ok adapter=\"%s\" dispatch=%u,%u,%u fence=%llu "
                ...
                (unsigned long long)fence_done, dst_w, dst_h, first, checksum);
    return 0;
}
"""
...
def compile_harness(vcvars: Path, cpp: Path, exe: Path, include_dir: Path | None) -> tuple[bool, str]:
    ...
    bat.write_text(
        ...
        f'cl /nologo /std:c++17 /EHsc /W4 /DUNICODE /D_UNICODE {include_flag}"{cpp}" '
...
def parse_harness_output(output: str) -> dict:
    ...
    for line in output.splitlines():
        ...
        if not line.startswith("RXGD_DISPATCH: ok "):
            ...
            continue
        # RXGD_DISPATCH: ok adapter="..." dispatch=1,1,1 fence=1 dst=1x1 dst_first=.. checksum=0x..
...
def main() -> int:
    ...
    _EVIDENCE_BASE = {
        ...
        "note": (
            "GRX-009 segment 4c real D3D12 dispatch smoke evidence only. Even a "
    ...
    # Stale/tampered artifacts must not be smoke-tested against the tracked evidence.
    ...
    # The bindings are derived from the descriptor layout; a layout that does not
    # match the tracked resource mapping means we would be guessing -> FAIL.
    ...
    if dxc_dir is None:
        return skip(
            ...
            "无法为编译器产出的 DXIL container 签名以在非 Developer-Mode device 上加载"
        )
    ...
    cpp = WORK / "dispatch_smoke_harness.cpp"
    exe = WORK / "dispatch_smoke_harness.exe"
    ...
    if p.returncode != 0 or "RXGD_DISPATCH: ok" not in output:
        return fail(
            "real D3D12 luminance dispatch smoke failed",
    ...
    print(f"[grx009-d3d12-dispatch-smoke] PASS measured real D3D12 dispatch; adapter={device_info['adapter']}")
    return 0
