...
"""G10.1 治理守卫 — 验收映射覆盖 / 空行 / 三向命名空间一致性（G10_CONTRACT §4.2 冻结面）。

对应 milestones/g10/CI_GATES.md §3 的 `g10.gov.acceptance_coverage`。

事实源三份，必须逐字一致（G10.1 冻结口径）：
  1. milestones/g10/G10_ACCEPTANCE_MAP.md §1（12 P0）与 §2（2 已 go P1：M133/M138）
  2. milestones/g10/G10_CONTRACT.md §4.2（12 P0 独立断言表；不载 P1 行）
  3. milestones/g10/CI_GATES.md §4（12 P0）与 §4A（2 已 go P1）
...
三向比对面（MAP §4.1 机器可核声明）：
  ...
  - evidence schema 目标路径：MAP §1 ↔ CI_GATES §4 逐行逐字相等，且全部命中 CONTRACT §4.2
    冻结的统一形态 `milestones/g10/g10_m<###>_<slug>_evidence_schema.json`（CONTRACT 只冻结
    形态不逐行载路径，故 CONTRACT 侧以形态机核、逐行字面由 MAP ↔ CI_GATES 双侧比对）；
  - P1 行：MAP §2 ↔ CI_GATES §4A 双向逐字比对（key/脚本/schema；CONTRACT §4.2 不载 P1 行）。
  - slug 同字面机核：key 末段 slug、脚本名 `ci/g10_<slug>_smoke.py`、schema 文件名
    `g10_m<###>_<slug>_evidence_schema.json` 三者同 slug（MAP §1 单一命名空间字面）。
...
CI_GATES_PATH = ROOT / "milestones/g10/CI_GATES.md"

# 冻结 12 行 P0 集合（2026-08-15 G10.1 立项裁决口径；go-P1 波次开工前只追加）。
EXPECTED_P0 = {
...
# 已 go P1 精确集合（G10_CONTRACT §4.2 末段「M133（清单冻结）/M138（阈值标定）为 P1，
# 入验收映射随主门核验」）。后续波次判 go 的 P1 只追加扩本集合 + MAP §2 + CI_GATES §4A。
EXPECTED_P1 = {"M133", "M138"}

# MAP §5：所有波次属于 G10.2|G10.3|G10.4|G10.5（M130 允许双阶段字面）。
ALLOWED_WAVES = {"G10.2", "G10.3", "G10.4", "G10.5", "G10.2 骨架 → G10.5 双端核验"}
...
KEY_RE = re.compile(r"^g10\.p0\.m\d{3}\.[a-z0-9_]+$")
KEY_IN_CELL_RE = re.compile(r"`(g10\.p0\.m\d{3}\.[a-z0-9_]+)`")
KEY_P1_RE = re.compile(r"^g10\.p1\.m\d{3}\.[a-z0-9_]+$")
KEY_P1_IN_CELL_RE = re.compile(r"`(g10\.p1\.m\d{3}\.[a-z0-9_]+)`")
KEY_P0_CELL_RE = re.compile(r"`(g10\.p0\.m\d{3}\.[a-z0-9_]+)`")
KEY_P1_CELL_RE = re.compile(r"`(g10\.p1\.m\d{3}\.[a-z0-9_]+)`")
SECTION_RE = re.compile(r"^## (\d+)\. ")
SCRIPT_RE = re.compile(r"ci/g10_[a-z0-9_]+_smoke\.py")
SCHEMA_RE = re.compile(r"`(milestones/g10/g10_(m\d{3})_[a-z0-9_]+_evidence_schema\.json)`")
...
def check_rows(
    ...
    for m in sorted(rows):
        ...
        if not schema_m:
            findings.append(f"[no-empty] {m} schema 路径不符 g10_m###_<slug>_evidence_schema.json：{row['schema']!r}")
        else:
            ...
            expected_schema = f"milestones/g10/g10_{m.lower()}_{slug}_evidence_schema.json"
...
def check(map_text: str, contract_text: str, ci_gates_text: str) -> Finding:
    ...
    if set(rows) != EXPECTED_P0:
        findings.append(
            f"[coverage] P0 集合不等于冻结 12 行：缺 {sorted(EXPECTED_P0 - set(rows))}，多 {sorted(set(rows) - EXPECTED_P0)}"
        )
    ...
    for m in sorted(rows):
        ...
        schema = schema_m.group(1) if schema_m else None
        for name, table in (("G10_CONTRACT §4.2", contract_rows), ("CI_GATES §4", ci_rows)):
            if m not in table:
        ...
        # 见 check_rows 的 g10_m###_<slug> 同字面断言）。
...
# ---------------------------------------------------------------------------
# selftest 合成夹具：12 行 P0 冻结集合 + 2 行已 go P1 的正本，不依赖树上文件。
# ---------------------------------------------------------------------------

CANONICAL_ROWS = [
    ("M128", "ue5_capture_environment", "G10.2"),
    ("M129", "ue5_reference_frames", "G10.2"),
    ("M130", "dual_determinism_contract", "G10.2 骨架 → G10.5 双端核验"),
    ("M131", "asset_license_registry", "G10.3"),
    ("M132", "corpus_loading", "G10.3"),
    ("M134", "frame_capture_pipeline", "G10.4"),
    ("M135", "flip_metric", "G10.4"),
    ("M136", "ssim_psnr_metric", "G10.4"),
    ("M137", "pixel_diff_report", "G10.4"),
    ("M139", "ab_comparison", "G10.5"),
    ("M140", "gap_registry", "G10.5"),
    ("M141", "perf_baseline", "G10.5"),
]

CANONICAL_P1_ROWS = [
    ("M133", "corpus_list_freeze", "G10.3"),
    ("M138", "metric_threshold_calibration", "G10.4"),
]


def _fixture() -> tuple[str, str, str]:
    map_lines = ["# fixture G10_ACCEPTANCE_MAP", "", "## 1. P0 硬门", ""]
    contract_lines = ["# fixture G10_CONTRACT", "", "### 4.2 P0 独立断言", ""]
    gates_lines = ["# fixture CI_GATES", "", "## 4. 12 个 P0 独立机器断言", ""]
    for m, slug, wave in CANONICAL_ROWS:
        key = f"g10.p0.{m.lower()}.{slug}"
        script = f"ci/g10_{slug}_smoke.py"
        schema = f"milestones/g10/g10_{m.lower()}_{slug}_evidence_schema.json"
        if m == "M130":
            cmd = (
                f"`py -3 {script} --gate {key} --phase g10.2`"
                f"<br>`py -3 {script} --gate {key} --phase g10.5`"
            )
        else:
...
def run_selftest() -> int:
    ...
    cases: list[tuple[str, str, str, str, str]] = [
        (
            "删除 M141 行 → coverage 必须红",
            "\n".join(l for l in map_text.splitlines() if not l.startswith("| **M141**")),
            contract_text,
            ci_text,
            "P0 集合不等于冻结 12 行",
        ),
        (
            "MAP 单侧改写 M137 key → three-way 必须红",
            map_text.replace("g10.p0.m137.pixel_diff_report", "g10.p0.m137.pixel_diff"),
            contract_text,
            ci_text,
            "[three-way] M137 key 漂移",
        ),
        (
            "恢复大写 key 写法 → coverage 必须红",
            map_text.replace("`g10.p0.m128.ue5_capture_environment`", "`G10.P0.M128.UE5_CAPTURE_ENVIRONMENT`", 1),
            contract_text,
            ci_text,
            "M128 必须恰有一个 canonical symbolic key",
        ),
        (
            "判据列置为待补 → no-empty 必须红",
            map_text.replace("合成判据 M135", "待补", 1),
            contract_text,
            ci_text,
            "[no-empty] M135",
        ),
        (
            ...
            map_text,
            contract_text,
            ci_text.replace("ci/g10_frame_capture_pipeline_smoke.py", "ci/g10_frame_capture_smoke.py"),
            "[three-way] M134 script 漂移",
        ),
        (
            "波次改写为非法 G10.6 → no-empty 必须红",
            map_text.replace("合成 RED 臂 M131 | host+device | **G10.3**", "合成 RED 臂 M131 | host+device | **G10.6**", 1),
            contract_text,
            ci_text,
            "[no-empty] M131 波次",
        ),
        (
            ...
            map_text.replace(
                "milestones/g10/g10_m132_corpus_loading_evidence_schema.json",
                "milestones/g10/g10_m133_corpus_loading_evidence_schema.json",
                1,
            ),
            contract_text,
            ci_text,
            "[no-empty] M132 schema 路径的 m### 段不符",
        ),
        (
            "numeric_step 列填入数字 → 预占必须红",
            map_text.replace(
                "host+device | **G10.2** | post-interlock actual-next-free allocation |",
                "host+device | **G10.2** | 173 |",
                1,
            ),
            contract_text,
            ci_text,
            "[no-empty] M128 numeric_step",
        ),
        (
            "M130 行删除 --phase 段 → coverage 必须红",
            map_text.replace(" --phase g10.2`", "`").replace(" --phase g10.5`", "`"),
            contract_text,
            ci_text,
            "[coverage] M130 同脚本多次调用必须以 --phase 区分阶段",
        ),
        (
            ...
            contract_text,
            ci_text.replace(
                "milestones/g10/g10_m136_ssim_psnr_metric_evidence_schema.json",
                "milestones/g10/g10_m136_ssim_metric_evidence_schema.json",
            ),
            "[three-way] M136 schema 漂移",
        ),
        (
            "删除 §2 M133 行 → P1 coverage 必须红",
            "\n".join(l for l in map_text.splitlines() if not l.startswith("| **M133**")),
            contract_text,
            ci_text,
            "P1 集合不等于 §2 声明集合",
        ),
        (
            ...
            map_text.replace(
                "`g10.p1.m138.metric_threshold_calibration`", "`g10.p0.m138.metric_threshold_calibration`", 1
            ),
            contract_text,
            ci_text,
            "出现 p0 key",
        ...
        (
            ...
            map_text,
            contract_text,
            ci_text.replace("ci/g10_corpus_list_freeze_smoke.py", "ci/g10_corpus_list_smoke.py"),
            "[two-way] M133 script 漂移",
        ),
        (
            ...
            contract_text,
            ci_text.replace(
                "milestones/g10/g10_m138_metric_threshold_calibration_evidence_schema.json",
                "milestones/g10/g10_m138_threshold_calibration_evidence_schema.json",
            ),
            "[two-way] M138 schema 漂移",
        ),
        (
            "脚本名与 key slug 不同字面 → coverage 必须红",
            map_text.replace("ci/g10_flip_metric_smoke.py", "ci/g10_flip_smoke.py"),
            contract_text,
            ci_text,
            "M135 脚本名",
        ),
    ...
    if failures:
        print(f"[check_g10_acceptance_map] SELFTEST FAIL ({failures})")
        return 1
    print("[check_g10_acceptance_map] SELFTEST PASS (15 RED + 1 GREEN)")
    return 0
...
def main() -> int:
    ...
    for path in (MAP_PATH, CONTRACT_PATH, CI_GATES_PATH):
        if not path.exists():
            print(f"[check_g10_acceptance_map] FAIL — 缺事实源 {path.relative_to(ROOT)}")
            return 1
    ...
    if findings:
        print(f"[check_g10_acceptance_map] FAIL ({len(findings)} 项)")
        for f in findings:
            print(f"  - {f}")
        return 1
    print(
        "[check_g10_acceptance_map] PASS"
        "（12 P0 + 2 已 go P1（M133/M138）覆盖齐备；14 key 唯一且同一命名空间；"
