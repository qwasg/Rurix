...
still fails closed with ``real_dispatch_path_not_linked``):
  ...
  * **Fallback red/green + visual stability**: all three legs must render via
    ...
    fallback entries with ``godot_fallback_active=true`` and ``telemetry_frame``
    equal to the measured capture frame index.
...
# Recording-smoke marker: must never appear here (the particles_copy
...
def write_smoke_project(
    ...
    *,
    ...
    byte-identical so the opt-in matrix is the only delta. The particles_copy
    dispatch_recording_smoke opt-in stays false in EVERY leg (the RECORD marker
    must never appear)."""
...
rurix_accel/passes/particles_copy/dispatch_recording_smoke=false
...
def run_matrix_leg(godot_exe: Path, *, leg: str, dll_path: Path) -> dict:
    ...
    # The real-pass marker prints once per dispatched frame; pin the first line
    # from the FULL output (the writeback scaffold marker contains the real-pass
    # marker as a prefix, so it is excluded here).
    real_pass_lines = [
        line.strip()
        for line in output.splitlines()
    ...
    for name, leg in legs.items():
        ...
        if leg["record_marker_observed"]:
            return fail(
                ...
                "stay off in the GRX-013 enablement matrix",
                extra=runs_extra,
            )
