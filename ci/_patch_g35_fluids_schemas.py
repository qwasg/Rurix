...
# G35 GPU 粒子系统 G35-7：check_schemas.py 三处纯追加注册（io.open 补丁法——
...
'''
LOAD_ADD = '''    # G35 GPU 粒子系统 G35-7 流体统一物理门前缀纯追加（重放幂等面；仅门
...
'''
VALIDATOR_ADD = '''    g35_fluids_gate_validator = (
...
PROBE = "g35_fluids_gate_schema = load"
...
def main() -> int:
    ...
    if PROBE in s:
        ...
        print(f"[patch_g35_fluids] 已驻留，核验 {'PASS' if ok else 'FAIL'}")
    ...
    if missing:
        print(f"[patch_g35_fluids] 锚缺失: {missing}", file=sys.stderr)
        return 1
    ...
    print(f"[patch_g35_fluids] 应用完成，重读核验 {'PASS' if ok else 'FAIL'}")
    return 0 if ok else 1
