...
"""G26.1 治理门 — 候选决策表闭集/锚纪律/横向对账（g26.wave.1.candidate_decisions，步骤 446）。

核验 `milestones/g26/G26_CANDIDATE_DECISIONS.md`：
冻结 8 行候选闭集全等（§1 G25 defer-to-G27+ 承接 3 行 + §3 G26 新增候选 5 行 G26-N1~N5）、
裁决枚举合法（go/closed-go/no-go/defer-to-G27+/strategic_override——G26 即本期，defer-to-G27+
不再合法）、零空行（全列非空）、承接锚纪律（§1/§3 行承接锚均含「重判条件 = …；兜底 = …」字面；
§1 行原触发条件字面转引含「→」分节）、
defer-to-G27+ 裁决行承接锚含 G2x 期别重评窗字面、
go 行验收映射锚义务（登记留痕位置含 G26_ACCEPTANCE_MAP，或依据面含 G26_CONTRACT §4.2 M 行锚定）、
...
用法：
  ...
  py -3 ci/g26_candidate_decisions_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g26.wave.1.candidate_decisions"
NUMERIC_STEP = 446  # 落盘前实测 registry/number_ledger.json CI_step.next_free=445 顺位领取
SUBJECT = "g26_candidate_decisions_check"
WAVE = "G26.1"
DECISIONS = ROOT / "milestones" / "g26" / "G26_CANDIDATE_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones" / "g26" / "g26_candidate_decisions_check_evidence_schema.json"
SOURCE_REF = (
    "G26_CONTRACT G-G26-1/§6/§7;G26_CANDIDATE_DECISIONS.md v1.0;G26_ACCEPTANCE_MAP §1/§2;"
    "G25_P2_DECISIONS.md §1（承接锚法定输入）+ G25_CONTRACT.md §8.7;"
...
SEC3_IDS = ["G26-N1", "G26-N2", "G26-N3", "G26-N4", "G26-N5"]
FROZEN_IDS = SEC1_IDS + SEC3_IDS
MAX_RD = 45
ALLOWED = frozenset({"go", "closed-go", "no-go", "defer-to-G27+", "strategic_override"})
...
def evaluate(text: str | None, deferred_doc: dict | None) -> list[dict]:
    ...
    facts: list[dict] = []
    if text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G26_CANDIDATE_DECISIONS.md 缺失（诚实红）"}]
    ...
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法（defer-to-G27+ 不再合法）" if not bad_dec else str(bad_dec)})
    ...
    for r in sec1 + sec3:
        if len(r) > 8 and _decision_of(r[4]) in ("go", "closed-go"):
            blob = r[6] + r[8]
            if "G26_ACCEPTANCE_MAP" not in blob and "G26_CONTRACT §4.2" not in blob:
                go_bad.append(r[0])
...
def emit(facts: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g26_candidate_decisions] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G26.1 治理门——候选决策表 8 行闭集（§1 三行 + §3 五行）+ 裁决枚举合法（defer 合法值 = defer-to-G27+ 点名期别）+ 零空行 + 承接锚纪律 + defer 行 G2x 窗 + go 行验收映射锚 + §2 RD 八条 status open 机核 + 零新 RD；只读文档与 registry，不代绿实现门",
...
def _fixture_text() -> str:
    def row10(rid: str, decision: str, trigger: str = "条件 → 兜底面") -> str:
        anchor = "重判条件 = G26 窗；兜底 = 维持" if decision.startswith("defer") else "重判条件 = 只追加重判；兜底 = 维持"
        trace = "本表行" if decision.startswith("defer") else "G26_ACCEPTANCE_MAP M-a"
    ...
    lines = ["# fixture", "", "## 1. 承接 3 行", "",
             ...
             "|---|---|---|---|---|---|---|---|---|---|"]
    for rid in SEC1_IDS:
        lines.append(row10(rid, "go" if rid in ("G13-N7", "RD-045-window", "G17-MD-F1") else "defer-to-G27+"))
    lines += ["", "## 2. open RD 八条", "",
...
def run_selftest() -> int:
    ...
    cases = [
        ("删除 §1 G13-N7 行 → 闭集红", "\n".join(l for l in text.splitlines() if not l.startswith("| G13-N7 ")),
         deferred, "sec1_closed_set"),
        ("裁决改 defer-to-G26+ → 枚举红", text.replace("| go |", "| defer-to-G26+ |", 1),
        ...
        ("go 行去映射锚 → go 锚红", text.replace("| G26_ACCEPTANCE_MAP M-a |", "| 本表行 |", 1),
    ...
    if failures:
        print(f"[g26_candidate_decisions] SELFTEST FAIL ({failures})")
        return 1
    print("[g26_candidate_decisions] SELFTEST PASS (8 RED + 1 GREEN + 真表臂)")
    return 0
