...
"""G17.1 治理门 — 候选决策表闭集/锚纪律/横向对账（g17.wave.1.candidate_decisions，步骤 294）。

G17_CONTRACT G-G17-1（G17.1 完成门 D-G17-2 候选决策表面）治理三门之一；G-G17-2 实现互锁
（ci/g17_interlock_check.py）事实门②消费本表 19 行零空行面、事实门④消费本门独立 PASS
机器事实。

核验 `milestones/g17/G17_CANDIDATE_DECISIONS.md`：
冻结 19 行候选闭集全等（§1 G16 defer-to-G17+ 承接 15 行 = 十四行 + G15-MD-F1；
§3 G17 新增候选 4 行 G17-N1~N4）、
裁决枚举合法（go/closed-go/no-go/defer-to-G18+/strategic_override——G17 即本期，defer-to-G17+
不再合法；closed-go = 兑现完结留痕变体，入 go 族不入 deferred 面）、
零空行（全列非空）、承接锚纪律（§1/§3 行承接锚均含「重判条件 = …；兜底 = …」字面；
§1 行原触发条件字面转引含「→」分节）、
defer-to-G18+ 裁决行承接锚含 G18+ 重评窗字面、
go 行验收映射锚义务（登记留痕位置含 G17_ACCEPTANCE_MAP，或依据面含 G17_CONTRACT.md §4.2 M 行
...
零新 RD max=RD-045）；外加横向机核——
  ① 与 G17_ACCEPTANCE_MAP 5 key（M-a~M-e 全 P0）互斥：候选行 ID 不得命中已 go 门裸 token；
  ② registry/deferred.json 对账：RD 八条目级 status 全 open、零新 RD（max=RD-045）。
（§2 RD 映射 8 行维持 open，不重复计入 19 行候选闭集三值枚举。）
...
用法：
  ...
  py -3 ci/g17_candidate_decisions_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g17.wave.1.candidate_decisions"
NUMERIC_STEP = 294  # 落盘前实测 registry/number_ledger.json CI_step.next_free=293 顺位领取
SUBJECT = "g17_candidate_decisions_check"
WAVE = "G17.1"
DECISIONS = ROOT / "milestones" / "g17" / "G17_CANDIDATE_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones" / "g17" / "g17_candidate_decisions_check_evidence_schema.json"
ACCEPTANCE_MAP = ROOT / "milestones" / "g17" / "G17_ACCEPTANCE_MAP.md"
DEFERRED = DEFERRED_PATH
SOURCE_REF = (
    "G17_CONTRACT G-G17-1/§6/§7;G17_CANDIDATE_DECISIONS.md v1.0;G17_ACCEPTANCE_MAP §1/§2;"
    "G15_P2_DECISIONS.md §4（承接锚法定输入）+ G16_CONTRACT.md §7 立项裁决 3;"
    "registry/deferred.json（RD 八条目级 status open 机核）"
)

# 冻结 ID 闭集（19 行）= §1 G16 defer-to-G17+ 承接 15 行 + §3 G17 新增 4 行。
SEC1_IDS = [
    "M61",          # G16 defer 承接：RD-039 mesh shader→M109
    "M52",          # G16 defer 承接：RD-040 SER→M108
    "M100-high",    # G16 defer 承接：RD-040 ReSTIR 高档
    "SAFE-GPU",     # G16 defer 承接：Safe GPU Operator Platform（独立期）
    "M127",         # G16 defer 承接：神经变形研究子轨
    "M98-l4",       # G16 defer 承接：M98 L4 Far Field
    "M114-strand",  # G16 defer 承接：毛发 strand 档
    "M118-hdr-cal", # G16 defer 承接：HDR 设备标定层
    "M125-adopt3",  # G16 defer 承接：Jolt 5.6 采纳三件
    "G10-N6",       # G16 defer 承接：BistroExterior 缺口
    "G10-N8",       # G16 defer 承接：-renderoffscreen 未测
    "G10-N17",      # G16 defer 承接：M137 scalars.flip 演进位
    "G11-N5",       # G16 defer 承接：锁定度量反向激励旁证
    "G13-N7",       # G16 defer 承接：帧生成 FG/MFG
    "G15-MD-F1",    # G17 主承接（本期唯一 go——M-a~M-e 五门承载）
]
...
# G17 即本期：defer-to-G17+ 不再合法，defer 合法值 = defer-to-G18+；
# closed-go = 兑现完结留痕变体（go 族终态，不充 G17 门绿、不入 deferred 面）。
ALLOWED = frozenset({"go", "closed-go", "no-go", "defer-to-G18+", "strategic_override"})
...
def _decision_base(decision: str) -> str | None:
    return (
        ...
        else "defer-to-G18+" if decision.startswith("defer-to-G18+")
...
def _row_findings(r: list[str], *, carry_over: bool) -> list[str]:
    ...
    if base is None:
        parts.append(f"非法裁决 {decision!r}（G17 合法枚举 go/closed-go/no-go/defer-to-G18+/strategic_override）")
    ...
    if base == "go" and "G17_ACCEPTANCE_MAP" not in r[8] and "G17_CONTRACT.md §4.2 M-" not in r[6]:
        parts.append("go 行缺验收映射锚（登记留痕位置须含 G17_ACCEPTANCE_MAP，或依据面含 G17_CONTRACT.md §4.2 M 行锚定）")
...
def validate(
    ...
    # RD max 断言：G17.1 治理波窗内零新 RD（≤45 = 治理门快照字面，RD-045 已存续 open）。
...
def run_check(
    ...
    if not p.is_file():
        # 诚实红：表未落盘不是绿
        return 1, [{"id": "file", "status": "FAIL", "detail": f"missing {p}（G17.1 候选决策表未落盘；诚实红，不假绿）"}]
    results = validate(
...
def emit(results: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g17_candidate_decisions] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G17.1 治理门——候选决策表 19 行闭集（§1 G16 defer-to-G17+ 承接 15 行 = 十四行 defer-to-G18+ + G15-MD-F1 go 主承接；§3 G17-N1~N4 新增 4 行全 go；§2 RD 映射 8 行维持 open 不重复计入三值枚举）：裁决枚举合法（go/closed-go/no-go/defer-to-G18+/strategic_override——G17 即本期，defer-to-G17+ 不再合法）/零空行/承接锚纪律（全表「重判条件 = …；兜底 = …」字面零缺项；§1 原触发条件字面转引含「→」分节）+ defer 行 G18+ 重评窗字面 + go 行验收映射锚义务 + closed-go 行 evidence/ 真跑件义务 + MAP 5 key 互斥 + deferred 对账（RD 八条目级 status 全 open，零新 RD max=RD-045）；no-go/defer 如实保持 open 不写进全绿叙述",
...
# ---------------------------------------------------------------------------
# selftest 合成夹具（十列形态；§1 决策分布 = defer-to-G18+ + go 正本）。
# ---------------------------------------------------------------------------
...
FIXTURE_SEC1_GO_IDS = frozenset({"G15-MD-F1"})


def _synth_sec1(rid: str) -> str:
    if rid in FIXTURE_SEC1_GO_IDS:
        decision = "go（G17 全期承接——M-a~M-e 五门承载）"
        ref = "milestones/g15/G15_P2_DECISIONS.md §4 行"
        anchor = "重判条件 = 门真跑产出面构成新事实时按只追加程序承接；兜底 = 维持未达标登记不冒充"
        reg = "本表 §1 行 + G17_ACCEPTANCE_MAP M-a~M-e 行（不新设 RD）"
        final = "go（五门承载面）"
    else:
        decision = "defer-to-G18+"
        ref = "milestones/g15/G15_P2_DECISIONS.md §4 行"
        anchor = "重判条件 = G18+ 重评窗触发条件齐备时按只追加程序重判；兜底 = 既有面维持"
        reg = "本表 §1 行（不新设 RD）"
        final = "open-defer（G18+）"
    return (
        f"| {rid} | 分项 | G16.1 defer-to-G17+ | 「G16+ 原触发条件齐备 → 兜底面维持（字面 0-byte）」 | "
...
def _synth_sec3(rid: str) -> str:
    return (
        f"| {rid} | 分项 | G17.1 新增 | 「口径争议时按只追加程序重判形态」 | go（G17.5 M-d 承载） | 理由 | "
        "milestones/g17/G17_CONTRACT.md §4.2 M-d 行 | "
        "重判条件 = 口径争议时按只追加程序重判形态并 §8 只追加修订；兜底 = 现口径维持 | "
        "本表 §3 行 + G17_ACCEPTANCE_MAP M-d 行 | go（M-d 承载面） |\n"
    )


def _full_fixture() -> str:
    sec1_head = "| ID | 分项名 | 来源波次 | 原触发条件字面 | 裁决 | 裁决理由 | 依据/证据路径 | 承接锚 | 登记留痕位置 | 最终状态 |\n|---|---|---|---|---|---|---|---|---|---|\n"
    sec2_head = "| RD | title（摘要） | 条目级 status | G17.1 处置 | 联动面 | 裁决理由 | 留痕位置 |\n|---|---|---|---|---|---|---|\n"
    sec3_head = sec1_head
    return (
        "## 1. G16 defer-to-G17+ 承接 15 行逐行转引终态裁决\n\n" + sec1_head + "".join(_synth_sec1(i) for i in SEC1_IDS)
...
def run_selftest() -> int:
    ...
    if not DECISIONS.is_file():
        ...
        if code != 0:
            ...
            print("[selftest] PASS: 真表 19 行闭集绿")

    with tempfile.TemporaryDirectory(prefix="g17_cand_selftest_") as td:
        ...
        _red(
            "§1 defer 行承接锚缺 G18+ 重评窗 → 红",
            full.replace(
                "重判条件 = G18+ 重评窗触发条件齐备时按只追加程序重判；兜底 = 既有面维持",
        ...
        _red(
            "非法裁决枚举（§1 defer-to-G17+ 本期不再合法）→ 红",
            full.replace("| defer-to-G18+ | 理由 |", "| defer-to-G17+ | 理由 |", 1),
            "row_M61",
        )
        _red(
            "空单元格（§3 裁决理由置空）→ 红",
            full.replace("| G17-N3 | 分项 | G17.1 新增 | 「口径争议时按只追加程序重判形态」 | go（G17.5 M-d 承载） | 理由 |",
                         "| G17-N3 | 分项 | G17.1 新增 | 「口径争议时按只追加程序重判形态」 | go（G17.5 M-d 承载） |  |", 1),
            "row_G17-N3",
        )
        _red(
        ...
        _red(
            "§1 go 行缺验收映射锚 → 红",
            full.replace("本表 §1 行 + G17_ACCEPTANCE_MAP M-a~M-e 行（不新设 RD）", "本表 §1 行（不新设 RD）", 1),
            "row_G15-MD-F1",
        )
        _red(
            "§3 go 行承接锚缺「兜底」→ 红",
    ...
    if failures:
        print(f"[g17_candidate_decisions] SELFTEST FAIL ({failures})")
        return 1
    print("[g17_candidate_decisions] SELFTEST PASS (11 RED + 真表/合成双臂 GREEN)")
    return 0
