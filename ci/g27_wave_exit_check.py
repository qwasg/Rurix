#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G27 波聚合门）
"""G27 波聚合门（参数化 g27.wave.{2..6}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g27/g27_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g27.wave.2.exit": ("G27.2", "g27_m_a_hzb_device_kernel", 465),
    "g27.wave.3.exit": ("G27.3", "g27_m_b_m61_mesh_shader_rejudgment", 467),
    "g27.wave.4.exit": ("G27.4", "g27_m_c_cluster_p4_gap_rejudgment", 469),
    "g27.wave.5.exit": ("G27.5", "g27_m_d_hlod_l4_counter_rejudgment", 471),
    "g27.wave.6.exit": ("G27.6", "g27_m_e_closed_gate_no_regression", 473),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g27_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G27_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g27_wave{n}_exit",
        notes=f"G27 wave {n} exit", host_section_pass=overall,
    )
