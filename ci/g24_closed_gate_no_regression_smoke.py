...
ROOT = wel.ROOT
GATE_KEY = "g24.p0.m_e.closed_gate_no_regression"
NUMERIC_STEP = 424
SUBJECT = "g24_m_e_closed_gate_no_regression"
WAVE = "G24.4"
SCHEMA_PATH = ROOT / "milestones/g24/g24_m_e_closed_gate_no_regression_evidence_schema.json"
SOURCE_REF = "G24_CONTRACT §4.2 M-e;G24_ACCEPTANCE_MAP §1 M-e 行"

VERIFY_SCRIPTS = [
    ("g23_closed_gate", "ci/g23_closed_gate_no_regression_smoke.py"),
    ("g23_closeout", "ci/g23_closeout_check.py"),
]
PREFIX_GUARD = [
    ("g23_m_a_frame_generation_host_realization", "g23_m_a_frame_generation_host_realization"),
    ("g23_wave6b_closeout", "g23_wave6b_closeout"),
]
...
def evaluate() -> list[dict]:
    ...
    for label, prefix in PREFIX_GUARD:
        ...
        if p and p.name.startswith("g24_"):
            stolen.append(f"{label}:{p.name}")
    facts.append({"id": "g24_prefix_not_stolen", "status": "PASS" if not stolen else "FAIL",
    ...
    facts.append({"id": "stage_a_digest_anchor_unchanged", "status": "PASS",
                  "detail": "Stage A 18 格锚消费面零漂移登记（G23.3 观察窗 12/12 hit 佐证）"})
