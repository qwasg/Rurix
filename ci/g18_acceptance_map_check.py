...
"""G18.1 治理门 — 验收映射覆盖 / 空行 / 双向命名空间一致性（g18.wave.1.acceptance_map，步骤 309）。

G18_CONTRACT G-G18-1（G18.1 完成门 D-G18-3 验收映射面）治理三门之一；G-G18-2 实现互锁
（ci/g18_interlock_check.py）事实门④消费本门与候选决策门的独立 PASS 机器事实。

核验 `milestones/g18/G18_ACCEPTANCE_MAP.md`：
§1 九行 P0（{M-a..M-i} 闭集全等）+ §2 零 go P1 空集断言 +
全部 symbolic key 匹配 `g18.p0.m_<a~e>.<slug>` 单一命名空间（key 的 m 段字母与行号一致、
脚本命令 `ci/g18_*_smoke.py` 每行单一且 --gate 参数 == canonical key、evidence schema
`g18_m_<a~e>_<key slug>_evidence_schema.json` 与 key 末段同 slug）+
波次 ∈ {G18.2,G18.3,G18.4,G18.5,G18.6} +
numeric_step 全列 `post-interlock actual-next-free allocation` 字面零预占 +
零空行/占位 + **双向一致**：MAP §1 与 G18_CONTRACT.md §4.2 对同一 P0 M 行给出的
判据与波次逐字相等（门冻结面 = 契约 §4.2 + MAP §1/§2，CI_GATES.md 为汇总视图不入比对，
沿 G15/G17 体例双向；G18 契约 §4.2 表 = M 行/判据（逐字）/波次 三列形态）。

MAP §1 = 七列形态（M 行 / symbolic gate key / 稳定脚本 / evidence schema 目标路径 /
判据（逐字）/ 波次 / numeric_step）。

只读文档，不代绿实现门；本门 PASS 只表示映射完整，不表示任何 P0 能力已实现。
用法：
  py -3 ci/g18_acceptance_map_check.py --gate g18.wave.1.acceptance_map
  py -3 ci/g18_acceptance_map_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g18.wave.1.acceptance_map"
NUMERIC_STEP = 309  # 落盘前实测 registry/number_ledger.json CI_step.next_free=293 顺位领取
SUBJECT = "g18_acceptance_map_check"
WAVE = "G18.1"
MAP_PATH = ROOT / "milestones" / "g18" / "G18_ACCEPTANCE_MAP.md"
CONTRACT_PATH = ROOT / "milestones" / "g18" / "G18_CONTRACT.md"
SCHEMA_PATH = ROOT / "milestones" / "g18" / "g18_acceptance_map_check_evidence_schema.json"
SOURCE_REF = (
    "G18_CONTRACT G-G18-1/§4.2;G18_ACCEPTANCE_MAP.md §1/§2/§4/§5;"
    "G17_P2_DECISIONS.md §3 defer-to-G18+ 十六行 + G17_CONTRACT.md §8.7 承接锚 + "
    "用户 U-59 指令（G18 全量方向）P0 清单 M-a~M-i"
)

# 冻结 9 行 P0 集合（G18.1 立项裁决口径；G18_CONTRACT §2/§4.2 M-a~M-i）。
EXPECTED_P0 = {"M-a", "M-b", "M-c", "M-d", "M-e", "M-f", "M-g", "M-h", "M-i"}
# MAP §5：所有波次属于 G18.2|G18.3|G18.4|G18.5|G18.6（§4.2 九行 P0 波次闭集）。
ALLOWED_WAVES = {"G18.2", "G18.3", "G18.4", "G18.5", "G18.6", "G18.7"}
...
KEY_RE = re.compile(r"^g18\.p0\.m_[a-i]\.[a-z0-9_]+$")
KEY_IN_CELL_RE = re.compile(r"`(g18\.p0\.m_[a-i]\.[a-z0-9_]+)`")
SECTION_RE = re.compile(r"^## (\d+)\. ")
SCRIPT_RE = re.compile(r"ci/g18_[a-z0-9_]+_smoke\.py")
SCHEMA_RE = re.compile(r"`(milestones/g18/g18_(m_[a-i])_[a-z0-9_]+_evidence_schema\.json)`")
BOLD_RE = re.compile(r"\*\*([^*]+)\*\*")
MAP_ROW_RE = re.compile(r"^\|\s*\*\*(M-[a-i])\*\*")
CONTRACT_ROW_RE = re.compile(r"^\|\s*\*\*(M-[a-i])\*\*")
...
def parse_contract_rows(text: str) -> dict[str, dict]:
    """解析 CONTRACT §4.2 形态的 `| **M-<a~e>** | 判据（逐字） | 波次 |` 三列行。"""
    out: dict[str, dict] = {}
    for line in text.splitlines():
...
def check_row(m: str, row: dict, seen_keys: dict[str, str], seen_schemas: dict[str, str]) -> list[str]:
    ...
    findings: list[str] = []
    m_seg = m.lower().replace("-", "_")  # M 行号 → key/schema m 段（m_a~m_i）
    if len(row["keys"]) != 1:
        findings.append(f"{m} 必须恰有一个 canonical symbolic key，实测 {row['keys']}")
        return findings
    key = row["keys"][0]
    if not KEY_RE.match(key):
        findings.append(f"{m} key `{key}` 不匹配 g18.p0.m_<a~e>.<slug>")
    ...
    slug = key.split(".")[3]
    if not row["scripts"]:
        findings.append(f"{m} 缺 `ci/g18_*_smoke.py` 脚本命令")
    ...
    if not schema_m:
        findings.append(f"{m} schema 路径不符 g18_m_<a~e>_<slug>_evidence_schema.json：{row['schema']!r}")
    else:
        path, schema_m_no = schema_m.group(1), schema_m.group(2)
        if schema_m_no != m_seg:
            findings.append(f"{m} schema 路径的 m 段不符：{path}")
        expected_schema = f"milestones/g18/g18_{m_seg}_{slug}_evidence_schema.json"
    ...
    if row["wave"] not in ALLOWED_WAVES:
        findings.append(f"{m} 波次 {row['wave']!r} 不在允许集合 {{G18.2,G18.3,G18.4,G18.5,G18.6}} 内")
...
def evaluate(map_text: str | None, contract_text: str | None) -> list[dict]:
    """12 facts：coverage_p0_set / coverage_p1_empty / row_M-a~M-i / two_way_M-a~M-i。"""
    results: list[dict] = []
    if map_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G18_ACCEPTANCE_MAP.md 缺失（诚实红，不假绿）"}]
    if contract_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G18_CONTRACT.md 缺失（诚实红，不假绿）"}]
    ...
    results.append({
        ...
        "detail": "§2 零 go P1 空集（G18.1 字面）" if p1_ok else f"§2 出现 P1 行 {sorted(p1_rows)}（G18.1 零 go P1 字面违例）",
    })
    ...
    for m in sorted(EXPECTED_P0):
        ...
        if len(row["keys"]) == 1:
            ...
            if other is None:
                two.append("G18_CONTRACT §4.2 缺行")
            else:
...
def emit(results: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g18_acceptance_map] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G18.1 治理门——验收映射覆盖/空行/双向命名空间一致性：5 P0（M-a~M-i）闭集全等 + §2 零 go P1 空集 + key/脚本/schema 单一命名空间同 slug + numeric_step 全列 post-interlock 字面零预占 + MAP §1 ↔ CONTRACT §4.2 判据/波次双向逐字一致（CI_GATES.md 为汇总视图不入比对）；本门 PASS 只表示映射完整，不表示任何 P0 能力已实现",
...
# ---------------------------------------------------------------------------
# selftest 合成夹具：9 行 P0 冻结集合的正本（七列形态），不依赖树上文件。
# ---------------------------------------------------------------------------

CANONICAL_ROWS = [
    ("M-a", "rurix_light_transport_depth", "ci/g18_rurix_light_transport_depth_smoke.py", "G18.2"),
    ("M-b", "presentation_pipeline_dual_profile", "ci/g18_presentation_pipeline_dual_profile_smoke.py", "G18.2"),
    ("M-c", "ue_arm_lighting_repair_and_render", "ci/g18_ue_arm_lighting_repair_and_render_smoke.py", "G18.3"),
    ("M-d", "dual_end_commercial_quality_verdict", "ci/g18_dual_end_commercial_quality_verdict_smoke.py", "G18.7"),
    ("M-e", "sl_runtime_upgrade_disposition", "ci/g18_sl_runtime_upgrade_disposition_smoke.py", "G18.4"),
    ("M-f", "fps_parity_reeval", "ci/g18_fps_parity_reeval_smoke.py", "G18.4"),
    ("M-g", "virtualized_geometry_p3", "ci/g18_virtualized_geometry_p3_smoke.py", "G18.5"),
    ("M-h", "frame_generation_independent_layer", "ci/g18_frame_generation_independent_layer_smoke.py", "G18.6"),
    ("M-i", "closed_gate_no_regression", "ci/g18_closed_gate_no_regression_smoke.py", "G18.7"),
]


def _fixture() -> tuple[str, str]:
    map_lines = ["# fixture G18_ACCEPTANCE_MAP", "", "## 1. P0 硬门（精确 9 行）", ""]
    contract_lines = ["# fixture G18_CONTRACT", "", "### 4.2 九行 P0", ""]
    for m, slug, script, wave in CANONICAL_ROWS:
        key = f"g18.p0.{m.lower().replace('-', '_')}.{slug}"
        schema = f"milestones/g18/g18_{m.lower().replace('-', '_')}_{slug}_evidence_schema.json"
        cmd = f"`py -3 {script} --gate {key}`"
        map_lines.append(
            f"| **{m}** | `{key}` | {cmd} | `{schema}` | 合成判据 {m} | **{wave}** | {NUMERIC_STEP_LITERAL} |"
        )
        contract_lines.append(f"| **{m}** | 合成判据 {m} | {wave} |")
    map_lines += ["", "## 2. 已 go P1 硬门（零行）", "", "G18.1 无 go 的 P1 行。", ""]
    return "\n".join(map_lines), "\n".join(contract_lines)
...
def run_selftest() -> int:
    ...
    cases: list[tuple[str, str, str, str]] = [
        ...
        (
            ...
            map_text.replace("--gate g18.p0.m_c.d3d12_host_lane_disposition`", "--gate g18.p0.m_c.d3d12_host_lane`"),
            contract_text,
            "row_M-c",
        ),
        (
            ...
            map_text.replace(
                "**G18.2** | post-interlock actual-next-free allocation |",
                "**G18.2** | 296 |",
                1,
            ),
            contract_text,
        ...
        (
            "波次改写为非法 G18.7c → 必须红",
            map_text.replace("合成判据 M-a | **G18.2**", "合成判据 M-a | **G18.7c**", 1),
            contract_text,
            "row_M-a",
        ),
        (
            ...
            map_text.replace(
                "G18.1 无 go 的 P1 行。",
                "| **M-a** | `g18.p0.m_a.dual_end_retest_warm_recalib` | `py -3 ci/g18_dual_end_retest_warm_recalib_smoke.py --gate g18.p0.m_a.dual_end_retest_warm_recalib` | `milestones/g18/g18_m_a_dual_end_retest_warm_recalib_evidence_schema.json` | 判据 | **G18.2** | post-interlock actual-next-free allocation |",
            ),
            contract_text,
            "coverage_p1_empty",
        ),
        ...
        (
            "schema 路径 m 段改写 → 必须红",
            map_text.replace(
                "milestones/g18/g18_m_c_d3d12_host_lane_disposition_evidence_schema.json",
                "milestones/g18/g18_m_d_d3d12_host_lane_disposition_evidence_schema.json",
                1,
            ),
            contract_text,
            "row_M-c",
    ...
    if failures:
        print(f"[g18_acceptance_map] SELFTEST FAIL ({failures})")
        return 1
    print("[g18_acceptance_map] SELFTEST PASS (9 RED + 1 GREEN + 真表臂)")
    return 0
