...
"""G17.2 实现互锁守卫（g17.gov.implementation_interlock，步骤 295；G17_CONTRACT G-G17-2）。

G17_CONTRACT G-G17-1（G17.1 完成门 D-G17-4 互锁诚实面）与 G-G17-2（实现互锁门）
双出处治理三门之一。读取事实源并逐项输出，最后给出 READY / BLOCKED / CLOSED。两类断言严格分开：

* **事实门**（当前允许为红，红即 BLOCKED，不是脚本失败）：
  ① G16_CONTRACT front matter status == closed（G16 close-out flip 终态，§8.4 签署块
    在位），且 G17.0 不可变 ref `8fc1fdaa` 已登记（G17_CONTRACT 立项裁决字面）；
  ② G17_CANDIDATE_DECISIONS 无空行（§1 G16 defer-to-G17+ 承接 15 行 + §3 新增候选 4 行
    = 19 行候选闭集全数在位〔§2 RD 映射 8 行不重复计入〕，全部数据行零空串/占位单元格）+
    registry/deferred.json history 只追加无静默改判（对 G17.0 不可变 ref 8fc1fdaa base：
    条目四字段 id/title/reason/backfill_condition 与 status/owner_milestone 0-byte、
    history 前缀只追加、条目零删除）+ G17_ACCEPTANCE_MAP §1 五行 P0（M-a~M-e 闭集全等，
    key/script/schema 路径/判据列无缺）与 §2 零 go P1 空集；
  ③ 用户 G17.2 开工指令留痕（「帮我一次性完成G17」字面在 G17_CONTRACT）+
    workflow 实测末号与 ledger namespaces.CI_step on_tree_max 一致且 next_free == on_tree_max + 1，
    且治理三门步骤 293~295 的 workflow/ledger 接线面在树（on_tree_max ≥ 295——共享编号按
    actual next_free 校准面；G17.1 期主会话提交批统一接线，接线前 = RED 诚实 BLOCKED）；
  ④ G17.1 治理两门独立 PASS（g17.wave.1.acceptance_map + g17.wave.1.candidate_decisions
    实测 run_check 全绿）。
* **一致性门**（红即脚本 FAIL，退出非零）：
  C1 双状态诚实：implementation_status == blocked，或事实门全绿（禁止治理完成冒充实现开工）；
  C2 §8 记录一致：§8 出现 G-G17-2 解锁记录 ⇔ 事实门全绿且 implementation_status != blocked；
  C3 数字步骤零预占：milestones/g17 P0 面无 numeric_step 数字赋值 / P0 数据行末列裸数字
    （治理三门步骤 293~295 白名单豁免——本波即落盘真脚本真步骤为例外合法面）、
    workflow 无 g17.p0./g17.p1./ci/g17_*_smoke.py token、ci/ 无 g17_*_smoke.py 预放；
  C4 src/spec/conformance 0-byte（治理期）：三面无 g17 实现面命中。
...
G-G17-2 重新开放凭据（契约 reopen 须新立项治理程序）。

诚实纪律：BLOCKED 是当前正确结论（治理三门步骤 293~295 workflow·ledger 接线面在树前——
主会话提交批统一接线），不得被当作 G-G17-2 PASS；
`--require-ready` 供 G17.2 实现 PR 作前置 required check（未 READY 即退出非零）；
`--gate g17.gov.implementation_interlock` 产 evidence（VERDICT 字面入档，BLOCKED 不充绿）。
`--selftest` 用可注入输入的受控负样本证明每组断言都能红/败。

用法：
  py -3 ci/g17_interlock_check.py --gate g17.gov.implementation_interlock
  py -3 ci/g17_interlock_check.py --require-ready
  py -3 ci/g17_interlock_check.py --selftest
"""
...
from g17_acceptance_map_check import run_check as acceptance_map_run_check  # noqa: E402
from g17_candidate_decisions_check import FROZEN_IDS as CANDIDATE_FROZEN_IDS  # noqa: E402
from g17_candidate_decisions_check import run_check as candidate_decisions_run_check  # noqa: E402

G17_CONTRACT = ROOT / "milestones/g17/G17_CONTRACT.md"
G16_CONTRACT = ROOT / "milestones/g16/G16_CONTRACT.md"
G17_CANDIDATES = ROOT / "milestones/g17/G17_CANDIDATE_DECISIONS.md"
G17_MAP = ROOT / "milestones/g17/G17_ACCEPTANCE_MAP.md"
LEDGER = ROOT / "registry/number_ledger.json"
DEFERRED = ROOT / "registry/deferred.json"
WORKFLOWS = ROOT / ".github/workflows"
SCHEMA_PATH = ROOT / "milestones/g17/g17_interlock_check_evidence_schema.json"

GATE_KEY = "g17.gov.implementation_interlock"
NUMERIC_STEP = 295  # 落盘前实测 registry/number_ledger.json CI_step.next_free=293 顺位领取
SUBJECT = "g17_interlock_check"
WAVE = "G17.1"
SOURCE_REF = (
    "G17_CONTRACT G-G17-2/§6/§7;G17_ACCEPTANCE_MAP §5/§6;"
    "registry/number_ledger.json;registry/deferred.json"
)

G17_0_IMMUTABLE_REF = "8fc1fdaa0a9c23c53d5a31b419ad5c9759aeaff8"  # G17.0 文档集不可变 ref
G17_USER_INSTRUCTION_LITERAL = "帮我一次性完成G17"
# 治理三门步骤（G17.1 落盘前实测 CI_step.next_free=293 顺位领取 293/294/295）；
# 接线面 = workflow 实测末号与 ledger on_tree_max 覆盖本批步骤（主会话提交批统一接线）。
GOVERNANCE_MAX_STEP = 295

# 空行门占位单元格闭集（MAP §5 no-empty 口径）。
PLACEHOLDER_CELLS = {"", "TBD", "TODO", "待定", "待补", "—"}
# 治理三门白名单（本波即落盘真脚本真步骤 293~295 的例外合法面；C3 数字预占扫描豁免）。
GOVERNANCE_GATE_KEYS = (
    "g17.wave.1.acceptance_map",
    "g17.wave.1.candidate_decisions",
    "g17.gov.implementation_interlock",
)
GOVERNANCE_SCHEMA_FILES = {
    "g17_acceptance_map_check_evidence_schema.json",
    "g17_candidate_decisions_check_evidence_schema.json",
    "g17_interlock_check_evidence_schema.json",
}

# 数字步骤零预占扫描面：numeric_step 数字赋值 / g17 P0 数据行末列裸数字 / workflow g17 实现面 token。
NUMERIC_ASSIGN_RE = re.compile(r"numeric_step[\"']?\s*[:=]\s*[\"']?\d")
NUMERIC_TAIL_CELL_RE = re.compile(r"\|\s*\d{1,4}\s*\|\s*$")
WORKFLOW_G17_RE = re.compile(r"g17\.p[01]\.|ci/g17_[a-z0-9_]+_smoke\.py")
WORKFLOW_STEP_RE = re.compile(r"步骤\s*(\d{1,4})")
# C4 扫描面：gate-key/脚本命名 token + g17 命名文件。
IMPL_SURFACE_G17_RE = re.compile(r"g17\.p[01]\.|g17\.wave\.|g17\.gov\.|ci/g17_")
# MAP §1 P0 行首单元格形态：| **M-a** | ...。
MAP_ROW_RE = re.compile(r"^\|\s*\*\*(M-[a-e])\*\*")
MAP_KEY_RE = re.compile(r"g17\.p0\.(m_[a-e])\.[a-z0-9_]+")
MAP_SCRIPT_RE = re.compile(r"ci/g17_[a-z0-9_]+_smoke\.py")
MAP_SCHEMA_RE = re.compile(r"milestones/g17/g17_m_[a-e]_[a-z0-9_]+_evidence_schema\.json")
EXPECTED_MAP_P0 = {"M-a", "M-b", "M-c", "M-d", "M-e"}
ACTIVATION_RE = re.compile(
    r"^#{2,5}\s*§?8\.\d+[^\n]*(implementation_status 解锁|G-G17-2)", re.MULTILINE
)
...
class TreeInputs:
    ...
    g17_contract_text: str | None = None
    ...
    workflow_g17_hits: list[str] = field(default_factory=list)
    g17_smoke_scripts: list[str] = field(default_factory=list)
...
def check_candidate_decisions(text: str | None) -> list[str]:
    """G17_CANDIDATE_DECISIONS 无空行：19 行候选闭集全数在位 + 全部数据行零空串/占位单元格。"""
    if text is None:
        return ["G17_CANDIDATE_DECISIONS.md 缺失"]
    findings: list[str] = []
...
def check_deferred_append_only(base_doc: dict | None, current_doc: dict | None) -> list[str] | None:
    """deferred.json history 只追加核验（vs G17.0 base）：条目零删除、条目级字段 0-byte、
...
def check_acceptance_map(text: str | None) -> list[str]:
    """G17_ACCEPTANCE_MAP §1 五行 P0（M-a~M-e 闭集全等）+ §2 零 go P1 空集 + 行面齐备。"""
    if text is None:
        return ["G17_ACCEPTANCE_MAP.md 缺失"]
    findings: list[str] = []
    ...
    if p1_rows:
        findings.append(f"MAP §2 P1 集合 {sorted(p1_rows)} 非空（G17.1 零 go P1 字面违例）")
    for mid, cells in p0_rows.items():
        ...
        if not MAP_SCRIPT_RE.search(blob):
            findings.append(f"MAP {mid} 行缺稳定脚本 ci/g17_*_smoke.py")
...
def _scan_numeric_step(root: Path) -> list[str]:
    """milestones/g17 P0 面数字步骤零预占扫描（治理三门白名单豁免）；返回违例行清单。"""
    hits: list[str] = []
    base = root / "milestones/g17"
    if not base.is_dir():
        return ["milestones/g17 目录缺失"]
    for path in sorted(base.rglob("*")):
        if not path.is_file() or path.suffix not in (".md", ".json", ".yml", ".yaml"):
            continue
        if path.name in GOVERNANCE_SCHEMA_FILES:
            continue  # 治理三门 schema 面（步骤 293~295 合法领取）
        try:
        ...
        for lineno, line in enumerate(text.splitlines(), 1):
            ...
            elif line.lstrip().startswith(("| **M-", "| `g17.")) and NUMERIC_TAIL_CELL_RE.search(line):
                hits.append(f"{rel}:{lineno} 数据行末列裸数字（疑似步骤预占）：{line.strip()[:80]}")
    return hits


def _scan_workflows(root: Path) -> list[str]:
    """workflow g17 实现面 token 扫描（g17.p0./g17.p1./ci/g17_*_smoke.py 即预占）。"""
    hits: list[str] = []
    ...
    for path in sorted(WORKFLOWS.glob("*.yml")) + sorted(WORKFLOWS.glob("*.yaml")):
        try:
            ...
            continue
        for lineno, line in enumerate(text.splitlines(), 1):
            if WORKFLOW_G17_RE.search(line):
...
def _scan_impl_surface(root: Path) -> list[str]:
    """src/spec/conformance 治理期 0-byte 扫描：g17 实现面 token / g17 命名文件即违例。"""
    hits: list[str] = []
    for sub in ("src", "spec", "conformance"):
        ...
        for path in sorted(base.rglob("*")):
            if not path.is_file():
                continue
            rel = path.relative_to(root)
            if "g17" in path.name.lower():
                hits.append(f"{rel}（文件名含 g17）")
                continue
            ...
            try:
                ...
                continue
            for lineno, line in enumerate(text.splitlines(), 1):
                if IMPL_SURFACE_G17_RE.search(line):
...
def load_inputs(root: Path) -> TreeInputs:
    ...
    base_text = _git_show_file(root, G17_0_IMMUTABLE_REF, "registry/deferred.json")
    ...
    return TreeInputs(
        g17_contract_text=_read(G17_CONTRACT),
        g16_contract_text=_read(G16_CONTRACT),
        candidate_findings=check_candidate_decisions(_read(G17_CANDIDATES)),
        deferred_findings=check_deferred_append_only(deferred_base, deferred_current),
        map_findings=check_acceptance_map(_read(G17_MAP)),
        ...
        workflow_g17_hits=_scan_workflows(root),
        g17_smoke_scripts=[p.name for p in sorted((root / "ci").glob("g17_*_smoke.py"))],
...
def evaluate_fact_gates(inp: TreeInputs) -> list[tuple[bool, str]]:
    ...
    g16_closeout_section = bool(inp.g16_contract_text and "§8.4" in inp.g16_contract_text)
    g17_ref_registered = bool(inp.g17_contract_text and G17_0_IMMUTABLE_REF in inp.g17_contract_text)
    facts.append(
        (
            g16_status == "closed" and g16_closeout_section and g17_ref_registered,
            f"① G16_CONTRACT status = {g16_status!r}（要求 closed）且 §8.4 签署块在位 = {g16_closeout_section}；"
            f"G17.0 不可变 ref {G17_0_IMMUTABLE_REF} 登记 = {g17_ref_registered}",
        )
    )
    ...
    if inp.deferred_findings is None:
        ...
        sub2.append("deferred history 只追加（vs G17.0 base 四字段 0-byte）")
    if inp.map_findings is None:
        ok2 = False
        sub2.append("G17_ACCEPTANCE_MAP 缺失")
    ...
    instruction_recorded = bool(
        inp.g17_contract_text and G17_USER_INSTRUCTION_LITERAL in inp.g17_contract_text
    )
    ...
    facts.append(
        (
            ...
            f"④ G17.1 治理两门独立 PASS（g17.wave.1.acceptance_map + g17.wave.1.candidate_decisions"
...
def evaluate_consistency_gates(inp: TreeInputs, facts_all_green: bool) -> list[tuple[bool, str]]:
    ...
    impl_status = front_matter_field(inp.g17_contract_text, "implementation_status") if inp.g17_contract_text else None
    consistency.append(
        (
            impl_status == "blocked" or facts_all_green,
            f"C1 G17_CONTRACT implementation_status = {impl_status!r}；事实门全绿 = {facts_all_green}"
            "（事实未全绿时必须保持 blocked，禁止治理完成冒充实现开工）",
        )
    )

    activated = inp.g17_contract_text is not None and bool(ACTIVATION_RE.search(inp.g17_contract_text))
    consistency.append(
        (
            activated == (facts_all_green and impl_status not in (None, "blocked")),
            f"C2 §8 G-G17-2 解锁记录存在 = {activated}；事实门全绿 = {facts_all_green}、"
            f"implementation_status = {impl_status!r}（双状态与 §8 记录必须一致）",
        )
    )

    preclaim = inp.numeric_step_violations + inp.workflow_g17_hits + inp.g17_smoke_scripts
    if impl_status == "blocked":
        consistency.append(
            (
                not preclaim,
                f"C3 数字步骤零预占：milestones/g17 numeric_step 违例 {len(inp.numeric_step_violations)} 处、"
                f"workflow g17 实现面 token {len(inp.workflow_g17_hits)} 处、ci/g17_*_smoke.py 预放 "
                f"{inp.g17_smoke_scripts or '无'}（治理三门 293~295 白名单豁免）"
        ...
        consistency.append(
            (
                ...
                f"C4 src/spec/conformance 治理期 0-byte：g17 实现面 token/命名命中 "
        ...
        consistency.append(
            (
                ...
                f"{len(inp.numeric_step_violations)} 处 / workflow g17 实现面 token {len(inp.workflow_g17_hits)} 处 / "
                f"ci/g17_*_smoke.py {len(inp.g17_smoke_scripts)} 件均为解锁后合法实现面，非预占；"
        ...
        consistency.append(
            (
                ...
                f"治理期口径不适用；skipped_reason=实现波合法改动三面，实测 g17 实现面 token/命名命中 "
...
def run(inp: TreeInputs, require_ready: bool = False, printer=print) -> tuple[int, str]:
    ...
    contract_status = (
        front_matter_field(inp.g17_contract_text, "status") if inp.g17_contract_text else None
    )
    if contract_status == "closed":
        printer(
            "[g17_interlock] 事实门/一致性门：not_applicable"
            "（status='closed' 收口终态，互锁使命完结；skipped_reason=G17.2+ 开工门问题"
            "不再适用——G17.7b close-out READY 后 status flip；active/blocked 态恢复原机核，"
            "判据语义 0-byte）"
        )
        printer("[g17_interlock] VERDICT = CLOSED")
        printer(
            "  CLOSED 是收口终态正确结论：本守卫回答「G17.2 可否开工」，契约 closed 后该问题"
            "不再适用；不得被当作 G-G17-2 重新开放凭据（契约 reopen 须新立项治理程序）。"
        )
        return 0, "CLOSED"
    ...
    printer("[g17_interlock] 事实门（当前可为红）：")
    for ok, msg in facts:
        printer(f"  {'PASS' if ok else 'RED '} {msg}")
    printer("[g17_interlock] 一致性门（红即脚本失败）：")
    for ok, msg in consistency:
        printer(f"  {'PASS' if ok else 'FAIL'} {msg}")

    verdict = "READY" if facts_all_green else "BLOCKED"
    printer(f"[g17_interlock] VERDICT = {verdict}")
    if verdict == "BLOCKED":
        missing = [msg.split("：", 1)[0] for ok, msg in facts if not ok]
        printer(f"  缺项清单：{missing}")
        printer(
            "  BLOCKED 是当前正确结论：G17.2+ 的 src/、spec/、conformance/ 与 P0 数字 workflow 步骤保持 0-byte；"
            "本输出不得被当作 G-G17-2 PASS。"
        )

    consistency_failed = [msg for ok, msg in consistency if not ok]
    if consistency_failed:
        printer(f"[g17_interlock] FAIL — {len(consistency_failed)} 项一致性门为红")
        return 1, verdict
    if require_ready and verdict != "READY":
        printer("[g17_interlock] FAIL — --require-ready 模式下互锁未 READY")
        return 1, verdict
    return 0, verdict
...
def run_gate() -> int:
    ...
    facts_all_green = all(ok for ok, _ in facts)
    impl_status = front_matter_field(inp.g17_contract_text, "implementation_status") if inp.g17_contract_text else None
    contract_status = front_matter_field(inp.g17_contract_text, "status") if inp.g17_contract_text else None
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g17_interlock] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    ecode, _ = wel.emit_wave_evidence(
        ...
        notes=(
            f"G17.1 治理门——实现互锁 validator 诚实报告：VERDICT={verdict}"
            + (
                "（BLOCKED 是当前正确结论：治理三门步骤 293~295 workflow·ledger 接线面在树前"
                "（主会话提交批统一接线），G17.2 不得开工；不充绿、不以叙述替代机器事实）"
                if verdict == "BLOCKED"
                else "（READY 是当前正确结论：事实门①~④全绿——G16 closed + 治理文档三面 + "
                "开工指令留痕与共享编号校准 + 治理两门独立 PASS，G17.2 开工面开放；不以叙述替代机器事实）"
            )
            + "；一致性门 C1~C4 全绿；"
            "--require-ready 供 G17.2 实现 PR 前置 required check"
        ),
...
- **立项裁决**：现在立项；G17.0 不可变 ref = `{G17_0_IMMUTABLE_REF}`。
- **用户开工指令留痕**：2026-08-24 用户指令——「{G17_USER_INSTRUCTION_LITERAL}」。
...
def _fixture_map_text() -> str:
    ...
    for letter, slug, script in (
        ("a", "dual_end_retest_warm_recalib", "ci/g17_dual_end_retest_warm_recalib_smoke.py"),
        ("b", "ngx_evolution_alignment", "ci/g17_ngx_evolution_alignment_smoke.py"),
        ("c", "d3d12_host_lane_disposition", "ci/g17_d3d12_host_lane_disposition_smoke.py"),
        ("d", "t100_final_verdict", "ci/g17_t100_final_verdict_smoke.py"),
        ("e", "closed_gate_no_regression", "ci/g17_closed_gate_no_regression_smoke.py"),
    ):
        lines.append(
            f"| **M-{letter}** | `g17.p0.m_{letter}.{slug}` `{script}` | "
            f"`milestones/g17/g17_m_{letter}_{slug}_evidence_schema.json` | 判据 | G17.2 | "
...
def _good_inputs() -> TreeInputs:
    ...
    return TreeInputs(
        g17_contract_text=_GOOD_G17_CONTRACT,
...
def run_selftest() -> int:
    ...
    case("G16 status 改 active → ① 红", inp, "① G16_CONTRACT status = 'active'", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.g17_contract_text = inp.g17_contract_text.replace(G17_0_IMMUTABLE_REF, "deadbeef")
    case("G17.0 不可变 ref 未登记 → ① 红", inp, "G17.0 不可变 ref", "BLOCKED", 0)
    ...
    inp = copy.deepcopy(_good_inputs())
    inp.g17_contract_text = inp.g17_contract_text.replace(G17_USER_INSTRUCTION_LITERAL, "路径追踪生产化")
    case("用户开工指令字面缺失 → ③ 红", inp, "③ 用户 G17.2 开工指令留痕", "BLOCKED", 0)
    ...
    case(
        "治理三门步骤 293~295 接线面缺失（主会话提交批前态）→ ③ 红",
        inp, "接线面在树 = False", "BLOCKED", 0,
    )

    inp = copy.deepcopy(_good_inputs())
    inp.governance_gates_green = False
    case("治理两门未绿注入 → ④ 红", inp, "④ G17.1 治理两门独立 PASS", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.numeric_step_violations = ["milestones/g17/G17_ACCEPTANCE_MAP.md:30 numeric_step 数字赋值：numeric_step: 296"]
    case("数字步骤预占注入 → C3 FAIL 退 1（VERDICT 不受一致性门影响）", inp, "C3 数字步骤零预占", "READY", 1)

    inp = copy.deepcopy(_good_inputs())
    inp.impl_surface_hits = ["spec/g17_perf_final.md（文件名含 g17）"]
    case("spec 面 g17 命中注入 → C4 FAIL 退 1", inp, "C4 src/spec/conformance 治理期 0-byte", "READY", 1)

    inp = copy.deepcopy(_good_inputs())
    inp.g17_contract_text = inp.g17_contract_text.replace(
        "## 8. Implementation activation / Close-out（只追加区）",
        "### §8.1 G-G17-2 implementation_status 解锁记录",
    )
    case("落 §8 解锁记录但 front matter 未翻 → C2 FAIL 退 1", inp, "C2 §8 G-G17-2 解锁记录存在 = True", "READY", 1)

    # 两态口径：unblocked 态 C3/C4 自动不适用（skipped_reason 登记）。
    unblocked_text = _GOOD_G17_CONTRACT.replace(
    ...
    inp = copy.deepcopy(_good_inputs())
    inp.g17_contract_text = unblocked_text
    inp.numeric_step_violations = ["milestones/g17/G17_ACCEPTANCE_MAP.md:30 numeric_step 数字赋值：numeric_step: 296"]
    inp.workflow_g17_hits = [".github/workflows/pr-smoke.yml:3200 g17.p0.m_a.dual_end_retest_warm_recalib"]
    inp.g17_smoke_scripts = ["g17_dual_end_retest_warm_recalib_smoke.py"]
    inp.impl_surface_hits = ["spec/perf_final.md:1 g17.p0. 新条款"]
    case(
    ...
    inp.g17_contract_text = unblocked_text
    inp.governance_gates_green = False
    case("unblocked 态事实门红 → C1 仍 FAIL 退 1（两态校准不遮蔽 C1/C2）", inp, "C1 G17_CONTRACT implementation_status = 'unblocked'", "BLOCKED", 1)

    # closed 三态口径：status==closed → VERDICT=CLOSED 退 0。
    inp = copy.deepcopy(_good_inputs())
    inp.g17_contract_text = unblocked_text.replace("status: active", "status: closed")
    case(
    ...
    if tree_verdict in ("BLOCKED", "READY", "CLOSED") and tree_code == expected_tree_exit:
        print(
            ...
            f"{'治理三门接线面在树前期' if tree_verdict == 'BLOCKED' else ('收口终态' if tree_verdict == 'CLOSED' else 'G-G17-2 解锁条件已齐')}，符合当前事实预期）"
        )
    else:
        print(
    ...
    if failures:
        print(f"[g17_interlock] SELFTEST FAIL ({failures})")
        return 1
    print("[g17_interlock] SELFTEST PASS (17 RED + 1 GREEN + 1 TREE)")
    return 0
