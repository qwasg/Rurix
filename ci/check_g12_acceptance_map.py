...
"""G12 治理守卫 — 验收映射覆盖 / 空行 / 三向命名空间一致性（G12_CONTRACT §4.2 冻结面）。

对应 milestones/g12/CI_GATES.md §3 的 `g12.gov.acceptance_coverage`
（G12.1 声明 validator 名与判据,materialize 期落盘——G12.1 起映射完整性机核由
check_g12_implementation_interlock 事实门③承载,G12.7b close-out 前三向独立
validator 按 CI_GATES §3 / MAP §4/§5 冻结判据成环;判据语义 0-byte）。

事实源三份，必须逐字一致（G12.1 冻结口径）：
  1. milestones/g12/G12_ACCEPTANCE_MAP.md §1（8 P0）与 §2（1 已 go P1：M166）
  2. milestones/g12/G12_CONTRACT.md §4.2（8 P0 独立断言表；不载 P1 行）
  3. milestones/g12/CI_GATES.md §4（8 P0）与 §4A（1 已 go P1）
...
三向比对面（MAP §4.1 机器可核声明）：
  ...
  - evidence schema 目标路径：MAP §1 ↔ CI_GATES §4 逐行逐字相等，且全部命中 CONTRACT §4.2
    冻结的统一形态 `milestones/g12/g12_m<###>_<slug>_evidence_schema.json`（CONTRACT 只冻结
    形态不逐行载路径，故 CONTRACT 侧以形态机核、逐行字面由 MAP ↔ CI_GATES 双侧比对）；
  - P1 行：MAP §2 ↔ CI_GATES §4A 双向逐字比对（key/脚本/schema；CONTRACT §4.2 不载 P1 行）。
  - slug 同字面机核：key 末段 slug、脚本名 `ci/g12_<slug>_smoke.py`、schema 文件名
    `g12_m<###>_<slug>_evidence_schema.json` 三者同 slug（MAP §1 单一命名空间字面）。
...
MAP_PATH = ROOT / "milestones/g12/G12_ACCEPTANCE_MAP.md"
CONTRACT_PATH = ROOT / "milestones/g12/G12_CONTRACT.md"
CI_GATES_PATH = ROOT / "milestones/g12/CI_GATES.md"

# 冻结 8 行 P0 集合（2026-08-17 G12.1 立项裁决口径；go-P1 波次开工前只追加）。
EXPECTED_P0 = {
    "M158", "M159", "M160", "M161",
    "M162",
    "M163", "M164",
...
# 已 go P1 精确集合（G12_CONTRACT §4.2 末段「M166（PT 生产化标定）为 P1，
# 入验收映射随主门核验」）。后续波次判 go 的 P1 只追加扩本集合 + MAP §2 + CI_GATES §4A。
EXPECTED_P1 = {"M166"}

# MAP §5：所有波次属于 G12.2|G12.3|G12.4|G12.5。
ALLOWED_WAVES = {"G12.2", "G12.3", "G12.4", "G12.5"}
...
KEY_RE = re.compile(r"^g12\.p0\.m\d{3}\.[a-z0-9_]+$")
KEY_IN_CELL_RE = re.compile(r"`(g12\.p0\.m\d{3}\.[a-z0-9_]+)`")
KEY_P1_RE = re.compile(r"^g12\.p1\.m\d{3}\.[a-z0-9_]+$")
KEY_P1_IN_CELL_RE = re.compile(r"`(g12\.p1\.m\d{3}\.[a-z0-9_]+)`")
KEY_P0_CELL_RE = re.compile(r"`(g12\.p0\.m\d{3}\.[a-z0-9_]+)`")
KEY_P1_CELL_RE = re.compile(r"`(g12\.p1\.m\d{3}\.[a-z0-9_]+)`")
SECTION_RE = re.compile(r"^## (\d+)\. ")
SCRIPT_RE = re.compile(r"ci/g12_[a-z0-9_]+_smoke\.py")
SCHEMA_RE = re.compile(r"`(milestones/g12/g12_(m\d{3})_[a-z0-9_]+_evidence_schema\.json)`")
...
def check_rows(
    ...
    for m in sorted(rows):
        ...
        if not schema_m:
            findings.append(f"[no-empty] {m} schema 路径不符 g12_m###_<slug>_evidence_schema.json：{row['schema']!r}")
        else:
            ...
            expected_schema = f"milestones/g12/g12_{m.lower()}_{slug}_evidence_schema.json"
...
def check(map_text: str, contract_text: str, ci_gates_text: str) -> Finding:
    ...
    if set(rows) != EXPECTED_P0:
        findings.append(
            f"[coverage] P0 集合不等于冻结 8 行：缺 {sorted(EXPECTED_P0 - set(rows))}，多 {sorted(set(rows) - EXPECTED_P0)}"
        )
    ...
    for m in sorted(rows):
        ...
        schema = schema_m.group(1) if schema_m else None
        for name, table in (("G12_CONTRACT §4.2", contract_rows), ("CI_GATES §4", ci_rows)):
            if m not in table:
        ...
        # 见 check_rows 的 g12_m###_<slug> 同字面断言）。
...
# ---------------------------------------------------------------------------
# selftest 合成夹具：8 行 P0 冻结集合 + 1 行已 go P1 的正本，不依赖树上文件。
# ---------------------------------------------------------------------------

CANONICAL_ROWS = [
    ("M158", "mis_full_surface", "G12.2"),
    ("M159", "russian_roulette_prod", "G12.2"),
    ("M160", "sampling_lds_upgrade", "G12.2"),
    ("M161", "convergence_criterion_prod", "G12.2"),
    ("M162", "denoise_pipeline_tsr", "G12.3"),
    ("M163", "ue_pt_parity", "G12.4"),
    ("M164", "regression_guard", "G12.4"),
    ("M165", "pt_throughput_baseline", "G12.5"),
]

CANONICAL_P1_ROWS = [
    ("M166", "pt_production_calibration", "G12.2"),
]


def _fixture() -> tuple[str, str, str]:
    map_lines = ["# fixture G12_ACCEPTANCE_MAP", "", "## 1. P0 硬门", ""]
    contract_lines = ["# fixture G12_CONTRACT", "", "### 4.2 P0 独立断言", ""]
    gates_lines = ["# fixture CI_GATES", "", "## 4. 8 个 P0 独立机器断言", ""]
    for m, slug, wave in CANONICAL_ROWS:
        key = f"g12.p0.{m.lower()}.{slug}"
        script = f"ci/g12_{slug}_smoke.py"
        schema = f"milestones/g12/g12_{m.lower()}_{slug}_evidence_schema.json"
...
def run_selftest() -> int:
    ...
    cases: list[tuple[str, str, str, str, str]] = [
        (
            "删除 M165 行 → coverage 必须红",
            "\n".join(l for l in map_text.splitlines() if not l.startswith("| **M165**")),
            contract_text,
            ci_text,
            "P0 集合不等于冻结 8 行",
        ),
        (
            "MAP 单侧改写 M163 key → three-way 必须红",
            map_text.replace("g12.p0.m163.ue_pt_parity", "g12.p0.m163.ue_pt"),
            contract_text,
            ci_text,
            "[three-way] M163 key 漂移",
        ),
        (
            "恢复大写 key 写法 → coverage 必须红",
            map_text.replace("`g12.p0.m158.mis_full_surface`", "`G12.P0.M158.MIS_FULL_SURFACE`", 1),
            contract_text,
            ci_text,
            "M158 必须恰有一个 canonical symbolic key",
        ),
        (
            "判据列置为待补 → no-empty 必须红",
            map_text.replace("合成判据 M159", "待补", 1),
            contract_text,
            ci_text,
            "[no-empty] M159",
        ),
        (
            ...
            map_text,
            contract_text,
            ci_text.replace("ci/g12_russian_roulette_prod_smoke.py", "ci/g12_russian_roulette_smoke.py"),
            "[three-way] M159 script 漂移",
        ),
        (
            "波次改写为非法 G12.6 → no-empty 必须红",
            map_text.replace("合成 RED 臂 M158 | host+device | **G12.2**", "合成 RED 臂 M158 | host+device | **G12.6**", 1),
            contract_text,
            ci_text,
            "[no-empty] M158 波次",
        ),
        (
            ...
            map_text.replace(
                "milestones/g12/g12_m160_sampling_lds_upgrade_evidence_schema.json",
                "milestones/g12/g12_m161_sampling_lds_upgrade_evidence_schema.json",
                1,
            ),
            contract_text,
            ci_text,
            "[no-empty] M160 schema 路径的 m### 段不符",
        ),
        (
            "numeric_step 列填入数字 → 预占必须红",
            map_text.replace(
                "host+device | **G12.2** | post-interlock actual-next-free allocation |",
                "host+device | **G12.2** | 218 |",
                1,
            ),
            contract_text,
            ci_text,
            "[no-empty] M158 numeric_step",
        ),
        (
            ...
            contract_text,
            ci_text.replace(
                "milestones/g12/g12_m164_regression_guard_evidence_schema.json",
                "milestones/g12/g12_m164_regression_evidence_schema.json",
            ),
            "[three-way] M164 schema 漂移",
        ),
        (
            "删除 §2 M166 行 → P1 coverage 必须红",
            "\n".join(l for l in map_text.splitlines() if not l.startswith("| **M166**")),
            contract_text,
            ci_text,
            "P1 集合不等于 §2 声明集合",
        ),
        (
            ...
            map_text.replace(
                "`g12.p1.m166.pt_production_calibration`", "`g12.p0.m166.pt_production_calibration`", 1
            ),
            contract_text,
            ci_text,
            "出现 p0 key",
        ...
        (
            ...
            map_text,
            contract_text,
            ci_text.replace("ci/g12_pt_production_calibration_smoke.py", "ci/g12_pt_production_smoke.py"),
            "[two-way] M166 script 漂移",
        ),
        (
            ...
            contract_text,
            ci_text.replace(
                "milestones/g12/g12_m166_pt_production_calibration_evidence_schema.json",
                "milestones/g12/g12_m166_pt_calibration_evidence_schema.json",
            ),
            "[two-way] M166 schema 漂移",
        ),
        (
            "脚本名与 key slug 不同字面 → coverage 必须红",
            map_text.replace("ci/g12_regression_guard_smoke.py", "ci/g12_regression_smoke.py"),
            contract_text,
            ci_text,
            "M164 脚本名",
        ),
    ...
    if failures:
        print(f"[check_g12_acceptance_map] SELFTEST FAIL ({failures})")
        return 1
    print("[check_g12_acceptance_map] SELFTEST PASS (14 RED + 1 GREEN)")
    return 0
...
def main() -> int:
    ...
    for path in (MAP_PATH, CONTRACT_PATH, CI_GATES_PATH):
        if not path.exists():
            print(f"[check_g12_acceptance_map] FAIL — 缺事实源 {path.relative_to(ROOT)}")
            return 1
    ...
    if findings:
        print(f"[check_g12_acceptance_map] FAIL ({len(findings)} 项)")
        for f in findings:
            print(f"  - {f}")
        return 1
    print(
        "[check_g12_acceptance_map] PASS"
        "（8 P0 + 1 已 go P1（M166）覆盖齐备；9 key 唯一且同一命名空间；"
