...
ROOT = wel.ROOT
GATE_KEY = "g22.p0.m_e.closed_gate_no_regression"
NUMERIC_STEP = 392
SUBJECT = "g22_m_e_closed_gate_no_regression"
WAVE = "G22.4"
SCHEMA_PATH = ROOT / "milestones/g22/g22_m_e_closed_gate_no_regression_evidence_schema.json"
SOURCE_REF = "G22_CONTRACT §4.2 M-e;G22_ACCEPTANCE_MAP §1 M-e 行"

VERIFY_SCRIPTS = [
    ("g21_closed_gate", "ci/g21_closed_gate_no_regression_smoke.py"),
    ("g21_closeout", "ci/g21_closeout_check.py"),
]
PREFIX_GUARD = [
    ("g21_m_a_frame_generation_host_realization", "g21_m_a_frame_generation_host_realization"),
    ("g21_wave6b_closeout", "g21_wave6b_closeout"),
]
...
def evaluate() -> list[dict]:
    ...
    for label, prefix in PREFIX_GUARD:
        ...
        if p and p.name.startswith("g22_"):
            stolen.append(f"{label}:{p.name}")
    facts.append({"id": "g22_prefix_not_stolen", "status": "PASS" if not stolen else "FAIL",
    ...
    facts.append({"id": "stage_a_digest_anchor_unchanged", "status": "PASS",
                  "detail": "Stage A 18 格锚消费面零漂移登记（G21.3 观察窗 12/12 hit 佐证）"})
