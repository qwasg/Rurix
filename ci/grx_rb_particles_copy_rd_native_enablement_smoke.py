...
STAGED_CONTAINER = ROOT / "target" / "grx" / "rd_containers" / "particles_copy.rd_container.bin"
...
ACTIVE_MARKER = "RXGD_RD_NATIVE_PARTICLES_COPY active"
...
GODOT_EXE_ENV = "RURIX_GRX_RB_PARTICLES_COPY_GODOT_EXE"
SCRATCH_SOURCE_ENV = "RURIX_GRX_RB_PARTICLES_COPY_GODOT_SOURCE"
SCRATCH_SOURCE_PROVENANCE_ENV = "RURIX_GRX_RB_PARTICLES_COPY_GODOT_SOURCE_PROVENANCE"
SCRATCH_BUILD_COMMAND_ENV = "RURIX_GRX_RB_PARTICLES_COPY_GODOT_BUILD_COMMAND"
SCRATCH_BUILD_LOG_ENV = "RURIX_GRX_RB_PARTICLES_COPY_GODOT_BUILD_LOG"
CAPTURE_PREFIX_ENV = "RURIX_GRX_RB_PARTICLES_COPY_CAPTURE_PREFIX"
CONTAINER_OVERRIDE_ENV = "RURIX_GRX_RB_PARTICLES_COPY_CONTAINER"
DXC_DIR_ENV = "RURIX_DXC_DIR"

TARGET_BACKEND = "Godot 4.7-dev Windows D3D12 Forward+"
BACKEND_SETTING = "rendering/rurix_accel/passes/particles_copy/backend"
CONTAINER_SETTING = "rendering/rurix_accel/passes/particles_copy/rd_container_path"
...
def write_evidence(status: str, *, reason: str | None = None, extra: dict | None = None) -> None:
    ...
    print(f"[grx-rb-particles-copy-rd-native-smoke] wrote {rel(EVIDENCE_OUT)} status={status}")
    if status == "success":
        ...
        print(
            "[grx-rb-particles-copy-rd-native-smoke] wrote "
            f"{rel(SUCCESS_EVIDENCE_OUT)} status=success (historical measured success)"
        )


def fail(msg: str, extra: dict | None = None) -> int:
    print(f"[grx-rb-particles-copy-rd-native-smoke] FAIL {msg}", file=sys.stderr)
...
def skip_environment(msg: str, extra: dict | None = None) -> int:
    ...
    print(f"[grx-rb-particles-copy-rd-native-smoke] SKIP {msg} (降级 SKIP,退出 0)")
...
def skip_measured_prerequisite(prerequisite: str, msg: str, extra: dict) -> int:
    print(
        "[grx-rb-particles-copy-rd-native-smoke] SKIP (measured) first missing "
...
def write_smoke_project(
    ...
    *,
    ...
    """Minimal deterministic Godot project for the particles_copy rd_native gate.
    Only the backend selector and rd_container_path differ between legs."""
    project_dir.mkdir(parents=True, exist_ok=True)

    project_text = f"""\
; Engine configuration file.
; Auto-generated by ci/grx_rb_particles_copy_rd_native_enablement_smoke.py

config_version=5

[application]

config/name="GRX Route B particles_copy rd_native enablement smoke"
...
rurix_accel/passes/particles_copy/backend={backend}
rurix_accel/passes/particles_copy/rd_container_path="{Path(container_path).as_posix() if container_path else ''}"
"""
    scene_text = """\
...
[node name="GRXRBParticlesRoot" type="Node3D"]
...
func _ready() -> void:
    ...
    draw_mat.albedo_color = Color(0.8, 0.6, 0.3)
...
func _capture(idx: int) -> void:
    ...
    if prefix.is_empty():
        printerr("GRXRBParticles: capture prefix env var missing")
        get_tree().quit(3)
        return
    ...
    if idx == 0:
        ...
        meta.close()
    print("GRXRBParticles: captured idx=%d frame=%d width=%d height=%d" % [idx, _frames, img.get_width(), img.get_height()])
"""
...
def main() -> int:
    ...
    _EVIDENCE_BASE.update(
        {
            ...
            "backend_selector": "rendering/rurix_accel/passes/particles_copy/backend",
            ...
            "performance_claim": "none",
            "kernel_subset": "COPY_MODE_FILL_INSTANCES 3D ALIGN_DISABLED/Z_BILLBOARD subset (!do_sort && copy_mode_2d==0)",
    ...
    for name in ("candidate", "fail_closed"):
        ...
        print(
            f"[grx-rb-particles-copy-rd-native-smoke] LDR absolute diff ({name} vs "
    ...
    print(
        "[grx-rb-particles-copy-rd-native-smoke] PASS measured rd_native REAL replacement "
