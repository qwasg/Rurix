#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GRX-019: gated real fused_post_chain pass enablement smoke.

This harness drives the opt-in real-pass enablement gate for the GRX-019
fused_post_chain path (fused luminance final level + tonemap). It is a strict,
fail-closed bring-up gate, NOT a default enablement:
``rendering/rurix_accel/passes/fused_post_chain/enabled`` stays ``false`` by
default, the ``.../fused_post_chain/dispatch_real_pass`` opt-in also defaults to
``false``, and NO FPS, GPU-timestamp, or performance claim is made anywhere in
this gate. Template copy of ``ci/grx013_particles_copy_real_pass_enablement_smoke.py``
pointed at the fused_post_chain pass and the 0001..0026 + 0036..0038 patch stack.

fused_post_chain is a texture pass hooked at the tonemap seam of
``_render_buffers_post_process_and_tonemap`` (the FUSION-FIRST gate wired by
patch 0036 runs BEFORE the per-member single-pass tonemap gate, so the two-level
fallback resolves fusion -> member -> native). The bridge binds FIVE textures
(src_color = SRV t0, lum_source = SRV t1, prev_luminance = SRV t2, dst_color =
UAV u0, dst_luminance = UAV u1) and a 64-byte b0 carrying i64 dimension pairs.
The scene enables AUTO-EXPOSURE (so the native luminance_reduction runs and the
1x1 current luminance buffer is valid, which the fusion call site needs) and a
FILMIC tonemapper.

Scaffold binding honesty (PASS_CONTRACT.md 3.4/5): the fused luminance segment's
distinct <=8x8 reduce level and double-buffered 1x1 prev/dst luminance targets
are NOT exposed by the public Luminance API at the tonemap call site, so patch
0037 binds the single publicly-available 1x1 current luminance buffer to
lum_source and prev_luminance (read-only) and the LDR tonemap destination (fully
overwritten by the native tonemapper continuation) to dst_color and
dst_luminance. A distinct, correct fused binding needs the deferred Luminance-API
extension / draw_graph route B. The strict-success arm therefore asserts ONLY
that a real D3D12 fused dispatch was recorded and the native-continuation visual
gate stayed bit-stable; it makes NO math-correctness, net-benefit, or
performance claim.

What it measures, honestly, against the scratch Godot console exe rebuilt with
the full 0001..0026 + 0036..0038 patch stack
(``RURIX_GRX019_FUSED_POST_CHAIN_GODOT_EXE``) and a ``rurix_godot.dll`` built
WITH the ``d3d12-recording-shim`` feature:

  * **Pass enable matrix (three legs)**: a *reference* leg (all fused_post_chain
    per-pass settings at their ``false`` defaults; native Godot
    luminance_reduction + tonemap chain), an *enabled_real_pass_optin* candidate
    leg (``enabled=true`` + ``dispatch_real_pass=true``), and a
    *forced_capability_downgrade* red leg (candidate settings plus the
    harness-only ``real_pass_force_capability_downgrade=true`` knob, which clears
    the shader-int64 capability so the bridge fused_post_chain dispatch
    eligibility must fail closed with ``unsupported_device``). NOTE:
    fused_post_chain's runtime binding preflight does NOT check the int64
    capability (only the bridge dispatch eligibility does), so the forced knob is
    caught one level later at the dispatch-eligibility gate
    (``dispatch_eligibility_failed``), NOT the preflight gate the other texture
    passes (luminance/tonemap/ssao_blur/taa_resolve) catch it at.
  * **Gated real-pass attempt**: the canonical fused_post_chain artifact paths
    carry the owner-approved hlsl_bridge workaround package (DXC ``cs_6_0``
    container validated by ``dxv``, per-slot texture2d/rwtexture2d binding kinds,
    Rurix-owned RTS0), and the fused EMA-x-tonemap math is CPU-proven
    (``math_parity_evidence.json``, pending GPU dispatch), so every software gate
    can pass and the candidate leg may print the
    ``RXGD_GODOT_RUNTIME_FUSED_POST_CHAIN_REAL_PASS`` marker (plus the patch 0038
    result writeback scaffold marker) after a real recorded dispatch. If the real
    dispatch cannot complete in this environment (e.g. the scaffold aliased-UAV
    binding is rejected, or no signed DXC dxil.dll) the candidate leg must
    instead print the tracked fallback marker AND the bridge's machine-readable
    ``RXGD_FUSED_POST_CHAIN_REAL_PASS_BLOCKED`` diagnostic naming the FIRST
    missing prerequisite (``real_dispatch_recording_failed``).
  * **Fallback red/green + visual stability**: all three legs must render via the
    native Godot luminance_reduction + tonemap chain and exit 0 (the patch 0038
    writeback scaffold deliberately keeps the native tonemap as the
    continuation/backstop, so the rendered image can never change); the candidate
    and forced-failure frames must match the reference frame within the pinned
    LDR absolute-diff thresholds, and a GRX-008-format
    ``evidence_level=measured_local`` telemetry document must record the fallback
    entries with ``godot_fallback_active=true`` and ``telemetry_frame`` equal to
    the measured capture frame index.
  * **Runtime log audit**: the FULL merged stdout+stderr of every leg is audited
    (GRX-009 policy): only the known ``Could not load global script cache``
    warning is tolerated; any other ``ERROR:`` line is an integrity FAIL.

Outcome semantics mirror GRX-013 (see that harness for the SKIP/FAIL/success
contract). ``status=success`` (strict) is written ONLY when the opt-in real
dispatch actually executed and completed, the visual diff stayed within
thresholds, and every audit passed; only then is ``real_gpu_pass=true`` recorded
and the historical measured success artifact written. Even a success keeps
...
# Reuse the tracked GRX-009 scratch-provenance and log-audit helpers.
...
# Compute the exact patch stack tuple + stack id with the SAME functions the
# scratch build uses, so the source-provenance sidecar audit matches byte for
# byte (fused_post_chain stacks 0001..0026 + 0036..0038; 0027-0035 are a
# reserved hole for GRX-015/016/018).
from ci.godot_rurix_scratch_build import resolve_patches, stack_id_for

_STACK_SPEC = "0001-0026,0036-0038"
_STACK_PATHS, _STACK_ERRS = resolve_patches(_STACK_SPEC)
if _STACK_ERRS:
    raise SystemExit(f"GRX-019 enablement smoke: bad patch stack spec: {_STACK_ERRS}")
PATCH_STACK_GRX019 = tuple(p.name for p in _STACK_PATHS)
PATCH_STACK_ID = stack_id_for(_STACK_PATHS)
...
# GRX-019 readiness gate advances off THIS file (it reads strict_success=true).
...
REFERENCE_FRAME = VISUAL_DIR / "fused_post_chain_real_pass_reference.rgb8"
CANDIDATE_FRAME = VISUAL_DIR / "fused_post_chain_real_pass_candidate.rgb8"
DIFF_ARTIFACT = VISUAL_DIR / "fused_post_chain_real_pass_diff.rgb8"

RURIX_GODOT_DLL = ROOT / "target" / "debug" / "rurix_godot.dll"
WORK = ROOT / "target" / "grx019_fused_post_chain_real_pass_enablement_smoke"
LOG_DIR = WORK / "logs"

SUBJECT = "grx019_fused_post_chain_real_pass_enablement_smoke"

# Visual gate pins (same caliber as GRX-009 4h / GRX-010 / GRX-013).
...
# Markers in the full 0001..0026 + 0036..0038 build. The fallback marker carries
# the patch 0037 native-resource-handle-mapping wording (NOT the 0036 gate-level
# wording).
FALLBACK_MARKER = (
    "RurixAccel: fused_post_chain native resource handle mapping fallback rc="
)
...
# once per session by the fail-closed FusedPostChainGate in
# src/rurix-godot/src/lib.rs).
REAL_PASS_BLOCKED_MARKER = "RXGD_FUSED_POST_CHAIN_REAL_PASS_BLOCKED"
# Real-pass markers: the bridge prints
# "RXGD_GODOT_RUNTIME_FUSED_POST_CHAIN_REAL_PASS recorded=1" and the patch 0038
...
REAL_PASS_MARKER = "RXGD_GODOT_RUNTIME_FUSED_POST_CHAIN_REAL_PASS"
# Patch 0038 result writeback scaffold marker: the native chain stays the
# continuation/backstop after a real dispatch, so the image never changes.
WRITEBACK_MARKER = "RXGD_GODOT_RUNTIME_FUSED_POST_CHAIN_REAL_PASS_WRITEBACK"
# Recording-smoke marker. GRX Wave 4: the candidate/forced legs arm the
# fused_post_chain dispatch_recording_smoke opt-in (it now gates the per-dispatch
# REAL_PASS/WRITEBACK instrumentation markers), and RECORD prints only on the
# real-pass OK path.
RECORD_MARKER = "RXGD_GODOT_RUNTIME_FUSED_POST_CHAIN_RECORD"
...
# fused_post_chain's runtime binding preflight does NOT check the int64
# capability (only the bridge dispatch eligibility does), so the forced
# capability downgrade is caught one level later at the dispatch-eligibility gate
# — NOT the preflight gate the other texture passes catch it at.
EXPECTED_FORCED_PREREQUISITE = "dispatch_eligibility_failed"
EXPECTED_FORCED_FALLBACK_REASON = "unsupported_device"

KNOWN_GAPS = [
    (
        "the fused luminance segment's distinct <=8x8 reduce level and "
        "double-buffered 1x1 prev/dst luminance targets are NOT exposed by the "
        "public Luminance API at the tonemap call site, so patch 0037 binds the "
        "single publicly-available 1x1 current luminance buffer to lum_source and "
        "prev_luminance (read-only) and the LDR tonemap destination (fully "
        "overwritten by the native tonemapper continuation) to dst_color and "
        "dst_luminance; a distinct, correct fused binding needs the deferred "
        "Luminance-API extension / draw_graph route B (PASS_CONTRACT.md 3.4/5)"
    ),
    (
        "math parity is CPU-proven for the fused EMA-x-tonemap (LINEAR + "
        "linear_to_srgb) subset only and still pending GPU observation "
        "(math_parity_evidence.json status=pending_gpu_dispatch); glow, "
        "tonemapper modes other than LINEAR, the rest of the auto-exposure "
        "consumer chain, and the one-frame-latency same-frame tonemap "
        "replacement are recorded gaps"
    ),
    (
        "canonical artifact provenance is hlsl_bridge_workaround "
        "(owner-approved): the fused kernel is a texture compute pass, so the "
        "GRX-009 texture-intrinsic llc blocker applies and the DXC/DXV/RTS0 "
        "hlsl_bridge package is the runtime-mappable canonical artifact "
        "(texture_artifact_provenance_policy.json)"
    ),
    (
        ...
        "real_dispatch_path_not_linked, and the patch 0038 result writeback is a "
        "SCAFFOLD: the native Godot luminance_reduction + tonemap chain "
        "re-renders every frame as the continuation/backstop"
    ),
]

GODOT_TIMEOUT_SECONDS = 200
REQUESTED_RENDERER = "d3d12"
REQUESTED_RENDERING_METHOD = "forward_plus"

GODOT_EXE_ENV = "RURIX_GRX019_FUSED_POST_CHAIN_GODOT_EXE"
SCRATCH_SOURCE_ENV = "RURIX_GRX019_FUSED_POST_CHAIN_GODOT_SOURCE"
SCRATCH_SOURCE_PROVENANCE_ENV = "RURIX_GRX019_FUSED_POST_CHAIN_GODOT_SOURCE_PROVENANCE"
SCRATCH_BUILD_COMMAND_ENV = "RURIX_GRX019_FUSED_POST_CHAIN_GODOT_BUILD_COMMAND"
SCRATCH_BUILD_LOG_ENV = "RURIX_GRX019_FUSED_POST_CHAIN_GODOT_BUILD_LOG"
CAPTURE_PREFIX_ENV = "RURIX_GRX019_FUSED_POST_CHAIN_CAPTURE_PREFIX"

TARGET_BACKEND = "Godot 4.7-dev Windows D3D12 Forward+"
PASS_SETTING_PREFIX = "rendering/rurix_accel/passes/fused_post_chain"
...
def dll_fingerprint(path: Path) -> dict:
    fp: dict = {
        ...
        "feature_note": (
            ...
            "fused_post_chain real-pass arm routes through the linked recording "
            "shim, so arming RXGD_CAP_FUSED_POST_CHAIN_REAL_PASS can make "
...
def parse_blocked_marker(line: str) -> dict[str, str]:
    """Parse the key=value tokens of an RXGD_FUSED_POST_CHAIN_REAL_PASS_BLOCKED line."""
    tokens: dict[str, str] = {}
...
def write_evidence(status: str, *, reason: str | None = None, extra: dict | None = None) -> None:
    ...
    print(
        f"[grx019-fused-post-chain-real-pass-smoke] wrote {rel(EVIDENCE_OUT)} status={status}"
    )

    if status == "success":
        success_doc = dict(doc)
        success_doc["evidence_kind"] = "historical_measured_success"
        # The field the GRX-019 gate (ci/grx_gates/grx019_fused_post_chain.py
        ...
        success_doc["success_evidence_note"] = (
            "Historical measured success artifact for the GRX-019 "
            "fused_post_chain real-pass enablement gate. It is written ONLY on a "
            ...
            "default_enable_state=disabled and performance_claim=none, and "
            "makes no math-correctness or net-benefit claim (the fused binding "
            "is a scaffold and the native chain re-renders every frame)."
        )
        _write_json(SUCCESS_EVIDENCE_OUT, success_doc)
        print(
            "[grx019-fused-post-chain-real-pass-smoke] wrote "
            f"{rel(SUCCESS_EVIDENCE_OUT)} status=success (historical measured success)"
        )


def fail(msg: str, extra: dict | None = None) -> int:
    print(f"[grx019-fused-post-chain-real-pass-smoke] FAIL {msg}", file=sys.stderr)
...
def skip_environment(msg: str, extra: dict | None = None) -> int:
    ...
    print(f"[grx019-fused-post-chain-real-pass-smoke] SKIP {msg}(降级 SKIP,退出 0)")
...
def skip_measured_prerequisite(prerequisite: str, msg: str, extra: dict) -> int:
    ...
    print(
        "[grx019-fused-post-chain-real-pass-smoke] SKIP (measured) first missing "
...
def locate_godot_exe() -> tuple[Path | None, str | None]:
    ...
    if not override:
        return None, (
            f"{GODOT_EXE_ENV} is not set; the GRX-019 fused_post_chain enablement "
...
def build_bridge_dll() -> tuple[bool, str]:
    ...
    fused_post_chain real-pass arm can only attempt a real dispatch when the
...
def scratch_source_provenance(godot_exe: Path) -> dict:
    ...
    the tracked 0001..0026 + 0036..0038 patch stack (GRX-009 machinery)."""
    ...
    provenance: dict = {
        ...
        "applied_patch_stack": patch_stack_identity(PATCH_STACK_GRX019, PATCH_STACK_ID),
        "godot_exe": {
    ...
    ok, errors, audit = verify_source_provenance_sidecar(
        sidecar,
        source_root,
        stack_names=PATCH_STACK_GRX019,
...
def write_smoke_project(
    ...
    *,
    ...
    fused_post_chain per-pass opt-in settings differ between legs; everything else
    (including the AUTO-EXPOSURE camera attributes that make the native
    luminance_reduction run and the 1x1 current luminance buffer valid, which the
    fusion call site needs, and the FILMIC tonemapper) is byte-identical so the
    opt-in matrix is the only delta. GRX Wave 4 print gating: the candidate/forced
    legs arm the fused_post_chain dispatch_recording_smoke opt-in because it now
    gates the per-dispatch REAL_PASS/WRITEBACK instrumentation markers this
    harness asserts on (the production dispatch_real_pass path emits zero
    per-dispatch stdout); the RECORD marker only prints on the real-pass OK path,
    so it must appear in the candidate leg IFF the real pass succeeded and never
    elsewhere."""
...
; Auto-generated by ci/grx019_fused_post_chain_real_pass_enablement_smoke.py

config_version=5

[application]

config/name="GRX-019 fused_post_chain real-pass enablement smoke"
...
rurix_accel/passes/fused_post_chain/enabled={flag(pass_enabled)}
rurix_accel/passes/fused_post_chain/dispatch_recording_smoke={flag(dispatch_recording_smoke)}
rurix_accel/passes/fused_post_chain/dispatch_real_pass={flag(dispatch_real_pass)}
rurix_accel/passes/fused_post_chain/real_pass_force_capability_downgrade={flag(force_capability_downgrade)}
"""
    scene_text = """\
...
[node name="GRX019FusedPostChainRoot" type="Node3D"]
...
"""
    # Deterministic 3D scene with AUTO-EXPOSURE enabled via CameraAttributesPractical
    # (this is what makes the native luminance_reduction run every frame, so the
    # 1x1 current luminance buffer is valid and the fusion call site can resolve
    # its luminance handle) and a FILMIC tonemapper. A bright emissive mesh over a
    # dark background gives the auto-exposure a stable target. fixed_fps makes the
    # auto-exposure temporal EMA converge deterministically across runs and legs,
    # so a fixed-frame capture is byte-exact; the fused pass never writes the real
    # luminance buffer (its dst_luminance is the LDR scaffold target), so the
    # auto-exposure evolution is identical in all three legs.
    script_text = f"""\
extends Node3D
...
func _ready() -> void:
    ...
    cam.position = Vector3(0.0, 1.5, 6.0)
    cam.rotation_degrees = Vector3(-10.0, 0.0, 0.0)
    var cam_attrs := CameraAttributesPractical.new()
    cam_attrs.auto_exposure_enabled = true
    cam_attrs.auto_exposure_min_sensitivity = 20.0
    cam_attrs.auto_exposure_max_sensitivity = 800.0
    ...
    # A bright emissive sphere over the dark background gives the auto-exposure a
    # strong, stable luminance signal so the native luminance_reduction produces
    # a well-defined 1x1 current buffer (the fusion call site's luminance input).
    var sphere := SphereMesh.new()
    sphere.radius = 1.2
    sphere.height = 2.4
    var mat := StandardMaterial3D.new()
    mat.albedo_color = Color(0.85, 0.72, 0.4)
    ...
    floor_mat.albedo_color = Color(0.15, 0.16, 0.2)
    var floor_inst := MeshInstance3D.new()
    floor_inst.mesh = floor_mesh
    floor_inst.material_override = floor_mat
    add_child(floor_inst)

    print("GRX019FusedPostChain: scene ready auto_exposure=%s tonemap=%d" % [str(cam_attrs.auto_exposure_enabled), env.tonemap_mode])
...
func _capture() -> void:
    ...
    if prefix.is_empty():
        printerr("GRX019FusedPostChain: capture prefix env var missing")
        get_tree().quit(3)
        return
    ...
    meta.close()
    print("GRX019FusedPostChain: captured frame=%d width=%d height=%d" % [_frames, img.get_width(), img.get_height()])
    get_tree().quit()
"""
...
def run_godot(godot_exe: Path, project_dir: Path, capture_prefix: Path, log_name: str) -> tuple[int, str]:
    ...
    # the strict-success marker/evidence semantics are preserved.
...
def run_matrix_leg(godot_exe: Path, *, leg: str, dll_path: Path) -> dict:
    ...
    return {
        ...
        "frame_bytes": data,
        "runtime_log_audit": runtime_log_audit(output, PATCH_STACK_GRX019),
...
def telemetry_entries_issue(
    ...
    """First incoherence in the generated GRX-019 telemetry entries, or None."""
    ...
    if not isinstance(passes, list) or len(passes) != expected_count:
        return (
            ...
            "fused_post_chain fallback entries"
        )
    by_leg = {
    ...
    for leg_name, expected_reason in expectations.items():
        ...
        if entry.get("pass_id") != "fused_post_chain":
            return f"{leg_name} entry pass_id is not fused_post_chain"
...
def main() -> int:
    ...
    _EVIDENCE_BASE = {
        ...
        "segment": "grx019_real_pass_enablement",
        "runtime_state": "fallback_only",
        "real_gpu_pass": False,
        "real_d3d12_dispatch_recorded": False,
        "godot_runtime_fused_post_chain_path_enabled": False,
        ...
        "note": (
            "GRX-019 fused_post_chain gated real-pass enablement gate evidence. "
            "The opt-in real-pass arm (dispatch_real_pass, default false) runs "
            "against the owner-approved hlsl_bridge workaround canonical package "
            "(five textures: src_color SRV t0, lum_source SRV t1, prev_luminance "
            "SRV t2, dst_color UAV u0, dst_luminance UAV u1; per-slot "
            "texture2d/rwtexture2d binding kinds; 64-byte i64-dims b0; fused "
            "EMA-x-tonemap math parity CPU-proven pending GPU) and a "
            "d3d12-recording-shim bridge DLL, so every software gate can pass and "
            "a real recorded dispatch may return RXGD_STATUS_OK; when the dispatch "
            "cannot complete the gate reports "
            "first_missing_prerequisite=real_dispatch_recording_failed instead of "
            "claiming success. The fused binding is a SCAFFOLD (lum_source and "
            "prev_luminance alias the single public 1x1 current luminance buffer; "
            "dst_color and dst_luminance target the LDR tonemap destination the "
            "native tonemapper fully overwrites), so the patch 0038 result "
            "writeback keeps the native Godot luminance_reduction + tonemap chain "
            "as the continuation/backstop, default_enable_state stays disabled, "
            "and no math-correctness, net-benefit, FPS, or GPU-timestamp claim is "
            "made."
        ),
    }
    ...
    if godot_exe is None:
        return skip_environment(godot_reason or "GRX-019 fused_post_chain Godot exe unavailable")
    ...
    _EVIDENCE_BASE["patch_stack_identity"] = patch_stack_identity(
        PATCH_STACK_GRX019, PATCH_STACK_ID
    )
    ...
    for name, leg in legs.items():
        ...
        if leg["record_marker_observed"] and name != "candidate":
            return fail(
                f"{name} run printed the fused_post_chain recording-smoke marker "
    ...
    for marker_key, marker_name in (
        ...
        if reference[marker_key]:
            return fail(
                "reference run (all fused_post_chain per-pass settings at their "
    ...
    for name in ("candidate", "forced_fallback"):
        ...
        print(
            f"[grx019-fused-post-chain-real-pass-smoke] LDR absolute diff ({name} vs "
    ...
    telemetry_doc = {
        ...
        "target_backend": TARGET_BACKEND,
        "note": (
            "GRX-019 measured fused_post_chain real-pass enablement telemetry: "
            "with the default-false dispatch_real_pass opt-in explicitly "
            "enabled, the tracked Godot tonemap seam invoked the Rurix fusion "
            "bridge through the patch 0037 native texture-handle binding; every "
            "fallback entry records the fail-closed gate outcome "
            "(validation_failed when the linked real dispatch cannot complete; "
            "unsupported_device under the forced capability downgrade) while the "
            "native Godot luminance_reduction + tonemap chain rendered every "
            "frame. The patch 0038 writeback is a scaffold (native continuation "
            "active), runtime_state stays fallback_only for the default path, and "
            "no performance or FPS claim is made."
        ),
        "passes": (
            []
            if real_pass_success
    ...
    if real_pass_success:
        ...
        print(
            "[grx019-fused-post-chain-real-pass-smoke] PASS measured opt-in real "
    ...
    return skip_measured_prerequisite(
        ...
        "hlsl_bridge canonical fused_post_chain package, but the linked real "
        "dispatch recording did not complete in this environment (e.g. no signed "
        "DXC dxil.dll, or the scaffold aliased-UAV binding was rejected), so the "
        "gate honestly reports real_dispatch_recording_failed instead of claiming "
        "success",
        measured_extra,
    )
