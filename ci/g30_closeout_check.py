#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G30.6 close-out 收尾脚本派生）
"""G30.6 close-out 终审门（g30.wave.6b.closeout，步骤 524）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g30.wave.6b.closeout"
NUMERIC_STEP = 524
SUBJECT = "g30_wave6b_closeout"
WAVE = "G30.6"
SCHEMA_PATH = ROOT / "milestones/g30/g30_wave6b_closeout_evidence_schema.json"
HANDOVER_REGISTRY = ROOT / "milestones/g30/g30_campaign_handover_registry.json"
RFC_PATH = ROOT / "rfcs/0047-campaign-final-review.md"
P0_SUBJECTS = [
    ("m_a", "g30_m_a_tail_anchor_rejudgment_closure"),
    ("m_b", "g30_m_b_commercial_final_review"),
    ("m_c", "g30_m_c_campaign_full_chain_no_regression"),
    ("m_d", "g30_m_d_campaign_handover_ledger"),
    ("m_e", "g30_m_e_closed_gate_no_regression"),
]
...
def evaluate() -> tuple[list[dict], str]:
    ...
    p2 = wel.load_latest_evidence("g30_p2_decisions_check")
    ...
    ledger_ok = HANDOVER_REGISTRY.is_file() and md_ok
    facts.append(fact("handover_ledger_chain", ledger_ok,
                      f"{HANDOVER_REGISTRY.name}+{md.name}（campaign handover registry 主锚 + M-d 绿件在档）"
                      if (HANDOVER_REGISTRY.is_file() and md) else
                      f"registry={'present' if HANDOVER_REGISTRY.is_file() else 'missing'};"
                      f"m_d={md.name if md else 'missing'}"))
    rfc_ok, rfc_detail = wel.rfc_agent_approved(RFC_PATH)
    facts.append(fact("rfc_0047_archived", rfc_ok, rfc_detail))
    me_ok, me_d = _latest_green("g30_m_e_closed_gate_no_regression")
    ...
    soak = wel.load_latest_evidence("g30_stabilization_soak")
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes=f"G30.6 closeout 八 facts VERDICT={verdict}",
        host_section_pass=ok,
    )
    print(f"[g30_closeout] VERDICT={verdict}")
