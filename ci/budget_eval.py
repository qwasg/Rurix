...
def eval_entry(entry: dict, strict: bool) -> None:
    ...
    if eid.startswith("g14.ue_variance_band."):
        ...
        return
    if eid.startswith("g15.quality_guard.ue_variance_band_"):
...
def eval_counter(entry: dict, strict: bool) -> None:
    ...
    if eid == "m0.counter.env_profile_required_fields":
        ...
        # g8_m31_reflection_hash_*.json 中 checks.* 全 true 的字段数(16 个独立断言)。
        # host/compile 纯 host 门,device 段 not_applicable,无 GPU 依赖,host 恒跑不 SKIP。
        n = 0
        for f in (ROOT / "evidence").glob("g8_m31_reflection_hash_*.json"):
            doc = json.loads(f.read_text(encoding="utf-8"))
            if doc.get("host_section_pass") is not True:
                continue
            checks = doc.get("checks") or {}
        ...
        count_or_gate(
            ...
            n,
            1,
            "份 reflection_hash 六腿判据全绿见证(checks.* 16/16 true)",
    ...
    elif eid == "g8.counter.ddc_content_address_legs":
        n = 0
        for f in (ROOT / "evidence").glob("g8_m80_ddc_content_address_*.json"):
            doc = json.loads(f.read_text(encoding="utf-8"))
            if doc.get("host_section_pass") is not True:
                continue
            checks = doc.get("checks") or {}
            if sum(1 for v in checks.values() if v is True) >= 19:
                n += 1
        count_or_gate(
            eid,
            n,
            1,
            "份 ddc_content_address 19 腿全绿见证",
            "G8.3 M80 实现回填前为正常状态",
            strict,
        )
    ...
    elif eid == "g8.counter.fracture_pipeline_checks":
        n = 0
        for f in (ROOT / "evidence").glob("g8_m68_fracture_pipeline_*.json"):
            doc = json.loads(f.read_text(encoding="utf-8"))
            if doc.get("host_section_pass") is not True:
                continue
            checks = doc.get("checks") or {}
        ...
        count_or_gate(
            ...
            n,
            1,
            "份 fracture_pipeline 12 腿 host 全绿见证",
