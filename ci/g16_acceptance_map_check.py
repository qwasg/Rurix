...
"""G16.1 治理门 — 验收映射覆盖 / 空行 / 双向命名空间一致性（g16.wave.1.acceptance_map，步骤 281）。

G16_CONTRACT G-G16-1（G16.1 完成门 D-G16-3 验收映射面）治理三门之一；G-G16-2 实现互锁
（ci/g16_interlock_check.py）事实门④消费本门与候选决策门的独立 PASS 机器事实。

核验 `milestones/g16/G16_ACCEPTANCE_MAP.md`：
§1 四行 P0（{M-a,M-b,M-c,M-d} 闭集全等）+ §2 零 go P1 空集断言 +
全部 symbolic key 匹配 `g16.p0.m_<a~d>.<slug>` 单一命名空间（key 的 m 段字母与行号一致、
脚本命令 `ci/g16_*_smoke.py` 每行单一且 --gate 参数 == canonical key、evidence schema
`g16_m_<a~d>_<key slug>_evidence_schema.json` 与 key 末段同 slug——schema 路径
与 key 末段同字面形态）+
波次 ∈ {G16.2,G16.3,G16.4,G16.5} +
numeric_step 全列 `post-interlock actual-next-free allocation` 字面零预占 +
零空行/占位 + **双向一致**：MAP §1 与 G16_CONTRACT.md §4.2 对同一 P0 M 行给出的
判据与波次逐字相等（G16 治理三件套无独立 CI_GATES——门冻结面 = 契约 §4.2 + MAP §1/§2，
沿 G15/G14 体例精简为双向；G16 契约 §4.2 表 = M 行/判据（逐字）/波次 三列形态，
判据/波次双向逐字一致机核沿 g15 脚本 two_way 牌段语义承载）。
...
用法：
  ...
  py -3 ci/g16_acceptance_map_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g16.wave.1.acceptance_map"
NUMERIC_STEP = 281  # 落盘前实测 registry/number_ledger.json CI_step.next_free=281 顺位领取
SUBJECT = "g16_acceptance_map_check"
WAVE = "G16.1"
MAP_PATH = ROOT / "milestones" / "g16" / "G16_ACCEPTANCE_MAP.md"
CONTRACT_PATH = ROOT / "milestones" / "g16" / "G16_CONTRACT.md"
SCHEMA_PATH = ROOT / "milestones" / "g16" / "g16_acceptance_map_check_evidence_schema.json"
SOURCE_REF = (
    "G16_CONTRACT G-G16-1/§4.2;G16_ACCEPTANCE_MAP.md §1/§2/§4/§5;"
    "G15_CONTRACT.md §8.9 G16 承接锚 + G15_CANDIDATE_DECISIONS.md defer-to-G16+ 14 行 + "
    "G15-MC-F1/G15-MD-F1（G16 法定输入）P0 清单 M-a~M-d"
)

# 冻结 4 行 P0 集合（G16.1 立项裁决口径；G16_CONTRACT §2/§4.2 M-a~M-d）。
EXPECTED_P0 = {"M-a", "M-b", "M-c", "M-d"}
# MAP §5：所有波次属于 G16.2|G16.3|G16.4|G16.5（§4.2 四行 P0 波次闭集）。
ALLOWED_WAVES = {"G16.2", "G16.3", "G16.4", "G16.5"}
...
KEY_RE = re.compile(r"^g16\.p0\.m_[a-d]\.[a-z0-9_]+$")
KEY_IN_CELL_RE = re.compile(r"`(g16\.p0\.m_[a-d]\.[a-z0-9_]+)`")
SECTION_RE = re.compile(r"^## (\d+)\. ")
SCRIPT_RE = re.compile(r"ci/g16_[a-z0-9_]+_smoke\.py")
SCHEMA_RE = re.compile(r"`(milestones/g16/g16_(m_[a-d])_[a-z0-9_]+_evidence_schema\.json)`")
BOLD_RE = re.compile(r"\*\*([^*]+)\*\*")
MAP_ROW_RE = re.compile(r"^\|\s*\*\*(M-[a-d])\*\*")
CONTRACT_ROW_RE = re.compile(r"^\|\s*\*\*(M-[a-d])\*\*")
...
def parse_contract_rows(text: str) -> dict[str, dict]:
    """解析 CONTRACT §4.2 形态的 `| **M-<a~d>** | 判据（逐字） | 波次 |` 三列行。"""
    out: dict[str, dict] = {}
    for line in text.splitlines():
...
def check_row(m: str, row: dict, seen_keys: dict[str, str], seen_schemas: dict[str, str]) -> list[str]:
    ...
    findings: list[str] = []
    m_seg = m.lower().replace("-", "_")  # M 行号 → key/schema m 段（m_a~m_d，连字符不入 key 字符集）
    if len(row["keys"]) != 1:
        findings.append(f"{m} 必须恰有一个 canonical symbolic key，实测 {row['keys']}")
        return findings
    key = row["keys"][0]
    if not KEY_RE.match(key):
        findings.append(f"{m} key `{key}` 不匹配 g16.p0.m_<a~d>.<slug>")
    ...
    slug = key.split(".")[3]
    if not row["scripts"]:
        findings.append(f"{m} 缺 `ci/g16_*_smoke.py` 脚本命令")
    if len(set(row["scripts"])) > 1:
        findings.append(f"{m} 一个 key 只能绑定一个脚本，实测 {sorted(set(row['scripts']))}")
    # 注：MAP §5 判据字面不设脚本↔key 同 slug 机核（脚本名可为 key 末段缩写稳定形态，
    ...
    if not schema_m:
        findings.append(f"{m} schema 路径不符 g16_m_<a~d>_<slug>_evidence_schema.json：{row['schema']!r}")
    else:
        path, schema_m_no = schema_m.group(1), schema_m.group(2)
        if schema_m_no != m_seg:
            findings.append(f"{m} schema 路径的 m 段不符：{path}")
        expected_schema = f"milestones/g16/g16_{m_seg}_{slug}_evidence_schema.json"
    ...
    if row["wave"] not in ALLOWED_WAVES:
        findings.append(f"{m} 波次 {row['wave']!r} 不在允许集合 {{G16.2,G16.3,G16.4,G16.5}} 内")
...
def evaluate(map_text: str | None, contract_text: str | None) -> list[dict]:
    """10 facts：coverage_p0_set / coverage_p1_empty / row_M-a~M-d / two_way_M-a~M-d。"""
    results: list[dict] = []
    if map_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G16_ACCEPTANCE_MAP.md 缺失（诚实红，不假绿）"}]
    if contract_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G16_CONTRACT.md 缺失（诚实红，不假绿）"}]
    ...
    results.append({
        ...
        "detail": "§2 零 go P1 空集（G16.1 字面）" if p1_ok else f"§2 出现 P1 行 {sorted(p1_rows)}（G16.1 零 go P1 字面违例）",
    })
    ...
    for m in sorted(EXPECTED_P0):
        ...
        if len(row["keys"]) == 1:
            ...
            if other is None:
                two.append("G16_CONTRACT §4.2 缺行")
            else:
...
def emit(results: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g16_acceptance_map] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G16.1 治理门——验收映射覆盖/空行/双向命名空间一致性：4 P0（M-a~M-d）闭集全等 + §2 零 go P1 空集 + key/脚本/schema 单一命名空间同 slug + numeric_step 全列 post-interlock 字面零预占 + MAP §1 ↔ CONTRACT §4.2 判据/波次双向逐字一致（G16 无独立 CI_GATES）；本门 PASS 只表示映射完整，不表示任何 P0 能力已实现",
...
# ---------------------------------------------------------------------------
# selftest 合成夹具：4 行 P0 冻结集合的正本（七列形态），不依赖树上文件。
# ---------------------------------------------------------------------------

CANONICAL_ROWS = [
    ("M-a", "ue5_ref_arm_black_fix", "ci/g16_ue5_ref_arm_black_fix_smoke.py", "G16.2"),
    ("M-b", "quality_gap_closure", "ci/g16_quality_gap_closure_smoke.py", "G16.3"),
    ("M-c", "perf_parity_recovery", "ci/g16_perf_parity_recovery_smoke.py", "G16.4"),
    ("M-d", "regression_guard", "ci/g16_regression_guard_smoke.py", "G16.5"),
]


def _fixture() -> tuple[str, str]:
    map_lines = ["# fixture G16_ACCEPTANCE_MAP", "", "## 1. P0 硬门（精确 4 行）", ""]
    contract_lines = ["# fixture G16_CONTRACT", "", "### 4.2 四行 P0", ""]
    for m, slug, script, wave in CANONICAL_ROWS:
        key = f"g16.p0.{m.lower().replace('-', '_')}.{slug}"
        schema = f"milestones/g16/g16_{m.lower().replace('-', '_')}_{slug}_evidence_schema.json"
        cmd = f"`py -3 {script} --gate {key}`"
        map_lines.append(
            f"| **{m}** | `{key}` | {cmd} | `{schema}` | 合成判据 {m} | **{wave}** | {NUMERIC_STEP_LITERAL} |"
        )
        contract_lines.append(f"| **{m}** | 合成判据 {m} | {wave} |")
    map_lines += ["", "## 2. 已 go P1 硬门（零行）", "", "G16.1 无 go 的 P1 行。", ""]
    return "\n".join(map_lines), "\n".join(contract_lines)
...
def run_selftest() -> int:
    ...
    if not MAP_PATH.is_file():
        ...
        if code != 0:
            ...
            print("[selftest] PASS: 真表 4 行绿")

    cases: list[tuple[str, str, str, str]] = [
        (
            "删除 M-d 行 → coverage 必须红",
            "\n".join(l for l in map_text.splitlines() if not l.startswith("| **M-d**")),
            contract_text,
            "coverage_p0_set",
        ),
        (
        ...
        (
            ...
            map_text.replace("--gate g16.p0.m_c.perf_parity_recovery`", "--gate g16.p0.m_c.perf_parity`"),
            contract_text,
            "row_M-c",
        ),
        (
            ...
            map_text.replace(
                "**G16.2** | post-interlock actual-next-free allocation |",
                "**G16.2** | 284 |",
                1,
            ),
            contract_text,
        ...
        (
            "波次改写为非法 G16.6c → 必须红",
            map_text.replace("合成判据 M-a | **G16.2**", "合成判据 M-a | **G16.6c**", 1),
            contract_text,
            "row_M-a",
        ),
        (
            ...
            map_text.replace(
                "G16.1 无 go 的 P1 行。",
                "| **M-a** | `g16.p0.m_a.ue5_ref_arm_black_fix` | `py -3 ci/g16_ue5_ref_arm_black_fix_smoke.py --gate g16.p0.m_a.ue5_ref_arm_black_fix` | `milestones/g16/g16_m_a_ue5_ref_arm_black_fix_evidence_schema.json` | 判据 | **G16.2** | post-interlock actual-next-free allocation |",
            ),
            contract_text,
            "coverage_p1_empty",
        ),
        ...
        (
            "schema 路径 m 段改写 → 必须红",
            map_text.replace(
                "milestones/g16/g16_m_c_perf_parity_recovery_evidence_schema.json",
                "milestones/g16/g16_m_d_perf_parity_recovery_evidence_schema.json",
                1,
            ),
            contract_text,
            "row_M-c",
    ...
    if green and len(results) == 10:
        print("  GREEN ok — 合成夹具正本 PASS（10 facts）")
    else:
        print(f"  GREEN MISS — 合成夹具正本本应 PASS（10 facts），实测 code={code} facts={len(results)}")
        for r in results:
            if r["status"] != "PASS":
                print(f"    - {r}")
        failures += 1
    ...
    if failures:
        print(f"[g16_acceptance_map] SELFTEST FAIL ({failures})")
        return 1
    print("[g16_acceptance_map] SELFTEST PASS (9 RED + 1 GREEN + 真表臂)")
    return 0
