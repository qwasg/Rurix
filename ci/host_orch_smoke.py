...
ACCEPTS = ["mod_file", "extern_link", "saxpy_single_source"]
...
def run(cmd, **kw):
    ...
    r = subprocess.run(cmd, capture_output=True, cwd=ROOT, **kw)
...
def device_segment(saxpy: Path, td: Path):
...
def write_evidence(adapter: str):
    ...
    doc = {
        ...
        "notes": (
            ...
            "device 真跑数值自校验 exit 0;reject 六类编译期拦截(RX1005/RX2010/RX3015/"
            "RX6024/RX4001)+ 篡改内嵌 PTX 装载协商拒 + 桩化写回数值红 + 复原绿"
            "(G-MS1-2,RFC-0009 / RXS-0189~0196)"
        ),
    }
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="host_orch_smoke_") as tdname:
        td = Path(tdname)
        saxpy = host_segment(td)
        device_run, adapter = device_segment(saxpy, td)
    if device_run:
        write_evidence(adapter)
        print("[host_orch_smoke] PASS host 段 + device 真跑 + 红①篡改 PTX + 红②桩化写回 + 复原绿(红绿闭合)")
    else:
        print("[host_orch_smoke] PASS host 段(reject 六类拦截 + accept 三例 + 链接面);device 段 SKIP")
    sys.exit(0)
