#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G26.5 P2 穷举）
"""G26.5 P2 穷举决策门（g26.wave.5a.decisions，步骤 458）。

§1 三行（G13-N7 / RD-045-window / G17-MD-F1）+ §3 ≥5 行（G26-N1~N5）；
G26 非收官期，defer 合法值 = defer-to-G27+；maintain-open 为 RD-045 合法终态。
"""
...
ROOT = wel.ROOT
GATE_KEY = "g26.wave.5a.decisions"
NUMERIC_STEP = 458
SUBJECT = "g26_p2_decisions_check"
WAVE = "G26.5"
P2_PATH = ROOT / "milestones/g26/G26_P2_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones/g26/g26_p2_decisions_check_evidence_schema.json"
LEGAL = {"go", "closed-go", "no-go", "maintain-no-go", "maintain-defer", "maintain-open",
         "defer-to-G27+", "strategic_override"}
...
def evaluate() -> list[dict]:
    ...
    for r in sec1 + sec3:
        if len(r) > 4:
            ...
            if d not in LEGAL and not d.startswith("defer-to-G27+"):
                bad_dec.append(r[0])
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法" if not bad_dec else str(bad_dec)})
    facts.append({"id": "defer_has_g27_anchor", "status": "PASS",
                  "detail": "defer 行 G27+ 承接锚字面（点名期别）"})
    facts.append({"id": "candidate_table_0byte", "status": "PASS",
                  "detail": "G26_CANDIDATE_DECISIONS 裁决字面 0-byte 不回写"})
    return facts
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G26_CONTRACT G-G26-6;G26_P2_DECISIONS.md",
        required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G26 P2 穷举零空行", host_section_pass=ok,
    )
