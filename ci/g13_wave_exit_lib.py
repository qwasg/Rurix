...
"""G13 波次聚合门共享库（G13_CONTRACT §2.2；同构 ci/g12_wave_exit_lib.py）。

只读汇总独立门 evidence，不重跑 smoke、不代绿。任一 required gate 缺失 /
非 PASS / SKIP / DEV_ENV_DEGRADE → 聚合红。供 G13.2+ 波聚合门薄壳复用。
...
# g11_wave_exit_lib 为通用只读聚合实现（命名空间参数化）；G13 共享库直接
# 复用其判定面，零语义改动（实现形态 = 对 g11 通用库的命名空间转接，判定
# 逻辑 0-byte，沿 g12_wave_exit_lib 同模）。
...
def run_selftest() -> int:
    """gate_pass_reason 红绿两臂自检（g13 key 面；不依赖树上 evidence）。"""
    key = "g13.p0.m_a.vendor_upscale_integration"
    good = {
    ...
    red("key 漂移", {**good, "symbolic_gate_key": "g13.p0.m_b.tsr_device_kernel"})
    ...
    if failures:
        print(f"[g13_wave_exit_lib] SELFTEST FAIL ({failures})")
        return 1
    print("[g13_wave_exit_lib] SELFTEST PASS (6 RED + 1 GREEN)")
    return 0
