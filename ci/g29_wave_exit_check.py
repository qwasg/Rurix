#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G29 波聚合门）
"""G29 波聚合门（参数化 g29.wave.{2..6}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g29/g29_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g29.wave.2.exit": ("G29.2", "g29_m_a_slab_device_kernel", 497),
    "g29.wave.3.exit": ("G29.3", "g29_m_b_slab_side_table_arm", 499),
    "g29.wave.4.exit": ("G29.4", "g29_m_c_svt_ktx2_gap_rejudgment", 501),
    "g29.wave.5.exit": ("G29.5", "g29_m_d_wg_dgc_capability_recheck", 503),
    "g29.wave.6.exit": ("G29.6", "g29_m_e_closed_gate_no_regression", 505),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g29_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G29_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g29_wave{n}_exit",
        notes=f"G29 wave {n} exit", host_section_pass=overall,
    )
