#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G9.5 M119 后处理骨架门冒烟(g9.p1.m119.post_processing_skeleton;RFC-0025
§4.J;spec/display_pipeline.md RXS-0370;G9_ACCEPTANCE_MAP §3 M119;G9_CONTRACT
§8.1 裁决① P1 全进)。
...
实记 device_name=host-only/validation=not_applicable)。三段判据:
  ...
  host 段:rurix-render display::post_chain 4 单测逐名锚定(五级显式排序
    可检测/HDR 线性域探针抓隐式 clamp/曝光状态跨帧持久/tonemap 消费 view
    transform 插件)+ conformance display_pipeline M119 双件语料锚定
    (//@ spec: RXS-0370 + 门 key/脚本名留痕)+ 冻结带
    milestones/g9/g9_m119_post_chain_band.json provenance 机器核验(五级
    排序字面 + 四插件 chain digest)。
  harness 段:持锁(gpu_device_lock)真跑 g9_m119_post_chain --evidence
    (直出件落 .tmp 工作区不覆盖 evidence/ harness 直出件;schema/spec_anchor/
    status==pass + 8 判据闭集全真)+ --red-arm order-swap/implicit-clamp
    子模式独立复跑抽检。

命名差如实登记:门 key 按 MAP §3 实记 = g9.p1.m119.post_processing_skeleton;
harness 直出件 assertion_id = g9.p1.m119.post_chain(实现波实记字面),门
harness 段按该字面如实核验,不改写 harness 身份充门 key。

用法:
  py -3 ci/g9_post_processing_skeleton_smoke.py --gate g9.p1.m119.post_processing_skeleton
  py -3 ci/g9_post_processing_skeleton_smoke.py --selftest
"""
...
SCHEMA_PATH = ROOT / "milestones" / "g9" / "g9_m119_post_processing_skeleton_evidence_schema.json"
BAND_PATH = ROOT / "milestones" / "g9" / "g9_m119_post_chain_band.json"
CORPUS_DIR = ROOT / "conformance" / "display_pipeline"
WORK_DIR = ROOT / ".tmp" / "g95_gates" / "m119"
HARNESS_EVIDENCE = WORK_DIR / "harness_evidence.json"

sys.path.insert(0, str(ROOT / "ci"))
from gpu_device_lock import gpu_device_lock  # noqa: E402

GATE_KEY = "g9.p1.m119.post_processing_skeleton"
NUMERIC_STEP = 163
SOURCE_REF = "RFC-0025 §4.J;spec/display_pipeline.md RXS-0370;G9_ACCEPTANCE_MAP §3 M119"
TAG = "g9_m119"
SUBJECT = "g9_m119_post_processing_skeleton"
MATRIX_ROW = "M119"

MODULE_TESTS = {
    "display::post_chain": [
        "five_stage_explicit_order_detectable",
        "hdr_linear_domain_probe_catches_implicit_clamp",
        "exposure_state_persists_across_frames",
        "tonemap_consumes_view_transform_plugin",
    ],
}
CORPUS_FILES = [
...
BAND_SCHEMA = "rurix.g9m119.post_chain_band.v1"
BAND_SPEC_ANCHOR = "RXS-0370"
BAND_STAGE_ORDER = ["exposure", "bloom", "tonemap", "color_grading", "output_transform"]
HARNESS_BIN = "g9_m119_post_chain"
HARNESS_SCHEMA = "rurix.g9m119.post_chain.v1"
# 命名差如实登记:harness 直出件 assertion_id 实记字面(门 key = MAP post_processing_skeleton)。
HARNESS_ASSERTION = "g9.p1.m119.post_chain"
HARNESS_TAG = "G9_M119_POST"
HARNESS_CHECKS = [
    "conformance_corpus_anchored",
    "five_stage_explicit_order",
    "order_swap_detectable_red",
    "hdr_linear_domain_probe_red",
    "exposure_state_persist_red",
    "four_plugins_wired",
    "double_run_bit_equal",
    "golden_frozen_equal",
]
RED_ARMS = ["order-swap", "implicit-clamp"]
...
def host_band_provenance() -> bool:
    ...
    need(band.get("stage_order") == BAND_STAGE_ORDER, "stage_order ≠ 五级显式排序冻结字面")
    plugins = band.get("plugins")
    need(isinstance(plugins, dict) and len(plugins) == 4, "plugins ≠ 四内置插件")
    for k, v in (plugins or {}).items():
        need(is_hex(v, 64), f"plugins.{k} 非 64-hex")
    return ok
...
def main() -> int:
    ...
    with gpu_device_lock(purpose="g9_m119 post_chain harness 腿"):
        exe = build_harness()
        if exe:
            ...
            note(
                "harness:g9_m119_post_chain 全档真跑(五级显式排序 golden + HDR 线性域探针 + "
                "曝光状态帧间持久)+ RED 双臂子模式复跑;命名差如实登记"
                "(harness assertion_id=g9.p1.m119.post_chain vs 门 key=post_processing_skeleton)"
            )
