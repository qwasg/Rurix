#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G22.5 soak 波）
"""G22.5 稳定 soak（g22.wave.5a.soak，步骤 395）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g22.wave.5a.soak"
NUMERIC_STEP = 395
SUBJECT = "g22_stabilization_soak"
WAVE = "G22.5"
SCHEMA_PATH = ROOT / "milestones/g22/g22_stabilization_soak_evidence_schema.json"
BIN = ROOT / "target/release/g14_3_pipeline_perf.exe"
SLAB_BIN = ROOT / "target/release/g22_slab_probe.exe"
MIN_SECONDS = 1800.0
...
def run_gate() -> int:
    facts: list[dict] = []
    md = wel.load_latest_evidence("g22_m_d_work_graphs_fsr_reeval_disposition")
    ...
    if not md_ok or not BIN.is_file() or not SLAB_BIN.is_file():
        ...
        facts.extend([
            ...
            fact("slab_lane_interleaved", False, "未启动"),
        ])
        ok = False
    else:
        combos = [
        ...
        iters = fails = slab_iters = 0
        active = 0.0
        while time.perf_counter() - t0 < MIN_SECONDS:
            it0 = time.perf_counter()
            if iters % 5 == 4:
                out = ROOT / ".tmp" / "g22_soak_slab" / f"iter_{iters}.json"
                out.parent.mkdir(parents=True, exist_ok=True)
                r = subprocess.run([str(SLAB_BIN), "--out", str(out)],
        ...
        facts.extend([
            ...
            fact("no_sleep_between_iters", True, "sleep=0"),
            fact("slab_lane_interleaved", slab_iters > 0, f"slab_iters={slab_iters}（slab 白炉审计车道穿插复跑）"),
        ])
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G22 soak ≥1800s（管线四组合 + slab 车道穿插）", host_section_pass=ok,
    )
