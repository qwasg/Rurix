#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G9.5 M112 大气 Froxel 门冒烟(g9.p1.m112.atmosphere_froxel;RFC-0025 §4.C;
spec/world_partition.md RXS-0365;G9_ACCEPTANCE_MAP §3 M112;G9_CONTRACT §8.1
裁决① P1 全进)。
...
实记 device_name=host-only/validation=not_applicable)。三段判据:
  ...
  host 段:rurix-render world::atmosphere 5 单测逐名锚定(Froxel 统一基础
    设施单入口/逐帧计数面非空/零散射+网格篡改 RED/weather map 签名篡改 RED/
    时序链初始化+断裂 RED)+ conformance world_partition M112 双件语料锚定
    (//@ spec: RXS-0365 + 门 key/脚本名留痕)+ 冻结带
    milestones/g9/g9_m112_atmosphere_froxel_band.json provenance 机器核验。
  harness 段:持锁(gpu_device_lock)真跑 g9_m112_atmosphere_froxel
    --evidence(直出件落 .tmp 工作区不覆盖 evidence/ harness 直出件;
    schema/spec_anchor/assertion_id/status==pass + 9 判据闭集全真)+
    --red-arm grid-tamper/weather-tamper 子模式独立复跑抽检。

用法:
  py -3 ci/g9_atmosphere_froxel_smoke.py --gate g9.p1.m112.atmosphere_froxel
  py -3 ci/g9_atmosphere_froxel_smoke.py --selftest
"""
...
SCHEMA_PATH = ROOT / "milestones" / "g9" / "g9_m112_atmosphere_froxel_evidence_schema.json"
BAND_PATH = ROOT / "milestones" / "g9" / "g9_m112_atmosphere_froxel_band.json"
CORPUS_DIR = ROOT / "conformance" / "world_partition"
WORK_DIR = ROOT / ".tmp" / "g95_gates" / "m112"
HARNESS_EVIDENCE = WORK_DIR / "harness_evidence.json"

sys.path.insert(0, str(ROOT / "ci"))
from gpu_device_lock import gpu_device_lock  # noqa: E402

GATE_KEY = "g9.p1.m112.atmosphere_froxel"
NUMERIC_STEP = 157
SOURCE_REF = "RFC-0025 §4.C;spec/world_partition.md RXS-0365;G9_ACCEPTANCE_MAP §3 M112"
TAG = "g9_m112"
...
MODULE_TESTS = {
    "world::atmosphere": [
        "froxel_unified_infrastructure_single_entry",
        "per_frame_counters_nonempty",
        "zero_scattering_and_grid_tamper_red",
        "weather_map_signature_tamper_red",
        "temporal_chain_init_and_break_red",
    ],
}
CORPUS_FILES = [
...
BAND_SCHEMA = "rurix.g9m112.atmosphere_froxel_band.v1"
BAND_SPEC_ANCHOR = "RXS-0365"
BAND_DIGEST_FIELDS = ["scene_digest"]
HARNESS_BIN = "g9_m112_atmosphere_froxel"
HARNESS_SCHEMA = "rurix.g9m112.atmosphere_froxel.v1"
HARNESS_ASSERTION = "g9.p1.m112.atmosphere_froxel"
HARNESS_TAG = "G9_M112_ATMO"
HARNESS_CHECKS = [
    "conformance_corpus_anchored",
    "froxel_unified_infrastructure",
    "fog_density_height_decay",
    "per_frame_counters_nonempty",
    "double_run_bit_equal",
    "golden_frozen_equal",
    "red_arm_grid_tamper",
    "red_arm_zero_scatter",
    "red_arm_weather_and_temporal",
]
RED_ARMS = ["grid-tamper", "weather-tamper"]
...
def main() -> int:
    ...
    with gpu_device_lock(purpose="g9_m112 atmosphere_froxel harness 腿"):
        exe = build_harness()
        if exe:
            ...
            note("harness:Froxel 统一基础设施 + 雾前端 golden + 计数面 + RED 双臂子模式复跑")
