...
# Recording-smoke marker: must never appear here (the taa_resolve
...
def write_smoke_project(
    ...
    *,
    ...
    matrix is the only delta. The taa_resolve dispatch_recording_smoke opt-in
...
rurix_accel/passes/taa_resolve/dispatch_recording_smoke=false
    ...
    for name, leg in legs.items():
        ...
        if leg["record_marker_observed"]:
            return fail(
                ...
                "stay off in the GRX-012 enablement matrix",
                extra=runs_extra,
            )
