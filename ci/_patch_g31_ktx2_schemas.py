#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# G31+ 波 C Task C14：check_schemas.py 三处纯追加注册（io.open 补丁法——
# newline="" 字节面保全，既有路由 0-byte；打补丁后立即重读验证驻留）。
# 本脚本幂等：已驻留即只做核验（B4 _patch_g31_texture_schemas.py 同法）。
import io
import sys

P = r"h:\rurix\ci\check_schemas.py"

LOAD_ANCHOR = '''    g31_svt_gate_schema = load(
        ROOT / "milestones/g31/g31_svt_gate_evidence_schema.json"
    )
'''
LOAD_ADD = '''    # G31+ 波 C Task C14 KTX2 三行门/harness 双 schema 纯追加（重放幂等面）
    g31_ktx2_ab_schema = load(
        ROOT / "milestones/g31/g31_ktx2_ab_evidence_schema.json"
    )
    g31_ktx2_gate_schema = load(
        ROOT / "milestones/g31/g31_ktx2_gate_evidence_schema.json"
    )
'''

VALIDATOR_ANCHOR = '''    g31_svt_gate_validator = (
        jsonschema.Draft7Validator(g31_svt_gate_schema)
        if g31_svt_gate_schema is not None
        else None
    )
'''
VALIDATOR_ADD = '''    g31_ktx2_ab_validator = (
        jsonschema.Draft7Validator(g31_ktx2_ab_schema)
        if g31_ktx2_ab_schema is not None
        else None
    )
    g31_ktx2_gate_validator = (
        jsonschema.Draft7Validator(g31_ktx2_gate_schema)
        if g31_ktx2_gate_schema is not None
        else None
    )
'''

ROUTE_ANCHOR = '''        elif (
            f.name.startswith("g31_svt_harness_")
            and g31_svt_validator is not None
        ):
            # G31+ 波 C Task C13 SVT harness 真跑证据 →
            # milestones/g31/g31_svt_evidence_schema.json。
            validator = g31_svt_validator
'''
ROUTE_ADD = '''        elif (
            f.name.startswith("g31_ktx2_gate_")
            and g31_ktx2_gate_validator is not None
        ):
            # G31+ 波 C Task C14 KTX2 三行门裁决证据 →
            # milestones/g31/g31_ktx2_gate_evidence_schema.json
            # （ci/g31_ktx2_smoke.py --gate g31.waveC.ktx2 产;长前缀先匹配）。
            validator = g31_ktx2_gate_validator
        elif (
            f.name.startswith("g31_ktx2_ab_")
            and g31_ktx2_ab_validator is not None
        ):
            # G31+ 波 C Task C14 KTX2 A/B measured harness 证据 →
            # milestones/g31/g31_ktx2_ab_evidence_schema.json
            # （ci/g31_ktx2_smoke.py --gate g31.waveC.ktx2 产）。
            validator = g31_ktx2_ab_validator
'''


def main() -> int:
    with io.open(P, "r", encoding="utf-8", newline="") as f:
        src = f.read()

    # 三处插入（幂等：已驻留即跳过插入只做核验;route = add 前置于锚……
    # 本面两个锚块各自唯一,add 追加在锚后保持 elif 链合法）。
    for name, anchor, add, token in [
        ("load", LOAD_ANCHOR, LOAD_ADD, "g31_ktx2_ab_schema = load("),
        ("validator", VALIDATOR_ANCHOR, VALIDATOR_ADD, "g31_ktx2_ab_validator = ("),
        ("route", ROUTE_ANCHOR, ROUTE_ADD, 'f.name.startswith("g31_ktx2_gate_")'),
    ]:
        n = src.count(token)
        if n == 1:
            print(f"[patch_g31_ktx2] {name} 已驻留（幂等跳过插入）")
            continue
        if n != 0:
            print(f"[patch_g31_ktx2] FAIL: {name} token 驻留数 {n}（≠0≠1 拒改）",
                  file=sys.stderr)
            return 1
        if src.count(anchor) != 1:
            print(f"[patch_g31_ktx2] FAIL: {name} 锚出现 {src.count(anchor)} 次（≠1 拒改）",
                  file=sys.stderr)
            return 1
        src = src.replace(anchor, anchor + add, 1)
        print(f"[patch_g31_ktx2] {name} 插入完成")

    # 驻留核验（各 1）+ 链合法 + 语法编译。
    misses = []
    for name, token in [
        ("load", "g31_ktx2_ab_schema = load("),
        ("load_gate", "g31_ktx2_gate_schema = load("),
        ("validator", "g31_ktx2_ab_validator = ("),
        ("validator_gate", "g31_ktx2_gate_validator = ("),
        ("route_gate", 'f.name.startswith("g31_ktx2_gate_")'),
        ("route", 'f.name.startswith("g31_ktx2_ab_")'),
    ]:
        n = src.count(token)
        if n != 1:
            misses.append(f"{name} 驻留数 {n} ≠ 1")
    if src.count('f.name.startswith("g31_svt_gate_")') != 1:
        misses.append("svt 路由非单例")
    if misses:
        for m in misses:
            print(f"[patch_g31_ktx2] FAIL {m}", file=sys.stderr)
        return 1

    with io.open(P, "w", encoding="utf-8", newline="") as f:
        f.write(src)
    # 立即重读验证驻留（防字节面漂移）。
    with io.open(P, "r", encoding="utf-8", newline="") as f:
        back = f.read()
    for token in [
        "g31_ktx2_ab_schema = load(",
        "g31_ktx2_gate_validator = (",
        'f.name.startswith("g31_ktx2_gate_")',
    ]:
        if token not in back:
            print(f"[patch_g31_ktx2] FAIL: 重读未驻留 {token!r}", file=sys.stderr)
            return 1
    import py_compile
    py_compile.compile(P, doraise=True)
    print("[patch_g31_ktx2] PASS：三处纯追加驻留（load/validator/route）+ 重读核验 + py_compile 绿")
    return 0


if __name__ == "__main__":
    sys.exit(main())
