#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G18 波聚合门）
"""G18 波聚合门（参数化 g18.wave.{2..10}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g18/g18_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g18.wave.2.exit": ("G18.2", "g18_m_a_rurix_light_transport_depth", 313),
    "g18.wave.3.exit": ("G18.3", "g18_m_b_presentation_pipeline_dual_profile", 315),
    "g18.wave.4.exit": ("G18.4", "g18_m_c_ue_arm_lighting_repair_and_render", 317),
    "g18.wave.5.exit": ("G18.5", "g18_m_d_dual_end_commercial_quality_verdict", 319),
    "g18.wave.6.exit": ("G18.6", "g18_m_e_sl_runtime_upgrade_disposition", 321),
    "g18.wave.7.exit": ("G18.7", "g18_m_f_fps_parity_reeval", 323),
    "g18.wave.8.exit": ("G18.8", "g18_m_g_virtualized_geometry_p3", 325),
    "g18.wave.9.exit": ("G18.9", "g18_m_h_frame_generation_independent_layer", 327),
    "g18.wave.10.exit": ("G18.10", "g18_m_i_closed_gate_no_regression", 329),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g18_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G18_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g18_wave{n}_exit",
        notes=f"G18 wave {n} exit", host_section_pass=overall,
    )
