...
"""G14.6 波次聚合门 g14.wave.6.exit（步骤 258；G14_CONTRACT §7 裁决 7 延续波程序面/
§2.2 同律；G14_ACCEPTANCE_MAP §1；同构 ci/g14_wave4_exit_check.py）。

只读汇总 G14.6 波 M-f(M176) 门最新 evidence——生产口径双列 + vendor Stage A
位级零漂移（步骤 257）——+ 六 facts:
① M-f 门 RED 臂独立有效（caliber-masquerade/digest-drift 双臂）;
② M-c 回归面最新 evidence 绿（Stage A 后 M-c 门复跑 PASS——既有判据零降级）;
③ M-d v2 守护面绿（最新 M-d evidence checks production_caliber_v2 +
   stage_a_digest_drift_guard 双真 + digest 锚 18 格在树）;
④ g14_budget production 口径 3 条目 measured_local + budget_eval 全 PASS;
...
用法:
  py -3 ci/g14_wave6_exit_check.py --gate g14.wave.6.exit
  py -3 ci/g14_wave6_exit_check.py --selftest
"""
...
GATE_KEY = "g14.wave.6.exit"
NUMERIC_STEP = 258  # 落盘前实测 registry/number_ledger.json CI_step.next_free=258 顺位领取
SUBJECT = "g14_wave6_exit"
WAVE = "G14.6"
SOURCE_REF = (
    "G14_CONTRACT §7 裁决 7/§2.2 同律;G14_ACCEPTANCE_MAP §1;M-f gate red arms independently effective;"
    "M-c regression latest green;M-d v2 caliber+digest guards green;budget production entries measured;"
    "M-d pass-line honest red registered;G5~G13 closed 0-byte"
)
SCHEMA_PATH = ROOT / "milestones" / "g14" / "g14_wave6_exit_evidence_schema.json"
...
def collect_facts() -> list[dict]:
    ...
    facts.append(_fact(
        "m_f_red_arms_independently_effective",
        bool(red_checks) and all(v is True for v in red_checks.values()),
        f"M-f 门 red 面 checks 全真（{len(red_checks)} 面：caliber-masquerade/digest-drift）"
    ...
    facts.append(_fact(
        "m_d_v2_guards_green",
        ok3,
        f"M-d v2 守护双真 + digest 锚 {anchor_n}/18 格在树"
        if ok3 else f"M-d v2 守护面缺: cal={md_checks.get('production_caliber_v2')} "
        f"dig={md_checks.get('stage_a_digest_drift_guard')} anchors={anchor_n}",
    ))

    # ④ budget production 条目 + budget_eval 全 PASS
    bud_bad: list[str] = []
    if not BUDGET_PATH.is_file():
        ...
        want = {f"g14.pipeline_perf.prod_frame_ms.cornell-box_t67_{b}"
    ...
    facts.append(_fact(
        "budget_production_entries_measured",
        not bud_bad,
        "g14_budget production 口径 3 条目齐备 measured_local + budget_eval 全 PASS"
...
def run_gate(*, evidence_dir: Path | None = None) -> int:
    ...
    if evidence_dir is not None:
        extras = [
            _fact("m_f_red_arms_independently_effective", False, "selftest 空目录"),
            _fact("m_c_regression_latest_green", False, "selftest 空目录"),
            _fact("m_d_v2_guards_green", False, "selftest 空目录"),
            _fact("budget_production_entries_measured", False, "selftest 空目录"),
            _fact("m_d_pass_line_honest_face_registered", False, "selftest 空目录"),
            _fact("legacy_criteria_0byte", False, "selftest 空目录"),
        ]
    notes_parts = [
        "implemented: G14.6 M-f(M176) production caliber + vendor Stage A gate (step 257)",
        "aggregate read-only: no smoke re-run, no substitute green, no RURIX_REQUIRE_REAL",
        "facts: M-f red arms + M-c regression + M-d v2 guards + budget production entries + honest red face + legacy 0-byte",
...
def run_selftest() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="g14_wave6_selftest_") as td:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="G14.6 wave6.exit 聚合门（只读汇总）")
