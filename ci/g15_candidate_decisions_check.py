...
"""G15.1 治理门 — 候选决策表闭集/锚纪律/横向对账（g15.wave.1.candidate_decisions，步骤 267）。

G15_CONTRACT G-G15-1（G15.1 完成门 D-G15-2 候选决策表面）治理三门之一；G-G15-2 实现互锁
（ci/g15_interlock_check.py）事实门②消费本表 35 行零空行面、事实门④消费本门独立 PASS
机器事实。

核验 `milestones/g15/G15_CANDIDATE_DECISIONS.md`（2026-08-23 v1.0 落盘）：
冻结 35 行候选闭集全等（§1 G14 defer-to-G15+ 29 行承接——G14_P2_DECISIONS §5 承接锚法定输入
逐行转引终态裁决〔7 closed-go G14-N8~N14 G14plus 兑现完结留痕 + 8 go G15 承接兑现 +
14 defer-to-G16+〕；§3 G15 新增候选 6 行 G15-N1~N6 全 go）、
裁决枚举合法（go/closed-go/no-go/defer-to-G16+/strategic_override——G15 即本期，defer-to-G15+
不再合法；closed-go = G14plus 兑现完结留痕变体，入 go 族不入 deferred 面）、
零空行（全列非空）、承接锚纪律（§1/§3 行承接锚均含「重判条件 = …；兜底 = …」字面；
§1 行原触发条件字面 = G14 P2 §5 承接锚 0-byte 转引含「→」分节）、
defer-to-G16+ 裁决行承接锚含 G16+ 重评窗字面、
go 行验收映射锚义务（登记留痕位置含 G15_ACCEPTANCE_MAP，或依据面含 G15_CONTRACT §4.2 M 行
锚定——同族登记行经契约 §4.2 M 行锚定同满足，沿 G14-N7 先例语义）、
...
零新 RD max=RD-045）；外加横向机核——
  ① 与 G15_ACCEPTANCE_MAP 5 key（M-a~M-e 全 P0）互斥：候选行 ID 不得命中已 go 门裸 token；
  ② registry/deferred.json 对账：RD-034/039/040/041/042/043/044/045 八条目级 status 全 open、
    零新 RD（max=RD-045）。
（§2 RD 映射 8 行维持 open，不重复计入 35 行候选闭集三值枚举。）
...
用法：
  ...
  py -3 ci/g15_candidate_decisions_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g15.wave.1.candidate_decisions"
NUMERIC_STEP = 267  # 落盘前实测 registry/number_ledger.json CI_step.next_free=266 顺位领取
SUBJECT = "g15_candidate_decisions_check"
WAVE = "G15.1"
DECISIONS = ROOT / "milestones" / "g15" / "G15_CANDIDATE_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones" / "g15" / "g15_candidate_decisions_check_evidence_schema.json"
ACCEPTANCE_MAP = ROOT / "milestones" / "g15" / "G15_ACCEPTANCE_MAP.md"
DEFERRED = DEFERRED_PATH

# 冻结 ID 闭集（35 行）= §1 G14 defer-to-G15+ 29 行承接 + §3 G15 新增候选 6 行。
# （§2 open RD 8 行 = 映射行维持 open，不重复计入候选闭集；行集与 status 由
# sec2_rd_set / row_RD-* / deferred_rd_open_reconcile 三面独立机核。）
SEC1_IDS = [
    "M61",          # G14 defer 承接：RD-039 mesh shader→M109（G13.4/G14 双窗未命中在案）
    "M52",          # G14 defer 承接：RD-040 SER→M108（G13.4/G14 双窗未命中在案）
    "M100-high",    # G14 defer 承接：RD-040 ReSTIR 高档（G14 窗登记 = 未齐备）
    "SAFE-GPU",     # G14 defer 承接：Safe GPU Operator Platform（独立期）
    "M127",         # G14 defer 承接：神经变形研究子轨
    "M98-l4",       # G14 defer 承接：M98 L4 Far Field
    "M114-strand",  # G14 defer 承接：毛发 strand 档（G14 窗登记 = 数据面部分落地）
    "M118-hdr-cal", # G14 defer 承接：HDR 设备标定层
    "M125-adopt3",  # G14 defer 承接：Jolt 5.6 采纳三件
    "G10-N6",       # G14 defer 承接：BistroExterior 缺口（G15 双场景闭集 0-byte 维持未入清单）
    "G10-N8",       # G14 defer 承接：-renderoffscreen 未测（G14 窗登记）
    "G10-N17",      # G14 defer 承接：M137 scalars.flip 演进位（G14 窗 = 未消费）
    "G11-N3",       # G14 defer 承接：GPU 管线画质差距面（锚定 G15 → go：M-a/M-c 承载）
    "G11-N5",       # G14 defer 承接：锁定度量反向激励旁证（G14 窗 = 未齐备）
    "G11-N8",       # G14 defer 承接：太阳穿玻璃高光尾（锚定 G15 → go：M-b 评估承载）
    "G11-N9",       # G14 defer 承接：c1_ue_specular_ibl 实测上界（锚定 G15 → go：M-b 评估承载）
    "G12-N10",      # G14 defer 承接：材质链（锚定 G15 → go：M-b 评估承载）
    "G12-N12",      # G14 defer 承接：UE PT 差距登记表 10 行处置（锚定 G15 → go：M-a 承载；只消费不回写）
    "G12-N13",      # G14 defer 承接：M165 间歇非确定性事件（→ go：M-e 漂移监控臂 + RD-045 观察窗承接）
    "G13-N7",       # G14 defer 承接：帧生成 FG/MFG（G14 重评窗不立项在案，G15 画质收口期不承接）
    "G13-N8",       # G14 defer 承接：UE 超分差距登记表 8 行处置（锚定 G15 → go：M-a 承载；只消费不回写）
    "G13-N9",       # G14 defer 承接：UE Lumen 差距登记表 2 行处置（锚定 G15 → go：M-a 承载；只消费不回写）
    "G14-N8",       # G14 P2 后事件承接：M-d 通过线 ×1.00 未达标处置（closed-go：G14plus 兑现完结留痕）
    "G14-N9",       # G14 P2 后事件承接：readback 内存型 HOST_CACHED 化（closed-go：G14.9 兑现完结留痕）
    "G14-N10",      # G14 P2 后事件承接：fence/readback N 帧流水（closed-go：G14.10 兑现完结留痕）
    "G14-N11",      # G14 P2 后事件承接：mv host 计算 GPU 化（closed-go：G14.10 兑现完结留痕）
    "G14-N12",      # G14 P2 后事件承接：vendor Stage B 零拷贝（closed-go：遥测重估让位维持登记）
    "G14-N13",      # G14 P2 后事件承接：raster-primary MVP 车道（closed-go：条件波不立项登记）
    "G14-N14",      # G14 P2 后事件承接：阴影射线结构面（closed-go：条件波不立项登记）
]
SEC2_IDS = ["RD-034", "RD-039", "RD-040", "RD-041", "RD-042", "RD-043", "RD-044", "RD-045"]
SEC3_IDS = [
    "G15-N1",  # 绝对画质通过线量化口径（go → M-c 承载）
    "G15-N2",  # AI 读图强制门形态（go → M-a 基线臂 + M-c 终审承载）
    "G15-N3",  # 材质链表达面立项评估程序（go → M-b 承载）
    "G15-N4",  # 商用收口判定口径（go → M-c 承载）
    "G15-N5",  # 20 行处置三态枚举口径（go → M-b 承载）
    "G15-N6",  # 性能零降级守护口径（go → M-d 承载）
]
FROZEN_IDS = SEC1_IDS + SEC3_IDS
# G15 即本期：defer-to-G15+ 不再合法，defer 合法值 = defer-to-G16+；
# closed-go = G14plus 兑现完结留痕变体（go 族终态，不充 G15 门绿、不入 deferred 面）。
ALLOWED = frozenset({"go", "closed-go", "no-go", "defer-to-G16+", "strategic_override"})
EMPTY_RE = re.compile(r"^(TBD|TODO|待定|待补|—|-)?$", re.I)
ID_RE = re.compile(r"^(M\d|RD-\d|SAFE-GPU|G1[012345]-N\d|M-[a-e])")
...
def _decision_base(decision: str) -> str | None:
    return (
        ...
        else "defer-to-G16+" if decision.startswith("defer-to-G16+")
...
def _row_findings(r: list[str], *, carry_over: bool) -> list[str]:
    ...
    if base is None:
        parts.append(f"非法裁决 {decision!r}（G15 合法枚举 go/closed-go/no-go/defer-to-G16+/strategic_override）")
    for i, cell in enumerate(r):
        if cell_empty(cell):
            parts.append(f"空单元格 col{i}")
            break
    if carry_over and "→" not in r[3]:
        parts.append("原触发条件字面缺「→」分节（G14 P2 §5 承接锚 0-byte 转引口径）")
    anchor = r[7]
    if "重判条件" not in anchor or "兜底" not in anchor:
        parts.append("承接锚缺「重判条件 = …；兜底 = …」字面")
    if base == "defer-to-G16+" and "G16+" not in anchor:
        parts.append("defer 缺 G16+ 重评窗字面")
    if base == "go" and "G15_ACCEPTANCE_MAP" not in r[8] and "G15_CONTRACT.md §4.2 M-" not in r[6]:
        parts.append("go 行缺验收映射锚（登记留痕位置须含 G15_ACCEPTANCE_MAP，或依据面含 G15_CONTRACT §4.2 M 行锚定——同族登记同满足面）")
...
def validate(
    ...
    """48 facts：set_equality_frozen / no_duplicate_ids / row×43 / sec2_rd_set / 两横向机核。"""
    results: list[dict] = []
    ...
    # --- §2 行集闭集：RD-034/039/040/041/042/043/044/045 八行全等（映射行不入 35 行候选闭集） ---
    ...
    # RD max 断言：G15.1 治理波窗内零新 RD（≤45 = 治理门快照字面，RD-045 已存续 open 承接
    # M165 漂移监控观察窗；后事件合法加性须按只追加程序登记并校准本断言面）。
...
def run_check(
    ...
    if not p.is_file():
        # 诚实红：表未落盘不是绿
        return 1, [{"id": "file", "status": "FAIL", "detail": f"missing {p}（G15.1 候选决策表未落盘；诚实红，不假绿）"}]
    results = validate(
...
def emit(results: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g15_candidate_decisions] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G15_CONTRACT G-G15-1/§6/§7;G15_CANDIDATE_DECISIONS.md v1.0;G15_ACCEPTANCE_MAP §1/§2;G14_P2_DECISIONS.md v1.0 §5（29 行承接锚法定输入）;registry/deferred.json（RD 八条目级 status open 机核）",
        required_gate_rows=[],
        extra_facts=results,
        subjects=[],
        schema_path=SCHEMA_PATH,
        evidence_basename=SUBJECT,
        notes="G15.1 治理门——候选决策表 35 行闭集（§1 G14 defer-to-G15+ 29 行承接〔7 closed-go G14plus 兑现完结留痕 + 8 go G15 承接兑现 + 14 defer-to-G16+〕+ §3 G15-N1~N6 新增 6 行全 go；§2 RD 映射 8 行维持 open 不重复计入三值枚举）：裁决枚举合法（go/closed-go/no-go/defer-to-G16+/strategic_override——G15 即本期，defer-to-G15+ 不再合法）/零空行/承接锚纪律（全表「重判条件 = …；兜底 = …」字面零缺项；§1 原触发条件字面 0-byte 转引含「→」分节）+ defer 行 G16+ 重评窗字面 + go 行验收映射锚义务（同族登记经契约 §4.2 M 行锚定同满足）+ closed-go 行 evidence/ 真跑件义务 + MAP 5 key 互斥 + deferred 对账（RD-034/039/040/041/042/043/044/045 条目级 status 全 open，零新 RD max=RD-045）；no-go/defer 如实保持 open 不写进全绿叙述",
...
# ---------------------------------------------------------------------------
# selftest 合成夹具（十列形态；§1 决策分布 = 14 defer + 8 go + 7 closed-go 正本）。
# ---------------------------------------------------------------------------
...
FIXTURE_MAP_TEXT = "\n".join(
    f"| **M-{l}** | `g15.p0.m_{l}.{s}` | ... |"
    for l, s in (
        ("a", "dual_end_quality_reharvest"),
        ("b", "gap_fix_closure_loop"),
        ("c", "absolute_quality_final_review"),
        ("d", "perf_parity_zero_regression"),
        ("e", "regression_drift_guard"),
    )
)

FIXTURE_SEC1_GO_IDS = frozenset({"G11-N3", "G11-N8", "G11-N9", "G12-N10", "G12-N12", "G12-N13", "G13-N8", "G13-N9"})
FIXTURE_SEC1_CLOSED_GO_IDS = frozenset({"G14-N8", "G14-N9", "G14-N10", "G14-N11", "G14-N12", "G14-N13", "G14-N14"})


def _synth_sec1(rid: str) -> str:
    if rid in FIXTURE_SEC1_CLOSED_GO_IDS:
        decision = "closed-go（G14plus 兑现完结留痕）"
        ref = "evidence/fixture_closed_go_20260823T000000Z.json（真跑件）"
        anchor = "重判条件 = 已兑现完结无重判面；兜底 = 终态维持"
        reg = "本表 §1 行（不新设 RD）"
        final = "closed-go"
    elif rid in FIXTURE_SEC1_GO_IDS:
        decision = "go（G15 承接兑现——M-a 承载）"
        ref = "milestones/g14/G14_P2_DECISIONS.md §5 行"
        anchor = "重判条件 = 重评产出面构成新事实时按只追加程序新立分项；兜底 = 既有面维持"
        reg = "本表 §1 行 + G15_ACCEPTANCE_MAP M-a 行（不新设 RD）"
        final = "go（M-a 承载面）"
    else:
        decision = "defer-to-G16+"
        ref = "milestones/g14/G14_P2_DECISIONS.md §5 行"
        anchor = "重判条件 = G16+ 重评窗触发条件齐备时按只追加程序重判；兜底 = 既有面维持"
        reg = "本表 §1 行（不新设 RD）"
        final = "open-defer（G16+）"
    return (
        f"| {rid} | 分项 | G14.5a defer | 「G15+ 原触发条件齐备 → 兜底面维持（字面 0-byte）」 | "
...
def _synth_sec3(rid: str) -> str:
    return (
        f"| {rid} | 分项 | G15.1 新增 | 「口径争议时按只追加程序重判形态」 | go（G15.4 M-c 承载） | 理由 | "
        "milestones/g15/G15_CONTRACT.md §4.2 M-c 行 | "
        "重判条件 = 口径争议时按只追加程序重判形态并 §8 只追加修订；兜底 = 现口径维持 | "
        "本表 §3 行 + G15_ACCEPTANCE_MAP M-c 行 | go（M-c 承载面） |\n"
    )


def _full_fixture() -> str:
    sec1_head = "| ID | 分项名 | 来源波次 | 原触发条件字面 | 裁决 | 裁决理由 | 依据/证据路径 | 承接锚 | 登记留痕位置 | 最终状态 |\n|---|---|---|---|---|---|---|---|---|---|\n"
    sec2_head = "| RD | title（摘要） | 条目级 status | G15.1 处置 | 联动面 | 裁决理由 | 留痕位置 |\n|---|---|---|---|---|---|---|\n"
    sec3_head = sec1_head
    return (
        "## 1. G14 defer-to-G15+ 29 行逐行转引终态裁决\n\n" + sec1_head + "".join(_synth_sec1(i) for i in SEC1_IDS)
...
def run_selftest() -> int:
    ...
    if not DECISIONS.is_file():
        ...
        if code != 0:
            ...
            print("[selftest] PASS: 真表 35 行闭集绿")

    with tempfile.TemporaryDirectory(prefix="g15_cand_selftest_") as td:
        ...
        _red(
            "§1 defer 行承接锚缺 G16+ 重评窗 → 红",
            full.replace(
                "重判条件 = G16+ 重评窗触发条件齐备时按只追加程序重判；兜底 = 既有面维持",
        ...
        _red(
            "§1 原触发条件字面缺「→」分节 → 红",
            full.replace(
                "「G15+ 原触发条件齐备 → 兜底面维持（字面 0-byte）」",
                "「G15+ 原触发条件齐备兜底面维持（字面 0-byte）」",
                1,
            ),
            "row_M61",
        )
        _red(
            "非法裁决枚举（§3 go→maybe）→ 红",
            full.replace("| G15-N1 | 分项 | G15.1 新增 | 「口径争议时按只追加程序重判形态」 | go（G15.4 M-c 承载） |",
                         "| G15-N1 | 分项 | G15.1 新增 | 「口径争议时按只追加程序重判形态」 | maybe |", 1),
            "row_G15-N1",
        )
        _red(
            "空单元格（§3 裁决理由置空）→ 红",
            full.replace("| G15-N3 | 分项 | G15.1 新增 | 「口径争议时按只追加程序重判形态」 | go（G15.4 M-c 承载） | 理由 |",
                         "| G15-N3 | 分项 | G15.1 新增 | 「口径争议时按只追加程序重判形态」 | go（G15.4 M-c 承载） |  |", 1),
            "row_G15-N3",
        )
        _red(
        ...
        _red(
            "§1 go 行缺验收映射锚 → 红",
            full.replace("本表 §1 行 + G15_ACCEPTANCE_MAP M-a 行（不新设 RD）", "本表 §1 行（不新设 RD）", 1),
            "row_G11-N3",
        )
        _red(
            "§3 go 行承接锚缺「兜底」→ 红",
        ...
        _red(
            ...
            full.replace("evidence/fixture_closed_go_20260823T000000Z.json（真跑件）", "milestones/g14/G14PLUS_RECORD.md 登记面", 1),
            "row_G14-N8",
        )
        _red(
    ...
    if failures:
        print(f"[g15_candidate_decisions] SELFTEST FAIL ({failures})")
        return 1
    print("[g15_candidate_decisions] SELFTEST PASS (12 RED + 真表/合成双臂 GREEN)")
    return 0
