...
"""G15.1 治理门 — 验收映射覆盖 / 空行 / 双向命名空间一致性（g15.wave.1.acceptance_map，步骤 266）。

G15_CONTRACT G-G15-1（G15.1 完成门 D-G15-3 验收映射面）治理三门之一；G-G15-2 实现互锁
（ci/g15_interlock_check.py）事实门④消费本门与候选决策门的独立 PASS 机器事实。

核验 `milestones/g15/G15_ACCEPTANCE_MAP.md`（2026-08-23 v1.0 落盘）：
§1 五行 P0（{M-a,M-b,M-c,M-d,M-e} 闭集全等——M-a 双端画质对拍链路全量复跑+20 行登记表逐项重评+
G15 差距处置表落盘+UE 方差带程序产+AI 读图基线臂 / M-b measured 主差修复闭环逐项处置三态穷举+
材质链表达面立项评估结论登记 / M-c 绝对画质通过线程序产标定+18 格逐格 AI 读图严格画面审查+
商用收口判定 / M-d 性能零降级守护（G14 M-d 同口径复跑 18 格 ×1.00 维持+画质锚带复核）/
M-e 回归门+RD-045/M165 漂移监控）+ §2 零 go P1 空集断言 +
全部 symbolic key 匹配 `g15.p0.m_<a~e>.<slug>` 单一命名空间（key 的 m 段字母与行号一致、
脚本命令 `ci/g15_*_smoke.py` 每行单一且 --gate 参数 == canonical key、evidence schema
`g15_m_<a~e>_<key slug>_evidence_schema.json` 与 key 末段同 slug——schema 路径
与 key 末段同字面形态）+
波次 ∈ {G15.2,G15.3,G15.4,G15.5,G15.6a} +
numeric_step 全列 `post-interlock actual-next-free allocation` 字面零预占 +
零空行/占位 + **双向一致**：MAP §1 与 G15_CONTRACT.md §4.2 对同一 P0 M 行给出的
判据与波次逐字相等（G15 治理三件套无独立 CI_GATES——门冻结面 = 契约 §4.2 + MAP §1/§2，
沿 G14 体例精简为双向；G15 契约 §4.2 表 = M 行/判据（逐字）/波次 三列形态，
判据/波次双向逐字一致机核沿 g14 脚本 two_way 牌段语义承载）。
...
用法：
  ...
  py -3 ci/g15_acceptance_map_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g15.wave.1.acceptance_map"
NUMERIC_STEP = 266  # 落盘前实测 registry/number_ledger.json CI_step.next_free=266 顺位领取
SUBJECT = "g15_acceptance_map_check"
WAVE = "G15.1"
MAP_PATH = ROOT / "milestones" / "g15" / "G15_ACCEPTANCE_MAP.md"
CONTRACT_PATH = ROOT / "milestones" / "g15" / "G15_CONTRACT.md"
SCHEMA_PATH = ROOT / "milestones" / "g15" / "g15_acceptance_map_check_evidence_schema.json"

# 冻结 5 行 P0 集合（2026-08-23 G15.1 立项裁决口径；G15_CONTRACT §2/§4.2 M-a~M-e）。
EXPECTED_P0 = {"M-a", "M-b", "M-c", "M-d", "M-e"}
# MAP §5：所有波次属于 G15.2|G15.3|G15.4|G15.5|G15.6a（§4.2 五行 P0 波次闭集）。
ALLOWED_WAVES = {"G15.2", "G15.3", "G15.4", "G15.5", "G15.6a"}
...
KEY_RE = re.compile(r"^g15\.p0\.m_[a-e]\.[a-z0-9_]+$")
KEY_IN_CELL_RE = re.compile(r"`(g15\.p0\.m_[a-e]\.[a-z0-9_]+)`")
SECTION_RE = re.compile(r"^## (\d+)\. ")
SCRIPT_RE = re.compile(r"ci/g15_[a-z0-9_]+_smoke\.py")
SCHEMA_RE = re.compile(r"`(milestones/g15/g15_(m_[a-e])_[a-z0-9_]+_evidence_schema\.json)`")
...
def check_row(m: str, row: dict, seen_keys: dict[str, str], seen_schemas: dict[str, str]) -> list[str]:
    ...
    if not KEY_RE.match(key):
        findings.append(f"{m} key `{key}` 不匹配 g15.p0.m_<a~e>.<slug>")
    ...
    slug = key.split(".")[3]
    if not row["scripts"]:
        findings.append(f"{m} 缺 `ci/g15_*_smoke.py` 脚本命令")
    if len(set(row["scripts"])) > 1:
        findings.append(f"{m} 一个 key 只能绑定一个脚本，实测 {sorted(set(row['scripts']))}")
    # 注：MAP §5 判据字面不设脚本↔key 同 slug 机核（M-b/M-c/M-d 脚本名为 key 末段缩写稳定形态，
    # 双向一致由 MAP §1 ↔ CONTRACT §4.2 判据/波次逐字比对承载）；schema 路径维持 key 同 slug 机核。
    ...
    if not schema_m:
        findings.append(f"{m} schema 路径不符 g15_m_<a~e>_<slug>_evidence_schema.json：{row['schema']!r}")
    else:
        path, schema_m_no = schema_m.group(1), schema_m.group(2)
        if schema_m_no != m_seg:
            findings.append(f"{m} schema 路径的 m 段不符：{path}")
        expected_schema = f"milestones/g15/g15_{m_seg}_{slug}_evidence_schema.json"
    ...
    if row["wave"] not in ALLOWED_WAVES:
        findings.append(f"{m} 波次 {row['wave']!r} 不在允许集合 {{G15.2,G15.3,G15.4,G15.5,G15.6a}} 内")
...
def evaluate(map_text: str | None, contract_text: str | None) -> list[dict]:
    ...
    results: list[dict] = []
    if map_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G15_ACCEPTANCE_MAP.md 缺失（诚实红，不假绿）"}]
    if contract_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G15_CONTRACT.md 缺失（诚实红，不假绿）"}]
    ...
    results.append({
        ...
        "detail": "§2 零 go P1 空集（G15.1 字面）" if p1_ok else f"§2 出现 P1 行 {sorted(p1_rows)}（G15.1 零 go P1 字面违例）",
    })
    ...
    for m in sorted(EXPECTED_P0):
        ...
        if len(row["keys"]) == 1:
            ...
            if other is None:
                two.append("G15_CONTRACT §4.2 缺行")
            else:
...
def emit(results: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g15_acceptance_map] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G15_CONTRACT G-G15-1/§4.2;G15_ACCEPTANCE_MAP.md §1/§2/§4/§5;G14PLUS_RECORD.md §6.3 G15 承接锚 + G14_P2_DECISIONS.md §5 defer-to-G15+ 29 行 + G13/G12 三表 20 行终态（G15 法定输入）P0 清单 M-a~M-e",
        required_gate_rows=[],
        extra_facts=results,
        subjects=[],
        schema_path=SCHEMA_PATH,
        evidence_basename=SUBJECT,
        notes="G15.1 治理门——验收映射覆盖/空行/双向命名空间一致性：5 P0（M-a 双端画质对拍链路全量复跑+20 行登记表逐项重评+G15 差距处置表+UE 方差带程序产+AI 读图基线臂 / M-b measured 主差修复闭环三态穷举+材质链立项评估登记 / M-c 绝对画质通过线程序产标定+18 格 AI 读图严格审查+商用收口判定 / M-d 性能零降级守护 G14 M-d 同口径复跑 ×1.00 维持+画质锚带复核 / M-e 回归门+RD-045/M165 漂移监控）闭集全等 + §2 零 go P1 空集 + key/脚本/schema 单一命名空间同 slug + numeric_step 全列 post-interlock 字面零预占 + MAP §1 ↔ CONTRACT §4.2 判据/波次双向逐字一致（G15 无独立 CI_GATES）；本门 PASS 只表示映射完整，不表示任何 P0 能力已实现",
...
# ---------------------------------------------------------------------------
# selftest 合成夹具：5 行 P0 冻结集合的正本（七列形态），不依赖树上文件。
# ---------------------------------------------------------------------------

CANONICAL_ROWS = [
    ("M-a", "dual_end_quality_reharvest", "ci/g15_dual_end_quality_reharvest_smoke.py", "G15.2"),
    ("M-b", "gap_fix_closure_loop", "ci/g15_gap_fix_closure_smoke.py", "G15.3"),
    ("M-c", "absolute_quality_final_review", "ci/g15_absolute_quality_review_smoke.py", "G15.4"),
    ("M-d", "perf_parity_zero_regression", "ci/g15_perf_parity_guard_smoke.py", "G15.5"),
    ("M-e", "regression_drift_guard", "ci/g15_regression_drift_guard_smoke.py", "G15.6a"),
]


def _fixture() -> tuple[str, str]:
    map_lines = ["# fixture G15_ACCEPTANCE_MAP", "", "## 1. P0 硬门（精确 5 行）", ""]
    contract_lines = ["# fixture G15_CONTRACT", "", "### 4.2 五行 P0", ""]
    for m, slug, script, wave in CANONICAL_ROWS:
        key = f"g15.p0.{m.lower().replace('-', '_')}.{slug}"
        schema = f"milestones/g15/g15_{m.lower().replace('-', '_')}_{slug}_evidence_schema.json"
        cmd = f"`py -3 {script} --gate {key}`"
        map_lines.append(
            f"| **{m}** | `{key}` | {cmd} | `{schema}` | 合成判据 {m} | **{wave}** | {NUMERIC_STEP_LITERAL} |"
        )
        contract_lines.append(f"| **{m}** | 合成判据 {m} | {wave} |")
    map_lines += ["", "## 2. 已 go P1 硬门（零行）", "", "G15.1 无 go 的 P1 行。", ""]
    return "\n".join(map_lines), "\n".join(contract_lines)
...
def run_selftest() -> int:
    ...
    cases: list[tuple[str, str, str, str]] = [
        ...
        (
            ...
            map_text.replace("--gate g15.p0.m_c.absolute_quality_final_review`", "--gate g15.p0.m_c.absolute_quality_review`"),
            contract_text,
            "row_M-c",
        ),
        (
            ...
            map_text.replace(
                "**G15.2** | post-interlock actual-next-free allocation |",
                "**G15.2** | 269 |",
                1,
            ),
            contract_text,
        ...
        (
            "波次改写为非法 G15.6c → 必须红",
            map_text.replace("合成判据 M-a | **G15.2**", "合成判据 M-a | **G15.6c**", 1),
            contract_text,
            "row_M-a",
        ),
        (
            ...
            map_text.replace(
                "G15.1 无 go 的 P1 行。",
                "| **M-a** | `g15.p0.m_a.dual_end_quality_reharvest` | `py -3 ci/g15_dual_end_quality_reharvest_smoke.py --gate g15.p0.m_a.dual_end_quality_reharvest` | `milestones/g15/g15_m_a_dual_end_quality_reharvest_evidence_schema.json` | 判据 | **G15.2** | post-interlock actual-next-free allocation |",
            ),
            contract_text,
            "coverage_p1_empty",
        ),
        (
            ...
            map_text,
            contract_text.replace("合成判据 M-d", "合成判据 M-d 漂移", 1),
            "two_way_M-d",
        ),
        (
            "schema 路径 m 段改写 → 必须红",
            map_text.replace(
                ...
                "milestones/g15/g15_m_d_absolute_quality_final_review_evidence_schema.json",
                1,
            ),
            contract_text,
            "row_M-c",
    ...
    if failures:
        print(f"[g15_acceptance_map] SELFTEST FAIL ({failures})")
        return 1
    print("[g15_acceptance_map] SELFTEST PASS (9 RED + 1 GREEN + 真表臂)")
    return 0
