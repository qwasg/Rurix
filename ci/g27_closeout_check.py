#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G27.6 close-out）
"""G27.6 close-out 终审门（g27.wave.6b.closeout，步骤 476）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g27.wave.6b.closeout"
NUMERIC_STEP = 476
SUBJECT = "g27_wave6b_closeout"
WAVE = "G27.6"
SCHEMA_PATH = ROOT / "milestones/g27/g27_wave6b_closeout_evidence_schema.json"
P0_SUBJECTS = [
    ("m_a", "g27_m_a_hzb_device_kernel"),
    ("m_b", "g27_m_b_m61_mesh_shader_rejudgment"),
    ("m_c", "g27_m_c_cluster_p4_gap_rejudgment"),
    ("m_d", "g27_m_d_hlod_l4_counter_rejudgment"),
    ("m_e", "g27_m_e_closed_gate_no_regression"),
]
...
def evaluate() -> tuple[list[dict], str]:
    ...
    p2 = wel.load_latest_evidence("g27_p2_decisions_check")
    ...
    facts.append(fact("handover_ledger_chain", ma_ok,
                      f"{ma.name}（HZB device kernel 主锚绿件）" if ma else "missing"))
    hits = list((ROOT / "rfcs").glob("0044-*.md"))
    facts.append(fact("rfc_0044_archived", bool(hits), hits[0].name if hits else "RFC-0044 missing"))
    me_ok, me_d = _latest_green("g27_m_e_closed_gate_no_regression")
    ...
    soak = wel.load_latest_evidence("g27_stabilization_soak")
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes=f"G27.6 closeout 八 facts VERDICT={verdict}",
        host_section_pass=ok,
    )
    print(f"[g27_closeout] VERDICT={verdict}")
