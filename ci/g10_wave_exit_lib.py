...
"""G10 波次聚合门共享库(milestones/g10/CI_GATES.md §5;同构 ci/g9_wave_exit_lib.py)。

只读汇总独立门 evidence,不重跑 smoke、不代绿。任一 required gate 缺失 /
非 PASS / SKIP / DEV_ENV_DEGRADE → 聚合红。供 G10.2~G10.8b 薄壳复用。

与 g9 版差异:docstring 对照说明、selftest 临时目录前缀、示例注释 subject 前缀;
...
# UTC stamp in evidence filenames: g10_<slug>_YYYYMMDDTHHMMSSZ.json
...
def load_latest_evidence(subject_prefix: str, evidence_dir: Path | None = None) -> Path | None:
    ...
    subject_prefix 例如 ``g10_m128_ue5_capture_environment``(匹配
    ``g10_m128_ue5_capture_environment_<UTC>.json``)。
    """
...
def run_selftest_missing_gate(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="g10_wave_exit_selftest_") as td:
        empty = Path(td)
...
def run_selftest() -> int:
    ...
    key = "g10.p0.m128.ue5_capture_environment"
    good = {
        "symbolic_gate_key": key,
    ...
    red("key 漂移", {**good, "symbolic_gate_key": "g10.p0.m129.ue5_reference_frames"})
    ...
    if failures:
        print(f"[g10_wave_exit_lib] SELFTEST FAIL ({failures})")
        return 1
    print("[g10_wave_exit_lib] SELFTEST PASS (6 RED + 1 GREEN)")
    return 0
