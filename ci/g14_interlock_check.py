...
"""G14.2 实现互锁守卫（g14.gov.implementation_interlock，步骤 249；G14_CONTRACT G-G14-3）。

G14_CONTRACT G-G14-1（G14.1 完成门 D-G14-4 互锁诚实 BLOCKED 面）与 G-G14-3（实现互锁门）
双出处治理三门之一。读取事实源并逐项输出，最后给出 READY / BLOCKED / CLOSED。两类断言严格分开：

* **事实门**（当前允许为红，红即 BLOCKED，不是脚本失败）：
  ① G13_CONTRACT front matter status == closed（G13.5b close-out flip 终态，§8.9 签署块
    在位），且 G14.0 不可变 ref `f4c8da0b` 已登记（G14_CONTRACT §7 立项裁决 1 字面）；
  ② G14_CANDIDATE_DECISIONS 无空行（§1 G13 defer 24 行 + §3 新增候选 7 行 = 31 行候选
    闭集全数在位〔§2 RD 映射 7 行不重复计入〕，全部数据行零空串/占位单元格）+
    registry/deferred.json history 只追加无静默改判（对 G14.0 不可变 ref f4c8da0b base：
    条目四字段 id/title/reason/backfill_condition 与 status/owner_milestone 0-byte、
    history 前缀只追加、条目零删除）+ G14_ACCEPTANCE_MAP §1 五行 P0（M-a~M-e 闭集全等，
    key/script/schema 路径/判据列无缺）与 §2 零 go P1 空集；
  ③ 用户 G14.2 开工指令留痕（2026-08-19 指令全期授权面——「帧率对标UE5略高（不降级画质）」
    字面在 G14_CONTRACT §7 裁决 2）+ workflow 实测末号与 ledger namespaces.CI_step
    on_tree_max 一致且 next_free == on_tree_max + 1，且治理三门步骤 247~249 的
    workflow/ledger 接线面在树（on_tree_max ≥ 249——共享编号按 actual next_free 校准面；
    G14.1 期主会话提交批统一接线，接线前 = RED 诚实 BLOCKED）；
  ④ G14.1 治理两门独立 PASS（g14.wave.1.acceptance_map + g14.wave.1.candidate_decisions
    实测 run_check 全绿——G14_ACCEPTANCE_MAP §6 条件 3 字面）。
  （G14 无许可前置门——G13 的 M-a 许可门是 G13 独有面，本守卫事实门为四项非五项。）
* **一致性门**（红即脚本 FAIL，退出非零）：
  C1 双状态诚实：implementation_status == blocked，或事实门全绿（禁止治理完成冒充实现开工）；
  C2 §8 记录一致：§8 出现 G-G14-3 解锁记录 ⇔ 事实门全绿且 implementation_status != blocked；
  C3 数字步骤零预占：milestones/g14 P0 面无 numeric_step 数字赋值 / P0 数据行末列裸数字
    （治理三门步骤 247~249 白名单豁免——本波即落盘真脚本真步骤为例外合法面）、
    workflow 无 g14.p0./g14.p1./ci/g14_*_smoke.py token、ci/ 无 g14_*_smoke.py 预放；
  C4 src/spec/conformance 0-byte（治理期）：三面无 g14 实现面命中。

**C3/C4 两态口径（沿 G10.4b/G11/G12/G13 先例，判据语义 0-byte）**：implementation_status
== blocked 时维持原机核；已解锁时 C3/C4 自动不适用（skipped_reason 登记并按通过处理）；
blocked 态恢复原机核。

**closed 三态口径（沿 G10.8b/G11/G12/G13 先例，判据语义 0-byte）**：status == closed 时
互锁使命完结，事实门/一致性门整体不适用，VERDICT=CLOSED、exit=0；CLOSED 不得被当作
G-G14-3 重新开放凭据（契约 reopen 须新立项治理程序）。

诚实纪律：BLOCKED 是当前正确结论（用户 G14.2 开工指令留痕核验面/治理三门步骤 247~249
workflow·ledger 接线面在树前——主会话提交批统一接线），不得被当作 G-G14-3 PASS；
`--require-ready` 供未来 G14.2 实现 PR 作前置 required check（未 READY 即退出非零）；
`--gate g14.gov.implementation_interlock` 产 evidence（VERDICT 字面入档，BLOCKED 不充绿）。
`--selftest` 用可注入输入的受控负样本证明每组断言都能红/败。

用法：
  py -3 ci/g14_interlock_check.py --gate g14.gov.implementation_interlock
  py -3 ci/g14_interlock_check.py --require-ready
  py -3 ci/g14_interlock_check.py --selftest
"""
...
from g14_acceptance_map_check import run_check as acceptance_map_run_check  # noqa: E402
from g14_candidate_decisions_check import FROZEN_IDS as CANDIDATE_FROZEN_IDS  # noqa: E402
from g14_candidate_decisions_check import run_check as candidate_decisions_run_check  # noqa: E402

G14_CONTRACT = ROOT / "milestones/g14/G14_CONTRACT.md"
G13_CONTRACT = ROOT / "milestones/g13/G13_CONTRACT.md"
G14_CANDIDATES = ROOT / "milestones/g14/G14_CANDIDATE_DECISIONS.md"
G14_MAP = ROOT / "milestones/g14/G14_ACCEPTANCE_MAP.md"
LEDGER = ROOT / "registry/number_ledger.json"
DEFERRED = ROOT / "registry/deferred.json"
WORKFLOWS = ROOT / ".github/workflows"
SCHEMA_PATH = ROOT / "milestones/g14/g14_interlock_check_evidence_schema.json"

GATE_KEY = "g14.gov.implementation_interlock"
NUMERIC_STEP = 249  # 落盘前实测 registry/number_ledger.json CI_step.next_free=247 顺位领取
SUBJECT = "g14_interlock_check"
WAVE = "G14.1"

G14_0_IMMUTABLE_REF = "f4c8da0b"  # G14.0 文档集不可变 ref（G13 close-out flip commit，tag g13-closed）
G14_USER_INSTRUCTION_LITERAL = "帧率对标UE5略高（不降级画质）"
# 治理三门步骤（G14.1 落盘前实测 CI_step.next_free=247 顺位领取 247/248/249）；
# 接线面 = workflow 实测末号与 ledger on_tree_max 覆盖本批步骤（主会话提交批统一接线）。
GOVERNANCE_MAX_STEP = 249

# 空行门占位单元格闭集（MAP §5 no-empty 口径）。
PLACEHOLDER_CELLS = {"", "TBD", "TODO", "待定", "待补", "—"}
# 治理三门白名单（本波即落盘真脚本真步骤 247~249 的例外合法面；C3 数字预占扫描豁免）。
GOVERNANCE_GATE_KEYS = (
    "g14.wave.1.acceptance_map",
    "g14.wave.1.candidate_decisions",
    "g14.gov.implementation_interlock",
)
GOVERNANCE_SCHEMA_FILES = {
    "g14_acceptance_map_check_evidence_schema.json",
    "g14_candidate_decisions_check_evidence_schema.json",
    "g14_interlock_check_evidence_schema.json",
}

# 数字步骤零预占扫描面：numeric_step 数字赋值 / g14 P0 数据行末列裸数字 / workflow g14 实现面 token。
NUMERIC_ASSIGN_RE = re.compile(r"numeric_step[\"']?\s*[:=]\s*[\"']?\d")
NUMERIC_TAIL_CELL_RE = re.compile(r"\|\s*\d{1,4}\s*\|\s*$")
WORKFLOW_G14_RE = re.compile(r"g14\.p[01]\.|ci/g14_[a-z0-9_]+_smoke\.py")
WORKFLOW_STEP_RE = re.compile(r"步骤\s*(\d{1,4})")
# C4 扫描面：gate-key/脚本命名 token + g14 命名文件。
IMPL_SURFACE_G14_RE = re.compile(r"g14\.p[01]\.|g14\.wave\.|g14\.gov\.|ci/g14_")
# MAP §1 P0 行首单元格形态：| **M-a** | ...。
MAP_ROW_RE = re.compile(r"^\|\s*\*\*(M-[a-e])\*\*")
MAP_KEY_RE = re.compile(r"g14\.p0\.(m_[a-e])\.[a-z0-9_]+")
MAP_SCRIPT_RE = re.compile(r"ci/g14_[a-z0-9_]+_smoke\.py")
MAP_SCHEMA_RE = re.compile(r"milestones/g14/g14_m_[a-e]_[a-z0-9_]+_evidence_schema\.json")
EXPECTED_MAP_P0 = {"M-a", "M-b", "M-c", "M-d", "M-e"}
ACTIVATION_RE = re.compile(
    r"^#{2,5}\s*§?8\.\d+[^\n]*(implementation_status 解锁|G-G14-3)", re.MULTILINE
)
...
class TreeInputs:
    ...
    workflow_g14_hits: list[str] = field(default_factory=list)
    g14_smoke_scripts: list[str] = field(default_factory=list)
...
def check_candidate_decisions(text: str | None) -> list[str]:
    """G14_CANDIDATE_DECISIONS 无空行：31 行候选闭集全数在位 + 全部数据行零空串/占位单元格。"""
    if text is None:
        return ["G14_CANDIDATE_DECISIONS.md 缺失"]
    findings: list[str] = []
...
def check_deferred_append_only(base_doc: dict | None, current_doc: dict | None) -> list[str] | None:
    """deferred.json history 只追加核验（vs G14.0 base）：条目零删除、条目级字段 0-byte、
...
def check_acceptance_map(text: str | None) -> list[str]:
    """G14_ACCEPTANCE_MAP §1 五行 P0（M-a~M-e 闭集全等）+ §2 零 go P1 空集 + 行面齐备。"""
    if text is None:
        return ["G14_ACCEPTANCE_MAP.md 缺失"]
    findings: list[str] = []
    ...
    if p1_rows:
        findings.append(f"MAP §2 P1 集合 {sorted(p1_rows)} 非空（G14.1 零 go P1 字面违例）")
    for mid, cells in p0_rows.items():
        ...
        if not MAP_SCRIPT_RE.search(blob):
            findings.append(f"MAP {mid} 行缺稳定脚本 ci/g14_*_smoke.py")
...
def _scan_numeric_step(root: Path) -> list[str]:
    """milestones/g14 P0 面数字步骤零预占扫描（治理三门白名单豁免）；返回违例行清单。"""
    hits: list[str] = []
    base = root / "milestones/g14"
    if not base.is_dir():
        return ["milestones/g14 目录缺失"]
    for path in sorted(base.rglob("*")):
        if not path.is_file() or path.suffix not in (".md", ".json", ".yml", ".yaml"):
            continue
        if path.name in GOVERNANCE_SCHEMA_FILES:
            continue  # 治理三门 schema 面（步骤 247~249 合法领取）
        try:
        ...
        for lineno, line in enumerate(text.splitlines(), 1):
            ...
            elif line.lstrip().startswith(("| **M-", "| `g14.")) and NUMERIC_TAIL_CELL_RE.search(line):
                hits.append(f"{rel}:{lineno} 数据行末列裸数字（疑似步骤预占）：{line.strip()[:80]}")
    return hits


def _scan_workflows(root: Path) -> list[str]:
    """workflow g14 实现面 token 扫描（g14.p0./g14.p1./ci/g14_*_smoke.py 即预占）。"""
    hits: list[str] = []
    ...
    for path in sorted(WORKFLOWS.glob("*.yml")) + sorted(WORKFLOWS.glob("*.yaml")):
        try:
            ...
            continue
        for lineno, line in enumerate(text.splitlines(), 1):
            if WORKFLOW_G14_RE.search(line):
...
def _scan_impl_surface(root: Path) -> list[str]:
    """src/spec/conformance 治理期 0-byte 扫描：g14 实现面 token / g14 命名文件即违例。"""
    hits: list[str] = []
    for sub in ("src", "spec", "conformance"):
        ...
        for path in sorted(base.rglob("*")):
            if not path.is_file():
                continue
            rel = path.relative_to(root)
            if "g14" in path.name.lower():
                hits.append(f"{rel}（文件名含 g14）")
                continue
            ...
            try:
                ...
                continue
            for lineno, line in enumerate(text.splitlines(), 1):
                if IMPL_SURFACE_G14_RE.search(line):
...
def load_inputs(root: Path) -> TreeInputs:
    ...
    base_text = _git_show_file(root, G14_0_IMMUTABLE_REF, "registry/deferred.json")
    ...
    return TreeInputs(
        ...
        candidate_findings=check_candidate_decisions(_read(G14_CANDIDATES)),
        deferred_findings=check_deferred_append_only(deferred_base, deferred_current),
        map_findings=check_acceptance_map(_read(G14_MAP)),
        ...
        workflow_g14_hits=_scan_workflows(root),
        g14_smoke_scripts=[p.name for p in sorted((root / "ci").glob("g14_*_smoke.py"))],
...
def evaluate_fact_gates(inp: TreeInputs) -> list[tuple[bool, str]]:
    ...
    g13_status = front_matter_field(inp.g13_contract_text, "status") if inp.g13_contract_text else None
    g13_closeout_section = bool(inp.g13_contract_text and "§8.9" in inp.g13_contract_text)
    g14_ref_registered = bool(inp.g14_contract_text and G14_0_IMMUTABLE_REF in inp.g14_contract_text)
    facts.append(
        (
            g13_status == "closed" and g13_closeout_section and g14_ref_registered,
            f"① G13_CONTRACT status = {g13_status!r}（要求 closed）且 §8.9 签署块在位 = {g13_closeout_section}；"
            f"G14.0 不可变 ref {G14_0_IMMUTABLE_REF} 登记 = {g14_ref_registered}",
        )
    )
    ...
    if inp.deferred_findings is None:
        ...
        sub2.append("deferred history 只追加（vs G14.0 base 四字段 0-byte）")
    if inp.map_findings is None:
        ok2 = False
        sub2.append("G14_ACCEPTANCE_MAP 缺失")
    ...
    instruction_recorded = bool(
        inp.g14_contract_text and G14_USER_INSTRUCTION_LITERAL in inp.g14_contract_text
    )
    ...
    facts.append(
        (
            ...
            f"③ 用户 G14.2 开工指令留痕（2026-08-19 全期授权面「{G14_USER_INSTRUCTION_LITERAL}」字面）"
            f" = {instruction_recorded}；workflow 实测末号 = {inp.workflow_max_step}、"
            f"ledger CI_step on_tree_max = {on_tree_max}、next_free = {next_free}"
            f"（一致 = {base_consistent}；治理三门步骤 247~249 workflow/ledger 接线面在树"
    ...
    facts.append(
        (
            ...
            f"④ G14.1 治理两门独立 PASS（g14.wave.1.acceptance_map + g14.wave.1.candidate_decisions"
            f" 实测 run_check 全绿，G14_ACCEPTANCE_MAP §6 条件 3 字面）= {inp.governance_gates_green}",
        )
    )
    return facts
...
def evaluate_consistency_gates(inp: TreeInputs, facts_all_green: bool) -> list[tuple[bool, str]]:
    ...
    impl_status = front_matter_field(inp.g14_contract_text, "implementation_status") if inp.g14_contract_text else None
    consistency.append(
        (
            impl_status == "blocked" or facts_all_green,
            f"C1 G14_CONTRACT implementation_status = {impl_status!r}；事实门全绿 = {facts_all_green}"
            "（事实未全绿时必须保持 blocked，禁止治理完成冒充实现开工）",
        )
    )

    activated = inp.g14_contract_text is not None and bool(ACTIVATION_RE.search(inp.g14_contract_text))
    consistency.append(
        (
            activated == (facts_all_green and impl_status not in (None, "blocked")),
            f"C2 §8 G-G14-3 解锁记录存在 = {activated}；事实门全绿 = {facts_all_green}、"
            f"implementation_status = {impl_status!r}（双状态与 §8 记录必须一致）",
        )
    )

    preclaim = inp.numeric_step_violations + inp.workflow_g14_hits + inp.g14_smoke_scripts
    if impl_status == "blocked":
        consistency.append(
            (
                not preclaim,
                f"C3 数字步骤零预占：milestones/g14 numeric_step 违例 {len(inp.numeric_step_violations)} 处、"
                f"workflow g14 实现面 token {len(inp.workflow_g14_hits)} 处、ci/g14_*_smoke.py 预放 "
                f"{inp.g14_smoke_scripts or '无'}（治理三门 247~249 白名单豁免）"
        ...
        consistency.append(
            (
                ...
                f"C4 src/spec/conformance 治理期 0-byte：g14 实现面 token/命名命中 "
                f"{len(inp.impl_surface_hits)} 处"
                + (f"；首处 {inp.impl_surface_hits[0]}" if inp.impl_surface_hits else ""),
            )
        )
    else:
        # 两态口径（沿 G10.4b/G11/G12/G13 先例，判据语义 0-byte）：已解锁后 C3/C4 治理期口径
        ...
        consistency.append(
            (
                ...
                f"{len(inp.numeric_step_violations)} 处 / workflow g14 实现面 token {len(inp.workflow_g14_hits)} 处 / "
                f"ci/g14_*_smoke.py {len(inp.g14_smoke_scripts)} 件均为解锁后合法实现面，非预占；"
        ...
        consistency.append(
            (
                ...
                f"治理期口径不适用；skipped_reason=实现波合法改动三面，实测 g14 实现面 token/命名命中 "
...
def run(inp: TreeInputs, require_ready: bool = False, printer=print) -> tuple[int, str]:
    ...
    # closed 三态口径（沿 G10.8b/G11/G12/G13 先例，判据语义 0-byte）。
    contract_status = (
        front_matter_field(inp.g14_contract_text, "status") if inp.g14_contract_text else None
    )
    if contract_status == "closed":
        printer(
            "[g14_interlock] 事实门/一致性门：not_applicable"
            "（status='closed' 收口终态，互锁使命完结；skipped_reason=G14.2+ 开工门问题"
            "不再适用——G14.5b close-out READY 后 status flip；active/blocked 态恢复原机核，"
            "判据语义 0-byte）"
        )
        printer("[g14_interlock] VERDICT = CLOSED")
        printer(
            "  CLOSED 是收口终态正确结论：本守卫回答「G14.2 可否开工」，契约 closed 后该问题"
            "不再适用；不得被当作 G-G14-3 重新开放凭据（契约 reopen 须新立项治理程序）。"
        )
        return 0, "CLOSED"
    ...
    printer("[g14_interlock] 事实门（当前可为红）：")
    for ok, msg in facts:
        printer(f"  {'PASS' if ok else 'RED '} {msg}")
    printer("[g14_interlock] 一致性门（红即脚本失败）：")
    for ok, msg in consistency:
        printer(f"  {'PASS' if ok else 'FAIL'} {msg}")

    verdict = "READY" if facts_all_green else "BLOCKED"
    printer(f"[g14_interlock] VERDICT = {verdict}")
    if verdict == "BLOCKED":
        missing = [msg.split("：", 1)[0] for ok, msg in facts if not ok]
        printer(f"  缺项清单：{missing}")
        printer(
            "  BLOCKED 是当前正确结论：G14.2+ 的 src/、spec/、conformance/ 与 P0 数字 workflow 步骤保持 0-byte；"
            "本输出不得被当作 G-G14-3 PASS。"
        )

    consistency_failed = [msg for ok, msg in consistency if not ok]
    if consistency_failed:
        printer(f"[g14_interlock] FAIL — {len(consistency_failed)} 项一致性门为红")
        return 1, verdict
    if require_ready and verdict != "READY":
        printer("[g14_interlock] FAIL — --require-ready 模式下互锁未 READY")
        return 1, verdict
    return 0, verdict
...
def run_gate() -> int:
    ...
    facts_all_green = all(ok for ok, _ in facts)
    impl_status = front_matter_field(inp.g14_contract_text, "implementation_status") if inp.g14_contract_text else None
    contract_status = front_matter_field(inp.g14_contract_text, "status") if inp.g14_contract_text else None
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g14_interlock] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    ecode, _ = wel.emit_wave_evidence(
        ...
        source_ref="G14_CONTRACT G-G14-3/§6/§7;G14_ACCEPTANCE_MAP §5/§6;registry/number_ledger.json;registry/deferred.json",
        required_gate_rows=[],
        ...
        notes=(
            f"G14.1 治理门——实现互锁 validator 诚实报告：VERDICT={verdict}"
            + (
                "（BLOCKED 是当前正确结论：用户 G14.2 开工指令留痕核验面/治理三门步骤 247~249 "
                "workflow·ledger 接线面在树前（主会话提交批统一接线），G14.2 不得开工；"
                "不充绿、不以叙述替代机器事实）"
                if verdict == "BLOCKED"
                else "（READY 是当前正确结论：事实门①~④全绿——G13 closed + 治理文档三面 + "
                "开工指令留痕与共享编号校准 + 治理两门独立 PASS，G14.2 开工面开放；不以叙述替代机器事实）"
            )
            + "；一致性门 C1~C4 全绿；"
            "--require-ready 供未来 G14.2 实现 PR 前置 required check"
        ),
...
- **立项裁决**：现在立项；G14.0 不可变 ref = `{G14_0_IMMUTABLE_REF}`（G13 close-out flip commit，tag `g13-closed`）。
- **用户开工指令留痕**：2026-08-19 用户指令全期授权面——「彻底完成对标UE5渲染器的目标……同时优化渲染管线效率，使{G14_USER_INSTRUCTION_LITERAL}……」。
...
def _fixture_candidates_text() -> str:
    ...
    for rid in CANDIDATE_FROZEN_IDS:
        lines.append(f"| {rid} | defer-to-G15+ | 重判条件 = x；兜底 = y |")
    return "\n".join(lines) + "\n"
...
def _fixture_map_text() -> str:
    ...
    for letter, slug, script in (
        ("a", "registry_variance_band_reconciliation", "ci/g14_registry_variance_band_smoke.py"),
        ("b", "ue_benchmark_arm_measurement", "ci/g14_ue_benchmark_arm_smoke.py"),
        ("c", "rurix_pipeline_perf", "ci/g14_rurix_pipeline_perf_smoke.py"),
        ("d", "dual_end_fps_parity", "ci/g14_dual_end_fps_parity_smoke.py"),
        ("e", "regression_drift_guard", "ci/g14_regression_drift_guard_smoke.py"),
    ):
        lines.append(
            f"| **M-{letter}** | `g14.p0.m_{letter}.{slug}` `{script}` | "
            f"`milestones/g14/g14_m_{letter}_{slug}_evidence_schema.json` | 判据 | G14.2 | "
...
def _good_inputs() -> TreeInputs:
    ...
    return TreeInputs(
        ...
        ledger={"namespaces": {"CI_step": {"on_tree_max": 249, "next_free": 250}}},
        workflow_max_step=249,
...
def run_selftest() -> int:
    ...
    inp.g13_contract_text = inp.g13_contract_text.replace("status: closed", "status: active")
    case("G13 status 改 active → ① 红", inp, "① G13_CONTRACT status = 'active'", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.g14_contract_text = inp.g14_contract_text.replace(G14_0_IMMUTABLE_REF, "deadbeef")
    case("G14.0 不可变 ref 未登记 → ① 红", inp, "G14.0 不可变 ref", "BLOCKED", 0)
    ...
    inp = copy.deepcopy(_good_inputs())
    inp.g14_contract_text = inp.g14_contract_text.replace(G14_USER_INSTRUCTION_LITERAL, "路径追踪生产化")
    case("用户开工指令字面缺失 → ③ 红", inp, "③ 用户 G14.2 开工指令留痕", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.workflow_max_step = 250
    case("workflow 实测末号 250 ≠ ledger on_tree_max 249 → ③ 红", inp, "workflow 实测末号 = 250", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.ledger["namespaces"]["CI_step"]["next_free"] = 252
    case("ledger next_free=252 ≠ on_tree_max+1 → ③ 红", inp, "next_free = 252", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.ledger = {"namespaces": {"CI_step": {"on_tree_max": 246, "next_free": 247}}}
    inp.workflow_max_step = 246
    case(
        "治理三门步骤 247~249 接线面缺失（主会话提交批前态）→ ③ 红",
        inp, "接线面在树 = False", "BLOCKED", 0,
    )

    inp = copy.deepcopy(_good_inputs())
    inp.governance_gates_green = False
    case("治理两门未绿注入 → ④ 红", inp, "④ G14.1 治理两门独立 PASS", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.numeric_step_violations = ["milestones/g14/G14_ACCEPTANCE_MAP.md:30 numeric_step 数字赋值：numeric_step: 250"]
    case("数字步骤预占注入 → C3 FAIL 退 1（VERDICT 不受一致性门影响）", inp, "C3 数字步骤零预占", "READY", 1)

    inp = copy.deepcopy(_good_inputs())
    inp.impl_surface_hits = ["spec/g14_fps_parity.md（文件名含 g14）"]
    case("spec 面 g14 命中注入 → C4 FAIL 退 1", inp, "C4 src/spec/conformance 治理期 0-byte", "READY", 1)

    inp = copy.deepcopy(_good_inputs())
    inp.g14_contract_text = inp.g14_contract_text.replace(
        "## 8. Implementation activation / Close-out（只追加区）",
        "### §8.1 G-G14-3 implementation_status 解锁记录",
    )
    case("落 §8 解锁记录但 front matter 未翻 → C2 FAIL 退 1", inp, "C2 §8 G-G14-3 解锁记录存在 = True", "READY", 1)

    # 两态口径（沿 G10.4b/G11/G12/G13 先例）：unblocked 态 C3/C4 自动不适用（skipped_reason 登记）。
    unblocked_text = _GOOD_G14_CONTRACT.replace(
    ...
    inp = copy.deepcopy(_good_inputs())
    inp.g14_contract_text = unblocked_text
    inp.numeric_step_violations = ["milestones/g14/G14_ACCEPTANCE_MAP.md:30 numeric_step 数字赋值：numeric_step: 250"]
    inp.workflow_g14_hits = [".github/workflows/pr-smoke.yml:2900 g14.p0.m_a.registry_variance_band_reconciliation"]
    inp.g14_smoke_scripts = ["g14_registry_variance_band_smoke.py"]
    inp.impl_surface_hits = ["spec/fps_parity.md:1 g14.p0. 新条款"]
    case(
    ...
    inp.g14_contract_text = unblocked_text
    inp.governance_gates_green = False
    case("unblocked 态事实门红 → C1 仍 FAIL 退 1（两态校准不遮蔽 C1/C2）", inp, "C1 G14_CONTRACT implementation_status = 'unblocked'", "BLOCKED", 1)

    # closed 三态口径：status==closed → VERDICT=CLOSED 退 0。
    inp = copy.deepcopy(_good_inputs())
    inp.g14_contract_text = unblocked_text.replace("status: active", "status: closed")
    case(
    ...
    # 当前树实测：VERDICT 与事实一致（G14.1 期开工指令留痕核验/治理三门接线面在树前 = BLOCKED／
    ...
    if tree_verdict in ("BLOCKED", "READY", "CLOSED") and tree_code == expected_tree_exit:
        print(
            ...
            f"{'开工指令留痕核验/治理三门接线面在树前期' if tree_verdict == 'BLOCKED' else ('收口终态' if tree_verdict == 'CLOSED' else 'G-G14-3 解锁条件已齐')}，符合当前事实预期）"
        )
    else:
        print(
    ...
    if failures:
        print(f"[g14_interlock] SELFTEST FAIL ({failures})")
        return 1
    print("[g14_interlock] SELFTEST PASS (17 RED + 1 GREEN + 1 TREE)")
    return 0
