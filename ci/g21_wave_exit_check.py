#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G21 波聚合门）
"""G21 波聚合门（参数化 g21.wave.{2..6}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g21/g21_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g21.wave.2.exit": ("G21.2", "g21_m_a_restir_high_reservoir_realization", 369),
    "g21.wave.3.exit": ("G21.3", "g21_m_b_ser_capability_disposition", 371),
    "g21.wave.4.exit": ("G21.4", "g21_m_c_rd040_subitem_disposition", 373),
    "g21.wave.5.exit": ("G21.5", "g21_m_d_rd034_upstream_recheck", 375),
    "g21.wave.6.exit": ("G21.6", "g21_m_e_closed_gate_no_regression", 377),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g21_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G21_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g21_wave{n}_exit",
        notes=f"G21 wave {n} exit", host_section_pass=overall,
    )
