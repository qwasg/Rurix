#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G9.5 M116 地形 chunk≡cell 门冒烟(g9.p1.m116.terrain_chunk_cell;RFC-0025
§4.G;spec/world_partition.md RXS-0367;G9_ACCEPTANCE_MAP §3 M116;G9_CONTRACT
§8.1 裁决① P1 全进)。
...
实记 device_name=host-only/validation=not_applicable)。三段判据:
  ...
  host 段:rurix-render world::terrain 6 单测逐名锚定(heightfield canonical
    往返+篡改/chunk≡cell+第二套分格 RED/全 compute LOD/剔除零 CPU 提交/邻级
    缝合+裂缝 RED/toroidal 复用+迟到页占位/零 SVT 依赖 RED)+ conformance
    world_partition M116 双件语料锚定(//@ spec: RXS-0367 + 门 key/脚本名
    留痕)+ 冻结带 milestones/g9/g9_m116_terrain_band.json provenance 机器核验。
  harness 段:持锁(gpu_device_lock)真跑 g9_m116_terrain --evidence(直出件
    落 .tmp 工作区不覆盖 evidence/ harness 直出件;schema/spec_anchor/
    assertion_id/status==pass + 11 判据闭集全真)+ --red-arm
    second-grid/stitch-crack 子模式独立复跑抽检。

用法:
  py -3 ci/g9_terrain_chunk_cell_smoke.py --gate g9.p1.m116.terrain_chunk_cell
  py -3 ci/g9_terrain_chunk_cell_smoke.py --selftest
"""
...
SCHEMA_PATH = ROOT / "milestones" / "g9" / "g9_m116_terrain_chunk_cell_evidence_schema.json"
BAND_PATH = ROOT / "milestones" / "g9" / "g9_m116_terrain_band.json"
CORPUS_DIR = ROOT / "conformance" / "world_partition"
WORK_DIR = ROOT / ".tmp" / "g95_gates" / "m116"
HARNESS_EVIDENCE = WORK_DIR / "harness_evidence.json"

sys.path.insert(0, str(ROOT / "ci"))
from gpu_device_lock import gpu_device_lock  # noqa: E402

GATE_KEY = "g9.p1.m116.terrain_chunk_cell"
NUMERIC_STEP = 161
SOURCE_REF = "RFC-0025 §4.G;spec/world_partition.md RXS-0367;G9_ACCEPTANCE_MAP §3 M116"
TAG = "g9_m116"
SUBJECT = "g9_m116_terrain_chunk_cell"
MATRIX_ROW = "M116"

MODULE_TESTS = {
    "world::terrain": [
        "heightfield_canonical_roundtrip_and_tamper",
        "chunk_eq_cell_and_second_grid_red",
        "full_compute_lod_cull_zero_cpu_submit",
        "seam_stitch_and_crack_red",
        "toroidal_reuse_and_late_page_placeholder",
        "zero_svt_dependency_red",
    ],
}
CORPUS_FILES = [
...
BAND_SCHEMA = "rurix.g9m116.terrain_band.v1"
BAND_SPEC_ANCHOR = "RXS-0367"
BAND_DIGEST_FIELDS = ["scene_digest"]
HARNESS_BIN = "g9_m116_terrain"
HARNESS_SCHEMA = "rurix.g9m116.terrain.v1"
HARNESS_ASSERTION = "g9.p1.m116.terrain_chunk_cell"
HARNESS_TAG = "G9_M116_TERRAIN"
HARNESS_CHECKS = [
    "conformance_corpus_anchored",
    "chunk_eq_cell_structural",
    "full_compute_lod_cull_indirect",
    "cpu_zero_per_chunk_submit",
    "stitch_continuity_crack_zero",
    "toroidal_reuse_and_placeholder",
    "double_run_bit_equal",
    "golden_frozen_equal",
    "red_arm_second_grid_and_svt",
    "red_arm_lod_gap_and_crack",
    "red_arm_cpu_submit",
]
RED_ARMS = ["second-grid", "stitch-crack"]
...
def main() -> int:
    ...
    with gpu_device_lock(purpose="g9_m116 terrain harness 腿"):
        exe = build_harness()
        if exe:
            ...
            note("harness:chunk≡cell + 全 compute indirect + 缝合裂缝=0 + toroidal 复用 + 零 SVT + RED 双臂子模式复跑")
