#!/usr/bin/env python3
"""GRX-019 ``fused_post_chain`` gate module.

Exports ``evaluate() -> dict`` per the interface contract in
``ci/grx_gates/_common.py``. The probe registers this module eighth in
``GRX_GATE_SEQUENCE`` (after grx011..grx018) and walks it fail-closed.

GRX-019 fused_post_chain is at the GRX Wave 4 *bridge* stage: the contract trio, the
offline kernel package, the integer-exact CPU math-parity reference, the
fail-closed ``FusedPostChainGate`` (3 SRV + 2 UAV textures with its shim
recording entry), and the standalone D3D12 dispatch smoke (three tracked
digests, GPU-vs-CPU match within the pass's documented absolute tolerance) have all landed. The Godot patches (0036-0038), the gated
...
GATE_ID = "grx019"
# The action the probe advances to once THIS gate is fully ready: the GRX-020
# descriptor-cache hardening slice (last in this Wave 4 bridge gate run).
NEXT_ACTION = "start_grx020_descriptor_cache_hardening"
...
# The three fused_post_chain patches (PATCH_ALLOCATION.md §2 0036-0038). They are
...
PASS_ENUM_MARKER = "RXGD_PASS_FUSED_POST_CHAIN"
...
def _patch_applyability() -> tuple[bool, str | None]:
    # DEFERRED: the fused_post_chain patch block (0036-0038) is authored in the next
    ...
    for ordinal in FUSED_POST_CHAIN_PATCH_ORDINALS:
        if _patch_file(ordinal) is None:
            return False, (
                f"fused_post_chain patches {'/'.join(FUSED_POST_CHAIN_PATCH_ORDINALS)} not yet authored "
                "(DEFERRED to the next serial patch slice; the GRX Wave 4 bridge slice landed "
                "the fail-closed FusedPostChainGate + standalone D3D12 dispatch smoke only)"
            )
    prereqs = []
    for ordinal in PREREQ_ORDINALS:
        ...
        prereqs.append(path)
    top = _patch_file("0038")
    result = _common.evaluate_stacked_patch_applyability(ROOT, EXTERNAL_GODOT, prereqs, top, "0038")
    if result.get("ok") is not True:
        return False, f"fused_post_chain patch 0038 stacked applyability failed: {result.get('reason')}"
    return True, None
...
def _dispatch_smoke_ready() -> tuple[bool, str | None]:
    ...
    if doc is None:
        return False, (
            ...
            "ci/grx019_fused_post_chain_d3d12_dispatch_smoke.py"
        )
