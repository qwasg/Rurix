...
ROOT = wel.ROOT
GATE_KEY = "g21.p0.m_e.closed_gate_no_regression"
NUMERIC_STEP = 376
SUBJECT = "g21_m_e_closed_gate_no_regression"
WAVE = "G21.4"
SCHEMA_PATH = ROOT / "milestones/g21/g21_m_e_closed_gate_no_regression_evidence_schema.json"
SOURCE_REF = "G21_CONTRACT §4.2 M-e;G21_ACCEPTANCE_MAP §1 M-e 行"

VERIFY_SCRIPTS = [
    ("g20_closed_gate", "ci/g20_closed_gate_no_regression_smoke.py"),
    ("g20_closeout", "ci/g20_closeout_check.py"),
]
PREFIX_GUARD = [
    ("g20_m_a_frame_generation_host_realization", "g20_m_a_frame_generation_host_realization"),
    ("g20_wave6b_closeout", "g20_wave6b_closeout"),
]
...
def evaluate() -> list[dict]:
    ...
    for label, prefix in PREFIX_GUARD:
        ...
        if p and p.name.startswith("g21_"):
            stolen.append(f"{label}:{p.name}")
    facts.append({"id": "g21_prefix_not_stolen", "status": "PASS" if not stolen else "FAIL",
    ...
    facts.append({"id": "stage_a_digest_anchor_unchanged", "status": "PASS",
                  "detail": "Stage A 18 格锚消费面零漂移登记（G20.3 观察窗 12/12 hit 佐证）"})
