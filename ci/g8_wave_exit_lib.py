#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G8 波次聚合门共享库(CI_GATES §5;G8.6…§7.1)。

只读汇总独立门 evidence,不重跑 smoke、不代绿。任一 required gate 缺失 /
非 PASS / SKIP / DEV_ENV_DEGRADE → 聚合红。供 wave2~8b 薄壳复用。
"""
...
# UTC stamp in evidence filenames: g8_<slug>_YYYYMMDDTHHMMSSZ.json
...
def load_latest_evidence(subject_prefix: str, evidence_dir: Path | None = None) -> Path | None:
    ...
    subject_prefix 例如 ``g8_m31_reflection_hash``(匹配
    ``g8_m31_reflection_hash_<UTC>.json``)。
    """
...
def run_selftest_missing_gate(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="g8_wave_exit_selftest_") as td:
        empty = Path(td)
