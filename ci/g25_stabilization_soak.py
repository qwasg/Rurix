#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G25.5 soak 波）
"""G25.5 稳定 soak（g25.wave.5a.soak，步骤 443）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g25.wave.5a.soak"
NUMERIC_STEP = 443
SUBJECT = "g25_stabilization_soak"
WAVE = "G25.5"
SCHEMA_PATH = ROOT / "milestones/g25/g25_stabilization_soak_evidence_schema.json"
BIN = ROOT / "target/release/g14_3_pipeline_perf.exe"
HANDOVER_REG = ROOT / "milestones/g25/g25_campaign_handover_registry.json"
MIN_SECONDS = 1800.0
...
def run_gate() -> int:
    facts: list[dict] = []
    md = wel.load_latest_evidence("g25_m_d_campaign_handover_ledger")
    ...
    if not md_ok or not BIN.is_file() or not HANDOVER_REG.is_file():
        ...
        while time.perf_counter() - t0 < MIN_SECONDS:
            ...
            if iters % 5 == 4:
                ...
                pb = ROOT / "target/release" / probes[probe_iters % len(probes)]
                out = ROOT / ".tmp" / "g25_soak_probes" / f"iter_{iters}.json"
        ...
        facts.extend([
            ...
            fact("no_sleep_between_iters", True, "sleep=0"),
            fact("probe_lane_interleaved", probe_iters > 0, f"probe_iters={probe_iters}（战役四实现件探针轮换穿插复跑）"),
        ])
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G25 soak ≥1800s（管线四组合 + 战役探针轮换穿插）", host_section_pass=ok,
    )
