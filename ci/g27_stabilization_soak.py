#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G27.5 soak 波）
"""G27.5 稳定 soak（g27.wave.5a.soak，步骤 475）。

管线 bench 四组合轮转 + 探针轮换扩容至六车道（G27-N5）：每第 5 迭代按
probe_iters % 6 轮转六车道——g19/g20/g21/g22 战役四实现件探针 + 第五车道
g26_framegen_device（--probe ×2 档 8 帧快车道）+ 第六车道 g27_hzb_device
（--probe 零容差，reduce/test 双 SPV，无 --tol）。soak 启动时现场编译双 SPV
（rurixc --target vulkan + spirv-val）并构建两 device 探针；前置缺任一 →
facts 诚实红落盘。
"""
...
ROOT = wel.ROOT
GATE_KEY = "g27.wave.5a.soak"
NUMERIC_STEP = 475
SUBJECT = "g27_stabilization_soak"
WAVE = "G27.5"
SCHEMA_PATH = ROOT / "milestones/g27/g27_stabilization_soak_evidence_schema.json"
...
def prepare_device_lanes() -> tuple[bool, str, float | None]:
    """soak 启动时现场编译双 SPV + 构建两 device 探针（五/六车道前置）；返回 (ok, detail, tol)。"""
    tol: float | None = None
    ...
    return True, f"双 SPV+两 device 探针就绪 tol={tol}", tol


def run_gate() -> int:
    facts: list[dict] = []
    md = wel.load_latest_evidence("g27_m_d_hlod_l4_counter_rejudgment")
    ...
    if not md_ok or not BIN.is_file() or not lane_ok:
        ...
        while time.perf_counter() - t0 < MIN_SECONDS:
            ...
            if iters % 5 == 4:
                lane = probe_iters % 6
                out_dir = ROOT / ".tmp" / "g27_soak_probes"
        ...
        facts.extend([
            ...
            fact("probe_lane_interleaved", probe_iters > 0,
                 f"probe_iters={probe_iters}（战役四实现件 + framegen/hzb 两 device 六车道探针轮换穿插复跑）"),
        ])
        ok = all(f["status"] == "PASS" for f in facts)
    code, _ = wel.emit_wave_evidence(
        wave=WAVE, subject=SUBJECT, symbolic_gate_key=GATE_KEY, numeric_step=NUMERIC_STEP,
        source_ref="G27_CONTRACT G-G27-6", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G27 soak ≥1800s（管线四组合 + 六车道探针轮换扩容穿插，含 framegen/hzb 两 device 车道穿插（六车道轮换））",
