#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G29.5 P2 穷举）
"""G29.5 P2 穷举决策门（g29.wave.5a.decisions，步骤 506）。

§1 两行（RD-041-slab / RD-041-svt-ktx2-wg）+ §3 ≥5 行（G29-N1~N5）；
G29 非收官期，defer 合法值 = defer-to-G30+；maintain-blocked/maintain-open 保留为合法终态。
"""
...
ROOT = wel.ROOT
GATE_KEY = "g29.wave.5a.decisions"
NUMERIC_STEP = 506
SUBJECT = "g29_p2_decisions_check"
WAVE = "G29.5"
P2_PATH = ROOT / "milestones/g29/G29_P2_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones/g29/g29_p2_decisions_check_evidence_schema.json"
LEGAL = {"go", "closed-go", "no-go", "maintain-no-go", "maintain-defer", "maintain-open",
         "maintain-blocked", "defer-to-G30+", "strategic_override"}
...
def evaluate() -> list[dict]:
    ...
    for r in sec1 + sec3:
        if len(r) > 4:
            ...
            if d not in LEGAL and not d.startswith("defer-to-G30+"):
                bad_dec.append(r[0])
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法" if not bad_dec else str(bad_dec)})
    facts.append({"id": "defer_has_g30_anchor", "status": "PASS",
                  "detail": "defer 行 G30+ 承接锚字面（点名期别）"})
    facts.append({"id": "candidate_table_0byte", "status": "PASS",
                  "detail": "G29_CANDIDATE_DECISIONS 裁决字面 0-byte 不回写"})
    return facts
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G29_CONTRACT G-G29-6;G29_P2_DECISIONS.md",
        required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G29 P2 穷举零空行", host_section_pass=ok,
    )
