#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G9.5 M113 水体双管线门冒烟(g9.p1.m113.water_dual_pipeline;RFC-0025 §4.D;
spec/world_partition.md RXS-0366;G9_ACCEPTANCE_MAP §3 M113;G9_CONTRACT §8.1
裁决① P1 全进)。
...
实记 device_name=host-only/validation=not_applicable)。三段判据:
  ...
  host 段:rurix-render world::water 7 单测逐名锚定(谱资产往返+非法参数
    RED/IFFT vs 定义式 DFT 对拍/Jacobian 泡沫映射/双管线几何互斥+共享 RED/
    浅水越界写 RED/浮力预留不实现/CDLOD+blend 闭集)+ conformance
    world_partition M113 双件语料锚定(//@ spec: RXS-0366 + 门 key/脚本名
    留痕)+ 冻结带 milestones/g9/g9_m113_water_band.json provenance 机器核验。
  harness 段:持锁(gpu_device_lock)真跑 g9_m113_water --evidence(直出件
    落 .tmp 工作区不覆盖 evidence/ harness 直出件;schema/spec_anchor/
    assertion_id/status==pass + 12 判据闭集全真)+ --red-arm
    invalid-spectrum/geometry-shared 子模式独立复跑抽检。

用法:
  py -3 ci/g9_water_dual_pipeline_smoke.py --gate g9.p1.m113.water_dual_pipeline
  py -3 ci/g9_water_dual_pipeline_smoke.py --selftest
"""
...
SCHEMA_PATH = ROOT / "milestones" / "g9" / "g9_m113_water_dual_pipeline_evidence_schema.json"
BAND_PATH = ROOT / "milestones" / "g9" / "g9_m113_water_band.json"
CORPUS_DIR = ROOT / "conformance" / "world_partition"
WORK_DIR = ROOT / ".tmp" / "g95_gates" / "m113"
HARNESS_EVIDENCE = WORK_DIR / "harness_evidence.json"

sys.path.insert(0, str(ROOT / "ci"))
from gpu_device_lock import gpu_device_lock  # noqa: E402

GATE_KEY = "g9.p1.m113.water_dual_pipeline"
NUMERIC_STEP = 158
SOURCE_REF = "RFC-0025 §4.D;spec/world_partition.md RXS-0366;G9_ACCEPTANCE_MAP §3 M113"
TAG = "g9_m113"
SUBJECT = "g9_m113_water_dual_pipeline"
MATRIX_ROW = "M113"

MODULE_TESTS = {
    "world::water": [
        "spectrum_roundtrip_and_invalid_param_red",
        "ifft_vs_reference_dft_close",
        "jacobian_foam_mapping",
        "dual_pipeline_geometry_disjoint_and_shared_red",
        "shallow_out_of_bounds_write_red",
        "buoyancy_reserved_not_implemented",
        "cdlod_and_blend_closed",
    ],
}
CORPUS_FILES = [
...
BAND_SCHEMA = "rurix.g9m113.water_band.v1"
BAND_SPEC_ANCHOR = "RXS-0366"
BAND_DIGEST_FIELDS = ["ocean_digest", "shallow_digest"]
HARNESS_BIN = "g9_m113_water"
HARNESS_SCHEMA = "rurix.g9m113.water.v1"
HARNESS_ASSERTION = "g9.p1.m113.water_dual_pipeline"
HARNESS_TAG = "G9_M113_WATER"
HARNESS_CHECKS = [
    "conformance_corpus_anchored",
    "spectrum_asset_assembly",
    "ocean_ifft_three_maps",
    "ifft_vs_host_dft_measured",
    "jacobian_negative_drives_foam",
    "cdlod_tiling_blend_closed",
    "shallow_ping_pong",
    "dual_pipeline_geometry_disjoint",
    "shared_shading_closure_input_only",
    "buoyancy_reserved_not_implemented",
    "double_run_bit_equal",
    "golden_frozen_equal",
]
RED_ARMS = ["invalid-spectrum", "geometry-shared"]
...
def main() -> int:
    ...
    with gpu_device_lock(purpose="g9_m113 water harness 腿"):
        exe = build_harness()
        if exe:
            ...
            note("harness:大洋 IFFT 三贴图 vs host DFT 对拍 + 双管线几何互斥 + 浮力预留 + RED 双臂子模式复跑")
