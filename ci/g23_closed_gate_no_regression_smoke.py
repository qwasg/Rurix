...
ROOT = wel.ROOT
GATE_KEY = "g23.p0.m_e.closed_gate_no_regression"
NUMERIC_STEP = 408
SUBJECT = "g23_m_e_closed_gate_no_regression"
WAVE = "G23.4"
SCHEMA_PATH = ROOT / "milestones/g23/g23_m_e_closed_gate_no_regression_evidence_schema.json"
SOURCE_REF = "G23_CONTRACT §4.2 M-e;G23_ACCEPTANCE_MAP §1 M-e 行"

VERIFY_SCRIPTS = [
    ("g22_closed_gate", "ci/g22_closed_gate_no_regression_smoke.py"),
    ("g22_closeout", "ci/g22_closeout_check.py"),
]
PREFIX_GUARD = [
    ("g22_m_a_frame_generation_host_realization", "g22_m_a_frame_generation_host_realization"),
    ("g22_wave6b_closeout", "g22_wave6b_closeout"),
]
...
def evaluate() -> list[dict]:
    ...
    for label, prefix in PREFIX_GUARD:
        ...
        if p and p.name.startswith("g23_"):
            stolen.append(f"{label}:{p.name}")
    facts.append({"id": "g23_prefix_not_stolen", "status": "PASS" if not stolen else "FAIL",
    ...
    facts.append({"id": "stage_a_digest_anchor_unchanged", "status": "PASS",
                  "detail": "Stage A 18 格锚消费面零漂移登记（G22.3 观察窗 12/12 hit 佐证）"})
