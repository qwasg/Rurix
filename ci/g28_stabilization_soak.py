#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G28.5 soak 波）
"""G28.5 稳定 soak（g28.wave.5a.soak，步骤 491）。

管线 bench 四组合轮转 + 探针轮换扩容至七车道（G28-N5）：每第 5 迭代按
probe_iters % 7 轮转七车道——g19/g20/g21/g22 战役四实现件探针 + 第五车道
...
四 SPV（rurixc --target vulkan + spirv-val）并构建三 device 探针；前置缺任一
→ facts 诚实红落盘。
"""
...
ROOT = wel.ROOT
GATE_KEY = "g28.wave.5a.soak"
NUMERIC_STEP = 491
SUBJECT = "g28_stabilization_soak"
WAVE = "G28.5"
SCHEMA_PATH = ROOT / "milestones/g28/g28_stabilization_soak_evidence_schema.json"
...
def prepare_device_lanes() -> tuple[bool, str, float | None, float | None]:
    """soak 启动时现场编译四 SPV + 构建三 device 探针（五/六/七车道前置）；返回 (ok, detail, framegen_tol, restir_tol)。"""
    ...
    if framegen_tol is None:
        return False, f"budget 缺 {FRAMEGEN_TOL_ID}", None, None
    ...
    steps: list[list[str]] = [
        ...
        ["cargo", "build", "-p", "rurix-render", "--features", "vulkan",
         "--bin", "g26_framegen_device", "--bin", "g27_hzb_device", "--bin", "g28_restir_device"],
    ]
    for cmd in steps:
        try:
            r = subprocess.run(cmd, cwd=ROOT, capture_output=True, text=True)
        except OSError as e:
            return False, f"{cmd[0]} 不可用: {e}", framegen_tol, restir_tol
        if r.returncode != 0:
            return False, f"{Path(cmd[0]).name} rc={r.returncode}", framegen_tol, restir_tol
    restir_desc = str(restir_tol) if restir_tol is not None else "零容差（budget 无条目，省略 --tol）"
    return True, f"四 SPV+三 device 探针就绪 framegen_tol={framegen_tol} restir_tol={restir_desc}", framegen_tol, restir_tol


def run_gate() -> int:
    facts: list[dict] = []
    md = wel.load_latest_evidence("g28_m_d_rd034_upstream_recheck")
    ...
    lane_ok, lane_detail, framegen_tol, restir_tol = prepare_device_lanes()
    if not md_ok or not BIN.is_file() or not lane_ok:
        why = ("M-d 未绿" if not md_ok
               else "缺 release bin" if not BIN.is_file()
               else f"framegen/hzb/restir device 车道前置未就绪：{lane_detail}")
        facts.extend([
        ...
        while time.perf_counter() - t0 < MIN_SECONDS:
            ...
            if iters % 5 == 4:
                lane = probe_iters % 7
                out_dir = ROOT / ".tmp" / "g28_soak_probes"
        ...
        facts.extend([
            ...
            fact("probe_lane_interleaved", probe_iters > 0,
                 f"probe_iters={probe_iters}（战役四实现件 + framegen/hzb/restir 三 device 七车道探针轮换穿插复跑）"),
        ])
        ok = all(f["status"] == "PASS" for f in facts)
    code, _ = wel.emit_wave_evidence(
        wave=WAVE, subject=SUBJECT, symbolic_gate_key=GATE_KEY, numeric_step=NUMERIC_STEP,
        source_ref="G28_CONTRACT G-G28-6", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G28 soak ≥1800s（管线四组合 + 七车道探针轮换扩容穿插，含 framegen/hzb/restir 三 device 车道穿插（七车道轮换））",
