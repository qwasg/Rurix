#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G25 波聚合门）
"""G25 波聚合门（参数化 g25.wave.{2..6}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g25/g25_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g25.wave.2.exit": ("G25.2", "g25_m_a_quality_final_state_verification", 433),
    "g25.wave.3.exit": ("G25.3", "g25_m_b_fps_parity_final_verdict", 435),
    "g25.wave.4.exit": ("G25.4", "g25_m_c_campaign_full_chain_no_regression", 437),
    "g25.wave.5.exit": ("G25.5", "g25_m_d_campaign_handover_ledger", 439),
    "g25.wave.6.exit": ("G25.6", "g25_m_e_closed_gate_no_regression", 441),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g25_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G25_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g25_wave{n}_exit",
        notes=f"G25 wave {n} exit", host_section_pass=overall,
    )
