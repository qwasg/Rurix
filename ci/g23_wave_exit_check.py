#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G23 波聚合门）
"""G23 波聚合门（参数化 g23.wave.{2..6}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g23/g23_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g23.wave.2.exit": ("G23.2", "g23_m_a_jolt_56_adoption_rejudgment", 401),
    "g23.wave.3.exit": ("G23.3", "g23_m_b_neural_deform_rejudgment", 403),
    "g23.wave.4.exit": ("G23.4", "g23_m_c_research_track_disposition", 405),
    "g23.wave.5.exit": ("G23.5", "g23_m_d_physics_p3_subitem_disposition", 407),
    "g23.wave.6.exit": ("G23.6", "g23_m_e_closed_gate_no_regression", 409),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g23_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G23_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g23_wave{n}_exit",
        notes=f"G23 wave {n} exit", host_section_pass=overall,
    )
