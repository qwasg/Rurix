...
"""G29.1 治理门 — 候选决策表闭集/锚纪律/横向对账（g29.wave.1.candidate_decisions，步骤 494）。

核验 `milestones/g29/G29_CANDIDATE_DECISIONS.md`：
冻结 7 行候选闭集全等（§1 G28 defer-to-G30+ 承接 2 行 + §3 G29 新增候选 5 行 G29-N1~N5）、
裁决枚举合法（go/closed-go/no-go/defer-to-G30+/strategic_override——G29 即本期，defer-to-G30+
不再合法）、零空行（全列非空）、承接锚纪律（§1/§3 行承接锚均含「重判条件 = …；兜底 = …」字面；
§1 行原触发条件字面转引含「→」分节）、
defer-to-G30+ 裁决行承接锚含 G2x 期别重评窗字面、
go 行验收映射锚义务（登记留痕位置含 G29_ACCEPTANCE_MAP，或依据面含 G29_CONTRACT §4.2 M 行锚定）、
...
用法：
  ...
  py -3 ci/g29_candidate_decisions_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g29.wave.1.candidate_decisions"
NUMERIC_STEP = 494  # 落盘前实测 registry/number_ledger.json CI_step.next_free=493 顺位领取
SUBJECT = "g29_candidate_decisions_check"
WAVE = "G29.1"
DECISIONS = ROOT / "milestones" / "g29" / "G29_CANDIDATE_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones" / "g29" / "g29_candidate_decisions_check_evidence_schema.json"
SOURCE_REF = (
    "G29_CONTRACT G-G29-1/§6/§7;G29_CANDIDATE_DECISIONS.md v1.0;G29_ACCEPTANCE_MAP §1/§2;"
    "G28_P2_DECISIONS.md §1（承接锚法定输入）+ G28_CONTRACT.md §8.7;"
...
SEC3_IDS = ["G29-N1", "G29-N2", "G29-N3", "G29-N4", "G29-N5"]
FROZEN_IDS = SEC1_IDS + SEC3_IDS
MAX_RD = 45
ALLOWED = frozenset({"go", "closed-go", "no-go", "defer-to-G30+", "strategic_override"})
EMPTY_CELLS = {"", "TBD", "TODO", "待定", "待补", "—", "-"}
SECTION_HEAD_RE = re.compile(r"^## (\d+)\. ")
DEFER_WINDOW_RE = re.compile(r"G2[0-9]")
...
def evaluate(text: str | None, deferred_doc: dict | None) -> list[dict]:
    ...
    facts: list[dict] = []
    if text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G29_CANDIDATE_DECISIONS.md 缺失（诚实红）"}]
    ...
    facts.append({"id": "sec1_closed_set", "status": "PASS" if ok1 else "FAIL",
                  "detail": f"§1 {len(got1)}/2 行" + ("" if ok1 else f"；diff={sorted(set(SEC1_IDS) ^ set(got1))}")})
    ...
    facts.append({"id": "zero_empty_rows", "status": "PASS" if not empty_hits else "FAIL",
                  "detail": "7 行全列非空" if not empty_hits else f"空/占位行：{empty_hits}"})
    ...
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法（defer-to-G30+ 不再合法）" if not bad_dec else str(bad_dec)})
    ...
    for r in sec1 + sec3:
        if len(r) > 8 and _decision_of(r[4]) in ("go", "closed-go"):
            blob = r[6] + r[8]
            if "G29_ACCEPTANCE_MAP" not in blob and "G29_CONTRACT §4.2" not in blob:
                go_bad.append(r[0])
    ...
    facts.append({"id": "sec2_rd_closed_set", "status": "PASS" if ok2 else "FAIL",
                  "detail": f"§2 {len(got2)}/2 行" + ("" if ok2 else f"；diff={sorted(set(SEC2_IDS) ^ set(got2))}")})
...
def emit(facts: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g29_candidate_decisions] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G29.1 治理门——候选决策表 7 行闭集（§1 两行 + §3 五行）+ 裁决枚举合法（defer 合法值 = defer-to-G30+ 点名期别）+ 零空行 + 承接锚纪律 + defer 行 G2x 窗 + go 行验收映射锚 + §2 RD 八条 status open 机核 + 零新 RD；只读文档与 registry，不代绿实现门",
...
def _fixture_text() -> str:
    def row10(rid: str, decision: str, trigger: str = "条件 → 兜底面") -> str:
        anchor = "重判条件 = G29 窗；兜底 = 维持" if decision.startswith("defer") else "重判条件 = 只追加重判；兜底 = 维持"
        trace = "本表行" if decision.startswith("defer") else "G29_ACCEPTANCE_MAP M-a"
        return f"| {rid} | 名 | 来源 | {trigger} | {decision} | 理由 | 依据 | {anchor} | {trace} | 终态 |"

    lines = ["# fixture", "", "## 1. 承接 2 行", "",
             "| ID | 分项名 | 来源波次 | 原触发条件字面 | 裁决 | 裁决理由 | 依据/证据路径 | 承接锚 | 登记留痕位置 | 最终状态 |",
             "|---|---|---|---|---|---|---|---|---|---|"]
    for rid in SEC1_IDS:
        lines.append(row10(rid, "go" if rid in ("RD-041-slab", "RD-041-svt-ktx2-wg") else "defer-to-G30+"))
    lines += ["", "## 2. open RD 八条", "",
...
def run_selftest() -> int:
    ...
    cases = [
        ("删除 §1 RD-041-slab 行 → 闭集红", "\n".join(l for l in text.splitlines() if not l.startswith("| RD-041-slab ")),
         deferred, "sec1_closed_set"),
        ("裁决改 defer-to-G29+ → 枚举红", text.replace("| go |", "| defer-to-G29+ |", 1),
         deferred, "decision_enum_legal"),
        ("承接锚去掉兜底 → 锚纪律红", text.replace("重判条件 = 只追加重判；兜底 = 维持", "重判条件 = 只追加重判", 1),
         deferred, "anchor_discipline"),
        ("空单元格注入 → 零空行红", text.replace("| RD-041-svt-ktx2-wg | 名 |", "| RD-041-svt-ktx2-wg |  |", 1),
        ...
        ("go 行去映射锚 → go 锚红", text.replace("| G29_ACCEPTANCE_MAP M-a |", "| 本表行 |", 1),
    ...
    if failures:
        print(f"[g29_candidate_decisions] SELFTEST FAIL ({failures})")
        return 1
    print("[g29_candidate_decisions] SELFTEST PASS (8 RED + 1 GREEN + 真表臂)")
    return 0
