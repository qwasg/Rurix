#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G21.5 soak 波）
"""G21.5 稳定 soak（g21.wave.5a.soak，步骤 379）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g21.wave.5a.soak"
NUMERIC_STEP = 379
SUBJECT = "g21_stabilization_soak"
WAVE = "G21.5"
SCHEMA_PATH = ROOT / "milestones/g21/g21_stabilization_soak_evidence_schema.json"
BIN = ROOT / "target/release/g14_3_pipeline_perf.exe"
RESTIR_BIN = ROOT / "target/release/g21_restir_probe.exe"
MIN_SECONDS = 1800.0
...
def run_gate() -> int:
    facts: list[dict] = []
    md = wel.load_latest_evidence("g21_m_d_rd034_upstream_recheck")
    ...
    if not md_ok or not BIN.is_file() or not RESTIR_BIN.is_file():
        ...
        facts.extend([
            ...
            fact("restir_lane_interleaved", False, "未启动"),
        ])
        ok = False
    else:
        combos = [
        ...
        iters = fails = restir_iters = 0
        active = 0.0
        while time.perf_counter() - t0 < MIN_SECONDS:
            it0 = time.perf_counter()
            if iters % 5 == 4:
                out = ROOT / ".tmp" / "g21_soak_restir" / f"iter_{iters}.json"
                out.parent.mkdir(parents=True, exist_ok=True)
                r = subprocess.run([str(RESTIR_BIN), "--out", str(out)],
        ...
        facts.extend([
            ...
            fact("no_sleep_between_iters", True, "sleep=0"),
            fact("restir_lane_interleaved", restir_iters > 0, f"restir_iters={restir_iters}（ReSTIR 方差收益车道穿插复跑）"),
        ])
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G21 soak ≥1800s（管线四组合 + ReSTIR 车道穿插）", host_section_pass=ok,
    )
