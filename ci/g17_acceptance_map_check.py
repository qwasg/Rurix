...
"""G17.1 治理门 — 验收映射覆盖 / 空行 / 双向命名空间一致性（g17.wave.1.acceptance_map，步骤 293）。

G17_CONTRACT G-G17-1（G17.1 完成门 D-G17-3 验收映射面）治理三门之一；G-G17-2 实现互锁
（ci/g17_interlock_check.py）事实门④消费本门与候选决策门的独立 PASS 机器事实。

核验 `milestones/g17/G17_ACCEPTANCE_MAP.md`：
§1 五行 P0（{M-a,M-b,M-c,M-d,M-e} 闭集全等）+ §2 零 go P1 空集断言 +
全部 symbolic key 匹配 `g17.p0.m_<a~e>.<slug>` 单一命名空间（key 的 m 段字母与行号一致、
脚本命令 `ci/g17_*_smoke.py` 每行单一且 --gate 参数 == canonical key、evidence schema
`g17_m_<a~e>_<key slug>_evidence_schema.json` 与 key 末段同 slug）+
波次 ∈ {G17.2,G17.3,G17.4,G17.5,G17.6} +
numeric_step 全列 `post-interlock actual-next-free allocation` 字面零预占 +
零空行/占位 + **双向一致**：MAP §1 与 G17_CONTRACT.md §4.2 对同一 P0 M 行给出的
判据与波次逐字相等（门冻结面 = 契约 §4.2 + MAP §1/§2，CI_GATES.md 为汇总视图不入比对，
沿 G15/G16 体例双向；G17 契约 §4.2 表 = M 行/判据（逐字）/波次 三列形态）。
...
用法：
  ...
  py -3 ci/g17_acceptance_map_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g17.wave.1.acceptance_map"
NUMERIC_STEP = 293  # 落盘前实测 registry/number_ledger.json CI_step.next_free=293 顺位领取
SUBJECT = "g17_acceptance_map_check"
WAVE = "G17.1"
MAP_PATH = ROOT / "milestones" / "g17" / "G17_ACCEPTANCE_MAP.md"
CONTRACT_PATH = ROOT / "milestones" / "g17" / "G17_CONTRACT.md"
SCHEMA_PATH = ROOT / "milestones" / "g17" / "g17_acceptance_map_check_evidence_schema.json"
SOURCE_REF = (
    "G17_CONTRACT G-G17-1/§4.2;G17_ACCEPTANCE_MAP.md §1/§2/§4/§5;"
    "G15_P2_DECISIONS.md §4 G15-MD-F1 行 + G16_CONTRACT.md §7 立项裁决 3 + "
    "G15_CONTRACT.md §8.5~§8.7（G17 法定输入）P0 清单 M-a~M-e"
)

# 冻结 5 行 P0 集合（G17.1 立项裁决口径；G17_CONTRACT §2/§4.2 M-a~M-e）。
EXPECTED_P0 = {"M-a", "M-b", "M-c", "M-d", "M-e"}
# MAP §5：所有波次属于 G17.2|G17.3|G17.4|G17.5|G17.6（§4.2 五行 P0 波次闭集）。
ALLOWED_WAVES = {"G17.2", "G17.3", "G17.4", "G17.5", "G17.6"}
...
KEY_RE = re.compile(r"^g17\.p0\.m_[a-e]\.[a-z0-9_]+$")
KEY_IN_CELL_RE = re.compile(r"`(g17\.p0\.m_[a-e]\.[a-z0-9_]+)`")
SECTION_RE = re.compile(r"^## (\d+)\. ")
SCRIPT_RE = re.compile(r"ci/g17_[a-z0-9_]+_smoke\.py")
SCHEMA_RE = re.compile(r"`(milestones/g17/g17_(m_[a-e])_[a-z0-9_]+_evidence_schema\.json)`")
...
def check_row(m: str, row: dict, seen_keys: dict[str, str], seen_schemas: dict[str, str]) -> list[str]:
    ...
    findings: list[str] = []
    m_seg = m.lower().replace("-", "_")  # M 行号 → key/schema m 段（m_a~m_e）
    if len(row["keys"]) != 1:
        findings.append(f"{m} 必须恰有一个 canonical symbolic key，实测 {row['keys']}")
        return findings
    key = row["keys"][0]
    if not KEY_RE.match(key):
        findings.append(f"{m} key `{key}` 不匹配 g17.p0.m_<a~e>.<slug>")
    ...
    slug = key.split(".")[3]
    if not row["scripts"]:
        findings.append(f"{m} 缺 `ci/g17_*_smoke.py` 脚本命令")
    ...
    if not schema_m:
        findings.append(f"{m} schema 路径不符 g17_m_<a~e>_<slug>_evidence_schema.json：{row['schema']!r}")
    else:
        path, schema_m_no = schema_m.group(1), schema_m.group(2)
        if schema_m_no != m_seg:
            findings.append(f"{m} schema 路径的 m 段不符：{path}")
        expected_schema = f"milestones/g17/g17_{m_seg}_{slug}_evidence_schema.json"
    ...
    if row["wave"] not in ALLOWED_WAVES:
        findings.append(f"{m} 波次 {row['wave']!r} 不在允许集合 {{G17.2,G17.3,G17.4,G17.5,G17.6}} 内")
...
def evaluate(map_text: str | None, contract_text: str | None) -> list[dict]:
    ...
    results: list[dict] = []
    if map_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G17_ACCEPTANCE_MAP.md 缺失（诚实红，不假绿）"}]
    if contract_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G17_CONTRACT.md 缺失（诚实红，不假绿）"}]
    ...
    results.append({
        ...
        "detail": "§2 零 go P1 空集（G17.1 字面）" if p1_ok else f"§2 出现 P1 行 {sorted(p1_rows)}（G17.1 零 go P1 字面违例）",
    })
    ...
    for m in sorted(EXPECTED_P0):
        ...
        if len(row["keys"]) == 1:
            ...
            if other is None:
                two.append("G17_CONTRACT §4.2 缺行")
            else:
...
def emit(results: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g17_acceptance_map] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G17.1 治理门——验收映射覆盖/空行/双向命名空间一致性：5 P0（M-a~M-e）闭集全等 + §2 零 go P1 空集 + key/脚本/schema 单一命名空间同 slug + numeric_step 全列 post-interlock 字面零预占 + MAP §1 ↔ CONTRACT §4.2 判据/波次双向逐字一致（CI_GATES.md 为汇总视图不入比对）；本门 PASS 只表示映射完整，不表示任何 P0 能力已实现",
...
CANONICAL_ROWS = [
    ("M-a", "dual_end_retest_warm_recalib", "ci/g17_dual_end_retest_warm_recalib_smoke.py", "G17.2"),
    ("M-b", "ngx_evolution_alignment", "ci/g17_ngx_evolution_alignment_smoke.py", "G17.3"),
    ("M-c", "d3d12_host_lane_disposition", "ci/g17_d3d12_host_lane_disposition_smoke.py", "G17.4"),
    ("M-d", "t100_final_verdict", "ci/g17_t100_final_verdict_smoke.py", "G17.5"),
    ("M-e", "closed_gate_no_regression", "ci/g17_closed_gate_no_regression_smoke.py", "G17.6"),
]


def _fixture() -> tuple[str, str]:
    map_lines = ["# fixture G17_ACCEPTANCE_MAP", "", "## 1. P0 硬门（精确 5 行）", ""]
    contract_lines = ["# fixture G17_CONTRACT", "", "### 4.2 五行 P0", ""]
    for m, slug, script, wave in CANONICAL_ROWS:
        key = f"g17.p0.{m.lower().replace('-', '_')}.{slug}"
        schema = f"milestones/g17/g17_{m.lower().replace('-', '_')}_{slug}_evidence_schema.json"
        cmd = f"`py -3 {script} --gate {key}`"
        map_lines.append(
            f"| **{m}** | `{key}` | {cmd} | `{schema}` | 合成判据 {m} | **{wave}** | {NUMERIC_STEP_LITERAL} |"
        )
        contract_lines.append(f"| **{m}** | 合成判据 {m} | {wave} |")
    map_lines += ["", "## 2. 已 go P1 硬门（零行）", "", "G17.1 无 go 的 P1 行。", ""]
    return "\n".join(map_lines), "\n".join(contract_lines)
...
def run_selftest() -> int:
    ...
    cases: list[tuple[str, str, str, str]] = [
        ...
        (
            ...
            map_text.replace("--gate g17.p0.m_c.d3d12_host_lane_disposition`", "--gate g17.p0.m_c.d3d12_host_lane`"),
            contract_text,
            "row_M-c",
        ),
        (
            ...
            map_text.replace(
                "**G17.2** | post-interlock actual-next-free allocation |",
                "**G17.2** | 296 |",
                1,
            ),
            contract_text,
        ...
        (
            "波次改写为非法 G17.7c → 必须红",
            map_text.replace("合成判据 M-a | **G17.2**", "合成判据 M-a | **G17.7c**", 1),
            contract_text,
            "row_M-a",
        ),
        (
            ...
            map_text.replace(
                "G17.1 无 go 的 P1 行。",
                "| **M-a** | `g17.p0.m_a.dual_end_retest_warm_recalib` | `py -3 ci/g17_dual_end_retest_warm_recalib_smoke.py --gate g17.p0.m_a.dual_end_retest_warm_recalib` | `milestones/g17/g17_m_a_dual_end_retest_warm_recalib_evidence_schema.json` | 判据 | **G17.2** | post-interlock actual-next-free allocation |",
            ),
            contract_text,
            "coverage_p1_empty",
        ),
        ...
        (
            ...
            map_text.replace(
                ...
                "milestones/g17/g17_m_d_d3d12_host_lane_disposition_evidence_schema.json",
                1,
            ),
            contract_text,
            "row_M-c",
    ...
    if failures:
        print(f"[g17_acceptance_map] SELFTEST FAIL ({failures})")
        return 1
    print("[g17_acceptance_map] SELFTEST PASS (9 RED + 1 GREEN + 真表臂)")
    return 0
