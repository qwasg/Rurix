#!/usr/bin/env python3
"""G9.1 measured baseline 守卫（milestones/g9/g9_budget.json 治理闸门）。

判据（G9.1 governance-only 立项裁决：非空 measured_local 基线，零 estimated/unlocked）：

1. `milestones/g9/g9_budget.json` 的 `namespace` 必须为 `g9`，`entries` 非空；
2. 每条 id 以 `g9.` 前缀，`evidence` 必须为 `measured_local`，零 `estimated`/`unlocked`；
...
6. 诚实边界：G9 evidence 的指标是 `results.metrics` 字典（区别于 G8 的 `results.metric`
   单值），每条 entry 的 id 必须包含其对应 metrics key 名（不得改述被测对象，
   例如把 host 墙钟 AS 构建耗时冒充 GPU 帧耗时）。
...
BUDGET = ROOT / "milestones/g9/g9_budget.json"
...
def check(budget: dict, load_evidence) -> list[str]:
    findings: list[str] = []
    if budget.get("namespace") != "g9":
        findings.append(f"namespace 必须为 g9，实测 {budget.get('namespace')!r}")
    entries = budget.get("entries") or []
    if not entries:
        findings.append("entries 为空 —— G9.1 governance-only 要求非空 measured_local 基线")
    for entry in entries:
        eid = entry.get("id", "<无 id>")
        if not str(eid).startswith("g9."):
            findings.append(f"{eid}: id 必须以 g9. 前缀")
        ...
        metrics = (ev.get("results") or {}).get("metrics") or {}
        if isinstance(metrics, dict) and metrics:
            matched = [k for k in metrics if k in str(eid)]
            if not matched:
                findings.append(
                    f"{eid}: evidence results.metrics 的 metric 名无一出现在 entry id 中"
                    "（诚实边界：不得改述被测对象）"
                )
    return findings
...
def run_selftest() -> int:
    ...
    overthreshold["entries"][1]["threshold"] = 0.01
    ...
    renamed["entries"][1]["id"] = "g9.bench.gpu_frame_p95"
    cases.append(("把 AS 构建耗时改述为 GPU 帧 → 必须红", renamed, "metric 名无一出现在 entry id 中"))
    ...
    if failures:
        print(f"[check_g9_budget_baseline] SELFTEST FAIL ({failures})")
        return 1
    print(f"[check_g9_budget_baseline] SELFTEST PASS ({len(cases)} RED + 1 GREEN)")
    return 0
...
def main() -> int:
    ...
    if not BUDGET.exists():
        print(f"[check_g9_budget_baseline] FAIL — 缺 {BUDGET.relative_to(ROOT)}")
        return 1
    budget = json.loads(BUDGET.read_text(encoding="utf-8"))
    findings = check(budget, _loader(ROOT))
    if findings:
        print(f"[check_g9_budget_baseline] FAIL ({len(findings)} 项)")
        for f in findings:
            print(f"  - {f}")
        return 1
    n = len(budget.get("entries") or [])
    print(f"[check_g9_budget_baseline] PASS（{n} 条 measured_local entry，零 estimated/skip，evidence 可追溯）")
    print("  注意：baseline 只证明测量已建立，不证明任何 G9 实现达标或任何能力门已绿。")
    return 0
