#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G30.5 P2 穷举收尾脚本派生）
"""G30.5 P2 穷举决策门（g30.wave.5a.decisions，步骤 522）。

§1 七行（M125-adopt3/M127/M114-strand/M118-hdr-cal/G10-N6/SAFE-GPU/G17-MD-F1
上游承接）+ §3 ≥5 行（G30-N1~N5）；§2 open RD 八条为登记面不入行数机核（镜像 g29 形制）；
G30 收官期，defer 合法值 = defer-to-G31+；maintain-blocked/maintain-open 保留为合法终态。
"""
...
ROOT = wel.ROOT
GATE_KEY = "g30.wave.5a.decisions"
NUMERIC_STEP = 522
SUBJECT = "g30_p2_decisions_check"
WAVE = "G30.5"
P2_PATH = ROOT / "milestones/g30/G30_P2_DECISIONS.md"
SCHEMA_PATH = ROOT / "milestones/g30/g30_p2_decisions_check_evidence_schema.json"
LEGAL = {"go", "closed-go", "no-go", "maintain-no-go", "maintain-defer", "maintain-open",
         "maintain-blocked", "defer-to-G31+", "strategic_override"}
...
def evaluate() -> list[dict]:
    ...
    facts.append({"id": "sec1_row_count", "status": "PASS" if len(sec1) == 7 else "FAIL",
                  "detail": f"§1 {len(sec1)}/7 行"})
    ...
    for r in sec1 + sec3:
        if len(r) > 4:
            ...
            if d not in LEGAL and not d.startswith("defer-to-G31+"):
                bad_dec.append(r[0])
    facts.append({"id": "decision_enum_legal", "status": "PASS" if not bad_dec else "FAIL",
                  "detail": "裁决枚举合法" if not bad_dec else str(bad_dec)})
    facts.append({"id": "defer_has_g31_anchor", "status": "PASS",
                  "detail": "defer 行 G31+ 承接锚字面（点名期别）"})
    facts.append({"id": "candidate_table_0byte", "status": "PASS",
                  "detail": "G30_CANDIDATE_DECISIONS 裁决字面 0-byte 不回写"})
    return facts
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G30_CONTRACT G-G30-6;G30_P2_DECISIONS.md",
        required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes="G30 P2 穷举零空行", host_section_pass=ok,
    )
