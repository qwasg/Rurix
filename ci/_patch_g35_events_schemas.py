...
# G35 GPU 粒子系统 G35-6：check_schemas.py 三处纯追加注册（io.open 补丁法——
# newline="" 字节面保全，既有路由 0-byte；打补丁后立即重读验证驻留）。
# 本脚本幂等：已驻留即只做核验（ci/_patch_g35_particle_core_schemas.py 同法）。
# 锚 = G35-1 基元库块（本波前最近追加的 g35 家族块；并行波若在同锚后追加，
# replace(ANCHOR, ANCHOR + ADD, 1) 仍在锚后插入互不覆盖，追加序无路由语义）。
import io
import sys
...
LOAD_ANCHOR = '''    # G35 GPU 粒子系统 G35-1 基元库门前缀纯追加（重放幂等面；仅门裁决件
...
'''
LOAD_ADD = '''    # G35 GPU 粒子系统 G35-6 事件/数据通道门前缀纯追加（重放幂等面；仅门
...
'''
VALIDATOR_ADD = '''    g35_events_gate_validator = (
...
PROBE = "g35_events_gate_schema = load"
...
def main() -> int:
    ...
    if PROBE in s:
        ...
        print(f"[patch_g35_events] 已驻留，核验 {'PASS' if ok else 'FAIL'}")
    ...
    if missing:
        print(f"[patch_g35_events] 锚缺失: {missing}", file=sys.stderr)
        return 1
    ...
    print(f"[patch_g35_events] 应用完成，重读核验 {'PASS' if ok else 'FAIL'}")
    return 0 if ok else 1
