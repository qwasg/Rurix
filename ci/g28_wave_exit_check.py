#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G28 波聚合门）
"""G28 波聚合门（参数化 g28.wave.{2..6}.exit）。"""
...
ROOT = wel.ROOT
SCHEMA_PATH = ROOT / "milestones/g28/g28_wave_exit_evidence_schema.json"
WAVES: dict[str, tuple[str, str, int]] = {
    "g28.wave.2.exit": ("G28.2", "g28_m_a_restir_device_kernel", 481),
    "g28.wave.3.exit": ("G28.3", "g28_m_b_restir_spatial_reuse_arm", 483),
    "g28.wave.4.exit": ("G28.4", "g28_m_c_m52_rd040_workload_rejudgment", 485),
    "g28.wave.5.exit": ("G28.5", "g28_m_d_rd034_upstream_recheck", 487),
    "g28.wave.6.exit": ("G28.6", "g28_m_e_closed_gate_no_regression", 489),
}
...
def run_gate(gate_key: str) -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        wave=wave, subject=f"g28_wave{n}_exit", symbolic_gate_key=gate_key, numeric_step=step,
        source_ref="G28_CONTRACT §2", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=f"g28_wave{n}_exit",
        notes=f"G28 wave {n} exit", host_section_pass=overall,
    )
