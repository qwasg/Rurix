...
"""G11.2 实现互锁守卫（milestones/g11/CI_GATES.md §3 `g11.gov.implementation_interlock`）。

读取事实源并逐项输出，最后给出 READY / BLOCKED / CLOSED。两类断言严格分开：

* **事实门**（当前允许为红，红即 BLOCKED，不是脚本失败）：
  ① G11_CONTRACT 在树且 front matter status == active；
  ② G11.1 治理交付四件齐备（G11_PLAN / G11_CANDIDATE_DECISIONS /
    G11_ACCEPTANCE_MAP / CI_GATES）；
  ③ g11_budget.json 非空且 budget_eval 可加载（namespace=g11、entries 非空、零 estimated、
    每条 entry 的 evidence_file 在树且可解出 results.trimmed_mean）；
  ④ RFC-0028 在树且 Agent Approved，且 §9.1 有 ≠ 起草 provenance 的独立评审记录（D-409）；
  ⑤ registry/number_ledger.json reserved_in_flight 存在 owner 以 "G11" 开头的登记行；
  ⑥ check_g11_acceptance_map 三向比对 PASS（MAP §1/§2 ↔ CONTRACT §4.2 ↔ CI_GATES §4/§4A）。
* **一致性门**（红即脚本 FAIL，退出非零）：
  C1 双状态诚实：implementation_status == blocked，或事实门全绿（禁止治理完成冒充实现开工）；
  C2 §8 记录一致：§8 出现 G-G11-3 解锁记录 ⇔ 事实门全绿且 implementation_status != blocked
    （事实未全绿时落解锁记录 / 已解锁但无记录均为 FAIL）；
  C3 数字步骤零预占：milestones/g11 全域无 numeric_step 数字赋值、workflow 无 g11.* key /
    ci/g11_ 脚本引用、ci/ 无 g11_*_smoke.py 预放；
  C4 src/spec/conformance 0-byte（治理期）：三面无 g11 实现面命中。

**C3/C4 两态口径（沿 G10.4b 先例，判据语义 0-byte）**：C3/C4 是**治理期口径**——
...
**C4 扫描面校准（G11.1 立项实测登记）**：裸字面「G11」在 spec/visual_comparison.md
RXS-0391 合法存续（g11_anchor 字段与「G11 立项重裁」字面，G10.5b 落——非 G11
实现面）；故 C4 不扫裸里程碑名，扫描面 = gate-key/脚本命名 token（`g11.p0.` /
`g11.p1.` / `g11.wave.` / `ci/g11_`）+ g11 命名文件——命中即 G11 实现面预放。

**closed 三态口径（沿 G10.8b 先例，判据语义 0-byte）**：
front matter status == closed（G11.7b close-out READY 后 status flip 的收口
终态）时，本守卫回答的问题「G11.2 可否开工」不再适用——互锁使命完结，
事实门/一致性门整体不适用（skipped_reason 登记），VERDICT=CLOSED、exit=0；
active/blocked 态恢复原机核逐字维持。CLOSED 是终态正确结论，不得被当作
G-G11-3 重新开放凭据（契约 reopen 须新立项治理程序）。

诚实纪律：BLOCKED 是当前正确结论，不得被当作 G-G11-3 PASS；`--require-ready`
供未来 G11.2 实现 PR 作前置 required check（未 READY 即退出非零）。
...
BUDGET = ROOT / "milestones/g11/g11_budget.json"
WORKFLOWS = ROOT / ".github/workflows"

G11_1_DELIVERABLES = [
    "milestones/g11/G11_PLAN.md",
    "milestones/g11/G11_CANDIDATE_DECISIONS.md",
    "milestones/g11/G11_ACCEPTANCE_MAP.md",
    "milestones/g11/CI_GATES.md",
]
G11_RFCS = [
    "rfcs/0028-g11-gi-quality-closure.md",
]

# 数字步骤零预占扫描面：numeric_step 数字赋值 / g11 数据行末列裸数字 / workflow g11 token。
NUMERIC_ASSIGN_RE = re.compile(r"numeric_step[\"']?\s*[:=]\s*[\"']?\d")
NUMERIC_TAIL_CELL_RE = re.compile(r"\|\s*\d{1,4}\s*\|\s*$")
WORKFLOW_G11_RE = re.compile(r"g11\.[pw]|ci/g11_")
# C4 扫描面（G11.1 校准）：裸「G11」里程碑名在 spec RXS-0391 合法存续，不扫；
# 扫描 gate-key/脚本命名 token + g11 命名文件。
IMPL_SURFACE_G11_RE = re.compile(r"g11\.p[01]\.|g11\.wave\.|ci/g11_")
...
class TreeInputs:
    ...
    workflow_g11_hits: list[str] = field(default_factory=list)
    g11_smoke_scripts: list[str] = field(default_factory=list)
...
def _scan_numeric_step(root: Path) -> list[str]:
    """milestones/g11 全域数字步骤零预占扫描；返回违例行清单。"""
    hits: list[str] = []
    base = root / "milestones/g11"
    if not base.is_dir():
        return [f"milestones/g11 目录缺失"]
    for path in sorted(base.rglob("*")):
        ...
        for lineno, line in enumerate(text.splitlines(), 1):
            ...
            elif line.lstrip().startswith(("| **M", "| `g11.")) and NUMERIC_TAIL_CELL_RE.search(line):
    ...
    for sub in ("src", "spec", "conformance"):
        ...
        for path in sorted(base.rglob("*")):
            if not path.is_file():
                continue
            rel = path.relative_to(root)
            if "g11" in path.name.lower():
                hits.append(f"{rel}（文件名含 g11）")
                continue
            ...
            try:
                ...
                continue
            for lineno, line in enumerate(text.splitlines(), 1):
                if IMPL_SURFACE_G11_RE.search(line):
...
def load_inputs(root: Path) -> TreeInputs:
    ...
    return TreeInputs(
        ...
        deliverables_missing=[p for p in G11_1_DELIVERABLES if not (root / p).exists()],
        budget_doc=budget_doc,
        budget_evidence_ok=_budget_evidence_ok(root, budget_doc),
        rfc_texts={rfc: _read(root / rfc) for rfc in G11_RFCS},
        ledger=_read_json(LEDGER),
        acceptance_findings=acceptance_findings,
        numeric_step_violations=_scan_numeric_step(root),
        workflow_g11_hits=_scan_workflows(root),
        g11_smoke_scripts=[p.name for p in sorted((root / "ci").glob("g11_*_smoke.py"))],
...
def rfc_review_state_text(text: str | None) -> tuple[bool, str]:
    """(是否 Agent Approved 且有独立 provenance 评审记录, 说明)。同构 G10 逻辑（D-409）。"""
    if text is None:
        return False, "文件不存在"
    status = ""
    drafter = ""
...
ACTIVATION_RE = re.compile(
    r"^#{2,5}\s*§?8\.\d+[^\n]*(implementation_status 解锁|G-G11-3)", re.MULTILINE
)


def evaluate_budget(doc: dict | None, evidence_ok: dict[str, bool]) -> tuple[bool, str]:
    """事实门③：g11_budget 非空且 budget_eval 可加载（零 estimated）。"""
    if doc is None:
        return False, "g11_budget.json 缺失或不可解析"
    problems: list[str] = []
    if doc.get("namespace") != "g11":
        problems.append(f"namespace={doc.get('namespace')!r}（要求 'g11'）")
    entries = doc.get("entries") or []
    if not entries:
        problems.append("entries 为空")
    for entry in entries:
        eid = entry.get("id", "?")
        if not str(eid).startswith("g11."):
            problems.append(f"{eid} 未带 g11. 前缀")
...
def evaluate_fact_gates(inp: TreeInputs) -> list[tuple[bool, str]]:
    ...
    status = front_matter_field(inp.g11_contract_text, "status") if inp.g11_contract_text else None
    facts.append((status == "active", f"① G11_CONTRACT status = {status!r}（要求 active）"))

    facts.append(
        (
            not inp.deliverables_missing,
            f"② G11.1 治理交付四件齐备（缺 {inp.deliverables_missing or '无'}）",
        )
    )

    budget_ok, budget_msg = evaluate_budget(inp.budget_doc, inp.budget_evidence_ok)
    facts.append((budget_ok, f"③ g11_budget 非空可加载零 estimated：{budget_msg}"))
    ...
    g11_claim = [r for r in reserved if str(r.get("owner", "")).startswith("G11")]
    facts.append(
        (
            inp.ledger is not None and bool(g11_claim),
            f"⑤ ledger reserved_in_flight 存在 owner 以 \"G11\" 开头的行（实测 {len(g11_claim)} 行）",
        )
    )

    if inp.acceptance_findings is None:
        facts.append((False, "⑥ check_g11_acceptance_map 三向比对：比对源缺失，无法判定"))
    else:
        facts.append(
            (
                not inp.acceptance_findings,
                f"⑥ check_g11_acceptance_map 三向比对 = {'PASS' if not inp.acceptance_findings else f'FAIL（{len(inp.acceptance_findings)} 项）'}",
            )
        )
    return facts
...
def evaluate_consistency_gates(inp: TreeInputs, facts_all_green: bool) -> list[tuple[bool, str]]:
    ...
    impl_status = front_matter_field(inp.g11_contract_text, "implementation_status") if inp.g11_contract_text else None
    consistency.append(
        (
            impl_status == "blocked" or facts_all_green,
            f"C1 G11_CONTRACT implementation_status = {impl_status!r}；事实门全绿 = {facts_all_green}"
            "（事实未全绿时必须保持 blocked，禁止治理完成冒充实现开工）",
        )
    )

    activated = inp.g11_contract_text is not None and bool(ACTIVATION_RE.search(inp.g11_contract_text))
    consistency.append(
        (
            activated == (facts_all_green and impl_status not in (None, "blocked")),
            f"C2 §8 G-G11-3 解锁记录存在 = {activated}；事实门全绿 = {facts_all_green}、"
            f"implementation_status = {impl_status!r}（双状态与 §8 记录必须一致）",
        )
    )

    preclaim = inp.numeric_step_violations + inp.workflow_g11_hits + inp.g11_smoke_scripts
    if impl_status == "blocked":
        consistency.append(
            (
                not preclaim,
                f"C3 数字步骤零预占：milestones/g11 numeric_step 违例 {len(inp.numeric_step_violations)} 处、"
                f"workflow g11 token {len(inp.workflow_g11_hits)} 处、ci/g11_*_smoke.py 预放 "
                f"{inp.g11_smoke_scripts or '无'}"
        ...
        consistency.append(
            (
                ...
                f"C4 src/spec/conformance 治理期 0-byte：g11 实现面 token/命名命中 "
                f"{len(inp.impl_surface_hits)} 处"
                + (f"；首处 {inp.impl_surface_hits[0]}" if inp.impl_surface_hits else ""),
            )
        )
    else:
        # 两态口径（沿 G10.4b 先例，判据语义 0-byte）：已解锁后 C3/C4 治理期口径
        ...
        consistency.append(
            (
                ...
                f"{len(inp.numeric_step_violations)} 处 / workflow g11 token {len(inp.workflow_g11_hits)} 处 / "
                f"ci/g11_*_smoke.py {len(inp.g11_smoke_scripts)} 件均为解锁后合法实现面，非预占；"
        ...
        consistency.append(
            (
                ...
                f"治理期口径不适用；skipped_reason=实现波合法改动三面，实测 g11 实现面 token/命名命中 "
...
def run(inp: TreeInputs, require_ready: bool = False, printer=print) -> tuple[int, str]:
    ...
    contract_status = (
        front_matter_field(inp.g11_contract_text, "status") if inp.g11_contract_text else None
    )
    if contract_status == "closed":
        printer(
            "[check_g11_implementation_interlock] 事实门/一致性门：not_applicable"
            "（status='closed' 收口终态，互锁使命完结；skipped_reason=G11.2+ 开工门问题"
            "不再适用——G11.7b close-out READY 后 status flip；active/blocked 态恢复原机核，"
            "判据语义 0-byte）"
        )
        printer("[check_g11_implementation_interlock] VERDICT = CLOSED")
        printer(
            "  CLOSED 是收口终态正确结论：本守卫回答「G11.2 可否开工」，契约 closed 后该问题"
            "不再适用；不得被当作 G-G11-3 重新开放凭据（契约 reopen 须新立项治理程序）。"
        )
        return 0, "CLOSED"
    ...
    printer("[check_g11_implementation_interlock] 事实门（当前可为红）：")
    for ok, msg in facts:
        printer(f"  {'PASS' if ok else 'RED '} {msg}")
    printer("[check_g11_implementation_interlock] 一致性门（红即脚本失败）：")
    for ok, msg in consistency:
        printer(f"  {'PASS' if ok else 'FAIL'} {msg}")

    verdict = "READY" if facts_all_green else "BLOCKED"
    printer(f"[check_g11_implementation_interlock] VERDICT = {verdict}")
    if verdict == "BLOCKED":
        missing = [msg.split("：", 1)[0] for ok, msg in facts if not ok]
        printer(f"  缺项清单：{missing}")
        printer(
            "  BLOCKED 是当前正确结论：G11.2+ 的 src/、spec/、conformance/ 与数字 workflow 步骤保持 0-byte；"
            "本输出不得被当作 G-G11-3 PASS。"
        )

    consistency_failed = [msg for ok, msg in consistency if not ok]
    if consistency_failed:
        printer(f"[check_g11_implementation_interlock] FAIL — {len(consistency_failed)} 项一致性门为红")
        return 1, verdict
    if require_ready and verdict != "READY":
        printer("[check_g11_implementation_interlock] FAIL — --require-ready 模式下互锁未 READY")
        return 1, verdict
    return 0, verdict
...
| 状态 | Agent Approved |
| Provenance | 起草 `Assisted-by: Kimi-K3（G11.1 治理波 RFC 起草）` |

## §9.1 对抗性评审记录

评审记录 `Assisted-by: Kimi-K3（D-409 独立评审会话，与起草会话隔离）`
"""
            ...
            "entries": [
                {
                    ...
                    "evidence_file": "evidence/g11_baseline_sr_pipeline_l3_fixture.json",
                }
            ],
        },
        budget_evidence_ok={"evidence/g11_baseline_sr_pipeline_l3_fixture.json": True},
        rfc_texts={rfc: _RFC_APPROVED for rfc in G11_RFCS},
        ledger={
            "namespaces": {"CI_step": {"on_tree_max": 195, "next_free": 196}},
            "reserved_in_flight": [{"owner": "G11(画质修复期)"}],
        },
        acceptance_findings=[],
    )
...
def run_selftest() -> int:
    ...
    inp.g11_contract_text = inp.g11_contract_text.replace("status: active", "status: draft")
    case("G11 status 改 draft → ① 红", inp, "① G11_CONTRACT status = 'draft'", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.deliverables_missing = ["milestones/g11/G11_CANDIDATE_DECISIONS.md"]
    case("缺一件治理交付 → ② 红", inp, "② G11.1 治理交付四件齐备（缺 ['milestones/g11/G11_CANDIDATE_DECISIONS.md']）", "BLOCKED", 0)
    ...
    inp = copy.deepcopy(_good_inputs())
    inp.rfc_texts[G11_RFCS[0]] = _RFC_APPROVED.replace(
        "| 状态 | Agent Approved |", "| 状态 | **Draft** 待评审 |"
    )
    case("RFC-0028 改 Draft → ④ 红", inp, "状态未达 Agent Approved", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.ledger["reserved_in_flight"] = [{"owner": "G10(UE5 画面对标基线期)"}]
    case("ledger 无 G11 登记行 → ⑤ 红", inp, '⑤ ledger reserved_in_flight 存在 owner 以 "G11" 开头的行（实测 0 行）', "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.acceptance_findings = ["[three-way] M154 key 漂移"]
    case("三向比对注入漂移 → ⑥ 红", inp, "⑥ check_g11_acceptance_map 三向比对 = FAIL（1 项）", "BLOCKED", 0)

    inp = copy.deepcopy(_good_inputs())
    inp.numeric_step_violations = ["milestones/g11/CI_GATES.md:70 numeric_step 数字赋值：numeric_step: 196"]
    case("数字步骤预占注入 → C3 FAIL 退 1（VERDICT 不受一致性门影响）", inp, "C3 数字步骤零预占", "READY", 1)

    inp = copy.deepcopy(_good_inputs())
    inp.impl_surface_hits = ["spec/g11_gi.md（文件名含 g11）"]
    case("spec 面 g11 命中注入 → C4 FAIL 退 1", inp, "C4 src/spec/conformance 治理期 0-byte", "READY", 1)

    inp = copy.deepcopy(_good_inputs())
    inp.g11_contract_text = inp.g11_contract_text.replace(
        "## §8 Implementation activation / Close-out（只追加区）",
        "### §8.1 G-G11-3 implementation_status 解锁记录",
    )
    case("落 §8 解锁记录但 front matter 未翻 → C2 FAIL 退 1", inp, "C2 §8 G-G11-3 解锁记录存在 = True", "READY", 1)

    # 两态口径（沿 G10.4b 先例）：unblocked 态 C3/C4 自动不适用（skipped_reason 登记），
    # 预占/三面命中注入不再构成 FAIL；blocked 态上述两臂已实证原机核维持。
    unblocked_text = _GOOD_G11_CONTRACT.replace(
    ...
    inp = copy.deepcopy(_good_inputs())
    inp.g11_contract_text = unblocked_text
    inp.numeric_step_violations = ["milestones/g11/CI_GATES.md:70 numeric_step 数字赋值：numeric_step: 196"]
    inp.workflow_g11_hits = [".github/workflows/pr-smoke.yml:2000 g11.p0.m154.fix_r4_gi_multibounce_world_cache"]
    inp.g11_smoke_scripts = ["g11_fix_r4_gi_multibounce_world_cache_smoke.py"]
    inp.impl_surface_hits = ["spec/global_illumination.md:1 G11.4 新条款"]
    case(
    ...
    inp.g11_contract_text = unblocked_text
    inp.acceptance_findings = ["[three-way] M154 key 漂移"]
    case("unblocked 态事实门红 → C1 仍 FAIL 退 1（两态校准不遮蔽 C1/C2）", inp, "C1 G11_CONTRACT implementation_status = 'unblocked'", "BLOCKED", 1)

    # closed 三态口径（沿 G10.8b 先例）：status==closed → VERDICT=CLOSED 退 0，
    # 全门 not_applicable + skipped_reason 登记；active/blocked 态原机核维持
    #（上述红绿臂已实证）。
    inp = copy.deepcopy(_good_inputs())
    inp.g11_contract_text = unblocked_text.replace("status: active", "status: closed")
    case(
    ...
    # 当前树实测：VERDICT 与事实一致（治理未齐=BLOCKED／全绿=READY／
    ...
    if tree_verdict in ("BLOCKED", "READY", "CLOSED") and tree_code == expected_tree_exit:
        print(
            ...
            f"{'互锁条件未齐期' if tree_verdict == 'BLOCKED' else ('收口终态' if tree_verdict == 'CLOSED' else 'G-G11-3 解锁条件已齐')}，符合当前事实预期）"
        )
    else:
        print(
    ...
    if failures:
        print(f"[check_g11_implementation_interlock] SELFTEST FAIL ({failures})")
        return 1
    print("[check_g11_implementation_interlock] SELFTEST PASS (12 RED + 1 GREEN + 1 TREE)")
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        "--require-ready",
        action="store_true",
        help="G11.2 实现 PR 前置模式：未 READY 即退出非零",
    )
