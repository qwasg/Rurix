#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G23.5 soak 波）
"""G23.5 稳定 soak（g23.wave.5a.soak，步骤 411）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g23.wave.5a.soak"
NUMERIC_STEP = 411
SUBJECT = "g23_stabilization_soak"
WAVE = "G23.5"
SCHEMA_PATH = ROOT / "milestones/g23/g23_stabilization_soak_evidence_schema.json"
BIN = ROOT / "target/release/g14_3_pipeline_perf.exe"
SYS56_MANIFEST = ROOT / "src/rurix-physics-sys56/Cargo.toml"
MIN_SECONDS = 1800.0
...
def run_gate() -> int:
    facts: list[dict] = []
    md = wel.load_latest_evidence("g23_m_d_physics_p3_subitem_disposition")
    ...
    if not md_ok or not BIN.is_file() or not SYS56_MANIFEST.is_file():
        ...
        facts.extend([
            ...
            fact("sys56_lane_interleaved", False, "未启动"),
        ])
        ok = False
    else:
        combos = [
        ...
        facts.extend([
            ...
            fact("no_sleep_between_iters", True, "sleep=0"),
            fact("sys56_lane_interleaved", sys56_iters > 0, f"sys56_iters={sys56_iters}（sys56 评估臂构建新鲜车道穿插复跑）"),
        ])
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G23 soak ≥1800s（管线四组合 + sys56 车道穿插）", host_section_pass=ok,
    )
