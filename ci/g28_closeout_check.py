#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G28.6 close-out）
"""G28.6 close-out 终审门（g28.wave.6b.closeout，步骤 492）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g28.wave.6b.closeout"
NUMERIC_STEP = 492
SUBJECT = "g28_wave6b_closeout"
WAVE = "G28.6"
SCHEMA_PATH = ROOT / "milestones/g28/g28_wave6b_closeout_evidence_schema.json"
P0_SUBJECTS = [
    ("m_a", "g28_m_a_restir_device_kernel"),
    ("m_b", "g28_m_b_restir_spatial_reuse_arm"),
    ("m_c", "g28_m_c_m52_rd040_workload_rejudgment"),
    ("m_d", "g28_m_d_rd034_upstream_recheck"),
    ("m_e", "g28_m_e_closed_gate_no_regression"),
]
...
def evaluate() -> tuple[list[dict], str]:
    ...
    p2 = wel.load_latest_evidence("g28_p2_decisions_check")
    ...
    ma = wel.load_latest_evidence("g28_m_a_restir_device_kernel")
    ma_ok = ma is not None and wel.load_json(ma).get("host_section_pass") is True
    facts.append(fact("handover_ledger_chain", ma_ok,
                      f"{ma.name}（ReSTIR device kernel 主锚绿件）" if ma else "missing"))
    hits = list((ROOT / "rfcs").glob("0045-*.md"))
    facts.append(fact("rfc_0045_archived", bool(hits), hits[0].name if hits else "RFC-0045 missing"))
    me_ok, me_d = _latest_green("g28_m_e_closed_gate_no_regression")
    ...
    soak = wel.load_latest_evidence("g28_stabilization_soak")
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        notes=f"G28.6 closeout 八 facts VERDICT={verdict}",
        host_section_pass=ok,
    )
    print(f"[g28_closeout] VERDICT={verdict}")
