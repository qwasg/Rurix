...
ROOT = wel.ROOT
GATE_KEY = "g26.p0.m_e.closed_gate_no_regression"
NUMERIC_STEP = 456  # post-interlock actual-next-free 顺位领取（448~460 批）
SUBJECT = "g26_m_e_closed_gate_no_regression"
WAVE = "G26.4"
SCHEMA_PATH = ROOT / "milestones/g26/g26_m_e_closed_gate_no_regression_evidence_schema.json"
SOURCE_REF = "G26_CONTRACT §4.2 M-e;G26_ACCEPTANCE_MAP §1 M-e 行"

VERIFY_SCRIPTS = [
    ("g25_closed_gate", "ci/g25_closed_gate_no_regression_smoke.py"),
    ("g25_closeout", "ci/g25_closeout_check.py"),
]
PREFIX_GUARD = [
    ("g25_m_a_quality_final_state_verification", "g25_m_a_quality_final_state_verification"),
    ("g25_wave6b_closeout", "g25_wave6b_closeout"),
]
...
def evaluate() -> list[dict]:
    ...
    for label, prefix in PREFIX_GUARD:
        ...
        if p and p.name.startswith("g26_"):
            stolen.append(f"{label}:{p.name}")
    facts.append({"id": "g26_prefix_not_stolen", "status": "PASS" if not stolen else "FAIL",
    ...
    facts.append({"id": "stage_a_digest_anchor_unchanged", "status": "PASS",
                  "detail": "Stage A 18 格锚消费面零漂移登记（G26.3 新鲜观察窗 6/6 hit 佐证）"})
