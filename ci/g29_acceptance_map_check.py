...
"""G29.1 治理门 — 验收映射覆盖 / 空行 / 双向命名空间一致性（g29.wave.1.acceptance_map，步骤 493）。

核验 `milestones/g29/G29_ACCEPTANCE_MAP.md`：
§1 五行 P0（{M-a..M-e} 闭集全等）+ §2 零 go P1 空集断言 +
全部 symbolic key 匹配 `g29.p0.m_<a~e>.<slug>` 单一命名空间（key 的 m 段字母与行号一致、
脚本命令 `ci/g29_*_smoke.py` 每行单一且 --gate 参数 == canonical key、evidence schema
`g29_m_<a~e>_<key slug>_evidence_schema.json` 与 key 末段同 slug）+
波次 ∈ {G29.2,G29.3,G29.4} +
numeric_step 全列 `post-interlock actual-next-free allocation` 字面零预占 +
零空行/占位 + **双向一致**：MAP §1 与 G29_CONTRACT.md §4.2 对同一 P0 M 行给出的
判据与波次逐字相等（G28 同构体例）。

只读文档，不代绿实现门；本门 PASS 只表示映射完整，不表示任何 P0 能力已实现。
用法：
  py -3 ci/g29_acceptance_map_check.py --gate g29.wave.1.acceptance_map
  py -3 ci/g29_acceptance_map_check.py --selftest
"""
...
ROOT = wel.ROOT
GATE_KEY = "g29.wave.1.acceptance_map"
NUMERIC_STEP = 493  # 落盘前实测 registry/number_ledger.json CI_step.next_free=493 顺位领取
SUBJECT = "g29_acceptance_map_check"
WAVE = "G29.1"
MAP_PATH = ROOT / "milestones" / "g29" / "G29_ACCEPTANCE_MAP.md"
CONTRACT_PATH = ROOT / "milestones" / "g29" / "G29_CONTRACT.md"
SCHEMA_PATH = ROOT / "milestones" / "g29" / "g29_acceptance_map_check_evidence_schema.json"
SOURCE_REF = (
    "G29_CONTRACT G-G29-1/§4.2;G29_ACCEPTANCE_MAP.md §1/§2/§4/§5;"
    "G28_P2_DECISIONS.md §1 defer-to-G30+ 九行 + G28_CONTRACT.md §8.7 承接锚 + "
    "用户战役指令（帮我一次性完成G26-G30）P0 清单 M-a~M-e"
)

EXPECTED_P0 = {"M-a", "M-b", "M-c", "M-d", "M-e"}
ALLOWED_WAVES = {"G29.2", "G29.3", "G29.4"}
NUMERIC_STEP_LITERAL = "post-interlock actual-next-free allocation"

KEY_RE = re.compile(r"^g29\.p0\.m_[a-e]\.[a-z0-9_]+$")
KEY_IN_CELL_RE = re.compile(r"`(g29\.p0\.m_[a-e]\.[a-z0-9_]+)`")
SECTION_RE = re.compile(r"^## (\d+)\. ")
SCRIPT_RE = re.compile(r"ci/g29_[a-z0-9_]+_smoke\.py")
SCHEMA_RE = re.compile(r"`(milestones/g29/g29_(m_[a-e])_[a-z0-9_]+_evidence_schema\.json)`")
...
def check_row(m: str, row: dict, seen_keys: dict[str, str], seen_schemas: dict[str, str]) -> list[str]:
    ...
    if not KEY_RE.match(key):
        findings.append(f"{m} key `{key}` 不匹配 g29.p0.m_<a~e>.<slug>")
    ...
    slug = key.split(".")[3]
    if not row["scripts"]:
        findings.append(f"{m} 缺 `ci/g29_*_smoke.py` 脚本命令")
    ...
    if not schema_m:
        findings.append(f"{m} schema 路径不符 g29_m_<a~e>_<slug>_evidence_schema.json：{row['schema']!r}")
    else:
        path, schema_m_no = schema_m.group(1), schema_m.group(2)
        if schema_m_no != m_seg:
            findings.append(f"{m} schema 路径的 m 段不符：{path}")
        expected_schema = f"milestones/g29/g29_{m_seg}_{slug}_evidence_schema.json"
    ...
    if row["wave"] not in ALLOWED_WAVES:
        findings.append(f"{m} 波次 {row['wave']!r} 不在允许集合 {{G29.2,G29.3,G29.4}} 内")
...
def evaluate(map_text: str | None, contract_text: str | None) -> list[dict]:
    ...
    results: list[dict] = []
    if map_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G29_ACCEPTANCE_MAP.md 缺失（诚实红，不假绿）"}]
    if contract_text is None:
        return [{"id": "file", "status": "FAIL", "detail": "G29_CONTRACT.md 缺失（诚实红，不假绿）"}]
    ...
    results.append({
        ...
        "detail": "§2 零 go P1 空集（G29.1 字面）" if p1_ok else f"§2 出现 P1 行 {sorted(p1_rows)}（G29.1 零 go P1 字面违例）",
    })
    ...
    for m in sorted(EXPECTED_P0):
        ...
        if len(row["keys"]) == 1:
            ...
            if other is None:
                two.append("G29_CONTRACT §4.2 缺行")
            else:
...
def emit(results: list[dict], overall_ok: bool) -> int:
    ...
    if not SCHEMA_PATH.is_file():
        print(f"[g29_acceptance_map] FAIL: schema 缺失 {SCHEMA_PATH}", file=sys.stderr)
        return 1
    code, _ = wel.emit_wave_evidence(
        ...
        notes="G29.1 治理门——验收映射覆盖/空行/双向命名空间一致性：5 P0（M-a~M-e）闭集全等 + §2 零 go P1 空集 + key/脚本/schema 单一命名空间同 slug + numeric_step 全列 post-interlock 字面零预占 + MAP §1 ↔ CONTRACT §4.2 判据/波次双向逐字一致；本门 PASS 只表示映射完整，不表示任何 P0 能力已实现",
...
CANONICAL_ROWS = [
    ("M-a", "slab_device_kernel", "ci/g29_slab_device_kernel_smoke.py", "G29.2"),
    ("M-b", "slab_side_table_arm", "ci/g29_slab_side_table_arm_smoke.py", "G29.2"),
    ("M-c", "svt_ktx2_gap_rejudgment", "ci/g29_svt_ktx2_gap_rejudgment_smoke.py", "G29.3"),
    ("M-d", "wg_dgc_capability_recheck", "ci/g29_wg_dgc_capability_recheck_smoke.py", "G29.4"),
    ("M-e", "closed_gate_no_regression", "ci/g29_closed_gate_no_regression_smoke.py", "G29.4"),
]


def _fixture() -> tuple[str, str]:
    map_lines = ["# fixture G29_ACCEPTANCE_MAP", "", "## 1. P0 硬门（精确 5 行）", ""]
    contract_lines = ["# fixture G29_CONTRACT", "", "### 4.2 五行 P0", ""]
    for m, slug, script, wave in CANONICAL_ROWS:
        key = f"g29.p0.{m.lower().replace('-', '_')}.{slug}"
        schema = f"milestones/g29/g29_{m.lower().replace('-', '_')}_{slug}_evidence_schema.json"
        cmd = f"`py -3 {script} --gate {key}`"
        map_lines.append(
            f"| **{m}** | `{key}` | {cmd} | `{schema}` | 合成判据 {m} | **{wave}** | {NUMERIC_STEP_LITERAL} |"
        )
        contract_lines.append(f"| **{m}** | 合成判据 {m} | {wave} |")
    map_lines += ["", "## 2. 已 go P1 硬门（零行）", "", "G29.1 无 go 的 P1 行。", ""]
    return "\n".join(map_lines), "\n".join(contract_lines)
...
def run_selftest() -> int:
    ...
    cases: list[tuple[str, str, str, str]] = [
        ...
        (
            "--gate 参数漂移 → row 必须红",
            map_text.replace("--gate g29.p0.m_c.svt_ktx2_gap_rejudgment`", "--gate g29.p0.m_c.rd045_drift`"),
            contract_text,
            "row_M-c",
        ),
        (
        ...
        (
            "波次改写为非法 G29.5 → 必须红",
            map_text.replace("合成判据 M-a | **G29.2**", "合成判据 M-a | **G29.5**", 1),
            contract_text,
            "row_M-a",
        ),
        (
            ...
            map_text.replace(
                "G29.1 无 go 的 P1 行。",
                f"| **M-a** | `g29.p0.m_a.slab_device_kernel` | `py -3 ci/g29_slab_device_kernel_smoke.py --gate g29.p0.m_a.slab_device_kernel` | `milestones/g29/g29_m_a_slab_device_kernel_evidence_schema.json` | 判据 | **G29.2** | {NUMERIC_STEP_LITERAL} |",
            ),
            contract_text,
            "coverage_p1_empty",
        ),
        ...
        (
            "schema 路径 m 段改写 → 必须红",
            map_text.replace(
                "milestones/g29/g29_m_c_svt_ktx2_gap_rejudgment_evidence_schema.json",
                "milestones/g29/g29_m_d_svt_ktx2_gap_rejudgment_evidence_schema.json",
                1,
            ),
            contract_text,
            "row_M-c",
    ...
    if failures:
        print(f"[g29_acceptance_map] SELFTEST FAIL ({failures})")
        return 1
    print("[g29_acceptance_map] SELFTEST PASS (9 RED + 1 GREEN + 真表臂)")
    return 0
