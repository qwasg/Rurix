    ...
    if cells:
        ...
        if checks["gap_registry_schema_valid"]:
            ...
            if REGISTRY_PATH.is_file():
                ...
                # 对账语义面，M-d 同模）：跨会话样本级联逐位带 map——带 = 历史样本
                # 极差率 ×2.0 与当次同会话探针带取 max；带面于 fresh 样本追加前
                # 派生（不拟合当次测量，RXS-0392/P-09 维持）。
                _ue_band_map: dict = {}
                _ue_fresh_rows: list = []
                _old_vals: dict = {}
                ...
                for _it in (registry_doc.get("items") or []):
                    for _d in (_it.get("measured_delta") or []):
                        for _f in ("a_value", "b_value"):
                            ...
                            if (_classify_m_c(str(_d.get("metric")), _f, 0.0) == gaplib.PROVENANCE_UE
