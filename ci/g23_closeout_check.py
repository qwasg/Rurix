#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Assisted-by: Cursor Agent（G23.6 close-out）
"""G23.6 close-out 终审门（g23.wave.6b.closeout，步骤 412）。"""
...
ROOT = wel.ROOT
GATE_KEY = "g23.wave.6b.closeout"
NUMERIC_STEP = 412
SUBJECT = "g23_wave6b_closeout"
WAVE = "G23.6"
SCHEMA_PATH = ROOT / "milestones/g23/g23_wave6b_closeout_evidence_schema.json"
P0_SUBJECTS = [
    ("m_a", "g23_m_a_jolt_56_adoption_rejudgment"),
    ("m_b", "g23_m_b_neural_deform_rejudgment"),
    ("m_c", "g23_m_c_research_track_disposition"),
    ("m_d", "g23_m_d_physics_p3_subitem_disposition"),
    ("m_e", "g23_m_e_closed_gate_no_regression"),
]
...
def evaluate() -> tuple[list[dict], str]:
    ...
    p2 = wel.load_latest_evidence("g23_p2_decisions_check")
    ...
    hzb = wel.load_latest_evidence("g23_m_a_jolt_56_adoption_rejudgment")
    hzb_ok = hzb is not None and wel.load_json(hzb).get("host_section_pass") is True
    facts.append(fact("jolt_adoption_chain", hzb_ok, hzb.name if hzb else "missing"))
    hits = list((ROOT / "rfcs").glob("0040-*.md"))
    facts.append(fact("rfc_0040_archived", bool(hits), hits[0].name if hits else "RFC-0040 missing"))
    me_ok, me_d = _latest_green("g23_m_e_closed_gate_no_regression")
    ...
    soak = wel.load_latest_evidence("g23_stabilization_soak")
...
def run_gate() -> int:
    ...
    code, _ = wel.emit_wave_evidence(
        ...
        source_ref="G23_CONTRACT G-G23-6", required_gate_rows=[], extra_facts=facts, subjects=[],
        schema_path=SCHEMA_PATH, evidence_basename=SUBJECT,
        notes=f"G23.6 closeout 八 facts VERDICT={verdict}",
        host_section_pass=ok,
    )
    print(f"[g23_closeout] VERDICT={verdict}")
