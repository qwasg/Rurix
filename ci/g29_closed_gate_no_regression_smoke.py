...
ROOT = wel.ROOT
GATE_KEY = "g29.p0.m_e.closed_gate_no_regression"
NUMERIC_STEP = 504  # post-interlock actual-next-free 顺位领取（496~508 批）
SUBJECT = "g29_m_e_closed_gate_no_regression"
WAVE = "G29.4"
SCHEMA_PATH = ROOT / "milestones/g29/g29_m_e_closed_gate_no_regression_evidence_schema.json"
SOURCE_REF = "G29_CONTRACT §4.2 M-e;G29_ACCEPTANCE_MAP §1 M-e 行"

VERIFY_SCRIPTS = [
    ("g28_closed_gate", "ci/g28_closed_gate_no_regression_smoke.py"),
    ("g28_closeout", "ci/g28_closeout_check.py"),
]
PREFIX_GUARD = [
    ("g28_m_a_restir_device_kernel", "g28_m_a_restir_device_kernel"),
    ("g28_wave6b_closeout", "g28_wave6b_closeout"),
]
...
def evaluate() -> list[dict]:
    ...
    for label, prefix in PREFIX_GUARD:
        ...
        if p and p.name.startswith("g29_"):
            stolen.append(f"{label}:{p.name}")
    facts.append({"id": "g29_prefix_not_stolen", "status": "PASS" if not stolen else "FAIL",
