...
### G6.6 close-out · **已完成(2026-08-01:G-G6-8 收口门绿)**
...
- **全量回归冻结**:cargo fmt --check / cargo clippy --workspace --all-targets -D warnings 零警告 / cargo test --workspace **1174 passed 0 failed**(100 测试二进制,与 G6.5 基线一致)+ 治理检查面板 10 项 rc=0(含 budget_eval --strict 96 pass 0 skip 零 estimated)。
- **CI 步骤 88~92 五门真跑全绿**:88 PASS(cargo 三档 6/41/54 + A7 行为测试 7/7 + §4.C4 审计四项,evidence `physics_core_smoke_20260801T085308.json`)/ 89 PASS(device 腿真跑 changed_pixels=168,evidence `physics_bridge_smoke_20260801T085315.json`)/ 90 PASS(host 七判据,parity 实测 max 0.541314125m / 61.774520874°、接触集合重叠 1.0〔阈值 0.82m/93°/≥0.99 内〕,evidence `physics_rapier_parity_smoke_20260801T005320.json`)/ 91 PASS(device 腿真跑 16 断言,physics_step_ms=8.9776,evidence `uc08_physics_smoke_20260801T085707.json`)/ 92 PASS(device 五断言真跑 first_values 逐位,evidence `taichi_vulkan_spike_20260801T005315.json`);89/91/92 device 腿 RTX 4070 Ti 真跑零 SKIP。
- **门终审表 G-G6-1~G-G6-8 全过**,落 [G6_CONTRACT.md](G6_CONTRACT.md) §8.2;G6_CONTRACT status active→closed(close-out 只追加 §8,上方条款 0-byte)。
- **RD/SG 处置**:deferred.json v1.73——RD-042 研究轨(Newton/Genesis/MuJoCo Warp)观察 / RD-043 wgrapier GPU 刚体观察 / RD-044 物理 P3+ 长线,全 open 存续;spike_gating.json 0-byte(G6 零新 SG,SG-010 留续号维持);number_ledger v1.34 校准(RD on_tree_max 44 / next_free 45)。
...
| v1.7 | 2026-08-01 | G6.6 close-out 完成(agent 完全自主收口蜂群):全量回归冻结(cargo fmt --check / cargo clippy --workspace --all-targets -D warnings 零警告 / cargo test --workspace **1174 passed 0 failed**〔100 测试二进制,与 G6.5 基线一致〕+ 治理检查面板 10 项 rc=0,含 budget_eval --strict 96 pass 0 skip 零 estimated);CI 步骤 88~92 五门真跑全绿——88 PASS(cargo 三档 6/41/54 + A7 行为测试 7/7 + §4.C4 审计四项,evidence `physics_core_smoke_20260801T085308.json`)/ 89 PASS(device 腿真跑 changed_pixels=168,evidence `physics_bridge_smoke_20260801T085315.json`)/ 90 PASS(host 七判据,parity 实测 max 0.541314125m / 61.774520874°、接触集合重叠 1.0〔阈值 0.82m/93°/≥0.99 内〕,evidence `physics_rapier_parity_smoke_20260801T005320.json`)/ 91 PASS(device 腿真跑 16 断言,physics_step_ms=8.9776,evidence `uc08_physics_smoke_20260801T085707.json`)/ 92 PASS(device 五断言真跑 first_values 逐位,evidence `taichi_vulkan_spike_20260801T005315.json`),89/91/92 device 腿 RTX 4070 Ti 真跑零 SKIP;门终审表 G-G6-1~G-G6-8 全过落 [G6_CONTRACT.md](G6_CONTRACT.md) §8.2 + G6_CONTRACT status active→closed(close-out 只追加 §8,上方条款 0-byte);RD/SG 处置——deferred.json v1.73(RD-042 研究轨 Newton/Genesis/MuJoCo Warp 观察 / RD-043 wgrapier GPU 刚体观察 / RD-044 物理 P3+ 长线,全 open 存续)、spike_gating.json 0-byte(G6 零新 SG,SG-010 留续号维持)、number_ledger v1.34 校准(RD on_tree_max 44 / next_free 45);状态行翻 closed + §1 G6.6 波次标记已完成(2026-08-01:G-G6-8 收口门绿)+ CI_GATES v1.5 修订行同步;g6_budget.json 空壳维持零 estimated;§0/§2/§3/§4 既有裁决与冻结接口条文 0-byte 不动 |
