...
### §8.6 G13.5a P2 穷举决策 + M-e(M171) 回归门验收记录（2026-08-19）——G-G13-7 字面兑现：g13.wave.5.decisions（步骤 243 实测领取，31 行闭集零空行，34 facts 全绿）+ G-G13-8 前半：g13.p0.m_e.regression_drift_guard（步骤 244 实测领取，9 checks 全绿：71 门零降级 + 触改面抽检 7 门真跑 fresh + M165 漂移零检出 + RED 三臂独立有效）

- **① 独立断言全绿清单**：

  | gate（symbolic key） | 独立布尔断言 | host/device | evidence 路径 | 结果 |
  |---|---|---|---|---|
  | `g13.wave.5.decisions`（步骤 243） | G-G13-7 逐字：31 行闭集全等（§1 G12 defer 承接 22 行 + §2 G13 新增候选 7 行 + §3 期内新增 G13-N8/N9 双差距登记表处置 2 行）+ 裁决枚举合法 + 零空行 + 承接锚「重判条件+兜底」字面 + defer 行 G14+ 重评窗 + go 行 evidence 义务 + no-go 行锚义务；横向机核四面：MAP §1 五 P0 互斥 / deferred history 对账（RD-040 +1〔M52 重评窗未命中〕/ RD-041 +1〔G10-N5 closed-go〕，零新 RD，status 全 open）/ G13.1 候选决策表 22 行承接对账 n=22/22 / G13.4 双差距登记表 8+2 行全 P2 + G12 表 10 行终态 0-byte | host | evidence/g13_p2_decisions_20260819T020123Z.json（34/34 facts） | PASS |
  | `g13.p0.m_e.regression_drift_guard`（步骤 244） | §4.2 M-e 行逐字 9 checks 闭集：temporal 底座 0-byte（vs G13.0 ref 8c5dc5ee）+ 既有 71 门（G9 34 + G10 14 + G11 14 + G12 9 key）最新 evidence 全绿只读汇总（聚合不遮蔽，行集全量入 evidence）+ G13 触改面既有门重跑回归零降级（g9_m96 golden 门序面真跑抽检〔RURIX_REQUIRE_REAL=1 + RURIX_VK_VALIDATION=1〕+ G10 M139 A/B 对拍门全档真跑 + G10 M140 登记表门真跑 + G12 wave5 exit + G13 wave2/3/4 exit 聚合复跑——子进程 exit 0 + 最新 evidence PASS + 新鲜度机核 evidence stamp ≥ 会话起点，陈旧冒充即 RED）+ M165 漂移监控登记（G13 四门确定性键族 15 键全真 + 同型 digest 漂移检出计数 = 0 零检出字面入 evidence + M165 FAIL 件 0-byte 在档 + flip-trace 诊断臂在树）+ G5~G12 既有判据 0-byte（已提交 diff 闭集 = {ci/g10_gap_registry_lib.py 加性演进位}；工作树闭集 = {g12_pt_sampler_selection.json 异己登记面}）+ RED 三臂（degraded-gate / aggregate-masking / drift-unregistered）独立复跑检出 | host+device（抽检子进程真跑） | evidence/g13_m_e_regression_drift_guard_20260819T032053Z.json（9/9） | PASS |

- **② 波聚合门实测输出**：本波无新波聚合门（G13.5a 聚合面 = soak 门 g13.wave.5a.soak，下一波次面）；决策门 `py -3 ci/g13_p2_decisions_check.py --gate g13.wave.5.decisions` → **VERDICT = PASS，exit=0**（34 facts 逐行打印不遮蔽）；M-e 门 `py -3 ci/g13_regression_drift_guard_smoke.py --gate g13.p0.m_e.regression_drift_guard` → **VERDICT=PASS，checks=9/9，exit=0**（71 门只读汇总 71/71 PASS + 抽检 7 门全真跑 fresh + 漂移零检出）。

- **③ 验收命令逐字输出（2026-08-19 真跑留痕，仓库根目录）**：
  - `py -3 ci/g13_p2_decisions_check.py --selftest` → **SELFTEST PASS（2 GREEN + 4 RED），exit=0**（真表 31 行绿 + 合成全表绿 + 缺行→红 + defer 缺 G14+ 承接锚→红 + 非法裁决→红 + 已 go M### 裸 token 混入→红）。
  - `py -3 ci/g13_p2_decisions_check.py --gate g13.wave.5.decisions` → **VERDICT = PASS，exit=0**：set_equality_frozen PASS（n=31 unique=31）+ 31 行逐行 PASS（go 6 closed-go 留痕〔G10-N5 + G13-N1~N5〕+ no-go 1〔G13-N6〕+ defer-to-G14+ 24）+ acceptance_map_mutex PASS（MAP §1 P0 行=5；裸 token 命中无）+ deferred_history_reconcile PASS（RD-040 +1 / RD-041 +1；RD max=44 零新 RD；RD-034/039~044 status 全 open）+ candidate_decisions_reconcile PASS（承接行对账 n=22/22）+ gap_registries_reconcile PASS（upscale 8 行全 P2 + lumen 2 行全 P2 + g12 表 10 行终态 porcelain 空）——evidence/g13_p2_decisions_20260819T020123Z.json。
  - `py -3 ci/g13_regression_drift_guard_smoke.py --selftest` → **selftest PASS checks=9 schema_required=9（3 RED + 3 GREEN），exit=0**。
  - `py -3 ci/g13_regression_drift_guard_smoke.py --gate g13.p0.m_e.regression_drift_guard` → **VERDICT=PASS checks=9/9，exit=0**：71 门只读汇总 71/71 PASS；抽检 g9_m96 / g10_m139 / g10_m140 / g12_wave5_exit / g13_wave2_exit / g13_wave3_exit / g13_wave4_exit 七门 exit=0 status=PASS fresh=True；G5~G12 closed 面 committed 闭集=['ci/g10_gap_registry_lib.py']（允许面内）+ 工作树闭集=['milestones/g12/g12_pt_sampler_selection.json']（异己登记允许面内）；M165 漂移监控 15 键全真 + 漂移检出计数 = 0 + FAIL 件在档 + 诊断臂在树——evidence/g13_m_e_regression_drift_guard_20260819T032053Z.json。
  - 恢复实证：`py -3 ci/g10_ab_comparison_smoke.py --gate g10.p0.m139.ab_comparison`（帧库恢复后复跑）→ **checks 16/16，device=executed，exit=0**——evidence/g10_m139_ab_comparison_20260819T031611Z.json；M140 同波复跑 17/17+ PASS——evidence/g10_m140_gap_registry_20260819T031953Z.json。
  - 守卫套件全 PASS（逐字输出行）：`py -3 ci/check_structure.py` → `[check_structure] PASS (11 dirs, 6 files)`；`py -3 ci/check_schemas.py` → `[check_schemas] PASS`（本批新 evidence 经 G13.5a 三前缀路由校验过）；`py -3 ci/check_number_ledger.py` → `[check_number_ledger] PASS`（CI_step on_tree_max 244 / next_free 245 校准后实测）；`py -3 ci/budget_eval.py` → PASS（g13_budget 全条目 measured_local 零 estimated）。
  - 互锁维持：`py -3 ci/g13_interlock_check.py --require-ready` → **VERDICT=READY，exit=0**（G13.5a 全程 implementation_status=unlocked 维持）。

- **④ 门序 / 偏差 / not-triggered 登记面摘要**：
  - **门序**：G-G13-6（G13.4 波聚合门）于 commit `8958ea5c` 留痕（§8.5）→ 本波 G-G13-7（决策门）+ G-G13-8 前半（M-e P0）兑现；数字步骤 243~244 按落盘前实测 actual next_free=243 顺位领取（ledger v1.139 校准同批）；G13.5a soak（g13.wave.5a.soak，步骤 245 预留面——脚本 ci/g13_stabilization_soak.py 本批同模 materialize，workflow/ledger 注册随 soak full-run 批次，禁推测号口径维持）。
  - **UE 参照帧库改写事件（全程取证—恢复—复跑—登记，本波最大偏差）**：
    - **检出**：M-e 门首跑（evidence/g13_m_e_regression_drift_guard_20260819T020910Z.json，checks 8/9 FAIL 件在档）——触改面抽检 g10_m139 子进程红：UE HDR 库帧 digest 漂移（cornell-box）sha256:b3e6b530… ≠ 冻结注册 sha256:c7c6f2cf…，派生链互证失败 + 度量重算超 golden 统计面（ssim 0.348 匹配 golden 字面但 hdr_ue5 统计 nonzero_ratio 0.933 ≠ golden 0.184）；M140 因 M139 fail-closed 连锁红（缺 M139 PASS evidence 登记集）。
    - **取证**：①漂移帧 mtime = 2026-08-17 21:16:48 UTC（G12 close-out 后、G13.1 前）被整组四帧改写；②现采帧与漂移帧内容 digest 全等（b3e6b530）⇒ 漂移态对当前 UE 项目态是**确定性复现**；③G12.4 RT 项目 ini 偏差（DefaultEngine.ini RT 链补齐，G12.4 §8.4 登记面）排除——RT-off 复采同 digest b3e6b530；④图像面定性：冻结帧 c7c6f2cf = **无壳帧**（两侧墙/天花/地板整面缺失，非零率 18.4%），现采帧 = **有壳帧**（非零率 93.3%）——根因 = **G10.5b 冻结帧早于 G11.3 U1 cornell 壳体双面化修复（M150），当前建设脚本（含 G11 修复）重采恒产有壳帧，与 G10.5b 冻结常量不兼容**；G11.3 未重登记 M139 常量 ⇒ 潜伏断裂自 G11.3 起存在，任何重采覆写库帧即显性化；⑤改写行为归属 = 异己并发会话对共享 UE 参照项目的活跃改建（2026-08-19 全天多次 build/probe 留痕，本会话期实测 G10_CornellBox.umap / G10_BistroInterior.umap 被逐景重存）。
    - **恢复**：在 K:\rurix-ext\g11-frames\g11_2\ue\cornell-box\ 找到 G11.2 期归档帧（.0000 内容 digest == c7c6f2cf 冻结注册值逐位匹配），.0000~.0003 四帧整体恢复至 K:\rurix-ext\g10-frames\g10_5\ue\cornell-box\（恢复前漂移帧已保全至 .tmp\g13_5_drifted_cornell\ 作取证件不入 commit）；恢复后 M139 复跑 16/16 PASS（evidence 031611Z）、M140 复跑 PASS（031953Z）、M-e 门复跑 9/9 PASS（032053Z）。
    - **stale-FAIL 序贯教训（如实登记）**：M-e 第二跑聚合腿读到 M140 陈旧 FAIL 件（首跑遗留）即红——第三跑聚合腿读到当次新 PASS 件转绿；陈旧 FAIL 件（020910Z/021500Z/021356Z 等首跑面）全部保留在档作诚实轨迹，不删不改。
    - **schema-invalid 调试残留处置留痕**：G13.5a 决策门首跑（2026-08-19 01:00 UTC 前会话）经 wel.emit_wave_evidence 落盘一件 schema-invalid 调试件（该 emit 路径自标「仍落盘以便调试，exit 1」——非门产证据、未入档、未被任何 §8 记录引用），且当时 check_schemas 缺 G13.5a 三前缀路由致其落入 gpu 兜底校验持续报红；处置 = 删除该未跟踪调试残留一件 + 三前缀路由本批补齐（load/validator/路由纯追加）——evidence/ 已提交面 0-byte，删除对象仅限未跟踪 schema-invalid 调试残留，留痕本条。
    - **共享面并发回退偏差（G13.3 §8.4/G13.4 §8.5 同模）**：本波期间 ci/check_schemas.py 三处追加面一度被并发会话回退至 HEAD（编辑落盘后短时间窗内实测）；处置 = 幂等重放 + 提交前单命令链压死竞态窗口（重放→守卫套件真跑复核→立即提交）。
  - **not-triggered / 维持 open 面**（如实保持 open，不写进全绿叙述）：M52（G13.4 重评窗结论未命中，RD-040 +1 留痕）/ M100-high / RD040-nrd 维持 defer；G10-N11/N16/G11-N3/M114-strand 锚定 G14；G11-N5（G13.5a 触发评估结论 = 未齐备）/ G10-N17（G13.4 触发评估结论 = 未消费）维持 defer；G11-N8/N9/G12-N10/G12-N12 锚定 G15（G12 gap registry 只消费不回写）；G13-N6 异己面 no-go；G13-N7 FG/MFG defer-to-G14+；G13-N8/N9 锚定 G15 画质收口期；SG-010 软保留 not_triggered 维持；G13.5a soak 未跑（下一波面）；G14/G15 未立项。
  - **异己并发工作树面**：本批只含 G13 车道文件（块⑤清单按文件名显式择取）；异己会话 src/ 未提交面（apps/、src/rurix-asset、src/rurix-render 多面、src/rurix-rt/render_exec.rs、milestones/g12/g12_pt_sampler_selection.json 及各 evidence/ 异己新件）维持未提交、零消费、零混入（立项裁决 1）。

- **⑤ 签署块**：白栀（依 10 §7 / P-13 / D-406 v2.0 agent 完全自主签署，G10/G11/G12 §8.x 同模）。`Assisted-by: Kimi-K3（G13.5a P2 穷举 + 回归波）`（影响范围：milestones/g13/G13_P2_DECISIONS.md v1.0 新建 + ci/g13_p2_decisions_check.py + ci/g13_regression_drift_guard_smoke.py + ci/g13_stabilization_soak.py 三新建 + milestones/g13 三 evidence schema〔g13_p2_decisions_/g13_m_e_regression_drift_guard_/g13_stabilization_soak_〕新建 + ci/check_schemas.py〔G13.5a 三前缀 load/validator/路由三处纯追加〕+ .github/workflows/pr-smoke.yml〔步骤 243~244，步骤 242 块后追加〕+ registry/number_ledger.json〔CI_step 242→244/next_free 245 + revision_log v1.139〕+ registry/deferred.json〔RD-040 +1/RD-041 +1 history 只追加〕+ 本契约 §8.6 本条 + evidence/g13_p2_decisions_20260819T020123Z + g13_m_e_regression_drift_guard_20260819T032053Z 真跑件；验证方式：块③逐字命令输出——决策门 34 facts PASS + M-e 门 9/9 checks PASS〔71 门汇总 71/71 + 抽检 7/7 真跑零降级 fresh 全真 + 漂移零检出 + 0-byte 闭集 + RED 三臂〕+ 双 selftest 红绿留痕 + 帧库恢复后 M139 复跑 16/16 PASS + 守卫套件全 PASS + 互锁 --require-ready READY 维持）。

---

### §8.7 G13.5a stabilization soak 验收记录（2026-08-19）——G-G13-8 字面兑现：g13.wave.5a.soak（步骤 245 实测领取，六 facts 全绿：5 P0 真跑回归 + base_commit 同一候选基线 + 409 迭代 1806s 超分链路 soak 零失败 + budget --strict 212/0-skip + G5~G12 0-byte + 日期锚）+ UE 厂商随机运行间方差事件取证与双差距登记表再锚定登记

- **① 独立断言全绿清单**：

  | gate（symbolic key） | 独立布尔断言 | host/device | evidence 路径 | 结果 |
  |---|---|---|---|---|
  | `g13.wave.5a.soak`（步骤 245） | G-G13-8 逐字：①全量回归（G13 5 P0 = M-a~M-e 逐门真跑 --gate 机器核验最新 evidence 顶层 status=="pass" 字面 + wave2/3/4 exit + wave5 decisions 四聚合/决策门真跑核验；5 门 evidence base_commit 同值 == HEAD `563bda68` 且 9 门 evidence 文件名 UTC stamp ≥ run 起点 = 同一候选 close-out 基线）+ ②超分链路（三后端出图→差距登记表装配复核）连续复跑 soak 409 迭代 / 1805.97s 墙钟（≥1800s 沿 G12.7a 继承）/ 13088 帧出图 / 409 次登记表装配 / failures=0 / sleep_seconds 恒 0 / active_chain_seconds==seconds / 外测墙钟 1805.97s 交叉核验一致 + ③budget_eval --strict 212 pass / 0 skip / 零 estimated + ④G5~G12 既有判据 0-byte（已提交 diff 闭集 = {ci/g10_gap_registry_lib.py 加性演进位}；工作树闭集 = {g12_pt_sampler_selection.json 异己登记面}）+ 日期锚 utc_date=20260819 | host+device（回归腿子进程真跑 + soak 出图腿 device 真跑持 gpu_device_lock 串行） | evidence/g13_stabilization_soak_20260819T090227Z.json（六 facts 全绿） | PASS |

- **② 波聚合门实测输出**：`py -3 ci/g13_stabilization_soak.py --gate g13.wave.5a.soak` → **VERDICT = PASS，exit=0**（六 facts 逐行打印不遮蔽）。回归腿逐门真跑留痕：M-a 22/22（evidence/g13_m_a_vendor_upscale_integration_20260819T090228Z.json）· M-b 18/18（g13_m_b_tsr_device_kernel_20260819T090602Z.json）· M-c 20/20（g13_m_c_ue_upscale_parity_20260819T095337Z.json）· M-d 19/19（g13_m_d_ue_lumen_gi_parity_20260819T104418Z.json）· M-e 9/9（g13_m_e_regression_drift_guard_20260819T105009Z.json：71 门只读汇总 71/71 + 抽检 7 门真跑 fresh + M165 漂移零检出）· 四聚合/决策门（wave2/3/4 exit + wave5 decisions 105410Z）全 PASS；soak 腿三后端轮转（tsr_device/dlss_sr/fsr_3_1_5）converged_digest 逐迭代位级锚定零漂移（M165 同型监控面零检出）。

- **③ 验收命令逐字输出（2026-08-19 真跑留痕，仓库根目录）**：
  - `py -3 ci/g13_stabilization_soak.py --selftest` → **selftest PASS（1 GREEN + 8 RED 判读面），exit=0**（谎报/sleep 充时/迭代不足/计数面空/外测背离全检出）。
  - `py -3 ci/g13_stabilization_soak.py --gate g13.wave.5a.soak` → **VERDICT = PASS，exit=0**：FACT 行集 = regression_gates_all_pass PASS（5 P0 真跑 + 4 聚合/决策门核验全绿）/ base_commit_uniform PASS（5 门 evidence base_commit 同值 563bda686beb）/ soak_dual_threshold PASS（iterations=409 seconds=1806.0 active=1806.0 outer=1806.0 sleep=0.0 failures=0 frames=13088 registries=409）/ budget_strict PASS（212 pass, 0 skip, strict mode）/ legacy_criteria_0byte PASS（committed 闭集=['ci/g10_gap_registry_lib.py']；工作树闭集=['milestones/g12/g12_pt_sampler_selection.json'] 异己登记）/ date_anchor PASS（utc_date=20260819）——evidence/g13_stabilization_soak_20260819T090227Z.json。
  - 守卫套件全 PASS（逐字输出行）：`py -3 ci/check_structure.py` → `[check_structure] PASS`；`py -3 ci/check_schemas.py` → `[check_schemas] PASS`（本批 soak/回归 evidence 经 G13.5a 三前缀路由校验过）；`py -3 ci/check_number_ledger.py` → `[check_number_ledger] PASS`（CI_step on_tree_max 245 / next_free 246 校准后实测）；`py -3 ci/budget_eval.py --strict` → PASS（212 pass / 0 skip / 零 estimated）。
  - 互锁维持：`py -3 ci/g13_interlock_check.py --require-ready` → **VERDICT=READY，exit=0**（G13.5a 全程 implementation_status=unlocked 维持）。

- **④ 门序 / 偏差 / not-triggered 登记面摘要**：
  - **门序**：G-G13-7（决策门）+ G-G13-8 前半（M-e P0）于 commit `563bda68` 留痕（§8.6）→ 本波 G-G13-8 后半（soak 稳定门）兑现；数字步骤 245 按落盘前实测 actual next_free=245 顺位领取（ledger v1.140 校准同批）；G13.5b close-out（g13.wave.5b.closeout，步骤 246 预留面——脚本 ci/g13_closeout_check.py 本批同模 materialize 在树未提交面，workflow/ledger/schema 路由注册随 5b 批次，禁推测号口径维持）。
  - **UE 厂商随机运行间方差事件（全程取证—处置—再锚定—承接锚登记，本波最大偏差）**：
    - **检出**：本波首跑 soak（evidence/g13_stabilization_soak_20260819T034127Z.json，FAIL 件在档 0-byte 不删）——回归腿 M-c/M-d 子进程红：「登记表漂移（在树非逐字节相等）」硬门触发；M-c 新跑 evidence（g13_m_c_ue_upscale_parity_20260819T043101Z.json，FAIL 件在档）bistro-interior 探针档噪声谱 ue_hf_share=0.8518624602043453 ≠ G13.4 锚定登记表值 0.854056（232935Z 跑产）；M-d 同型（g13_m_d_ue_lumen_gi_parity_20260819T052207Z.json，FAIL 件在档）。
    - **取证（四跑独立进程实测数据集）**：bistro-interior/tier67 UE 臂末帧 HF share 四次独立真跑 = 0.854056（232935Z，G13.4 锚）/ 0.851862（043101Z）/ 0.854550（075128Z）/ 0.852789（095337Z，本 soak 回归腿锚）——最大两两相对差 ≈0.32%；**Rurix 侧同输入四跑位级一致**（rurix_hf=0.3622313729720505 逐位不变，双跑位级门逐跑核验在案）；cornell-box UE 侧四跑恒为结构零值 0.0（稳定）。根因判定 = **UE 5.8.1 deferred+Lumen+DLSS MRQ 臂 32 帧静态收敛序列末帧残余随机噪声（Lumen 随机采样面）跨进程运行间方差**——厂商侧非确定性，非我方链路缺陷，非帧库/资产/驱动内容变更（契约 digest 三方机核逐跑全等 == 冻结注册值 + ue_build_id == M128 机核逐跑在案）；G12 UE PT 臂无此面（固定 seed 路径追踪确定性收敛 + M163 门无逐字节冻结核验——本冻结核验 = G13.4 gaplib 正典化引入面，设计意图 = 世界内容变更检出，对厂商随机方差构成误报类）。
    - **处置（本波窗口内，如实登记不冒充结构性修复）**：漂移态双登记表归档至 .tmp\g13_5_drifted_registries\（取证件不入 commit）后删除在树旧表，M-c/M-d 门复跑产「首落盘」路径重建登记表（075128Z/085309Z 独立验证跑 → 本 soak 回归腿 095337Z/104418Z 再锚定跑）——**再锚定口径 = 登记表锚定最终确认跑（soak 回归腿）实测值**，gap_id 身份五节派生与测量值解耦（八+二行 id 集四跑逐字不变，机核在案），行集超容差判定四跑稳定（方差 ≪ 容差带，行集零翻转）；本 soak 全绿即建立在此再锚定面上。 FAIL 三件（034127Z soak / 043101Z M-c / 052207Z M-d）全部在档作诚实轨迹。
    - **结构性修复承接锚（G14 治理波法定输入，本波如实登记不静默）**：M-c/M-d 门「在树逐字节相等」冻结核验对厂商随机方差不稳健——G14 立项候选决策表**必须消费本锚**：登记表对账升级为结构化核验（身份面〔gap_id 集/场景集/metric/kind/模块归属/行数〕逐字节 + Rurix 侧测量值位级一致 + UE 侧测量值程序产方差带〔门内 UE 探针格双跑方差底 ×headroom，n≥5 标定样本量评估，P-09 程序产禁手写〕——真实内容变更（≫方差带）检出面维持，厂商随机方差吸收），并调研 UE 侧确定性控制面（r.RandomSeed/MRQ warm-up/收敛帧数）压缩方差底；本锚未兑现前任何 G14+ 复跑 M-c/M-d 全门（--gate）须先完成该修订，G15 全场景双端出图终审为其天然兑现窗。
    - **stale-FAIL 序贯教训（沿 §8.6 同模登记）**：首跑 soak FAIL 件（034127Z）与回归腿 FAIL 两件（043101Z/052207Z）保留在档不删不改；复跑后同 subject 新 PASS 件转绿，陈旧 FAIL 件不冒充当次面。
  - **not-triggered / 维持 open 面**（如实保持 open，不写进全绿叙述）：G13_P2_DECISIONS 24 行 defer-to-G14+ 承接锚字面 0-byte（G10-N11/N16/G11-N3/M114-strand/M100-high 锚定 G14 性能对标期；G11-N8/N9/G12-N10/N12/G13-N8/N9 锚定 G15 画质收口期；G12-N13 M-e 监控臂逐波登记〔本波零检出〕）；G13-N6 异己面 no-go 维持；SG-010 软保留 not_triggered 维持；G13.5b close-out 未跑（下一波面）；G14/G15 未立项。
  - **异己并发工作树面**：本批只含 G13 车道文件（块⑤清单按文件名显式择取）；异己会话 src/ 未提交面（apps/、src/rurix-asset、src/rurix-render 多面、src/rurix-rt/render_exec.rs、milestones/g12/g12_pt_sampler_selection.json 及各 evidence/ 异己新件）维持未提交、零消费、零混入（立项裁决 1）。

- **⑤ 签署块**：白栀（依 10 §7 / P-13 / D-406 v2.0 agent 完全自主签署，G10/G11/G12 §8.x 同模）。`Assisted-by: Kimi-K3（G13.5a soak 波）`（影响范围：milestones/g13/g13_ue_upscale_gap_registry.json + g13_ue_lumen_gap_registry.json 双表再锚定回写〔soak 回归腿 095337Z/104418Z 实测值，gap_id 闭集 0-byte〕+ 本契约 §8.7 本条 + .github/workflows/pr-smoke.yml〔步骤 245，步骤 244 块后追加〕+ registry/number_ledger.json〔CI_step 244→245/next_free 246 + revision_log v1.140〕+ evidence 本批真跑件〔g13_stabilization_soak_20260819T090227Z PASS + 首跑 FAIL 轨迹三件 034127Z/043101Z/052207Z + M-a 090228Z 四件 + M-b 090602Z + M-c 095337Z 四件 + 075128Z 独立验证跑 + M-d 104418Z 四件 + 085309Z 独立验证跑 + M-e 105009Z + 决策门 105410Z + wave2/3/4 exit 105405~105410Z 六件 + M-e 抽检产 g9_m96 105034Z/g10_m130 105301Z/g10_m139 105403Z/g10_m140 105403Z/g12_wave5_exit 105404Z〕；验证方式：块③逐字命令输出——soak 门六 facts PASS + 回归腿 5 P0 真跑 22/18/20/19/9 全绿 + 4 聚合/决策门 PASS + selftest 红绿留痕 + 守卫套件全 PASS + 互锁 --require-ready READY 维持）。

---

### §8.8 G13.5b close-out 终审记录（2026-08-19）——G-G13-9 收口门：八 facts 全 PASS，VERDICT=READY；超分/Lumen 双差距登记表 8+2 行终态（全 quality_gap/P2）终审锁定——残余差距/未闭环行如实登记不冒充全闭环，锁定面 = G14/G15 法定输入；status flip 独立 commit

- **① 独立断言全绿清单（终审门一行——host 只读终审面，不重跑子门）**：

  | gate（symbolic key） | 独立布尔断言 | host/device | evidence 路径 | 结果 |
  |---|---|---|---|---|
  | `g13.wave.5b.closeout`（步骤 246 终审门） | 八 facts 只读终审：①5 P0 key（M-a~M-e，G13_ACCEPTANCE_MAP §1 实记，与 ci/g13_stabilization_soak.py P0_GATES 同一闭集）逐门 PASS（wel 口径 + 顶层 status==pass 字面——G13 证据形态统一无豁免面）②wave2/3/4 exit + wave5 decisions + wave5a soak 五聚合/决策门全 PASS ③g13_acceptance_map_check 双向 exit=0 ④P2 表 31 行闭集最终状态无漂移（最新 evidence host_section_pass + FROZEN_IDS 在树，复用 ci/g13_p2_decisions_check.py 闭集单一事实源）⑤budget_eval --strict 非空零 estimated/skip ⑥5a full-run 先行（最新 g13_stabilization_soak evidence host_section_pass，base_commit_5a=563bda68 留痕；立项裁决 7 同日放行）⑦RD 最终状态逐字一致（deferred.json RD-034/039/040/041/042/043/044 七条目级 status 全 open 逐字 + G13_P2_DECISIONS 31 行 FROZEN_IDS 闭集在树两面一致；G13 无 defer 重评窗表，全表深对账由 g13.wave.5.decisions 门承载不重复）⑧超分/Lumen 双差距登记表终审锁定（g13_ue_upscale_gap_registry.json 8 行闭集 + g13_ue_lumen_gap_registry.json 2 行闭集：gap_id 集 == 本门冻结清单逐字对账 + 计数 8/5/3 与 2/1/1 重算一致 + generated_by == M-c/M-d 门字面 + 全行 quality_gap/P2 + measured_delta 非空 + delta==b−a 可溯源 + evidence_digest 溯源 + g11_anchor 锚定 G15）+ 最后新绿 UTC 日留痕 | host 只读（不重跑子门 smoke、不设 RURIX_REQUIRE_REAL；MAP 双向机核门 --gate 真跑产新鲜 evidence） | evidence/g13_wave5b_closeout_20260819T114727Z.json（facts 8/8 + checks 八键全真 + required_gates 10 行） | READY |

- **② 终审门实测输出**：`py -3 ci/g13_closeout_check.py --gate g13.wave.5b.closeout` → **VERDICT=READY，exit=0**（evidence/g13_wave5b_closeout_20260819T114727Z.json）——八 facts 全 PASS：`five_p0_pass`（pass=5/5）/ `wave_gates_2_to_5a`（pass=5/5）/ `acceptance_map_check`（g13.wave.1.acceptance_map exit=0，新鲜件 g13_acceptance_map_check_20260819T114725Z.json）/ `p2_decisions_31_frozen`（g13_p2_decisions_20260819T105410Z.json，frozen_31_in_tree=True）/ `budget_strict`（exit=0，212 pass 0 skip）/ `soak_5a_precedes`（g13_stabilization_soak_20260819T090227Z.json，base_commit_5a=563bda686beb7959a6288ed3165c9a989ad5b3ff，立项裁决 7 同日放行）/ `rd_final_state_consistent`（7 RD open 逐字一致 + P2 31 行闭集在树，G13 无重评窗表两面一致）/ `gap_registries_locked_and_green_recorded`（双表 gap_id 集对账 + 计数 8/5/3 与 2/1/1 重算一致 + 全行 quality_gap/P2 + measured_delta 对账齐 + 锚定 G15；last_green_utc=20260819 today=20260819 missing=[]）。required_gates 10 行（5 P0 + wave2/3/4 exit + decisions + soak）逐行 status==PASS；checks 八键闭集全真；utc_date=20260819、last_new_green_utc_date=20260819。

- **③ 验收命令逐字输出（2026-08-19 真跑留痕，仓库根目录）**：
  - `py -3 ci/g13_closeout_check.py --gate g13.wave.5b.closeout` → **VERDICT = READY**（八 facts 逐行打印全 PASS，字面见块②）。
  - `py -3 ci/g13_closeout_check.py --selftest` → `[selftest] OK materialized step 246`。
  - `py -3 ci/g13_stabilization_soak.py --verify-latest` → `[g13_5a_soak] verify-latest PASS（g13_stabilization_soak_20260819T090227Z.json，checks 7 键全绿 + soak 判读面复核过）`。
  - 守卫套件全 PASS（逐字输出行）：`py -3 ci/check_structure.py` → `[check_structure] PASS (11 dirs, 6 files)`；`py -3 ci/check_schemas.py` → `[check_schemas] PASS`（G13 前缀路由落——本波新增 g13_wave5b_closeout_ 一件，既有 0-byte）；`py -3 ci/check_number_ledger.py` → `[check_number_ledger] PASS`（CI_step on_tree_max 246 / next_free 247 校准后实测）；`py -3 ci/budget_eval.py --strict` → `[budget_eval] PASS (212 pass, 0 skip, strict mode)`；`py -3 ci/g13_interlock_check.py --require-ready` → VERDICT=READY（事实门④实测 workflow 末号 246 == ledger on_tree_max 246 == next_free−1）；`py -3 ci/g13_acceptance_map_check.py --gate g13.wave.1.acceptance_map` → PASS（双向机核，新鲜件 114725Z）。

- **④ 门序 / 终审锁定 / 纪律面摘要**：
  - **门序**：G-G13-3 互锁 READY（§8.2）维持 → G13.5a（§8.6/§8.7）→ 本波 G-G13-9 终审门 materialize（纯治理核验面无新 RXS 条款/conformance 语料/src 改动）；数字步骤 246 按落盘前实测 actual next_free=246 顺位领取（ledger v1.141 校准同批）；同日放行先例继承（立项裁决 7：5a full-run 先行完成后允许同日 close-out——5a 090227Z 先行完成留痕）。
  - **超分/Lumen 双差距登记表终审锁定（G-G13-9 字面兑现）**：`milestones/g13/g13_ue_upscale_gap_registry.json` 8 行终态（cornell t50/t67 dlss_sr deficit 2 行 + probe 档噪声谱双端差 6 行）+ `milestones/g13/g13_ue_lumen_gap_registry.json` 2 行终态（cornell/bistro GI 能量 + 间接光派生 LDR 双端差）——全 quality_gap/P2（measured_delta 非空 + delta==b−a 可溯源 + evidence_digest 溯源 receipt + g11_anchor 锚定 G15）；计数重算一致（upscale 8/cornell 5/bistro 3；lumen 2/cornell 1/bistro 1；not_ready_scenes==[]）；generated_by == M-c/M-d 门字面。**残余差距/未闭环行如实登记不冒充全闭环**；终审锁定 0-byte（终审不重写清单本体）——**锁定面 = G14/G15 法定输入**：G14 帧率对标期与 G15 画质收口期只消费本双表、G12 表 10 行终态与 G13_P2_DECISIONS 承接锚，不得另起无锚差距面；G13-N8/N9 锚定 G15 逐项重评面维持。
  - **RD 最终状态**：RD-034/039/040/041/042/043/044 七条维持 open 逐字一致（分项 go/no-go/defer 已由候选决策表、G13_P2_DECISIONS 31 行闭集与 deferred history 只追加留痕；G13 无 defer 重评窗表，全表深对账由 g13.wave.5.decisions 门承载不重复）。
  - **G14 法定输入面清点（如实登记）**：G10-N11（三轮进程级独立运行 + MRQ 开销剥离口径）/ G10-N16 + G11-N3（GPU 管线双端 A/B 出图与帧率/画质 measured 面）/ M114-strand + M100-high（G14 性能对标期 benchmark/多灯对照窗）锚定 G14；§8.7 结构性修复承接锚（M-c/M-d 登记表 UE 方差带结构化对账修订 + UE 确定性控制面调研）= G14 治理波候选决策表必须消费行；G13-N7 FG/MFG 帧生成独立层另判维持。
  - **not-triggered / 维持 open 面**：M52 / M61 / M98-l4 / M118-hdr-cal / M125-adopt3 / M127 / SAFE-GPU / G10-N6/N8/N17 / G11-N5 / G11-N8 / G11-N9 / G12-N10 / G12-N12 / G12-N13（M-e 监控臂逐波登记，G13.5a 零检出）/ G13-N6 / G13-N7 / G13-N8 / G13-N9 与 SG-010 软保留 not_triggered——§8.6/§8.7 登记字面 0-byte。
  - **纪律面**：只读终审不重跑子门 smoke、不设 RURIX_REQUIRE_REAL；G5~G12 closed 判据与既有门脚本 0-byte；evidence/ 只增不删不改；RD 条目级 status 与四字段 0-byte；双差距登记表终态 0-byte；异己会话 src/ 未提交面维持未提交、零消费、零混入（立项裁决 1）；本批 commit 只含本车道文件（块⑤清单按文件名显式择取）。**VERDICT=READY ⇒ status flip 独立洁净 commit（active→closed + §8.9 签署块）**。
