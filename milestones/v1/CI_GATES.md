...
| v1.1 | 2026-07-14 | **V1.2 步骤 50 落地回填(G-V1-3,MR-0008,RXS-0185~0186)**:计划步骤 50 落地为 `.github/workflows/pr-smoke.yml` 真实步骤 `ci/channel_manifest_smoke.py`(步骤 49 之后;**纯 host/CPU-only,不 SKIP 充绿(除无 cargo 工具链)**)——① green:`rurixup release`(缺省 channel=stable)产出 channel_manifest.json,channel=stable / rurix_version==workspace 版号 / bundle_manifest_sha256==bundle.json 字节流 SHA-256(内容寻址引用)/ components 与 bundle 全集对应;② 确定性:同一输入两次生成逐字节一致(无时间戳);③ red→绿闭合:`--simulate-channel-drift` → 第 8 子门 channel-manifest 红 → 发布阻断 exit 2 / `--channel nightly` → 用法错误 exit 1(零新 RX 码)/ 复原绿(反 YAML-only)。`release.yml` 同步:Release pipeline gate 之后加同款步骤 + upload-artifact path 追加 channel_manifest.json / gate_decision.json。配套:spec/release.md v1.3(RXS-0185/0186 条款体)+ `src/rurixup/src/channel.rs`(+gate 第 8 子门/lib 编排/main CLI)+ stable 快照重 bless 180→182(bless_log 同 diff 追加)。本机真实红绿:`py -3 ci/channel_manifest_smoke.py` PASS(green + 确定性 + 漂移红 + 未知 channel 红 + 复原绿);`cargo test -p rurixup` 14 passed;步骤 49 `ci/edition_smoke.py` 复绿。run URL 随 PR 合入后归档契约 §8(不伪造) |
