...
### §8.6 G-G11-8 G11.6 P2/留档/未触发分项穷举决策波验收记录（2026-08-16/17）——**28 行闭集穷举零空行：go 3 closed-go 留痕 + no-go 6 + defer-to-G12+ 19；步骤 214 门 PASS 33/33 facts，no-go/defer 如实保持 open 不写进全绿叙述、不阻塞 G11.7a soak**

- **波次来源与法定输入**：G-G11-8「G11 期全部 P2/留档/未触发分项逐条 go/no-go/defer-to-G12+，零空行；defer 必有承接锚（机核同构 ci/g10_p2_decisions_check.py）；no-go/defer 如实保持 open，不阻塞 soak 且不得写进全绿叙述」。候选全集 = [G11_CANDIDATE_DECISIONS.md](G11_CANDIDATE_DECISIONS.md) v1.0（39 行实记全集）未进 14 key 验收面者 20 行（§2 defer 15 + §4 G11-N3 + §3 RD 级 RD-034/042/043/044）+ G11.2~G11.5b 期内新增 not-triggered/no-go/留档/closed-go 登记面 G11-N4~G11-N11 八行（逐波验收记录 §8.2~§8.5b 穷举），去重后 **28 行闭集**（`ci/g11_p2_decisions_check.py` 内置 `FROZEN_IDS` 逐字对账）；已进 14 key 验收面独立绿的 go 16 行（§1 法定输入 11 + §2 M99-clipmap/G10-N7/G10-N10 + §4 G11-N1/G11-N2）不进表（沿 G9/G10 范式）。
- **① 决策表落地**：[G11_P2_DECISIONS.md](G11_P2_DECISIONS.md) v1.0——**go 3 行**（G11-N4 M147 判据双 phase 修订留痕 / G11-N6 G11.4 串扰标定中间件删除留痕 / G11-N7 G11.5 首跑 FAIL→G11.5b 诊断修复→复测收敛全链留痕，均 closed-go 有 evidence 在树不再产生后续分项）+ **no-go 6 行**（RD034 blocked 维持 / RD042、RD043 观察维持 / RD044 maintain_no_go 维持 / G11-N10 HEAD 预存 fmt-clippy 漂移面 G11 零修复纪律不回写〔G10-N15 同族〕/ G11-N11 异己并发会话 src/ 未提交面维持未提交不混入〔G10.8b §8.10 先例同模〕）+ **defer-to-G12+ 19 行**（G10 defer 承接 15 行承接锚字面 0-byte 逐字承接 + G11-N3 锚定 G14 + G11-N5 度量口径修订评估面 + G11-N8/G11-N9 锚定 G15；每行承接锚「重判条件 = …；兜底 = …」+ G12+ 重评窗字面齐备）+ strategic_override 0 行；零空行（全列非空机核）。**触发评估兑现三行**：M100-high（G11.6 兑现：A/B 复测臂 = host CPU 参考管线未消费低档 MegaLights GPU 管线，多灯 measured 对照未产出，证据未齐备维持 defer）/ G10-N6（G11.3 兑现：U2 修复消费面 = DDS→PNG 派生转码链非 FBX2glTF 替代臂，未触发维持 defer）/ G10-N17（G11.5 兑现：复测闭环经门侧独立重算面消费标量，diff 报告 `scalars.flip` 字段面消费需求不成立，维持 null 演进位）——触发评估如实兑现不以绿色冒充触发。
- **② 门禁落地（步骤 214，落盘前实测 `CI_step.next_free=214` 顺位领取）**：`ci/g11_p2_decisions_check.py --gate g11.wave.6.decisions`（骨架期行级机核扩 28 行闭集 materialize，同 G10 先例「骨架期行级机核 → materialize 期扩闭集」；host 普通检查门非聚合门，required_gates 空）——行级机核（裁决枚举合法 go/no-go/defer-to-G12+/strategic_override + 零空行 + 承接锚「重判条件+兜底」字面 + defer 行 G12+ 重评窗 + go 行 evidence 义务 + no-go 行 RD/矩阵/契约锚义务）+ 三横向机核（① G11_ACCEPTANCE_MAP 14 key〔13 P0 + 1 已 go P1〕互斥——P2 行 ID 无一命中已 go M### 裸 token，M98-l4/M100-high 子项级 key 不互斥沿 G9/G10 范式；② deferred.json history 对账——G11.6 P2 登记恰好 RD-039 +1〔M61〕/RD-040 +3〔M52/M99-clipmap/M100-high——M99-clipmap = G11.4 承接兑现完结登记〕，零新 RD max=RD-044，RD-039/040 条目级 status=open 0-byte；③ G11.1 候选决策表对账——15 行 G10 defer 承接 + G11-N3 == CANDIDATE §2/§4 行集 defer-to-G12+ 字面逐字承接 n=16/16）。evidence schema `milestones/g11/g11_p2_decisions_evidence_schema.json`（numeric_step const 214）+ `ci/check_schemas.py` 三处纯追加（load/validator/`g11_p2_decisions_` 前缀路由，与既有全族互不包含，既有路由 0-byte）+ pr-smoke.yml 步骤 214（步骤 213 块后追加）+ CI_GATES v1.7 修订行（§5 表体 0-byte）+ ledger CI_step on_tree_max 213→214、next_free 214→215（v1.121）同批。
- **③ 门禁真跑实测输出（evidence 落盘 evidence/g11_p2_decisions_20260817T044009Z.json）**：`--gate g11.wave.6.decisions` **PASS 33/33 facts**（set_equality_frozen 28/28 unique + no_duplicate_ids + 28 行 row_* 全 PASS + acceptance_map_mutex〔MAP 实解 P0=13 P1=1，命中已 go 裸 token：无〕+ deferred_history_reconcile〔RD-039 ×1 / RD-040 ×3〕+ candidate_decisions_reconcile〔n=16/16〕，VERDICT=PASS）；`--selftest` **ALL PASS 10 检**（真表 28 行绿 + 合成全表绿 + 缺行→红 + defer 缺 G12+ 承接锚→红 + 非法裁决枚举→红 + 互斥违例〔已 go P0 裸 token 入表〕→红 + 空单元格→红 + deferred history 缺登记→红 + 候选决策表对账失配→红——红绿双全）。
- **④ deferred.json history 只追加四条（2026-08-16 G11.6 P2 行）**：RD-039 +1（M61——G11.6 P2 穷举裁决 defer-to-G12+，承接锚字面 0-byte 维持，G12+ 重评窗顺延）/ RD-040 +3（M52 defer-to-G12+ 锚定 G12 / M99-clipmap G11.4 承接兑现完结登记——M154 门闭环 PASS + RXS-0360 世界级登记翻转修订行落地，RD-040 总体 open 维持 / M100-high G11.6 触发评估兑现=证据未齐备维持 defer-to-G12+）；零新 RD（max=RD-044 不动）；RD-034/039/040/041/042/043/044 条目级 status 与 id/title/reason/backfill_condition 四字段 0-byte；SAFE-GPU/M127/M98-l4/M114-strand/M118-hdr-cal/M125-adopt3/G10-N5~N17/G11-N3~N11 无 RD 条目归属**不新设 RD**（沿 G9.7/G10.7 先例），留痕位置 = 决策表 + CANDIDATE + 本契约 §8.2~§8.5b 登记面。
- **⑤ 验收命令逐字输出（守卫套件）**：
  ```text
  py -3 ci/g11_p2_decisions_check.py --gate g11.wave.6.decisions
    → PASS 33/33 facts，VERDICT=PASS（evidence g11_p2_decisions_20260817T044009Z.json）
  py -3 ci/g11_p2_decisions_check.py --selftest
    → ALL PASS 10 检（真表绿 + 合成全表绿 + 八负样本全红）
  py -3 ci/check_schemas.py
    → PASS（g11_p2_decisions_ 前缀路由消费新 evidence，既有路由 0-byte）
  py -3 ci/check_number_ledger.py
    → PASS（CI_step on_tree_max=214/next_free=215，v1.121 登记在树）
  py -3 ci/check_g11_acceptance_map.py
    → PASS（13 P0 + 1 已 go P1 三向逐字一致；P2 表互斥零命中）
  ```
- **⑥ 纪律面与异己并发工作树留痕**：no-go/defer 25 行如实保持 open/留档/观察/blocked，**无一行写进全绿叙述、无一行阻塞 G11.7a soak**（G-G11-8 字面）；G5~G10 closed 契约与 00~14 文档 0-byte；G10_P2_DECISIONS 27 行与 G10_DEFER_REEVALUATION 十锚行裁决字面 0-byte 不回写；G11_CANDIDATE_DECISIONS 39 行裁决字面 0-byte 不回写；既有门脚本与 evidence/ 只增不删不改（G11-N6 登记的串扰中间件删除为 G11.4 已结案受控例外留痕）；RXS/RD/U/SG/RX_error/MR/D/RFC 字段 0-byte（本波纯决策面零新 spec 条款/零新 RD/U/SG/MR/RFC）。**工作树并发安全留痕**：本波期间异己会话持续活跃（src/rurix-asset/rurix-render 未提交面 + evidence/d3d12_interop_smoke.json 异己改写面维持未提交不混入；pr-smoke.yml/registry/number_ledger.json 本车道编辑一度被并发回退后重放落盘并以 git hash-object 逐文件复核）；决策表初版 13 行裁决列带括号批注形态经 selftest 红绿校准为裸枚举字面（批注全额保留于裁决理由/最终状态列，G10 表同构口径），判据语义 0-byte。本批 commit 按文件名显式择取、异己面零混入（立项裁决 1 / MAP §3.1 / G10.8b §8.10 先例同模）；新文件 LF + 尾换行。
- **签署**：白栀（依 10 §7 / P-13 / D-406 v2.0 agent 完全自主签署，G10 §8.1 / G11 §8.2/§8.3/§8.3a/§8.3b/§8.4/§8.5/§8.5b 同模）。`Assisted-by: Kimi-K3（G11.6/G11.7 收口波）`（影响范围：§8.6 本条 + 同批文件集（② 节字面）；验证方式：③ 节门禁真跑实测 + ⑤ 节验收命令逐字输出 + G11_P2_DECISIONS.md v1.0 28 行闭集在树）。

### §8.7a G-G11-9 G11.7a stabilization soak 验收记录（2026-08-17）——**19 门全量回归真跑全绿 + 修复链路 soak 29 迭代 1859.9s ≥1800s 零失败 + budget strict 166 pass 0 skip + G5~G10 既有判据 0-byte；VERDICT=PASS**

- **波次来源与法定输入**：G-G11-9「G11.7a 稳定门：全部 P0 与所有 go 的 P1 全量回归；G5~G10 既有判据 0-byte；修复链路（复测出图/度量/差距清单装配）连续复跑 soak（量级沿 G10.8a 继承〔≥1800s〕或 measured 证明更短足够，阈值 G11.1 裁决 measured 标定）；strict budget 非空、零 estimated/skip；同日放行按立项裁决 7（7a full-run 先行完成后允许同日进 7b）」。门 = `ci/g11_stabilization_soak.py --gate g11.wave.7a.soak`（步骤 215，落盘前实测 `CI_step.next_free=215` 顺位领取；同构 ci/g10_stabilization_soak.py 全量形态），pr-smoke 侧 = `--verify-latest` 秒级核最新 full-run evidence（沿 G10.8a 步骤 194 体例）。
- **① 全量回归 19 门真跑全绿（同一候选 close-out 基线 base_commit=`413c58af`）**：13 P0 + 1 go P1 = 14 key 逐门 `--gate` 真跑后机器核验最新 evidence 顶层 `status=="pass"`——M144/M145/M146/M157（G11.2 口径对齐四门）/ M147（`--phase g11.3` 登记面腿 12/12，**双 phase 机核**：最新 evidence phase==g11.3 + 最新 g11.5 phase 件 closure.verdict==converged，§8.3a definitive 收敛断言面不遮蔽）/ M148 14/14 / M149 14/14 / M150 15/15 / M151 15/15 / M152 13/13 / M153 15/15 / M154 18/18 / M155 27/27 / M156 16/16（48 门 + G11 已绿门全绿汇总 + M130 双 phase/M139/M140/M141/G9 M96/M94/M110 八面真跑抽检零降级——含 M141 全量 benchmark 墙钟同协议复跑，G10-N11 登记面兑现）+ wave2/wave3/wave4/wave5 exit + wave6 decisions 五聚合/决策门真跑核验全 PASS；14 门 evidence `base_commit` 同值==HEAD 且 19 门 evidence 文件名 UTC stamp ≥ run 起点新鲜度机核（沿 G10.8a MAP §7 口径）。
- **② 修复链路 soak ≥1800s 诚实全绿**：`iterations=29 / seconds=1859.9 / active_chain_seconds=1859.9（≈seconds）/ sleep_seconds=0 / failures=0 / subject='chain-soak'`，计数非空——rurix_frames_rendered=58 / ldr_derivations=116 / frames_decoded=464 / metric_row_recomputes=319（29×11 行）/ gap_registry_assemblies=29；gate 外测墙钟交叉核验无谎报。链路逐迭代真跑（与 M155 门数据面同构）：Rurix HDR release 全修复旗标面（--material-pbr/--light-seed-set/--gi-multibounce/--sky-ibl 双场景同消费）重渲染 digest 逐位复现 g11_5b 帧区库帧（复测出图）→ LDR 派生双臂 ×1.0 逐字节复现（派生）→ 11 行复测 delta 门侧独立重算 == `g11_5b_rerun_report.json` closure_faces 登记值逐位（度量）→ 复测差距清单 `stage_registry` 当次重装配与在树幂等复核（**rerun_report_digest 自证指针字段外逐字段全等**——G11.5b 落盘时序既存差异〔在树清单登记 `6fb18192…` vs 当前报告 `7ea9eb62…`〕如实登记不重写 G11.5b 终态 0-byte，重装配值==当前报告 digest 机核，装配后在树回写 0-byte 恢复）（差距清单装配）。host CPU 参考管线链无 device 面，不以字面量 0 充 device 零错门；UE 帧为帧区只读解码面（不重复 MRQ，G10.8a 同口径）。
- **③ budget strict + ④ 日期锚 + 0-byte fact**：`budget_eval --strict` **PASS 166 pass 0 skip**（非空零 estimated/skip）；utc_date=20260817 纪律日期锚；**G5~G10 既有判据 0-byte**（`git status --porcelain -- ci/g9_*.py ci/g10_*.py milestones/g9 milestones/g10 spec conformance` 闭集面空集，与 M156 门 ⑤ 同闭集字面；异己 src/ 未提交面属立项裁决 1 登记面不在本闭集）。
- **④ 门禁落地与实测输出（evidence 落盘 `evidence/g11_stabilization_soak_20260817T064847Z.json`，VERDICT=PASS，checks 七键全真）**：`ci/g11_stabilization_soak.py`（四腿 + 0-byte fact）+ evidence schema `milestones/g11/g11_stabilization_soak_evidence_schema.json`（numeric_step const 215，required_gates 19 行，checks 七键闭集，soak 块 honesty 字段全必填）+ `ci/check_schemas.py` 三处纯追加（load/validator/`g11_stabilization_soak_` 前缀路由，与既有全族互不包含，既有路由 0-byte）+ pr-smoke.yml 步骤 215（步骤 214 块后追加）+ CI_GATES v1.8 修订行（§5 表体 0-byte）+ ledger CI_step on_tree_max 214→215、next_free 215→216（v1.122）同批。**红绿双全**：`--selftest` ALL PASS（5 红臂——sleep 充墙钟/外测墙钟戳穿谎报/迭代不足/计数面空/缺 honesty 字段 + 1 绿臂诚实样本）；装配腿单迭代实测 66s 全绿后启动 full-run；`--verify-latest` PASS（pr-smoke 腿语义全绿）。
- **⑤ 验收命令逐字输出（守卫套件）**：
  ```text
  py -3 ci/g11_stabilization_soak.py --gate g11.wave.7a.soak
    → 19 门回归全 PASS（base_commit=413c58af 同值）+ soak 29 迭代 1859.9s 零失败
    → budget 166 pass 0 skip + 0-byte 闭集面空集 + date 20260817 → VERDICT=PASS
  py -3 ci/g11_stabilization_soak.py --verify-latest
    → PASS(honest chain soak)← evidence/g11_stabilization_soak_20260817T064847Z.json
  py -3 ci/g11_stabilization_soak.py --selftest → ALL PASS（5 红臂 + 1 绿臂）
  py -3 ci/check_schemas.py            → PASS（g11_stabilization_soak_ 前缀路由消费新 evidence，既有路由 0-byte）
  py -3 ci/check_number_ledger.py      → PASS（CI_step on_tree_max=215/next_free=216，v1.122 登记在树）
