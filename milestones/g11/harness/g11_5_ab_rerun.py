...
"""G11.5 A/B 复测波 同契约双端全量复跑驱动（G11_CONTRACT §4.2 M155 + G-G11-7；
g10_gap_registry 11 行锁定清单 0-byte 消费 + 逐项承接锚字面；spec/visual_comparison.md
RXS-0393 修复闭环判据〔quality_gap 款 / caliber_diff 款分款机核〕+ RXS-0392 口径对齐面；
契约 §8.3a M147 双 phase 修订句——R1 行收敛断言 definitive 测量面 = 本驱动同契约复跑）。

链路面（与 G11.2/G11.3/G11.4 同构；G10/G11.2/G11.3/G11.4 帧库只读——本批全部产物落
G11.5 帧区 K:/rurix-ext/g11-frames/g11_5/）：

  契约参数（digest == G10.5 锁定值 0-byte，三面绑定机核——不等即 SystemExit 停线，
  不等仍出报告即 RED 门序硬约束）→
  ├─ Rurix 端：g10_5_scene_render --render --exposure-scale 2^(−EV100)（C2 对齐面
  │   维持）+ 全修复旗标——bistro = `--material-pbr --light-seed-set <契约光照>
  │   --gi-multibounce`（R1/U2 材质纹理 + R3 灯种子集 + R4 多反弹/世界级辐射缓存
  │   全修复面）；cornell = `--smooth-normals --gi-multibounce`（R2 法线 + R4
  │   cornell 投影面，契约 sun+sky 灯面 0-byte）；parity 面 = 双场景无旗标
  │   （== G10.5 锁定 digest 回归锚）→ HDR EXR + stdout JSON 闭集块
  │   （animations/materials/lights/world_cache 计数面）
  └─ UE 端：双场景全量重跑——g10_5_build_scenes.py（G10_5_SCENE 逐场景：cornell
     壳体双面化〔U1〕/ bistro DDS→PNG 派生链 + MIC 绑定〔U2〕+ 点光源 4 盏
     spawn〔R3〕）+ MRQ Phase B 双场景出帧（G11_2_OUT_ROOT → g11_5 帧区）
  双端 HDR → LDR 派生（×1.0 双端统一，C2 对齐后口径）→ HDR 覆盖/亮度统计 +
  FLIP/SSIM/PSNR（ci/g10_*_lib 单一事实源）→ g11_5_rerun_report.json
  （11 行修复前后 delta 对账面：锁定基线 = g10_gap_registry 0-byte 消费 + G11.2
  域统一换算面）→ registry 段：复测差距清单 11 行闭集落盘
  milestones/g11/g11_5_retest_gap_registry.json（行集 == G10.8b 锁定清单逐字对账；
  逐项 closed/converged/partial/aligned_closed 显式判定——partial/未收敛行如实登记
  不充绿并带 G12+ 承接锚；收敛阈 = g11_budget g11.fix.* 标定条目消费〔标定程序产，
  禁手写〕；quality_gap 行收敛判据按 RXS-0393 L2 字面，caliber_diff 行按 L2 同条款
  caliber_diff 款〔口径对齐完成 + 残余显式登记 + 复测 delta 与登记残余一致〕）。

G11 不设绝对画质通过线：全部数字 measured_local 登记，收敛判定归 M155 门机核；
R1 行不收敛则整波 FAIL（契约 §8.3a 不弱化声明）。

用法：
  py -3 milestones/g11/harness/g11_5_ab_rerun.py --stage all
  py -3 milestones/g11/harness/g11_5_ab_rerun.py --stage contract|rurix|ue|derive|metrics|registry
  G11_5_UE_COLLECT_ONLY=1 时 ue 段只解码既有帧补登记（不重跑 UE）。
"""
...
FRAMES_G11 = Path(r"K:\rurix-ext\g11-frames\g11_5")
...
# G11.5 全修复旗标面（G11.3 + G11.4 修复面合集 = definitive 复测面）。
SCENES = {
    "cornell-box": {
...
# G11.2 域统一换算基线（C2 对齐面：基线原域 a 未施曝光 ⇒ b − a×2^(−EV100) 对齐域
# 换算——M144/M153/M154 门登记面同一事实源字面）。
...
# 收敛阈消费面 = g11_budget g11.fix.* 标定条目 id（标定程序产，禁手写——M155 门核
# threshold == 标定 evidence results.trimmed_mean × k provenance）。
ROW_THRESHOLD_IDS = {
...
def log(msg: str) -> None:
    print(f"[g11_5_rerun] {msg}", flush=True)
...
def stage_contract() -> dict:
    ...
    G10.5 锁定值（修复旗标默认关——默认面与 G10.5 逐字节一致）；联合 digest 复核。"""
    ...
    # 联合 digest（M130 事实源口径：双场景各自 hex digest 字典序拼接 sha256——
    # ci/g10_dual_determinism_contract_smoke.py joint 计算同字面）。
...
def _render_one(scene_id: str, out_name: str, flags: list[str], exposure: float | None = None,
    ...
    #（G10.5 锁定 digest = 无 --exposure-scale 旗标面，G11.3/G11.4 parity 同口径）。
...
def stage_rurix() -> dict:
    """Rurix 端渲染：双场景全修复面 + parity 双场景无旗标面（独立目录，
    == G10.5 锁定 digest 回归锚）。"""
    out = {}
    for scene_id, s in SCENES.items():
        out[scene_id] = _render_one(scene_id, scene_id, s["rurix_flags"])
        log(f"Rurix {scene_id}（全修复面）: digest={out[scene_id]['frame_content_digest'][:32]}…")
    for scene_id in SCENES:
...
def stage_ue() -> dict:
    """UE 端双场景全量重跑：逐场景关卡建设（G11.3 双面化/DDS 派生链 + G11.4
    点光源面同消费——build_scenes 当前面）+ MRQ Phase B 双场景出帧。
    G11_5_UE_COLLECT_ONLY=1 时只解码既有帧与探针补登记（分阶段增量合并面）。"""
    out = {}
    collect_only = os.environ.get("G11_5_UE_COLLECT_ONLY") == "1"
...
def stage_metrics() -> dict:
    ...
    # U3 host 面（bistro 渲染输出 animations 闭集块对账——显式剥离登记后残余静默通道）。
    ...
    # C2 面尺度实测（复跑报告当次登记值——禁手写）：Rurix 管线内烘焙尺度 + LDR 派生尺度。
...
def stage_registry() -> dict:
    ...
    caliber_diff 款（口径对齐完成 + 残余显式登记 + 复测 delta 与登记残余一致）；
    partial/未收敛行如实登记不充绿并带 G12+ 承接锚）。收敛判定机器形态与
    ci/g11_3_fix_lib.evaluate_closure 同一判定层（0-byte 语义）。"""
    ...
    for lk in locked["items"]:
        ...
        if lk["kind"] == "quality_gap":
            ...
            if ev["converged"]:
                ...
                item["disposition_anchor"] = "G11.5 复测闭环（本清单终态）"
            else:
            ...
            # caliber_diff 款（RXS-0393 L2 同条款 C 族字面）：口径对齐完成（G11.2 门绿面
            # 由 M155 门机核）+ 残余口径差显式登记 + 复测 delta 与登记残余一致。
            if prefix == "C1":
                consistent = (
                ...
                item["attribution"] = (
                    ...
                    "收敛后残余 + 登记残余项 c1_ue_specular_ibl（UE 镜面 IBL 结构差，G15 画质量级收口面候选）"
                    "+ c3_source_bit_depth_quantization（源位深量化差，C3 行承接面）——无未归因余量"
                )
                item["residual_ids"] = [r for r in residual_ids if r.startswith("c1_")] + ["c3_source_bit_depth_quantization"]
            elif prefix == "C2":
    ...
    doc = {
        "schema_version": 1,
        "registry": "g11_5_retest_gap_registry",
        "generated_by": "milestones/g11/harness/g11_5_ab_rerun.py --stage registry",
        "upstream_registry": "milestones/g10/g10_gap_registry.json（G10.8b 终审锁定 11 行闭集 0-byte 只读消费；行集逐字对账）",
        "semantics": "G11.5 同契约双端复跑复测差距清单：11 行逐项闭环核验终态（修复前锁定 delta〔清单字面〕vs 复测 delta〔当次 measured〕→ RXS-0393 L2 分款收敛断言；收敛阈标定程序产禁手写；partial/未收敛行如实登记不充绿并带 G12+ 承接锚）；R1 行不收敛则整波 FAIL（契约 §8.3a 不弱化声明）",
        "contract_digest": dict(LOCKED_DIGEST, joint=LOCKED_DIGEST_JOINT),
        "frames_root": str(FRAMES_G11),
        "rerun_report": "milestones/g11/g11_5_rerun_report.json",
...
def main() -> int:
    ...
    report.update({
        "schema_version": 1,
        "report": "g11_5_ab_rerun",
        "generated_by": "milestones/g11/harness/g11_5_ab_rerun.py",
        "frames_root": str(FRAMES_G11),
        "locked_contract_digest": LOCKED_DIGEST,
        "locked_contract_digest_joint": LOCKED_DIGEST_JOINT,
    ...
    for st in stages:
        if st == "contract":
            ...
            result["registry"] = stage_registry()
        report["stages"][st] = "done"
        # 每阶段即时落盘（长 UE 段中断可续——分阶段增量合并面）。
        ...
        prior_commands = merged_commands
        COMMANDS.clear()
