...
### 8.4 操作者终审记录(2026-06-14)

**G-M4-1 measured 硬证据已回填。**本机 RTX 4070 Ti 锁频 L0 成功:
`nvidia-smi -lgc 2610,2610` + `nvidia-smi -lmc 10501`;读回 graphics/SM = 2610 MHz,
memory = 10501 MHz。随后执行 `py -3 bench/rurix_saxpy_triple.py`,三次进程级独立运行均为
`evidence_level=measured_local`:

| 证据 | trimmed_mean |
|---|---:|
| `evidence/rurix_saxpy_20260614_1.json` | 413.2139 GB/s |
| `evidence/rurix_saxpy_20260614_2.json` | 412.5728 GB/s |
| `evidence/rurix_saxpy_20260614_3.json` | 412.9157 GB/s |
| `evidence/rurix_saxpy_20260614_agg.json` | 412.9008 GB/s |

预算回填:`milestones/m4/m4_budget.json` 新增 numerator
`m4.bench.saxpy.effective_bandwidth_gbps = 412.9008 GB/s`;比值
`m4.ratio.saxpy_vs_m0_baseline = 1.0001`(M0 denominator = 412.8698 GB/s,阈值 0.95)。

`py -3 ci/budget_eval.py --strict` 输出:

```text
  PASS m0.bench.saxpy.effective_bandwidth_gbps: PASS — 412.870 GB/s vs min 392.23
  PASS m0.bench.bandwidth.h2d_pinned_gbps: PASS — 23.962 GB/s vs min 22.76
  PASS m0.bench.bandwidth.h2d_pageable_gbps: PASS — 20.337 GB/s vs min 19.32
  PASS m0.bench.bandwidth.d2h_pinned_gbps: PASS — 26.265 GB/s vs min 24.95
  PASS m0.bench.bandwidth.d2h_pageable_gbps: PASS — 20.454 GB/s vs min 19.43
  PASS m0.bench.bandwidth.d2d_gbps: PASS — 426.786 GB/s vs min 405.45
  PASS m0.ratio.saxpy_vs_d2d_bandwidth: PASS — ratio 0.9674 vs min 0.919
  PASS m0.counter.env_profile_required_fields: PASS(delegated to check_schemas.py)
  PASS m0.counter.evidence_files_saxpy_runs: PASS — 4 份 measured_local 证据
  PASS m1.bench.lexer.throughput_mbps: PASS — 213.183 MB/s vs min 202.52
  PASS m1.bench.parser.throughput_kloc_per_s: PASS — 3485.183 kloc/s vs min 3310.92
  PASS m1.counter.syntax_corpus_size: PASS — 108 个语法样例(要求 ≥100)
  PASS m1.counter.ui_golden_path1_snapshots: PASS — 55 条 .stderr snapshot(要求 ≥10)
  PASS m1.counter.spec_clause_test_anchoring: PASS — 77 条款全部 ≥1 测试锚定
  PASS m2.bench.cold_compile_hello_world_ms: PASS — 122.652 ms vs max 183.98
  PASS m2.bench.check_latency_ms: PASS — 6.912 ms vs max 10.37
  PASS m2.counter.ui_golden_path2_snapshots: PASS — 14 条 .stderr snapshot(要求 ≥10)
  PASS m3.counter.ui_golden_path3_snapshots: PASS — 10 条 .stderr snapshot(要求 ≥10)
  PASS m3.counter.borrowck_conformance_categories: PASS — 7 个预设错误类别目录(要求 ≥7)
  PASS m4.bench.saxpy.effective_bandwidth_gbps: PASS — 412.901 GB/s vs min 392.26
  PASS m4.ratio.saxpy_vs_m0_baseline: PASS — ratio 1.0001 vs min 0.95
  PASS m4.counter.launch_conformance_categories: PASS — 4 个预设错误类别目录(要求 ≥4)
  PASS m4.counter.ui_golden_path4_snapshots: PASS — 11 条 .stderr snapshot(要求 ≥10)
[budget_eval] PASS (23 pass, 0 skip, strict mode)
```

GitHub run URL 归档(查询于 2026-06-14 12:40 CST,不将 pending/cancelled 伪记为 green):
- PR #11(M4.3 + M4.4 runtime/launch/ptxas + Rurix SAXPY on GPU):https://github.com/qwasg/Rurix/pull/11
- PR run 27488276501: https://github.com/qwasg/Rurix/actions/runs/27488276501
  (event=pull_request,HEAD `13f0d2c`,conclusion=cancelled,merge 后取消,无 jobs/check-rollup)。
- main push run 27488292211: https://github.com/qwasg/Rurix/actions/runs/27488292211
  (event=push,HEAD `5e55807`,status=pending,jobs=0 at 2026-06-14 12:40 CST)。

人工终审结论:本地锁频 measured_local 硬证据与 `budget_eval --strict` 已满足 G-M4-1;G-M4-2~G-M4-5 证据见
§8.1/CI_GATES v1.3~v1.7。契约 YAML 头已由 `status: active` 落笔为 `status: closed`;
§1~§7 条款保持 0-byte。
