...
"""G26.0 baseline 快检：Stage A digest 锚在档计数 + 战役七期收口 tag 计数实测。"""
...
OUT_DIGEST = ROOT / "evidence/g26_baseline_stage_a_digest_guard.json"
OUT_TAGS = ROOT / "evidence/g26_baseline_campaign_tags.json"
CAMPAIGN_TAGS = ["g19-closed", "g20-closed", "g21-closed", "g22-closed",
                 "g23-closed", "g24-closed", "g25-closed"]
...
def main() -> int:
    ...
    OUT_DIGEST.write_text(
        json.dumps({
            ...
            "notes": f"G26.0 Stage A digest 锚在档格数实测（anchors={n_anchor}）",
        }, ensure_ascii=False) + "\n",
    ...
    OUT_TAGS.write_text(
        json.dumps({
            ...
            "notes": f"G26.0 上游战役七期收口 tag 实测（{n_tags}/7：{CAMPAIGN_TAGS}）",
        }, ensure_ascii=False) + "\n",
        encoding="utf-8", newline="\n",
    )
    print(f"[g26_baseline] anchors={n_anchor} campaign_tags={n_tags}/7")
    return 0 if (n_anchor == 18 and n_tags == 7) else 1
