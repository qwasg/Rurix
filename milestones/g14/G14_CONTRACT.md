...
### §8.11 G14.10 帧循环重构波验收记录（2026-08-22）——RFC-0030 §4.1/§4.3 六子批兑现：统一 session（tsr/dlss 双臂）+ mv GPU 化 + cornell 拆散 + SSBO DEVICE_LOCAL 驻留 + DLSS buffer 共享驻留 + 曝光显示域修正；tsr/dlss 12 格帧率 PASS + AI 读图三场景 PASS；「digest 双跑一致 ≠ 内容正确」教训面登记

- **① 六子批**（G14PLUS_RECORD §4 G14.10 逐批详录）：主体批 b75c773d（tsr 统一四 pass 单 session + mv GPU 化〔NoContraction 注入,漂移归因=FDiv 2.5 ULP 病态反投影放大,L1 读图带内〕+ vendor 驻留输出 API）/10b（exportable 纹理导出/导入基建 + LUID 对拍;OPAQUE_WIN32=0x2 踩坑）/10c e2d84cd3（TSR LocalSize 32×4 六候选扫描位级不变 + cornell 拆散六 pass digest == 锚逐字）/10d df6c985d 前半（SSBO 三路内存机核,bistro t50 tsr 21.47→1.227ms 首格达标）/10e df6c985d（dlss 驻留统一车道,手编 pack SPV;15.62→2.56ms）/10f 2da40f4c（见②）。
- **② 10f 读图抓获双缺陷与修正（AI 读图硬门价值实证）**：a) **OPAQUE_WIN32 OPTIMAL image 跨 device 布局不一致**——10e 的 dlss 输出图确定性块状乱序（digest 双跑一致故门全绿而图毁;render_exec 侧 dump 正常 vs DLSS 侧 device 回读同一 memory 乱序,双侧 dump 对拍实锤）→ 弃 image 共享改 **exportable buffer**（线性无歧义;pack SPV 改 PackHalf2x16 RTE 写 RGBA16F 8B/px 紧凑 = host `f32_to_f16` 同语义;depth/mv SSBO 紧凑直出;DLSS 侧 `import_win32_buffer_input`+`upscale_resident_buffers` copy 进自建 input image——输入位面回归 host 路径 f16/D32/RG32F）;b) **vendor 臂 scene 域 vs tsr 臂显示域语义分裂**（TSR resample `o=v·exposure` 契约字面;bistro ev100=−4 → vendor 输出暗 2^4,G14.3 以来存量;G13.4 M-a 门 = scene 域语义 + 度量侧 ×2^(−ev100) 尺度链故不显形）→ pack SPV `rgb×exposure` 转显示域（**host `pack_vendor_inputs` 共享面零触碰保 M-a 锚**）。验证:cornell（exposure=1.0 IEEE 位恒等）converged digest 修正前后逐字同 + bistro 亮度对齐 tsr（lum mean 0.00961 vs 0.00984,HDR max 13.3 恢复）+ 三场景读图 PASS（cornell dlss 结构锐利/bistro dlss 吊灯吧台桌椅全可见/bistro tsr 对照正常）。
- **③ 波终态（60 帧快扫 + 双格 160 帧确认）**：tsr 6 格 PASS（ratio 1.72~9.13）+ dlss 6 格 PASS（bistro t67 实测 2.88ms ratio 1.13/cornell t67 0.90ms ratio 2.84）;fsr 6 格 MISS（host 链）归 G14.11 条件波（触发条件命中——M-d 探针复跑仍有未达格）。**教训登记:「固定 seed 双跑位级一致」只证确定性不证正确性——确定性的坏内容照样全绿;AI 读图为改图波硬门（方案 C 协议追认强制,G14.12 收口读图三证之一）**。
- **④ 签署块**：白栀（依 10 §7 / P-13 / D-406 v2.0 agent 完全自主签署）。`Assisted-by: Claude Fable 5（G14plus G14.10 波,主会话 + 执行域两 agent 并行）`（影响范围：src/rurix-rt/{render_exec.rs,vendor_upscale.rs} + src/rurix-render/kernels/g14_mv.rx 新建 + src/rurix-render/src/bin/g14_3_pipeline_perf.rs + ci/g14_rurix_pipeline_perf_smoke.py〔_ensure_spv 七对〕+ G14PLUS_RECORD §4 + 本契约 §8.11 本条 + .tmp/g14plus_review/ 读图产物〔不入 commit〕；验证方式：五 commit（b75c773d/e2d84cd3/df6c985d/2da40f4c 及门修订批）+ 读图 PNG 三曝光档 + dump 对拍 EXR + cornell digest 位保持逐字 + 快扫 18 格表）。

### §8.12 G14.11 结构条件波验收记录（2026-08-22/23）——触发条件命中（G14.10 后 fsr 6 格未达）；fsr 臂 D3D12 反向共享驻留 + 跨 device image 共享**真因勘误**（推翻 §8.11「布局解释不一致」结论）+ f16 subnormal 解码缺陷修复；**18/18 全绿达成**

- **① fsr 臂 D3D12 反向共享驻留（9e144bbd）**：D3D12 建 `D3D12_HEAP_FLAG_SHARED` committed BUFFER（三段 256B 行距：color f16 RGBA / depth f32 / mv f32 RG，64KB 对齐）→ `CreateSharedHandle` → Vulkan `D3D12_RESOURCE_BIT` + dedicated 导入为 SSBO → pack SPV v2 按行距直写（color rgb ×exposure 后 PackHalf2x16；depth/mv Bitcast 位拷贝）→ 帧末 EXTERNAL release → D3D12 侧 3× `CopyTextureRegion` 搬入三输入纹理 → ffx dispatch；CPU 序跨界同步 + LUID/staging 布局双对拍 fail-closed。**6 格全绿 ratio 1.65~3.55**（bistro t67 host 链 31.71→1.46ms，21.8×）；digest 双跑位级一致 + cornell exposure=1.0 位保持 + bistro 亮度对齐 tsr（0.00966 vs 0.00992）+ 双侧内容对拍真差异 0（352,947 值）+ 读图非乱序 + tsr/dlss 零回归。
- **② 真因勘误（凌驾 §8.11 ②/G14PLUS_RECORD §4 两处结论；治理项①裁决=更正登记）**：dlss（OPAQUE_WIN32）与 fsr（D3D12_RESOURCE）两次共享 image 的块状乱序，**真因非「两 VkDevice 对同一显存 OPTIMAL tiling 解释不一致」的硬件事实（该判定错误）**，而是 `vendor_upscale.rs::import_win32_input` 的 `image_type: 2`（=`VK_IMAGE_TYPE_3D`，注释却写 "2D"）与导出侧 `IMAGE_TYPE_2D`（=1）不匹配。memreq 实证：1920×1080 RGBA16F OPTIMAL，2D 需 17694720B、3D 需 16588800B。修正后跨 device image 共享直接成立，DLSS 侧三条 copy（41.5MB/帧、0.6ms）整体消失。**同源笔误另存于 `mk_image`（session 自有 image；同 device 写读内容自洽无可见损坏，但 3D image 上建 2D view 触 VUID-VkImageViewCreateInfo-image-06728 十条 validation）——治理项②裁决=修，实测 digest 逐字不变（`sha256:7aa5774b…`）为纯收益，host 路径语义等价（单层 image 2D vs 3D depth=1 纹素值同一），G13.4 bin 构建通过（d6eab741）**。教训与 §8.11 ③「digest 双跑一致 ≠ 内容正确」并列登记：**把可复现的自研缺陷误判为平台固有限制并据此绕道，代价 = 两条独立路线各绕远路 + 每帧 0.6ms 常驻税；判定「硬件不支持」前须逐字段对拍两侧 create info**（本例错误侧注释与取值自相矛盾，代码审查即可发现）。
- **③ 末格 bistro t100 dlss 达标（37ac2688）**：三项优化 = 消 copy（pack 直写三 exportable storage image）+ **跨界 image layout 跨帧常驻 GENERAL**（免每帧 3 次全表面压缩元数据重初始化，4.24→4.075）+ **同形帧跳过命令体重录**（`readback_subset` 逐帧同形且无 TLAS/binding/push override 时命令体逐字节不变原样重放；`cpu_record` 229µs→5.9µs，4.075→3.545）——②③是①的必要配套。prod 4.358→3.846（7 样本均值；中位 3.911/最好 3.545/最差 4.032），ratio 0.992→1.124（最差样本 1.072）；7 跑 digest 位级一致；lum mean 0.009795 + 读图无乱序；四格无回归；validation 零错误。**弃案实测登记**：LINEAR 别名路线走不通（硬件 `linearTilingFeatures` 全支持 ≠ NGX 接受——`vkCreateImage(LINEAR,fmt=97)` 返 `VK_ERROR_FORMAT_NOT_SUPPORTED` 并崩于 `slEvaluateFeature`），已全部回退；`RURIX_G14_DLSS_VK_VALIDATION=1` 为陷阱（NGX 首帧崩于 `vkCreateCuModuleNVX`，层×驱动兼容问题，仅供建面期查错）。治理项⑤（按 tier 切 SL DLSS mode）**裁决=否决**：G13.4 契约 `ue_dlss_quality_map` 的 `tier_note` 限定该映射为 UE 臂插件枚举面且规定 Rurix 臂档位语义 = tier% 内部分辨率，改 mode 反偏离契约（NGX 日志亦实证 t100 in=out 时内部已走 DLAA 路径）。
- **④ f16 subnormal 解码缺陷修复（9e144bbd 同批）**：fsr 双侧对拍暴露 173 处 a=2b 表观差 → `f16_to_f32` 的 `e` 初值 −1 使 f32 指数字段少 1，**全体 f16 次正规数（<2⁻¹⁴，深阴影像素）解码为正确值一半**；该函数为 host 链与 vendor 输出转换共用面、涉 digest 锚（G14.12 统一重收割吸收）。加**全 65536 位型枚举 vs 位精确公式**回归锚测试永久钉死。G13.4 M-a 门为容差带口径（无输出 digest 锚），既存 evidence 不因本修复失效。
