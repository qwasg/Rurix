...
"""G29.0 baseline 快检：Stage A digest 锚在档计数 + 上游十期收口 tag 计数实测。"""
...
OUT_DIGEST = ROOT / "evidence/g29_baseline_stage_a_digest_guard.json"
OUT_TAGS = ROOT / "evidence/g29_baseline_campaign_tags.json"
...
def main() -> int:
    ...
    OUT_DIGEST.write_text(
        json.dumps({
            ...
            "notes": f"G29.0 Stage A digest 锚在档格数实测（anchors={n_anchor}）",
        }, ensure_ascii=False) + "\n",
    ...
    OUT_TAGS.write_text(
        json.dumps({
            ...
            "notes": f"G29.0 上游十期收口 tag 实测（{n_tags}/10：{CAMPAIGN_TAGS}）",
        }, ensure_ascii=False) + "\n",
        encoding="utf-8", newline="\n",
    )
    print(f"[g29_baseline] anchors={n_anchor} campaign_tags={n_tags}/10")
    return 0 if (n_anchor == 18 and n_tags == 10) else 1
