...
status: closed            # active → closed(close-out 只追加,既有条款 0-byte 修改;M7 close-out 终审 §8 自主签署)
version: v1.0
date: 2026-06-15
...
deferred_refs: [RD-007]   # RD-007(const 泛型值运行期单态化,agent M6→M7 顺延评估,inherited;非本契约验收门,执行期处置留痕);M7 不预造新 deferred,执行期按需登记 RD-###(14 §4)
deliverables:
  - id: D-M7-1
...
guardrails:
  ...
  - "registry/deferred.json 与 registry/spike_gating.json 只追加(既有条目修改触发审查);RD-007 仅允许 inherited→closed 的状态留痕追加(agent M6→M7 顺延为 inherited 生命周期既定动作);SG 复评只追加 decisions"
...
| 编号 | 内容摘要 | 承接 |
|---|---|---|
| RD-007 | const 泛型值运行期单态化(turbofish const 实参 → 实例值代入 + codegen)+ 运行期数组 aggregate codegen | M7(M6 close-out agent M6→M7 顺延,inherited;M7 标准库充实与 G0 图形演示作用面——几何原语 / 数组长度类 const 泛型可能触发运行期单态化,届时按需接通或继续留痕,spec/consteval.md RXS-0064 语义不变,回填仅补实现侧。**非本契约验收门**,接通与否执行期处置留痕) |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.0 | 2026-06-15 | 初版契约固化(M7 开工脚手架;基准 ref 维持 m6-closed 无需再切;deferred RD-007 agent M6→M7 顺延承接、维持 inherited;新建 spec/stdlib.md RXS-0104 续号预留,条款体随 M7.1+ 与测试同 PR;新段位错误码首批分配随 M7.1+ 诊断 PR) |
| v1.1 | 2026-06-16 | 自主签署 §8.11:白栀/agent 裁决 M7 正式关闭(`active→closed`)并批准软光栅 L3 阈值定档(1.8333 ms,实测 1.2222 ms ×1.5,max 上界);契约 YAML 头落为 `status: closed`;guardrail 默认基准切换 `m6-closed→m7-closed`。AI 仅代录签署事实并执行机械落档,不代作关闭判定。 |
...
> **草拟声明(M7.5,档位 Direct)**:本 §8 由 M7.5 PR 草拟(契约 `rfc_required: none`,基准回填 / 报告 / 收官为已锁定决策的工程动作)。**M7 close-out 关闭判定、`m6-closed → m7-closed` 基准切换、`m7-closed` tag 由白栀 / agent 自主签署兑现**,agent 自主签署;下方 §8.10 留自主签署位。本轮**未新增 RXS 条款**(L3 基准为工程编排,复用 RXS-0118~0121 kernel 语义);**未触及禁区**(未改 device kernel 签名 / RXS-0118~0121 PTX golden / spec 既有条款本体 / closed 契约 / plan 文档 / error_codes 既有含义)。
...
- **阈值裁定(Direct)**:`direction=max`(帧时间越小越好,阈值为上界);`threshold = 实测 × 安全系数 1.5 = 1.8333 ms`(50% 回归裕度,远低于行业线软光栅 L3 帧天花板)。预算条目 [m7_budget.json](m7_budget.json) `m7.bench.soft_raster_l3_frame_ms` 由 `estimated` 占位转 `measured_local`(单向合法转换,guardrail `check_budget`)。**安全系数与阈值口径待 agent 终审确认**。
...
判据见 §8.1 / §8.2:measured_local 1.2222 ms ≤ threshold 1.8333 ms,`budget_eval --strict` PASS,全局零 estimated。占位在 M7 内生灭(14 §3)。harness 真跑(非 YAML-only):四 stage 逐 stage 向量化 host 参考一致(correctness PASS)。判定:**PASS**(待 agent 确认安全系数口径)。
...
| RD | 状态 | M7 触发 | 处置 |
|---|---|---|---|
| RD-007 | 维持 `inherited`(agent M7) | M7 未触发 | const 泛型值运行期单态化:M7.1~M7.5(数学库 / image-io / 软光栅 / UC-03 demo / L3 基准)均用固定维度常量实现,未依赖 turbofish const 实参实例值代入,**非 M7 验收门**,M7 未接通;承接随后续 device codegen / 运行期数组 aggregate codegen 扩展评估接通(M8 开工按先例顺延),RXS-0064 语义不变。对齐 M5/M6 close-out 顺延先例([../../registry/deferred.json](../../registry/deferred.json) history 2026-06-16 / revision_log v1.8) |

RD-001 / RD-006 维持 open(M8);RD-002 / RD-003 / RD-004 / RD-005 维持 closed。M7 开工无预造新 deferred,执行期无新增 RD-###。

### 8.10 验收门汇总 + 关闭判定(自主签署位)
...
**以下由白栀 / agent 自主兑现(agent 自主签署)**:

- [ ] M7 close-out 关闭判定(G-M7-1~G-M7-5 全绿确认 + §8.1 安全系数 / 阈值口径终审)。
- [ ] 基准 ref 切换 `m6-closed → m7-closed`(M8 开工基准;按 `check_*` 守卫风格 + 双基准核对)。
- [ ] 打 `m7-closed` tag。
- [ ] RD-007 agent_milestone M7→M8 顺延(M8 开工承接留痕)。
- [ ] 各验收门 PR Smoke self-hosted runner 红绿 run URL 归档(#43 步骤 31 / #44 步骤 32 / 本 PR L3 基准)。
- 签署:____________(白栀 / agent) 日期:__________

### 8.11 自主签署 + 收官落档留痕(2026-06-16)

> 本节记录 agent 对 M7 close-out 人工门的签署指令;Codex 仅代录签署事实并执行机械落档,不代作关闭判定、不伪造远端 runner 结果。

**(a) 自主签署:**

- **M7 正式关闭判定**(status `active → closed`):签署人:白栀/agent 日期:2026-06-16 裁决:closed。依据:§8.1~§8.9 验收记录 + 本节本地复核输出(G-M7-1~G-M7-5 全部 PASS)。
- **软光栅 L3 阈值定档**(硬规则 1,agent 提案待人工终审):签署人:白栀/agent 日期:2026-06-16 裁决:批准定档 `m7.bench.soft_raster_l3_frame_ms` max 阈值 1.8333 ms(实测 1.2222 ms × 1.5 安全系数,G-M7-2)。
...
queued/pending 项由 agent 在本人工门中接受为远端归档待补事实;后续若 runner 完成或重跑,仅可在本区后续追加补录,不得回写 §8.11 既有行。
...
RD-007 在 M7 close-out 维持 `inherited` 且 agent_milestone 维持 M7(见 §8.9 / `registry/deferred.json` v1.8);M8 开工时按既有先例追加顺延到 M8,本自主签署不改写 deferred 既有条目。
