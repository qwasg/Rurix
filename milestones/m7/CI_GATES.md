...
| 31 | 软光栅 kernel 冒烟 + safe 覆盖(契约 G-M7-3 通道;M7.3 落地接入):binning/tile 光栅/深度/tonemap kernel 确定性帧像素 + safe 覆盖计数。**实测命令(M7.3 回填)**:计数核对 `py -3 ci/budget_eval.py`(`m7.counter.soft_raster_kernels_safe ≥` 预设软光栅 kernel 数,计数源 = `evidence/soft_raster_*.json` 的 `safe_kernels` 去重基数);软光栅 device kernel 纳入 Compute Sanitizer nightly + PTX/IR golden bless。建设期软光栅未落地 → 0 → normal SKIP 属预期 | 是 |
