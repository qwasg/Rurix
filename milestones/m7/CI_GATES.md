...
| 32 | UC-03 demo 单 EXE 端到端 + 确定性图像序列(契约 G-M7-1;**M7.4 已落地接入**):确定性 SPH 仿真 + G0 软光栅出图(`src/uc03-demo`,复用 RXS-0104~0121,工程编排不新增 spec 语义面)产 host 单 EXE,运行输出确定性图像序列,逐帧 content SHA-256 两次运行逐字节一致。**实测命令(M7.4 回填)**:`py -3 ci/uc03_demo_smoke.py`(`cargo build -p uc03-demo --bin uc03_demo` 产单 EXE → 固定初值/固定步长两次运行落盘逐帧 content SHA-256 逐字节一致 + 序列含运动 + 内建篡改一帧像素 R/B 通道序红绿,应红却绿即脚本 FAIL,反 YAML-only;写唯一 `evidence/uc03_demo_smoke.json`,`image_sequence_ok=true`)。计数核对 `py -3 ci/budget_eval.py`(`m7.counter.uc03_demo_image_sequence ≥1` → PASS)。单 EXE 为 host 本地构建产物不打包再分发(M8/G1),`rx build` 经包管理对接见 PR 描述 | 是 |
...
| v1.1 | 2026-06-16 | M7 close-out 人工签署落档:owner 裁决 M7 正式关闭(`active→closed`)并批准软光栅 L3 阈值定档(1.8333 ms,实测 1.2222 ms ×1.5,max 上界);guardrail 回退基准默认值 `m6-closed→m7-closed`(`ci/check_guardrails.py` docstring / `resolve_base()` 注释与返回值)。本地签署前复核:`budget_eval --strict` PASS(46 pass / 0 skip)、`trace_matrix --check` PASS(121/121)、`check_schemas` PASS、`check_guardrails.py m6-closed` PASS(85 changed paths);`m7-closed` tag 锚定本 close-out 签署提交后 `check_guardrails.py m7-closed` PASS(0 changed paths)。远端 PR Smoke run URL 已归档到契约 §8.11,其中 #43 success,#44/#45 在签署时仍为 queued/pending,不伪造成已绿。 |
