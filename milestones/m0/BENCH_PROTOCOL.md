...
|---|---|---|
| v1.0 | 2026-06-11 | 初版 |
| v1.1 | 2026-06-11 | 追加计量约定:§4.2 的 D2D 方向按 NVIDIA bandwidthTest 参考工具约定计双向字节(读+写,2×);H2D/D2H 计单向;§3 的 bootstrap CI 与中心趋势统一为中位数统计量 |
| v1.2 | 2026-06-11 | §2.2 进程隔离条款可执行化勘误(M0 close-out §8.2.3 留痕兑现):桌面 WDDM 环境 NVML 计算进程枚举含大量图形进程(本机实测 37),"无其他计算进程"不可达成——条款执行口径修订为**记录进程概况入环境画像而非中止本轮**;探测器现行为即此口径,正文 0-byte |
