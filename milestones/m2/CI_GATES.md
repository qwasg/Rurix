...
| v1.4 | 2026-06-12 | 步骤 14 PR 级红绿完成(PR [#6](https://github.com/qwasg/Rurix/pull/6)):红 = 故意篡改阶段基线(link → borrowck)→ 步骤 14 `self-profile check` 失败、run 红([27418261653](https://github.com/qwasg/Rurix/actions/runs/27418261653));revert 后同 PR 转绿([27418345621](https://github.com/qwasg/Rurix/actions/runs/27418345621));另有篡改前基线绿 run([27416608296](https://github.com/qwasg/Rurix/actions/runs/27416608296),步骤 12/13/14 全过)。三 URL 随 M2 close-out 引用本行(契约 §8.3) |
| v1.5 | 2026-06-12 | M2 close-out 配套两项:(1)§4 guardrail 回退基准切换落地——`ci/check_guardrails.py` 的本地/push 路径回退基准 `m0-baseline → m1-closed`(PR 路径不变,仍以 GITHUB_BASE_REF 为准);切换前已以 `py -3 ci/check_guardrails.py m1-closed` 双基准核对 PASS。(2)§3 Nightly 首跑完成(workflow_dispatch 触发):run [27418751059](https://github.com/qwasg/Rurix/actions/runs/27418751059) `success`,self-profile artifact 归档确认(346 bytes);schedule 通道自此每日生效 |
