...
6. 贡献校验门(`ci/check_contribution.py`,G1.4)落地后,以 geometry dogfood PR(MR-0004 + `src/rurix-geometry/`)走通样例:构造**缺 provenance / 缺条款号**的 commit → 贡献门守卫步骤红 → 补 trailer / 条款号转绿,run URL 归档(反 YAML-only)。本机 CPU-only 可完整演示(`py -3 ci/check_contribution.py` 退出 1 → 修复退出 0);GitHub Actions run URL 由 owner 自托管 runner 上线回填(对齐 G1.1/G1.2/G1.3 runner-offline 先例)。
