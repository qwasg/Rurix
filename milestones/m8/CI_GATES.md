...
- **Python 互操作链**(G-M8-1):Python ≥3.10 + PyTorch(CUDA 12.x build)+ nanobind + scikit-build-core,`rx build --emit=pyd` 产 PYD 经 `__cuda_array_interface__` v3 / DLPack 双协议接入 PyTorch 端到端真跑。
