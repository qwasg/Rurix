...
| 16 | const eval 冒烟:`py -3 ci/consteval_smoke.py compile-run`——`conformance/consteval/const_eval_run.rx`(const fn 算术·分支 + const item 引用链编译期求值)经全管线产出 EXE → 运行核对退出码 0 / stdout `consteval-ok`(契约 G-M3-4 通道,对齐步骤 12 真跑形态;M3.4 WP3 接入 pr-smoke,名为 "const eval smoke") | 是 |
...
| v1.6 | 2026-06-13 | §2 步骤 16(const eval 冒烟)落地(M3.4 WP3):实测命令 = `py -3 ci/consteval_smoke.py compile-run`,接入 pr-smoke 工作流(名为 "const eval smoke");语料 `conformance/consteval/const_eval_run.rx`(const fn 求值类,G-M3-4 标量优先,const 泛型值单态化随 M4+ / RD-007)。本地红绿验证(§5 第 2 项)均过:篡改 `const SIDE` 4→5 → const eval 算出 SUM=33≠24 → stdout `consteval-bad` → smoke exit 1 红;复原 → exit 0 绿。真实 CI 绿跑已归档:pr-smoke run [27460135247](https://github.com/qwasg/Rurix/actions/runs/27460135247)(PR #7,HEAD `88b6820`)conclusion=success,"const eval smoke" 步骤真实通过。预算回填(G-M3-3)落地:`m2.bench.*` 两条经 `bench/compile_triple_run.py` 三次进程级独立运行转 measured_local,`budget_eval --strict` 全局零 estimated 残留(阈值余量 ×1.5 待人工终审,硬规则 1)。新增 `milestones/m3/compile_evidence_schema.json`(编译证据 schema,check_schemas 路由 `compile_*`)。5xxx const eval 段位首批 RX5001~RX5003 启用 |
