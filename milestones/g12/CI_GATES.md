...
| v1.2 | 2026-08-17 | G12.3 降噪波两 CI 门 materialize 校准（落盘前实测 CI_step.next_free=223 顺位领取 223~224；判据事实源 = G12_CONTRACT §4.2 M162 行 + G-G12-5 + G12_ACCEPTANCE_MAP §1 + spec RXS-0402 + RFC-0029 §4.5）：步骤 223 = `g12.p0.m162.denoise_pipeline_tsr`（ci/g12_denoise_pipeline_tsr_smoke.py，host+device 门，env 双置 RURIX_REQUIRE_REAL=1 + RURIX_VK_VALIDATION=1——host 段：prod_denoise 单测逐名锚定 + conformance 3 件锚定 + **temporal 底座 0-byte 机核**〔目录级 git diff vs G12.0 不可变 ref + 工作树双面〕+ M96 冻结面 0-byte + NRD 评估报告落盘 + 树内零 vendor 接线符号 + 降噪标定腿〔harness --calibrate-denoise 纯 host 两跑逐字节一致 + 2 条 g12.pt.denoise_* 标定条目字节级纯追加入 g12_budget measured_local + budget_eval 全 PASS〕；device 段：双 kernel 持锁真跑〔g12_pt_production.rx 0-byte + g12_pt_denoise.rx〕——时域累积 + firefly 预钳位 + A-trous 双帧管线 + 噪声谱高频能量下降 ≥ 标定阈 + 帧均值能量守恒容差内 + 历史验证活性 + golden 对拍面不降级 + 帧型标签闭集 + 双跑位级 + RED 三臂 + 子模式复跑抽检）/ 224 = `g12.wave.3.exit`（ci/g12_wave3_exit_check.py，host 只读聚合门，G12 共享库 ci/g12_wave_exit_lib.py 复用——M162 最新 evidence 只读汇总 + 六 facts，聚合不代绿不遮蔽）。同批：两 evidence schema（g12_m162_denoise_pipeline_tsr_ + g12_wave3_exit_，§7 形态 numeric_step const 钉死真号）+ 降噪标定条目共享 schema g12_m162_calibration_entry_evidence_schema.json + ci/check_schemas.py 三处纯追加（g12_m162_denoise_pipeline_tsr_ / g12_m162_calibration_ / g12_wave3_exit_ 前缀路由，与既有全族互不包含，既有路由 0-byte）+ pr-smoke.yml 步骤 223~224（步骤 222 块后追加）+ registry/number_ledger.json CI_step on_tree_max 222→224、next_free 223→225 + revision_log v1.128 + registry/deferred.json RD-040 history 只追加一条（RD040-nrd 评估窗承接兑现登记，条目级字段与 status 0-byte）。§4/§4A/§5 表体 0-byte——numeric_step 经本行校准回填，不回写表体。 |
| v1.3 | 2026-08-17 | G12.4 UE Path Tracer 对标波三 CI 门 materialize 校准（落盘前实测 CI_step.next_free=225 顺位领取 225~227；判据事实源 = G12_CONTRACT §4.2 M163/M164 行 + G-G12-6 + G12_ACCEPTANCE_MAP §1 + spec/visual_comparison.md RXS-0403 + RFC-0029 §4.6）：步骤 225 = `g12.p0.m163.ue_pt_parity`（ci/g12_ue_pt_parity_smoke.py，host+device 门，env 双置 RURIX_REQUIRE_REAL=1 + RURIX_VK_VALIDATION=1——契约 digest 三方独立实现全等 ∧ == 冻结注册值〔不等仍出报告即 RED——门序拒产面〕+ UE build digest == M128 登记 ue_build_id 机核〔5.8.1-56057345〕+ M133 清单 digest 转引 + Rurix 臂〔g12_4_ue_pt_parity_render release 面：glTF→ProdScene→生产化 PT megakernel device 真跑，双场景 × 6 spp × 双 seed，固定 seed 双跑位级一致〕+ UE 臂〔5.8.1 PT MRQ 逐 job 真跑 + 新鲜度/真帧机核 + PT 接通收敛签名核验〕+ 收敛曲线逐段/噪声谱/能量守恒 measured 对拍〔容差标定腿双 seed 方差底 p100×2.0 产入 g12_budget 三条目 g12.pt.parity_*，禁手写 P-09〕+ UE PathTracing 模块归属差距登记表落盘〔milestones/g12/g12_ue_pt_gap_registry.json，RXS-0391 归属枚举口径继承 + 行集对账〕+ 残余口径差逐环节登记 + 不设绝对通过线 + RED 五臂）/ 226 = `g12.p0.m164.regression_guard`（ci/g12_regression_guard_smoke.py，纯 host 门——既有 62 门〔G9 34 + G10 14 + G11 14，M147 双 phase 纪律面〕最新 evidence 全绿只读汇总 + 生产化触改面既有门重跑回归零降级〔M96 golden 门序面真跑抽检 + G12.2 M158/M160 + G12.3 M162 + wave2/wave3 exit 聚合复跑 + G10 M140/G11 M157 host 面广幅抽检 + 新鲜度机核〕+ 既有判据 0-byte + RED 三臂）/ 227 = `g12.wave.4.exit`（ci/g12_wave4_exit_check.py，host 只读聚合门，G12 共享库复用——M163/M164 最新 evidence 只读汇总 + 六 facts，聚合不代绿不遮蔽）。同批：四 evidence schema（g12_m163_ue_pt_parity_ + g12_m164_regression_guard_ + g12_wave4_exit_〔const 钉死 numeric_step/gate key〕+ g12_m163_calibration_entry_〔对标标定条目共享〕）+ ci/check_schemas.py 三处纯追加（g12_m163_ue_pt_parity_ / g12_m163_calibration_ / g12_m164_regression_guard_ / g12_wave4_exit_ 前缀路由，与既有全族互不包含，既有路由 0-byte）+ pr-smoke.yml 步骤 225~227（步骤 224 块后追加，225 device 门 env 双置）+ registry/number_ledger.json CI_step on_tree_max 224→227、next_free 225→228 + revision_log v1.130 + registry/deferred.json RD-040 history 只追加一条（M100-high G12.4 触发评估登记——多灯 workload measured 对照面产出留痕，条目级字段与 status 0-byte）。同批实现面：src/rurix-render/kernels/g12_pt_production.rx 加性扩展（type=2 三角网格光 + lights stride 16→17 CDF 槽位〔NEE 二分选择〕——type 0/1 既有路径位级不变）+ src/rurix-render/src/gi/path_trace/prod.rs（ProdLight::Tri 变体 + 校验/分布/打包/host oracle 镜像 + 单遍分桶校验优化〔语义不变〕+ 单测 2 件）+ src/rurix-asset 新 bin g12_4_ue_pt_parity_render（+rurix-rt optional dep/vulkan feature 同构面）+ milestones/g12/g12_ue_pt_parity_contract.json（独立冻结契约）+ milestones/g12/harness/{ue_python/g12_pt_contract.py, ue_python/g12_4_build_pt_scenes.py, g12_4_ue_render.py}（UE 臂建设/渲染编排）。§4/§4A/§5 表体 0-byte——numeric_step 经本行校准回填，不回写表体。 |
| v1.4 | 2026-08-17 | G12.5 性能面波两 CI 门 materialize 校准（落盘前实测 CI_step.next_free=228 顺位领取 228~229；判据事实源 = G12_CONTRACT §4.2 M165 行 + G-G12-7 + G12_ACCEPTANCE_MAP §1 + milestones/m0/BENCH_PROTOCOL.md §3 + G10.5 M141 50×3 trimmed mean 同协议继承 + RFC-0029 §4.7 基线锚消费面）：步骤 228 = `g12.p0.m165.pt_throughput_baseline`（ci/g12_pt_throughput_baseline_smoke.py，host+device 门，env 双置 RURIX_REQUIRE_REAL=1 + RURIX_VK_VALIDATION=1——吞吐基线 measured〔rays/sec 主射线口径〔像素数 × spp，次级射线未计——显式登记不冒充全光线计数〕+ 帧时 at 固定 spp{16,64} × M133 双场景闭集 {cornell-box, bistro-interior}，warmup 10 + timed 150 = 3×50 块 IQR→块中位数→3 块均值 M141 冻结统计口径；计时口径 = host Instant 墙钟 around run_device 全帧〔G12.4 生产化出图路径逐帧全链路〕；采样腿持 gpu_device_lock 串行〕+ 8 条目入 g12_budget measured_local 零 estimated〔阈 = 实测 ×1.5〔frame_ms max 向〕/ ÷1.5〔rays_sec min 向〕回归守护语义非通过线，P-09 禁手写〕+ 不设通过线登记〔zero_pass_line 字面 + 条目逐个携带「不构成帧率对标通过线」字面——以基线冒充帧率对标即 RED，正式帧率对标锚定 G14，G10-N11/N16 承接锚字面维持〕+ 正确性锚固定 seed digest 0-byte〔逐 cell == M163 Rurix 臂 receipt 冻结锚 + 全 160 帧 distinct==1 + 演进位 null——digest 漂移未登记即 RED〕+ RED 三臂独立有效〔含 tamper 子进程真跑面〕）/ 229 = `g12.wave.5.exit`（ci/g12_wave5_exit_check.py，host 只读聚合门，G12 共享库 ci/g12_wave_exit_lib.py 复用——M165 最新 evidence 只读汇总 + 六 facts〔M96 锚 0-byte / 8 条目 budget measured + eval PASS / 不设通过线机核 / digest 0-byte 锚断言 / RED 臂独立有效 / 62 门零降级 + M163 复跑 base_commit 同窗二进制回归锚〕，聚合不代绿不遮蔽）。同批：三 evidence schema（g12_m165_pt_throughput_baseline_ + g12_m165_baseline_entry_〔8 基线条目共享〕+ g12_wave5_exit_，§7 形态 numeric_step const 钉死真号）+ ci/check_schemas.py 三处纯追加（g12_m165_pt_throughput_baseline_ / g12_m165_baseline_ / g12_wave5_exit_ 前缀路由，与既有全族互不包含，既有路由 0-byte）+ pr-smoke.yml 步骤 228~229（步骤 227 块后追加，228 device 门 env 双置）+ registry/number_ledger.json CI_step on_tree_max 227→229、next_free 228→230 + revision_log v1.131。同批实现面：src/rurix-asset/src/bin/g12_4_ue_pt_parity_render.rs 加性 --benchmark 子模式（--render/--contract-digest 既有面 0-byte；M163 全档复跑零降级机核同窗）。M165 纯测量面无新 RXS 条款/conformance 语料（G10.5 M141 先例同模——消费 BENCH_PROTOCOL §3 + 14 §5 既有口径面）。§4/§4A/§5 表体 0-byte——numeric_step 经本行校准回填，不回写表体。 |
| v1.5 | 2026-08-17 | G12.6 P2 穷举决策门 materialize 校准（落盘前实测 CI_step.next_free=230 顺位领取 230；判据事实源 = G12_CONTRACT G-G12-8 + G12_ACCEPTANCE_MAP §1/§2 + G12_CANDIDATE_DECISIONS v1.0 + registry/deferred.json + G12_CONTRACT §8.2~§8.5）：步骤 230 = `g12.wave.6.decisions`（ci/g12_p2_decisions_check.py，host 普通检查门非聚合门，required_gates 空——核验 milestones/g12/G12_P2_DECISIONS.md v1.0：33 行冻结候选闭集全等〔G12.1 决策表校准后冻结 §1 defer 19 + §3 G12-N10/N11 + §2 RD 级 4 + G12.2~G12.5 期内新增 G12-N12~G12-N19 八行〕+ 裁决枚举合法〔go/no-go/defer-to-G13+/strategic_override〕+ 零空行 + 承接锚「重判条件+兜底」、defer 行 G13+ 重评窗 + go 行 evidence 义务 + no-go 行 RD/矩阵/契约锚义务 + G12_ACCEPTANCE_MAP 9 key〔8 P0 + 1 已 go P1〕互斥 + deferred.json history 对账〔G12.6 P2 登记恰好 RD-039 +1〔M61〕/RD-040 +2〔M52/M100-high——M52 = G12.6 复评窗核验双条件未命中 maintain-defer、M100-high = G12.6 触发重判证据未齐备 maintain-defer 锚定 G14〕，零新 RD max=RD-044，RD-039/040 status open 0-byte〕+ G12.1 候选决策表对账〔19 行 G11 defer 承接 + G12-N10 == CANDIDATE §1/§3 行集 defer-to-G13+ 字面逐字承接〕；no-go/defer 如实保持 open 不写进全绿叙述）。同批：milestones/g12/G12_P2_DECISIONS.md v1.0（33 行穷举：go 5 closed-go 留痕 + no-go 6 + defer-to-G13+ 22 + strategic_override 0）+ evidence schema milestones/g12/g12_p2_decisions_evidence_schema.json（§7 形态 numeric_step const 钉死 230，extra_facts minItems=maxItems=38）+ ci/check_schemas.py 三处纯追加（g12_p2_decisions_ 前缀路由，与既有 g12_* 全族互不包含，既有路由 0-byte）+ pr-smoke.yml 步骤 230（步骤 229 块后追加）+ registry/number_ledger.json CI_step on_tree_max 229→230、next_free 230→231 + revision_log v1.132 + registry/deferred.json history 只追加三条（RD-039 +1〔M61〕/RD-040 +2〔M52/M100-high〕，条目级字段与 status 0-byte）+ **ci/check_g12_acceptance_map.py materialize**（本文 §3 `g12.gov.acceptance_coverage` 声明 validator 落盘——coverage + no-empty 两组断言 + P0 行 MAP §1 ↔ CONTRACT §4.2 ↔ CI_GATES §4 三向逐字 + P1 行 MAP §2 ↔ §4A 双向逐字，G11 同构；不占数字步骤；G12.1 起映射完整性机核由互锁 validator 事实门③承接口径不变，本 validator = G12.7b close-out 三向机核面成环，判据语义 0-byte）。§4/§4A/§5 表体 0-byte——numeric_step 经本行校准回填，不回写表体。 |
| v1.6 | 2026-08-17 | G12.7a stabilization soak 聚合门 materialize 校准（落盘前实测 CI_step.next_free=231 顺位领取 231；判据事实源 = G12_CONTRACT G-G12-9 + G12_PLAN §2 G12.7a + 本文 §5 wave7a 行 + G11.7a 同构体例）：步骤 231 = `g12.wave.7a.soak`（ci/g12_stabilization_soak.py——pr-smoke 侧 `--verify-latest` 秒级核最新 full-run evidence〔schema + host_section_pass + checks 七键全真 + soak 双阈值与 honesty 字段复核〕；full-run 本地/workflow_dispatch `--gate` 产 evidence——四腿 + G5~G11 既有判据 0-byte fact：①全量回归〔8 P0 + 1 go P1 = 9 key 逐门真跑 --gate 后机器核验最新 evidence 顶层 status==pass 字面〔G12 证据形态统一无豁免面——缺字段即红〕；wave2~wave5 exit + wave6 decisions 五聚合/决策门真跑核验；9 门 evidence base_commit 同值=HEAD 且 14 门 stamp 新鲜度机核=同一候选 close-out 基线，沿 G11.7a/G10.8a MAP §7 口径〕+ ②PT 生产化链路〔出图→降噪→对标装配→吞吐基线〕连续复跑 soak ≥1800s〔沿 G11.7a/G10.8a/G9.8a 继承：出图 g12_4_ue_pt_parity_render --render 双场景 spp16 receipt frame_content_digest == M163 Rurix 臂冻结锚 + 契约 digest 冻结拒产面 → 降噪 g12_pt_production --gate g12.p0.m162.denoise_pipeline_tsr 双 kernel 全档真跑〔时域累积 + firefly 预钳位 + A-trous 管线，SPV 循环外一次编译，pbrt 参照缓存面〕 → 对标装配 最新 M163 evidence parity 节重建 metrics → build_gap_registry 当次重装配与在树 g12_ue_pt_gap_registry.json 幂等复核〔逐字段全等 + evidence_digest 自证指针 == metrics 重算 digest〕 → 吞吐基线 --benchmark 轻量链复跑〔warmup 4 + timed 8 场景逐迭代轮转，首帧 digest == 冻结锚 + distinct==1 + timed_count==8——链复跑口径如实登记不冒充 M165 冻结 50×3 协议；吞吐守护阈归 M165 门本体〔①回归腿 50×3 全协议 ×1.5/÷1.5 守护〕，轻量腿帧时不充守护判定〕；sleep_seconds 恒 0，active_chain_seconds 逐迭代计时求和，gate 外测墙钟交叉核验谎报判红，迭代/出图/降噪/装配/吞吐计数非空零失败，device 腿持 gpu_device_lock 串行〕+ ③budget_eval --strict 非空零 estimated/skip + ④纪律日期锚 + G5~G11 既有判据 0-byte〔git status --porcelain 闭集面空集，与 M164 门 ③ 同闭集字面〕）。同批：evidence schema milestones/g12/g12_stabilization_soak_evidence_schema.json（§7 形态 numeric_step const 钉死 231，required_gates minItems=maxItems=14，soak 块 honesty 字段全必填，checks 七键闭集）+ ci/check_schemas.py 三处纯追加（g12_stabilization_soak_ 前缀路由，与 g12_p2_decisions_ 及既有 g12_* 全族互不包含，既有路由 0-byte）+ pr-smoke.yml 步骤 231（步骤 230 块后追加，--verify-latest）+ registry/number_ledger.json CI_step on_tree_max 230→231、next_free 231→232 + revision_log v1.133。§4/§4A/§5 表体 0-byte——numeric_step 经本行校准回填，不回写表体。 |
