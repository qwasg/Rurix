...
- 已 go 的 P1 精确集合：`{M92,M105,M106,M107,M99,M100,M101}`（G9.3 波四行 + G9.4 波三行 P1 全进裁决，[G9_CONTRACT.md](G9_CONTRACT.md) §8.1 裁决①；2026-08-11 / 2026-08-12 两批只追加登记，见 §3）。后续在对应波次开工前判 go 的 P1，须经治理流程**只追加**修订本表（§1 覆盖集合 + §3 行），不得静默并入现有 key；P0 集合变更属于契约变更，不得以勘误处理。
...
## 3. 已 go P1 硬门（G9.3 波四行 + G9.4 波三行）

G9.3 波 P1 全进裁决（[G9_CONTRACT.md](G9_CONTRACT.md) §8.1 裁决①：P1 全进，逐波经治理流程只追加进本节，不静默并入既有 key）首批登记 M92/M105/M106/M107 四行（2026-08-11，只追加）。M## 承接面按 G9.3 执行波口径（G9_PLAN §2 G9.3 D3 链路行）：M105 = command build node 全链路零 CPU 回读（RFC-0023 §4.4 语义面，在 M104 P0 已冻结的 AccessKind 新边与结构性零回读之上）、M106 = Execution Set 与 PSO 衔接（RFC-0023 §4.2，`submit.execution_set` 预留位转正）、M107 = shader library IR 链接 + 变体预算合并门（RFC-0023 §4.5/§4.6「同波不延后」字面）。G9.4 波 P1 全进裁决（同一裁决①）第二批登记 M99/M100/M101 三行（2026-08-12，只追加）。M## 承接面按 G9.4 GI 波口径（G9_PLAN §2 G9.4 门序行 + RFC-0022 §4.8 语义面）：M99 = 屏幕级 SPG 自适应细分 + Radiance Cache 双级（RD-040 条件分项：世界级 clipmap 未 measured 举证，分项 not-triggered 不充绿，[G9_CANDIDATE_DECISIONS.md](G9_CANDIDATE_DECISIONS.md) v1.3 校准注）、M100 = 低档多灯直接光默认档（RD-040 条件分项：高档 ReSTIR workload 证据不足，分项 not-triggered 不充绿，同注；多灯语义面 RFC-0022 §7 本章不冻结，RD-040 触发举证先行）、M101 = IF 体素网格档位阶梯 L0~L3（共享 probe 着色与八面体编码内核，全档判 go）。后续在对应波次开工前经治理流程判 go 的 P1，按 §1 覆盖集合条**只追加**进入本节（独立 key、独立脚本、独立 schema、独立判据、独立波次），并同步修订 [CI_GATES.md](CI_GATES.md) §4A 追加段；`no-go`/`defer` 项不入本表，不得冒充 PASS。
...
| **M99** | `g9.p1.m99.spg_radiance_cache`<br>`py -3 ci/g9_spg_radiance_cache_smoke.py --gate g9.p1.m99.spg_radiance_cache` | `milestones/g9/g9_m99_spg_radiance_cache_evidence_schema.json` | 屏幕级 SPG 自适应细分（深度/法线不连续性 + radiance 方差双判据驱动，16 px/probe 基线 + 3×3 空间滤波，G8 既有 1/16 均匀 probe + 3×3 滤波底座增量不重定）+ Radiance Cache 屏幕级（复用 probe 历史）双级语义产物 digest 等于 golden；第一反弹 BRDF×入射光 product importance sampling 关闭后方差回归必须可检测（关 product IS 负例 RED 臂独立有效）；细分判据阈值先 measured 后冻结（禁手写掩盖，P-09）；probe 历史/时域累积一律经 temporal 公共底座，私写重投影 variant 即 RED（D2-Q14）；**世界级 clipmap 证据不足登记 not-triggered 不充绿**（RD-040 measured 触发举证未满足，未举证只做屏幕级，不得以屏幕级绿色冒充世界级已触发）；按匹配深度（1/2/full bounce）对 M96 golden；门序硬约束同 M96 行（M96 未绿本门不得验收）。G9.4 波 P1 判 go（G9_CONTRACT §8.1 裁决①；RD-040 条件分项校准见 G9_CANDIDATE_DECISIONS v1.3 注）只追加登记。 | **G9.4** |
| **M100** | `g9.p1.m100.multi_light_low`<br>`py -3 ci/g9_multi_light_low_smoke.py --gate g9.p1.m100.multi_light_low` | `milestones/g9/g9_m100_multi_light_low_evidence_schema.json` | 低档多灯直接光（MegaLights 式固定随机选灯）为**默认档**：多灯场景出图与 golden 相等（容差域经 spec 明示冻结，禁手写掩盖）；**验证射线零跳过**统计性偏置硬契约（跳验证引入系统性变暗偏置、随场景复杂度放大、事后不可归因，D2-Q4——跳验证射线注入负例 RED 臂独立有效）；海量灯阴影统一接口随动；选灯种子流固定、同输入双运行逐位一致；**高档 ReSTIR reservoir 须附多灯 workload 证据，证据不足不做到场登记 not-triggered 不充绿**（RD-040 条件分项，M15 维持 open-留档，不得以默认档绿色冒充高档已验收）；按匹配深度（1/2/full bounce）对 M96 golden；门序硬约束同 M96 行（M96 未绿本门不得验收）。G9.4 波 P1 判 go（G9_CONTRACT §8.1 裁决①；RD-040 条件分项校准见 G9_CANDIDATE_DECISIONS v1.3 注）只追加登记。 | **G9.4** |
| **M101** | `g9.p1.m101.if_tier_ladder`<br>`py -3 ci/g9_if_tier_ladder_smoke.py --gate g9.p1.m101.if_tier_ladder` | `milestones/g9/g9_m101_if_tier_ladder_evidence_schema.json` | IF 体素网格档位阶梯 L0~L3（L0 屏幕空间 probe〔SPG 完整形态〕/ L1 clipmap 体积 probe〔DDGI 基线：八面体 irradiance 8×8 + visibility 16×16 **防漏光优先于提 irradiance 分辨率** + 每帧轮换更新摊销；DDGI Resampling 演进项非首版〕/ L2 空间哈希缓存 / L3 per-pixel 全分辨率参考档）**共享 probe 着色与八面体编码内核、只换空间索引**（档间 golden 对拍可归因到索引结构而非实现差异；共享内核同一函数实例断言）；每档强制含 AS 更新预算行且档位切换判据消费 AsStats 计数面（D2-Q10），**超 AS 更新预算必须强制降档**（超限未降档即 RED）；八面体编码为线性域（SRGB 编码注入即 RED）；档位切换对同输入确定（双运行逐位一致）；按匹配深度（1/2/full bounce）对 M96 golden；门序硬约束同 M96 行（M96 未绿本门不得验收）。G9.4 波 P1 判 go（G9_CONTRACT §8.1 裁决①）只追加登记。 | **G9.4** |
...
1. P0 行集合与 §1 的 15 项**集合全等**，无遗漏、无额外 P0、无重复；已 go P1 行集合与 §1 声明集合全等（G9.3 波四行 + G9.4 波三行 = `{M92,M105,M106,M107,M99,M100,M101}`）。
...
| v1.2 | 2026-08-12 | **G9.4 波 P1 全进裁决只追加登记**（[G9_CONTRACT.md](G9_CONTRACT.md) §8.1 裁决①）：§1 已 go P1 集合 `{M92,M105,M106,M107}` → `{M92,M105,M106,M107,M99,M100,M101}`；§3 追加三行（M99 `g9.p1.m99.spg_radiance_cache` / M100 `g9.p1.m100.multi_light_low` / M101 `g9.p1.m101.if_tier_ladder`，脚本 `ci/g9_<slug>_smoke.py` 同 slug，evidence schema 目标路径同命名空间只冻结不预建，最晚波次均 G9.4，numeric CI step 一律 `post-interlock actual-next-free allocation` 待 materialize 实测回填）；M99/M100 承接 RD-040 条件分项触发举证校准（M99 仅屏幕级判 go、世界级 clipmap 未 measured 举证分项 not-triggered 不充绿；M100 仅低档默认判 go、高档 ReSTIR workload 证据不足分项 not-triggered 不充绿，M15 维持 open-留档；[G9_CANDIDATE_DECISIONS.md](G9_CANDIDATE_DECISIONS.md) v1.3 校准注），M101 全档判 go；语义面 RFC-0022 §4.8（SPG 自适应细分 / Radiance Cache 双级 / IF L0~L3 档位阶梯）+ §7 跳验证射线否决（D2-Q4）+ §4.10 golden 门序硬约束；§4 validator 判据描述同步（P1 集合七行）。`ci/check_g9_acceptance_map.py` 同 PR 扩展（`EXPECTED_P1` 七行集合 + selftest 10 RED → 11 RED + 1 GREEN；§2 P0 十五行 coverage/no-empty/三向比对 0-byte 不改弱）。**§2 P0 15 行精确集合 0-byte**；G5~G8 closed 判据 0-byte；零脚本/schema/workflow 预放。 |
