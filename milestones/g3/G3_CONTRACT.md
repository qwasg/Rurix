...
status: closed            # active → closed（2026-07-19 G3.7 close-out,§8.1 终审;close-out 只追加 §8,上方条款 0-byte;基准 mb1-closed→g3-closed 切换 + g3-closed tag,agent 自主签署;EA1 仍 active,基准链 mb1→g3 单线性,EA1 日后收口另裁）
version: v1.0
date: 2026-07-18
...
### 8.1 G3 整体 close-out 终审(2026-07-19,agent 完全自主签署,AGENTS v3.0 硬规则 1)

G3 工业渲染期一次性推完(闸门→设计→处置→五面全量→收口),全程 agent 完全自主(D-406 v2.0),大量 Workflow/子 agent 编排。五面均达 device measured 真见证(开发机 RTX 4070 Ti 活 Vulkan 驱动),blocked 腿以 probe 证据落尾门照 G-MB1-6 先例不伪造。

#### 全量回归冻结(逐命令真实输出)

- `cargo fmt --all --check` PASS(第一道)
- `cargo test --workspace` PASS(83 test-suites,0 failed)
- `cargo clippy --workspace --all-targets --features "dxil-backend shader-stages vulkan-backend vulkan" -- -D warnings` PASS(0 warning)
- `ci/trace_matrix.py --check` PASS(245/245 条款全锚定)
- `ci/stable_snapshot.py --check` PASS(spec_clauses=245 / error_codes=103,同 edition 只增)
- `ci/bilingual_coverage.py` PASS(zh/en 103/103 成对)
- `ci/check_schemas.py` / `ci/check_structure.py` PASS
- `ci/budget_eval.py --strict` PASS(80 pass,**0 skip / 全局零 estimated**)
- `ci/check_guardrails.py mb1-closed` ADVISORY(agent 自主模式建议项不阻断)
- `ci/check_number_ledger.py` PASS(RXS 头 245 零同号碰撞;14 命名空间保留号被尊重)
- `ci/check_contribution.py` PASS(provenance + 条款号 + 验证 + 对抗性评审全过)
- `ci/check_redistribution.py` PASS(再分发面为空;spike cubin 工件驻 build/ 已清,不入库)
- **五面 device smoke `RURIX_REQUIRE_REAL=1` 真跑全 PASS**(RTX 4070 Ti,validation 零报错):present 6 帧+resize readback / 采样 7 模式数值判据 / bindless 3 判据 / render graph 3 判据(D6 互证+run_graph) / mesh-RT 4 stages
- `bench/saxpy_bench.py --smoke` correctness PASS(逐位相等)

#### 验收门终审表

| 门 | 判据 | 结论 | 证据 |
|---|---|---|---|
| G-G3-1 | RD-027 归因闸门(归因落地即开闸) | **✅ 达成** | 四层判别矩阵定罪 NVIDIA 优化后段(ptxas -O0 绿/-O1+ 挂/双装载路挂/封顶仍挂;evidence/rd027_pt_poison_spike_{report.md,20260718.json});护栏 MR-0011(-O0 pin,完整生产档 9.5s/帧可测)+ 上游 DRAFT 备包(do-NOT-file)。PR [#171](https://github.com/qwasg/Rurix/pull/171)/[#173](https://github.com/qwasg/Rurix/pull/173) |
| G-G3-2 | present 面真实红绿 | **✅ 达成(measured)** | 可见窗口 flip-model swapchain present 6 帧+ResizeBuffers 重建 readback 一致(RTX 4070 Ti);RD-019 close。PR [#174](https://github.com/qwasg/Rurix/pull/174) |
| G-G3-3 | 采样超集面真实红绿 | **✅ 达成(measured)** | 7 模式 device 数值判据(隐式 LOD/lod/grad/fetch/sampler 状态/shadow/gather/UAV 唯一写者/多分量);RD-022/023/024 close。PR [#175](https://github.com/qwasg/Rurix/pull/175) |
| G-G3-4 | bindless 面真实红绿 | **✅ 达成(measured)** | 四象限动态索引四色+篡改注册序换位 RED+feature chain Err;DXIL probe 绿;RD-018 close。PR [#176](https://github.com/qwasg/Rurix/pull/176) |
| G-G3-5 | render graph 面真实红绿 | **✅ 达成(measured)** | D6 双实现互证金标准(graph.rs 推导==uc04 手动锚点集)+run_graph 2-pass 自动 barrier 出图+漏声明拒 RED;RD-020 close。PR [#177](https://github.com/qwasg/Rurix/pull/177) |
| G-G3-6 | mesh/task/RT 面真实红绿 | **✅ 达成(measured;含 open 尾门)** | Vulkan 主腿全量:8 阶段 SPIR-V 双口径 spirv-val 16/16+SPIR-V 1.4 分叉零回归+vk mesh pipeline/AS/SBT/TraceRays;4 stages device 真跑(mesh 出图/RT 命中-miss);RD-029 close。**DXIL RT 腿 blocked-on-upstream = RD-034 open 尾门**(probe 实证 spirv-cross 拒 raygen builtin 5319;照 G-MB1-6 越 close-out 存续,步骤 69 blocked 探针防腐烂)。PR [#178](https://github.com/qwasg/Rurix/pull/178) |
| G-G3-7 | 条款锚定延续 | **✅ 达成** | trace 215→245/245 全锚定;stable 快照 5 次加性重 bless;新 RX 码 en/zh 成对(bilingual 96→103) |
| G-G3-8 | EI1 脚手架 | **✅ 达成** | milestones/ei1/ gated 契约四件套合入(并行会话 [#170](https://github.com/qwasg/Rurix/pull/170)),§0 gated on G3 close-out,零共享编号消费 |
| G-G3-9 | close-out 收口 | **✅ 达成** | 本节:全量回归冻结零 estimated + 终审表 + status flip + 基准切换 + g3-closed tag + deferred/SG 处置 + ledger 校准 |

**签署:agent 完全自主(D-406 v2.0);五面 device 见证均 evidence-based measured;唯一 open 尾门 RD-034(DXIL RT)照 G-MB1-6「不签、不伪造、越 close-out 存续、上游解锁后状态翻转不依赖新契约」。**

#### Deferred 处置复核(registry/deferred.json v1.55~v1.64)

| RD | 处置 | 依据 |
|---|---|---|
| RD-019/022/023/024/018/020/029 | **closed**(7 条,owner_milestone→G3) | 五面兑现+device measured |
| RD-012 | **open**(owner_milestone→G3) | mesh/task DXIL 兑现(RX6008 改接+probe 绿);RT DXIL 分量→RD-034 承接,不 force-close |
| RD-027 | **open**(闸门) | 归因 NVIDIA 优化后段(本仓不可修)+护栏 MR-0011+上游 DRAFT 备包;上游缺陷未修维持 open,越 close-out 存续 |
| RD-034 | **open**(新登,G3.6) | DXIL RT 腿 blocked-on-upstream 尾门,照 G-MB1-6;步骤 69 blocked 探针恒跑防腐烂;上游(spirv-cross/RD-015)解锁后全量落 |

三条 open 尾门(RD-012 RT 分量/RD-027/RD-034)均诚实存续,非验收缺失——measured-first/blocked-honest 高于「全量」表述(契约 §7 ③⑨)。RFC §8 earmark 的其余 RD-035+ 家族(可竞写模式/image atomics/heap 直索引/重排调度/多 queue/split barrier/storage-in-graph)本期未触发,不预登(以合入时实际续号)。

#### SG 复评(registry/spike_gating.json)

SG-001~002/004~005/007~009 维持 not_triggered;SG-003 维持 triggered(RFC-0011 多后端,不回翻);SG-006 维持 closed;SG-010 留续号(窗口/UI 框架进语言方向——present 面 D-130 红线使其不触)。**G3 五面均为既登记 deferred 兑现,非扩张诱惑方向,零 SG 消费**;spike_gating.json 追加复评 decisions 行。

#### Provisioning 注(工具件不入库)

Vulkan SDK 1.3.296.0(spirv-as/spirv-val/spirv-cross/glslang/dxc)、活 Vulkan 驱动(RTX 4070 Ti,VK_EXT_mesh_shader/VK_KHR_ray_tracing_pipeline/acceleration_structure/deferred_host_operations 四扩展)、CUDA 工具链(RD-027 spike ptxas/compute-sanitizer)、spike 分析工件(build/spike-rd027/ SASS/cubin/campaign.jsonl、build/spike-sampling-probe/ probe .spv/.hlsl/.dxil)——**全部驻 build/ gitignore 区不入库,可弃**。CI runner(自托管开发机)已同等 provisioning 方复现 device 真绿;缺 provisioning 环境 device 段 SKIP=dev-env degrade(REQUIRE_REAL=1 翻硬红),不伪造。上游备包(evidence/upstream-reports/rd027-pt-spin/,DRAFT — do NOT file)owner 复核门,agent 不对外提报。

#### 关键 PR / run 归档

脚手架 [#168](https://github.com/qwasg/Rurix/pull/168)(治理包)· [#169](https://github.com/qwasg/Rurix/pull/169)(编号更正伞形 RFC-0013)· spike [#171](https://github.com/qwasg/Rurix/pull/171)(开闸)· RFC [#172](https://github.com/qwasg/Rurix/pull/172)(伞形五章 Agent Approved+对抗性评审)· 处置 [#173](https://github.com/qwasg/Rurix/pull/173)· 五面 [#174](https://github.com/qwasg/Rurix/pull/174)/[#175](https://github.com/qwasg/Rurix/pull/175)/[#176](https://github.com/qwasg/Rurix/pull/176)/[#177](https://github.com/qwasg/Rurix/pull/177)/[#178](https://github.com/qwasg/Rurix/pull/178)· EI1 脚手架 [#170](https://github.com/qwasg/Rurix/pull/170)。

#### Status flip / 基准切换 / tag

- 本契约 `status: active → closed`(§8 追加生效,上方条款 0-byte)。
- `ci/check_guardrails.py` 默认 `resolve_base` 由 `mb1-closed` 切至 `g3-closed`(EA1 仍 active 未收口,基准链 mb1-closed→g3-closed 单线性;EA1 日后收口另裁,契约 §7 ⑧ 兼容两序)。
- 本 close-out PR 合入后推 annotated `g3-closed` tag(不匹配 release.yml 触发器 `v[0-9]+.[0-9]+.[0-9]+*`,零误触发)。
- number_ledger 校准:RXS on_tree_max→249/next_free→250(EI1 earmark 0250~0269 之前 G3 用满 0220~0249);RD next_free→35;CI 步骤 61~69 落地;RX 6027~6030+3016/3017+6008 改接;U30。reserved_in_flight[G3] 收口。
