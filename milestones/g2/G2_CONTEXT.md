...
**owner 开工裁决**（2026-06-23，经 AskUserQuestion；权威留痕 = [G2_CONTRACT.md](G2_CONTRACT.md) §7，引既有
...
> 须 owner 经 00 §6.3 独立规划文档勘误 PR 兑现（D-408 续号），与脚手架 PR 分离。
...
### 2.3 硬规则 5 禁区（仅人类经 Full RFC 落笔，[agents/AGENTS.md](../../agents/AGENTS.md) §2 / 10 §7）
UB 条款 / **内存模型映射（[06](../../06_GPU_GRAPHICS_PROGRAMMING_MODEL.md) §4.2，纹理路径 G2 引入时再扩展，禁区）** / FFI ABI / 安全包络边界。
→ G2 的 DXIL codegen 面、着色阶段语法/类型系统、纹理内存模型 **均须人工 Full RFC 前置**，AI 只登记 gating。

### 2.4 其他语义边界
- **单机单 GPU（A-06，[06](../../06_GPU_GRAPHICS_PROGRAMMING_MODEL.md) 范围红线表）**:MVP 语义边界；G2 碰多 GPU / VMM / NVLink / MIG 须人工 Full RFC（VMM 评估 [08](../../08_RUNTIME_AND_TOOLING.md) §2.2）。
...
状态翻转由 owner 人工签署（AI 备绿）。新做不完事项 → 14 §4 追加 RD-010+ 双侧标注。
...
| SG-003 | 多后端 | G2 完成 + owner 解除红线 3（D-008） | not_triggered（红线 3 维持，一次一条） |
| SG-007 | 包 registry | D-312 社区规模驱动（>50 包/强需求） | not_triggered（本期休眠，留社区规模触发） |

G2 开工复评 decisions 已追加（维持 not_triggered，trigger_condition 0-byte 不改）。新触发方向 → SG-010+（owner 裁决）。
...
- **红线解除一次一条**（10 §9.2）:死亡路线解除只能 owner 批准且一次一条。
...
- 关键复诵:**规范先行（7）**（改 src 前读 spec，语义 PR 引 RXS-####，缺条款先补）/ **判档向上取严（8）**（AI 不自判 Direct）/ **不代签（1）**（status 翻转/基准切换/tag/红线解除/RD·SG 翻转由 owner 人工签署）。
...
- **自托管环境**:runner `rurix-dev-4070ti`（RTX 4070 Ti，driver 591.86，CUDA Toolkit 13.3，VS 2022 MSVC + Windows SDK）；脚本用 `py -3`；**evidence 由 owner 桌面会话兑现**（本地跑 `*_smoke.py` 后 `git restore -- evidence/`，AI 不提交 evidence；device 真实红绿 run URL 由 owner 上线回填，对齐 G1.1~G1.5 先例）。
