...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.4 | 2026-06-28 | **G2.3 owner 收口（步骤 47 落地）**：owner 白栀于本工作会话明确签署 G-G2-3 并批准 close-out 代录；AI 代录、非代签 G2 整体 close-out。计划步骤 47（绑定布局推导冒烟，G-G2-3，RFC-0005 / RXS-0163~0166）已落地为 `.github/workflows/pr-smoke.yml` 真实步骤 `ci/dxil_binding_device_smoke.py`（默认 `RURIX_DXC_DIR=H:\dxc-round7\extracted\bin\x64`，可由 repo variable 覆盖，`RURIX_REQUIRE_REAL=1` 缺 validator/D3D12/MSVC 即红），并由 PR [#109](https://github.com/qwasg/Rurix/pull/109) 最新 `pr-smoke` [run 28319166995](https://github.com/qwasg/Rurix/actions/runs/28319166995) 全量 success（head `84324f0`）验证。步骤 47 device 见证日志含 `DXIL_BIND: ok adapter="NVIDIA GeForce RTX 4070 Ti" rurix_rts0=accept tamper_rts0=reject sampled=64,127,255,255 draw=ok`；E2b-3 推导产物 host golden `tests/dxil/binding/fs_tex_samp.binding-golden`（`rts0.bytes.sha256=409b6a1e…c0a2`）owner bless（bless_log 2026-06-28 行），E2b-4 device witness 回填 `evidence/g2.3-binding-layout/binding_layout_device_smoke_20260628.json`（`run_url` = [run 28319066260](https://github.com/qwasg/Rurix/actions/runs/28319066260)）。G2 契约仍 active，不执行 `g2-closed` tag / 基准切换 / RD·SG 翻转。|
| v1.3 | 2026-06-27 | **G2.2 owner 收口**:owner 白栀于本工作会话监督确认 G-G2-2 device/run URL、DXIL 文本 golden bless 与子里程碑签字;AI 代录、非代签 G2 整体 close-out。远端 PR smoke [28284960733](https://github.com/qwasg/Rurix/actions/runs/28284960733) 全量 success(3m19s),步骤 46 同时跑 `ci/dxil_codegen_smoke.py` + `ci/dxil_device_smoke.py`,日志含 `DXIL_DEVICE: ok adapter="NVIDIA GeForce RTX 4070 Ti" pixel=64,127,255,255 draw=ok`。`tests/dxil/graphics/gfx_vs_min.dxil-disasm` 在 owner pin 环境(`H:\dxc-round7\extracted\bin\x64` signed DXC + `spirv-cross.exe`)经 `RURIX_BLESS=1 cargo test -p rurixc --features dxil-backend --test dxil_golden dxil_b_disasm_golden_matches_when_toolchain_present -- --exact --nocapture` 重 bless;入 golden 前 `dxv.exe` validator 接受。当前 golden 锁已登记 RD-013/RD-017 缺口下的 TEXCOORD baseline,不声称 output varying 用户语义保真已兑现。G2 契约仍 active,不执行 `g2-closed` tag / 基准切换 / RD 翻转。|
