...
| 27 | 三包 workspace 离线重建逐字节可复现门(契约 G-M6-1;M6.3 落地接入,GPU 队列无关 CPU-only):`rx build --locked --offline` 干净环境两次重建 content SHA-256 逐字节一致 + `rurix.lock` 解析图稳定。**实测命令(M6.3 回填)**:占位;计数核对 `py -3 ci/budget_eval.py`(`m6.counter.offline_rebuild_reproducible ≥1`)。**离线重建未落地 → 0 → normal SKIP**(M6.2 已落 manifest/lock/vendor 子系统 + 离线解析门 27a 铺底) | 是 |
