...
| v1.8 | 2026-06-15 | M6.5 回填 LSP 10k 行交互延迟 measured_local(契约 G-M6-2):新增 `bench/gen_lsp_workspace.py`(确定性生成 ~10k 行合法 .rx,前向调用链,来源留痕)+ `bench/lsp_bench.py`(经 `rurixc --tooling-server` 客户端 JSON-RPC 计时 completion/definition/publishDiagnostics(didChange 保存后),BENCH_PROTOCOL §3 trimmed mean;`rx bench lsp` 经 cmd_bench 泛分发自动路由,RD-003)+ `bench/lsp_latency_triple.py`(三次进程级独立运行聚合 + 回填)。`m6.bench.lsp_interaction_latency_ms` estimated → measured_local:实测(ms)completion 35.9975 / definition 29.9585 / publishDiagnostics 72.6226,逐交互阈值 = 实测 × 1.5 = 54.0 / 44.94 / 108.93(max 方向上界,publishDiagnostics 远在 07 §6 < 5s 行业线天花板内);标量 `threshold`=最坏 108.93(满足 check_schemas),逐交互上界在 `thresholds`,direction/unit/id 语义不改。新增 `milestones/m6/lsp_latency_evidence_schema.json` + `ci/check_schemas.py` 路由(`lsp_latency_` 前缀);`ci/budget_eval.py` 新增 `m6.bench.lsp_interaction_latency_ms` 特例分支(measured_local 读 `results.per_interaction` 逐交互对 `thresholds` 校验,normal/strict 同;estimated 路径不变)。证据 `evidence/lsp_latency_20260615_agg.json`(+ `_1`/`_2`/`_3` 三单 run)。nightly.yml 接入 "LSP 10k interaction latency trend archive"(`rx bench lsp --smoke` 功能哨兵 + 趋势 artifact,**趋势参考非硬阈门**;延迟达标判定在 close-out `--strict` 与 pr-smoke budget evaluator)。阈值为 agent 提案,待人工终审批准(硬规则 1)。`py -3 ci/budget_eval.py --strict` = PASS(全局零 estimated 残留)。真实 PR 红绿(#36,base main,§5 第 6 条):绿基线 commit `4f965cc` pr-smoke run [27537555626](https://github.com/qwasg/Rurix/actions/runs/27537555626) success;红验证 commit `8089870` 临时把 `thresholds.completion` 改到实测之下(54.0→10.0),pr-smoke run [27537714804](https://github.com/qwasg/Rurix/actions/runs/27537714804) 在 `budget evaluator load (CI_GATES §3.6)` 失败,日志含 `m6.bench.lsp_interaction_latency_ms.completion: FAIL — 35.998 违反 max 10.0`(后续步骤未执行,红精准落在 budget evaluator);绿验证 commit `25663c1` 用普通 `git revert` 复原阈值,pr-smoke run [27537807841](https://github.com/qwasg/Rurix/actions/runs/27537807841) success。#36 base `main`,普通 commit/revert 历史保留,未使用 reset/rebase/force-push(反 YAML-only,D11.8-2) |
| v1.9 | 2026-06-15 | M6 整里程碑 close-out 终审收官(CI_GATES §5 第 6 条):**guardrail 回退基准 `m5-closed → m6-closed`**(`ci/check_guardrails.py` `resolve_base()` 默认值 + 文件头 docstring + 行内注释;PR 路径仍以 `GITHUB_BASE_REF` 为准,既有逻辑不变;对齐 M5 v1.1/v1.5 `m4-closed→m5-closed` 切换先例)。切换后双基准核对(反 YAML-only,真实跑过,`m6-closed` annotated tag 锚定收官 PR 合入 main 的闭合 merge commit):`py -3 ci/check_guardrails.py m5-closed` = PASS + `py -3 ci/check_guardrails.py m6-closed` = PASS(0 changed paths,tag=HEAD),核对输出留痕 M6_CONTRACT §8.11(b)。close-out `budget_eval --strict` = PASS(42 pass / 0 skip,含 LSP 三类逐交互对阈 + 三计数器 6/1/6 + 全局零 estimated);`trace_matrix --check` = PASS(103/103);`check_schemas` = PASS;`check_fmt_idempotent` = PASS(108 files)。RD-003/RD-005 formal close(inherited→closed,registry/deferred.json v1.6,G-M6-4 rx bench/rx fmt 工具化兑现)、RD-004 已于 M6.4 closed、RD-007 维持 inherited 顺延;D-M6-5 VS Code 扩展/Natvis 首批人工部分顺延 RD-001/M8 留痕(M6_CONTRACT §8.10)。**M6 关闭判定(active→closed)与 LSP 延迟阈值定档由白栀/agent 自主签署(M6_CONTRACT §8.11),agent 自主签署**;LSP 逐交互阈值 54.0/44.94/108.93ms 受 #36 measured_local 0-byte 冻结,close-out 不改预算条目,签署"批准定档" |
