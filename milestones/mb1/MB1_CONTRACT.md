...
### G-MB1-7 全四要素 measured(2026-07-16 晚场,round-2)

同日上午 shell 直跑轮(上条)以 compute 数值对照 ✅ + 证据归档 ✅ 推进尾门,但 present(shell 进程无 ANativeWindow)+ on-device validation(production build loader 不为 shell 加载外部 layer)两要素归「最小 NativeActivity APK 壳」follow-up。**本轮经该 APK 壳(`com.rurix.vk` / `android.app.NativeActivity`,零 Java `hasCode=false` + `debuggable=true` + in-APK layer `.so`)在同一 HONOR BKQ-AN10 真机门控 RED→GREEN 补齐余两要素,四要素现全 measured。** 证据全文:[`evidence/mb1-android-ondevice/android_present_smoke_report.md`](../../evidence/mb1-android-ondevice/android_present_smoke_report.md)(+ `logcat_{red,green}.txt` / `present_result_{red,green,green2}.json` / `round1_halt_excerpt.md` 入库,PNG 记 scratch 路径 sha256 不入库)。装机 APK sha256 前32 `b1b2c99a…` == 预期,`librurix_vk.so` stage==APK 内嵌逐字节一致。

- **RED 机制迭代(诚实留痕)**:round-1「字节损坏 vertex SPIR-V 触 pCode-08742」在 Adreno/Android16+MTE 上令 **validation layer 自身 SIGSEGV**(signal 11 / `SEGV_ACCERR`,tagged-pointer use-after-free,栈顶 6 帧全在 `libVkLayer_khronos_validation.so`,VUID 未吐即崩)→ RED 取证失败,按协议 HALT。**此崩溃本身是 layer 上游鲁棒性 bug 被 MTE 硬抓的独立证据,非本项目缺陷**(逐字栈见 `round1_halt_excerpt.md`)。round-2 改 **graphics pipeline vertex stage 用模块内不存在的假入口名 `rurix_red_bogus_entry`(SPIR-V 完全合法,不喂非法字节)**触 `VUID-VkPipelineShaderStageCreateInfo-pName-00707`——天然消除 spec-UB/崩溃路径,不依赖 layer 解析非法字节的鲁棒性(`src/rurix-rt/src/vk.rs` `red_selftest` 分支;green = 真入口名 `c"main"`)。
- **RED measured PASS**:`validation_errors=1` + present fail-closed(`present_ok=false`)+ logcat 精确吐 `VUID-…-pName-00707`(RurixVK-VVL,含假入口名 + 运行期 module-inspect 细节)+ **进程 pid 18300 存活、全量 buffer 0 条 SIGSEGV**——layer 干净捕获故意 bug,round-1 崩溃模式消除。**RED 真红是 GREEN 零错可信的根基**(证 layer 确在看)。
- **GREEN measured PASS(两遍字节一致)**:`{present_ok:true, frames:3, ext_w:1256, ext_h:2808, covered:864256, corner_bg:true, center_covered:true, validation_errors:0}`——ext == WM `Display{#0 size=1256x2808}`;`present_frame_settled.png` 截屏实证插值三角形黑底满屏(像素反演覆盖率与 `covered` 吻合、覆盖块居中);green logcat 0 条 VVL/VUID。**独立 counter-agent 三路反证(layer-loaded / present-happened / provenance)全「无法反驳」high-confidence。**
- **DoD 四要素全 measured**:present ✅(§4 3 帧)/ validation ✅(§3 RED 干净捕获 → GREEN 零错)/ compute ✅(上午已档,`vk_saxpy` max_err=0 三厂商逐位一致)/ 证据归档 ✅(本 evidence 目录,LF 归一入库)。
- **G-MB1-7 维持 open,不自签**:四要素虽全 measured,**硬件尾门签署权归 owner**;agent 只归档 measured 证据、不代签、不伪造(区别于技术门 G-MB1-2~5 的 agent 自主签署)。gate 状态翻转 = owner 裁决,非 close-out 自动触发。工具件(NDK/build-tools/adb/layer `.so`/debug keystore/APK)不入库;`build_apk.ps1` 逐字记入 evidence 保可复现,CI runner 需同等 provisioning + 真机方复现真绿。
- **G-MB1-6(AMD 真卡)不受影响,维持 open**:Android 真机四要素达成 ≠ AMD 已验证。

### 进度记录(G-MB1-7 Android 真机首跑:compute + offscreen graphics measured,present/validation 余 APK 壳 — owner 提供设备,2026-07-16)

owner 提供 arm64 真机(**HONOR BKQ-AN10,Qualcomm SM8850〔Adreno 系〕,Android 16/SDK 36,production build**)并逐步授权工具下载(platform-tools/adb 经 sdkmanager;Khronos validation layer android-binaries-1.4.350.1),agent 经无线调试(mDNS 发现,USB 链路不稳弃用)真机真跑。证据全文:[`evidence/mb1-android-ondevice/android_ondevice_smoke_report.md`](../../evidence/mb1-android-ondevice/android_ondevice_smoke_report.md)。

- **compute 腿 measured PASS**:主机 `rurixc --target vulkan` 产 `saxpy.spv`(spirv-val accepted,sha256 对账)→ aarch64 release `vk_saxpy` 真机跑 → `VK_SAXPY: ok entry=rx_saxpy_8 n=1024 a=2 out[0]=0 out[1]=2.5 out[1023]=2557.5 max_err=0.00e0` + exit 0(复跑一致)。**out/max_err 与桌面 NVIDIA、lavapipe 归档值全等 → 同一 `.spv` 三家厂商驱动(NVIDIA/Mesa-lavapipe/Qualcomm-Adreno)数值逐位一致**(承 RXS-0207/0211 语义中性)。
- **offscreen graphics 腿 measured PASS(DoD 外附加证据)**:`vk_triangle tri_vs.spv tri_fs.spv` → `VK_TRIANGLE: ok W=64 H=64 covered=968 center=(130,59,65)` + exit 0,**covered=968 与桌面 NVIDIA(上方 G-MB1-4 记录)全等**。
- **validation layer 腿 BLOCKED(OS 策略,诚实不充绿)**:production build(`ro.debuggable=0`)loader 不为 shell 进程加载外部 layer(`vkCreateInstance` = -6 VK_ERROR_LAYER_NOT_PRESENT;`/data/local/debug/vulkan` Permission denied)→ 归 debuggable APK + `enable_gpu_debug_layers` 官方通道(下条)。
- **G-MB1-7 维持 open,不签**:DoD 四要素中 compute 数值对照 ✅ / 证据归档 ✅;**ANativeWindow present N 帧 ❌(shell 进程无窗口)+ on-device validation 零报错 ❌(上条)——两者共同归「最小 NativeActivity APK 壳」工件(Phase B,新 follow-up)**。尾门状态从「缺硬件 pending-hardware」推进为「**有硬件,余 APK 壳工件**」。工具件(adb/validation layer .so)不入库,provisioning 注同上。
