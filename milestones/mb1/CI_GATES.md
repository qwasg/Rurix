...
| §2.58 | `ci/vulkan_present_smoke.py` | MB1 W6 win32 swapchain present:本机 NVIDIA 隐藏 win32 窗口 + `VkSurfaceKHR`(`VK_KHR_win32_surface`)+ `VkSwapchainKHR`(`VK_KHR_swapchain`)→ 渲染数帧居中三角形 → `vkCmdCopyImageToBuffer` 回读像素对照(背景角==clear / 中心覆盖非背景 / covered>0)→ 转 `PRESENT_SRC_KHR` → `vkQueuePresentKHR` 逐帧成功 + `VK_LAYER_KHRONOS_validation` 零报错(messenger fail-closed)+ 内建 `red_self_test`(provenance-带保名 `.spv` 喂同 present 管线 → VUID-...-08742 → 退出码判红);**discharge RXS-0210 L4 present-defer(RD-032 code-deferral)**;非 Windows → SKIP(win32 surface 不可用,Android present = G-MB1-7);AMD 真卡 present 像素校验 = G-MB1-6 尾门;缺 Vulkan 设备 → SKIP dev-env degrade | G-MB1-4 | 是(GPU runner 硬红) |
