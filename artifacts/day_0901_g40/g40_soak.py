#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G40 收役 soak ≥1800s @缺省面(g39_soak.py 同型;锚断言钉死非自举)。

差异(G39 版 → G40 版):产物隔离 artifacts/day_0901_g40/soak/;其余口径
逐字沿用——32f 迭代锚钉死 G38 在案值 a8204b3b…(缺省面全役加性 off 预期
恒值,断言非自举);前置双锚 all-off==55e4a92d + full19==a5521e47;
Stage A 探针 it4/it9;frame_ms 对 11.111 预算记账;VUID=0 门。
"""
...
OUT = ROOT / "artifacts" / "day_0901_g40" / "soak"
EV = OUT / "soak_ev"
MIN_WALL_S = 1800.0
BUDGET_MS = 11.111
...
# G38 W6_FULL_SOAK.json anchor_32f 在案值(缺省面 32f/warmup2 口径,钉死非自举)。
...
def main() -> int:
    ...
    t_start = time.time()
    it = 0
    with gpu_device_lock(purpose="G40 收役 soak >=1800s(锚钉死)", timeout_s=4 * 3600.0):
    ...
    doc = {
        "schema": "rurix.day0901.g40.soak.v1",
        "caliber": "缺省面(=full19)32f/warmup2 迭代锚钉死 a8204b3b(G38 在案非自举)+ 前置 all-off/full19 双锚断言 + Stage A 探针 it4/it9",
        "iterations": it,
        "wall_s": round(wall, 1),
    ...
    (OUT / "G40_SOAK.json").write_text(
        json.dumps(doc, ensure_ascii=False, indent=1) + "\n", encoding="utf-8")
    print("G40SOAK", json.dumps({"verdict": doc["verdict"], "iterations": it,
