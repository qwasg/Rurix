#!/usr/bin/env python3
"""G40 W0 开役基线复证(断言型,零收割——本役预期零重锚;g39_baseline.py 同型)。
...
产物:artifacts/day_0901_g40/w0_baseline/G40_BASELINE.json + baseline_log.jsonl。
"""
...
ROOT = Path(r"H:\rurix")
G40 = ROOT / "artifacts" / "day_0901_g40" / "w0_baseline"
EV = G40 / "ev"
LOG = G40 / "baseline_log.jsonl"
...
def main() -> int:
    ...
    for exe in (WIN, PERF):
        if not exe.exists():
            ...
            return 2
    with gpu_device_lock(purpose="G40 W0 开役基线复证(三锚断言)"):
        # n1 all-off
        ...
        if not ok:
            FAILS.append("n4")
    out = {
        "schema": "rurix.day0901.g40.baseline.v1",
        "fails": len(FAILS),
    ...
    (G40 / "G40_BASELINE.json").write_text(
