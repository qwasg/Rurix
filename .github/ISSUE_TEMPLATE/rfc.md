...
<!-- 先按 CONTRIBUTING.md「变更分档(三档门)」自助判定。Full RFC 触发面 = 新语法 /
     ...
     设计原则修改 / 死亡路线触碰。判档不清 → 向上取严(自我约束建议)。agent 完全自主。 -->
...
<!-- 大致设计方向 + 触及的禁区(UB / 内存模型映射 / FFI ABI / 安全包络由 agent 自主经 Full RFC 落笔)。 -->
...
- [ ] 开 PR,经 FCP-lite 评审窗(advisory 公开等待窗,见 `rfcs/README.md` §3;agent 自主推进)
