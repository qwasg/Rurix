...
## Project status: language 1.0 released (`v1.0.0`); G5 and G6 closed; G7 Production Frame Closure is active

The first-layer full acceptance (01 §6) is met. The three flagship use cases run end-to-end on real hardware, the resource-lifetime error classes are 100% intercepted at compile time, and every existing performance threshold is backed by `measured_local` evidence with zero `estimated` entries. G6 deliberately defined no hard performance gate; G7 requires a non-empty measured frame budget before implementation can advance:
