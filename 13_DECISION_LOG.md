...
| D-408 | owner 保留权白名单（D-406 v2.0 完全自主默认下的 carve-out 集合显式化） | **状态：Proposed — 待 owner 签署**（不写「已批准/已选定」；本条定义 owner 自身权力面，agent 起草不自批，镜像 [RFC-0011](rfcs/0011-vulkan-spirv-backend.md) 红线 carve-out=owner 裁决 / agent 不自签先例）。D-406 v2.0（[10](10_GOVERNANCE.md) §7）声明「AI agent 完全自主行动与决策，无 agent 批准门 / 人类签字」为**默认治理口径**；但实操已积累七类 owner 保留权 carve-out，与该字面存在张力，本条显式化之。每项标注：**agent 可起草 / 代录机器事实，签署 / 触发权留 owner**。① **死亡路线 / 红线解除**——owner 主动裁决「一次一条」并留痕（依据 10 §9 第 2 条 + D-008 红线 3 解除 errata〔§8 v2.1〕；RFC-0011 状态字段记「10 §9.2 owner 主动决策，非 close-out 自动触发」carve-out）；② **真机 / 硬件验收签署**——owner 授权、agent 代录非代签（依据 MB1 acceptance_gates G-MB1-6〔AMD 真卡，open〕/ G-MB1-7〔Android 真机，owner 明确授权代录签署，§8 v2.2〕，milestones/mb1/MB1_CONTRACT.md）；③ **outward-facing 提报**（上游 issue / 以项目身份对外发布件）——owner 亲自，agent 只备 `DRAFT — do NOT file`（依据 milestones/ea1/EA1_CONTRACT.md §6 guardrail「agent 不对外提报」+ OWNER_DECISION_PACKAGE.md §2 ③）；④ **里程碑立项 / 期次拍板**——owner（依据 EA1_CONTRACT.md §7 开工裁决 ①「立项 = owner 拍板」+ D-005）；⑤ **生产签名 secret + 信任根锚 PR 合并**——人工门（依据 spec/release.md §4「生产签名经 CI secret + 人工门控」+ EA1 OWNER_DECISION_PACKAGE.md 裁决 A「信任根登记流…owner 合并 = 人工门」）；⑥ **NVIDIA 再分发白名单**——`pending-human-review`（依据 D-313 + spec/release.md §4 EULA Attachment A 白名单法律核对）；⑦ **使命 / 验收成败尺度定义**——owner（依据「agent 不自定成败尺」EA1 OWNER_DECISION_PACKAGE.md §1-C + MS1 主语言判据 owner 三裁定，milestones/ms1/MS1_CONTRACT.md §7 ③）。**本条为 P-13「agent 完全自主」的实质收窄**（准永久面），按 10 §9.3 额外 30 天公示（公示起点 = 本 errata PR 开启日，合并权限 = owner）、按 [04](04_DESIGN_PRINCIPLES.md) 头部属 Full-RFC 级最高档变更；**agent 不自批，待 owner 签署后由 owner 授权转「已选定 / 已批准」**。 | [10](10_GOVERNANCE.md) §7 v2.1 · [04](04_DESIGN_PRINCIPLES.md) P-13 |
...
| v2.3 | 2026-07-17 | **新增 D-408（owner 保留权白名单）——Proposed，待 owner 签署**：§6 治理决策追加 D-408，把 D-406 v2.0「agent 完全自主」默认之上实操已积累的七类 owner 保留权 carve-out（① 死亡路线 / 红线解除 ② 真机 / 硬件验收签署 ③ outward-facing 提报 ④ 里程碑立项 / 期次拍板 ⑤ 生产签名 secret + 信任根锚 PR 合并 ⑥ NVIDIA 再分发白名单 ⑦ 使命 / 验收成败尺度定义）显式化，逐项附依据先例。**本条定义 owner 自身权力面，agent 起草不自批**（镜像 RFC-0011 红线 carve-out = owner 裁决 / agent 不自签先例）；触 P-13 准永久面（实质收窄「agent 完全自主」），按 10 §9.3 额外 30 天公示（公示起点 = 本 errata PR 开启日，合并权限 = owner），按 04 头部属 Full-RFC 级最高档变更。配套：10 §7 追加 v2.1 调和条款 + 04 P-13 追加交叉引用一行（不改 P-13 陈述本体）。状态 = Proposed，owner 签署后转「已选定 / 已批准」。规划文档勘误（00 §6.3 追加式修订，独立 errata PR，check_planning_docs advisory 不阻断；**本 PR 由 owner 审签，agent 不自合**）。**与 D-409（P1-3 RFC 对抗性评审，另一 errata PR）在本文档 §6 决策表 / §8 修订记录相邻行有预期 rebase 冲突，建议本 PR(D-408) 先合。** |
