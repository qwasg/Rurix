...
| 状态 | **Draft / Awaiting Owner**。AI 起草骨架 + measured spike 留痕；**owner FCP-lite 批准前不得推进下游实现 PR（硬规则 1，AI 不代签、不自判 Direct）**。所有路径性抉择落 §9 待裁清单，AI 不自填默认、不自判 Direct |
| 承接里程碑 | G2.3（验收门 **G-G2-3**，D-G2-3），承 G2.1 着色阶段类型面（RFC-0002）+ G2.2 图形=B codegen（RFC-0004）；可与 G2.2 部分并行 |
| 关联条款 | 拟落 spec **RXS-0163 起**（区间随 §9 Q-Range 裁定，见 §5）；落点（延伸 `spec/dxil_backend.md` vs 新建 `spec/binding_layout.md`）= §9 Q-File 待裁。**本 RFC 不创建裸条款头**，trace 维持现状 |
...
| Provenance | `Assisted-by: kiro:claude-opus-4-8`。Human-in-the-loop（硬规则 1/2）：本草案由 AI 起草，§4 measured 锚定来自隔离 spike 真实命令输出，禁区子节仅作边界声明；§9 全部路径抉择留 owner，owner 批准前不推进下游实现 |
| Owner 批准 | 〈待 owner FCP-lite — Draft / Awaiting Owner〉 |

> **批准记录占位**：本 RFC 触及 🔒 签名/绑定二进制 ABI 布局（RFC-0004 §4.6(a) 同级:register/space/mask/packing/descriptor table 偏移/root parameter DWORD 物理布局）、纹理路径内存模型映射（06 §4.2）、DXIL·SPIR-V UB 边界——这些只能由人类经 Full RFC 落笔（硬规则 5）。本 RFC 在 §4/§8 仅作**边界声明**，不落禁区语义本体；§9 全部路径抉择（Q-Space / Q-RootShape / Q-Sampler / Q-Bindless / Q-Gate / Q-Err / Q-File / Q-Inference-vs-Explicit）留 owner 裁决，AI 不自填默认值「偷偷定型」、不自判 Direct。
...
绑定布局推导失败 = **结构化编译期 6xxx 诊断**（无运行期 fallback、无静默降级，P-01）。推导失败类别（错误码段位 = §9 Q-Err 待裁）：
...
落点（延伸 `spec/dxil_backend.md` vs 新建 `spec/binding_layout.md`）= **§9 Q-File 待裁**。**本 RFC 不创建 `### RXS-####` 裸条款头**——下表为条款新增的**计划表**，条款体随 owner 批准本 RFC 后的实现 PR 同落（条款 PR 先于实现 PR，硬规则 7；trace 维持全锚定）。**区间大小/拆分 = §9 Q-Range 待裁**，下表为 AI 建议形态（自 **RXS-0163** 起续号，当前最高现存 RXS-0162 @ `spec/dxil_backend.md`），非已锁定。

| 条款（计划，待 Q-Range 锁定） | 标题 | 测试锚定计划（每条 ≥1，`//@ spec`） |
|---|---|---|
...
> 实际条款数/拆分以 §9 Q-Range owner 裁决为准（可能并条或增条）；上表为计划锚点，**不预占编号**——条款体落地时按当时最高号续。

- **错误码策略**：推导失败归 **6xxx codegen 段**（只追加）。**不预留、不预造**：新可达类别随实现 PR 按**当时实际最高号续**分配 + en/zh message-key（`registry/error_codes.json` 只追加，`ci/bilingual_coverage.py` 覆盖）。注意 6xxx 段当前最高 = **RX6013**（RX6008/RX6009 已被 RD-012/RD-013 预引占用，不复用；**RX6014 在 `spec/dxil_backend.md` RXS-0160 IR3 作错链候选码被提及但 registry 未分配**——本 RFC 不预占 RX6014，落码时按当时 registry 实际最高号续，避免与 RXS-0160 错链码归类争号）。错误码段位最终归类 = §9 Q-Err 待裁。
- spec 条款 PR 先于实现 PR（硬规则 7）；trace_matrix 维持全锚定（沿用全局 counter）。

## 6. feature gate / tracking / 实现序（10 §3 要件）

- **feature gate**：复用 `dxil-backend` vs 新开 `binding-layout` = **§9 Q-Gate 待裁**（AI 倾向复用 `dxil-backend`，绑定推导为其图形分支的推导面，避免暴露新用户面组合维度——但留 owner 裁）。
- **栈式 PR（门控于本 RFC 批准 + §9 裁定后）**：
  - **PR-E1 spec 脚手架**：spec 落点（Q-File 裁定后）登记 RXS-0163 起预留区间（**不落裸条款头**）+ README §4/§5 同步；`trace_matrix --check` PASS（维持全锚定、零新 RXS、零裸条款头、零悬空锚点）。
...
- **register/space 分配轴：单一全局 binding 轴 vs 按种类分轴**：两者 measured 都确定性、都产合规 DXIL（§4.2 measured-1）——**留 §9 Q-Space owner 裁**，本 RFC 不预定。
- **SPIR-V 作绑定中间表示 vs 自写资源绑定 IR**：复用 B 链既有 SPIR-V 装饰通道（RFC-0004），不另起资源绑定 IR；SPIR-V 在此仅作 B 路内部中间表示（≠ 对外通用目标，D-008/SG-003 维持）。
- **bindless / descriptor heap 直索引**：是否进本期 = §9 Q-Bindless（AI 倾向 defer，取严不自裁）；否决「本期即做完整 bindless」——范围蔓延风险（复杂度黑洞 SG-005）。
...
- **bindless / unbounded descriptor array 完整支持**：是否 defer / out_of_scope = §9 Q-Bindless 待裁；若裁剪须登 owner 待裁（草拟 SG-010+/RD-018+ 文案进本 RFC，不擅自落 registry）。
...
## 9. §9 Q-Range owner 待裁问题清单（全部留 owner，AI 不自填默认、不代决、不自判 Direct）

> 以下为本 RFC 的**路径性抉择**，全部留 owner FCP-lite 裁决。每项给候选 + **AI 倾向（供参，不代决；判档/裁剪争议向上取严）** + 裁决列〈待 owner〉。AI **不**把任一倾向写进 §3~§8 作为已定型默认（§3 示例仅示意类型面声明，不示意已定布局策略）。涉裁剪（如 bindless out_of_scope）的项若 owner 倾向裁剪，须登 SG-010+ / RD-018+（AI 已草拟文案于本节，**不擅自落 registry**，owner 签）。

| Q | 待裁项 | 候选（2–3） | AI 倾向（供参，不代决；取严默认） | 裁决 |
|---|---|---|---|---|
| **Q-Space** | register/space 分配策略 | (A) 按声明序单一全局 binding 轴打包（spirv-cross 默认，t/s/b/u 索引跨种类连号，measured `b0,t1,t2,s3,s4`）；(B) 按资源种类分轴（每类 t/s/u/b 各自从 0 计数，可选各占独立 space）；(C) 纯推导 + 开放 `#[binding(space=, register=)]` 显式标注覆盖 | 倾向 **(B) 按种类分轴**（更贴 D3D12 惯例与 root signature descriptor range 组织，host 侧 binding 直观）；**取严默认**：先不开放 (C) 显式标注（显式标注触新语法面 + 与 Q-Inference-vs-Explicit 耦合，向上取严留后期）。两轴 measured 都确定性，**不自填**，留 owner | 〈待 owner〉 |
| **Q-RootShape** | root signature 形态推导规则 | (A) 全 descriptor table（最简、最省 root DWORD，但每次绑定走 heap）；(B) CBV 走 root descriptor + SRV/UAV/Sampler 走 descriptor table（混合）；(C) 小常量走 root constant + 其余混合 | 倾向 **(B) 混合：CBV root descriptor + 其余 descriptor table**（兼顾常量缓冲低延迟与上限友好）；**取严默认**：root constant (C) 本期不自动推（需常量大小阈值策略，易过度工程），先不做、留后期。**不自填**，留 owner | 〈待 owner〉 |
| **Q-Sampler** | RXS-0156 `Sampler` 默认归 static 还是 dynamic sampler | (A) 默认 static sampler（编译期固定采样状态，省 descriptor，但采样参数不可运行期改）；(B) 默认 dynamic sampler（descriptor table/heap 绑定，运行期可改）；(C) 由句柄类型/标注区分 | 倾向 **(B) dynamic sampler 为默认**（语义最不意外、运行期可配置；static sampler 需采样状态在编译期已知，属优化而非默认）；**取严默认**：static sampler 作后期可选优化，不在本期默认推。**不自填**，留 owner | 〈待 owner〉 |
...
| **Q-Gate** | feature gate | (A) 复用 `dxil-backend`（绑定推导为其图形分支推导面）；(B) 新开 `binding-layout` 子 gate | 倾向 **(A) 复用 `dxil-backend`**（避免暴露新用户面组合维度，对齐 RFC-0004 Q-Gate-B 复用先例）；**不自填**，留 owner | 〈待 owner〉 |
| **Q-Err** | 推导失败错误码段位 + 类别 | 归 6xxx codegen 段，**按当时 registry 实际最高号续**（当前最高 RX6013；RX6008/RX6009 已被 RD-012/RD-013 预引，**RX6014 在 RXS-0160 IR3 作错链候选码被提及但 registry 未分配**——本 RFC 不预占）；类别 = 超 64 DWORD 上限 / 资源种类不可映射（复用 RX6013 vs 新开）/ register 冲突 | 倾向 **落码时按当时实际最高空号续、不预占 RX6014**（与 RXS-0160 错链码归类解耦，避免抢号）；「资源种类不可映射」**倾向复用 RX6013**（同语义类），超上限 / register 冲突 **倾向新开**；**不自填**，留 owner（与 RXS-0160 RX6011 复用/RX6014 新开裁决一并看更佳） | 〈待 owner〉 |
| **Q-File** | spec 落点 | (A) 延伸 `spec/dxil_backend.md`（绑定推导承 DXIL 后端语义面，RXS 续号同文件）；(B) 新建 `spec/binding_layout.md`（绑定布局独立语义面文件） | 倾向 **(B) 新建 `spec/binding_layout.md`**（绑定布局是独立语义面、可与 G2.4 PSO/资源状态承接，避免 dxil_backend.md 过载；对齐 shader_stages.md 独立成文先例）；**不自填**，留 owner | 〈待 owner〉 |
| **Q-Range** | RXS 区间大小 / 拆分 | §5 计划表 AI 建议 4 条 RXS-0163~0166（资源绑定降级面 / register-space 推导 / root signature 形态+RTS0 / 一致性校验门+strict-only）；可并条或增条 | 倾向 **4 条 RXS-0163~0166**（与推导面四阶段对齐）；区间**不预占**、不预造，条款体落地时按当时最高号续；**不自填**，留 owner | 〈待 owner〉 |
| **Q-Inference-vs-Explicit** | 纯推导 vs 允许用户标注覆盖 | (A) 纯推导（P-11 推导优先，本期不开放覆盖）；(B) 推导优先 + 允许 `#[binding(...)]` 显式覆盖（本期纳入）；(C) 推导优先 + 覆盖留后期 | 倾向 **(C) 推导优先、覆盖留后期**（P-11 推导优先；显式覆盖触新语法面 + 与 Q-Space (C) 耦合，向上取严不在本期定型）；**取严默认**：本期纯推导，覆盖作后期独立判档。**不自填**，留 owner | 〈待 owner〉 |

**待 owner 裁决的 registry 文案草拟（AI 不擅自落 registry，owner 签后落）**：

- **RD-018（草拟，Q-Bindless 若裁 defer）**：title「bindless / unbounded descriptor array / descriptor heap 直索引绑定推导」；reason「本期 G2.3 绑定推导先收敛有界 descriptor 布局；bindless 属 SM6.6+ resource heap 大面，本期纳入与预算不成比例」；backfill_condition「UC-04 或后续里程碑出现 bindless 硬需求时按 10 §3 判档接通」；owner_milestone「G2」；status「open（待 owner）」。
- **SG-010（草拟，Q-Bindless 若裁 out_of_scope）**：direction「bindless / descriptor heap 直索引」；trigger_condition〈owner 定〉；permanence「conditional」；current_verdict「not_triggered（待 owner）」。

> 二选一（RD-018 defer 还是 SG-010 gating）由 owner 裁；AI **不**双开、不预判。

## 10. 稳定化与 provenance

- **稳定化**（10 §5）：绑定布局推导经 feature gate（Q-Gate 裁定后）→ tracking → 两里程碑无重大修订 → stabilization report → FCP-lite（10 §2.2）；推导产物面/root signature 形态在首个 stable 前不进 stable 面（随 RD-008）。register/space/packing 物理布局不进 stable 语言保证（§4.5(a) 🔒）。
- **Provenance**：`Assisted-by: kiro:claude-opus-4-8`。本草案由 AI 起草；§4 measured 锚定来自隔离分支 `spike/g2.3-binding-layout` 真实命令输出（§11）；§4.5 🔒 禁区维持边界声明不落语义本体；§9 全部路径抉择留 owner FCP-lite 裁决。**owner 批准前不推进下游实现 PR；AI 不代签、不代决、不自判 Direct**（硬规则 1/8）。下游 spec/实现 PR 按硬规则 7 序进（spec 先于实现）。
...
- **registry**：6xxx 段当前最高 RX6013（RX6008/RX6009 RD-012/RD-013 预引；RX6014 RXS-0160 错链候选未分配,本 RFC 不预占,§5/§9 Q-Err）· RD-018 / SG-010 草拟文案见 §9（owner 签后落,AI 不擅自落 registry）。
