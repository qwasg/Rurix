...
| 档位 | **Full RFC**（10 §3：新 codegen 路径 + 第二中间表示(SPIR-V) + 外部转译依赖；触 AGENTS 硬规则 5 禁区——DXIL/SPIR-V UB 边界 / 纹理路径内存模型映射(06 §4.2) / 签名·FFI ABI 二进制布局；§4.6 经 owner 授权代理裁决 PR-D2 的规范性边界；且触**准永久公理 P-01(strict-only)**——§4.4 为图形=B 的 strict-only 达标要求,不设例外） |
| 状态 | **Accepted / Owner Approved（2026-06-25）**。初版 RFC 经 owner FCP-lite 批准；PR #104 合入后，owner 在当前 Codex 会话进一步明确授权代理完成 PR-D2 前置人工裁决，§4.6 与 §9 新增 Q-ABI-B/Q-Texture-B/Q-UB-B/Q-D205-B 四项规范性边界。该授权不代表 G-G2-2 device 真跑、法律许可或生产证书签名已完成 |
...
| Provenance | `Assisted-by: kiro:claude-opus-4-8`（初版）+ `Assisted-by: codex:gpt-5`（PR-D2 owner 裁决增补）。owner/qwasg 于 2026-06-25 当前 Codex 会话明确授权 AI 代理完成技术裁决、落笔与机械合并，并保留事后 review；代录不得被解释为已完成设备、法律或密钥验证 |
| Owner 批准 | **Approved — owner（Language Lead）2026-06-25；PR-D2 增补授权同日确认**。批准范围：§4.4 strict-only 达标要求；§4.6(a) packing/寄存器布局降级为外部 conformance 说明、(b) 未建模纹理操作显式拒绝、(c) 不建立独立 DXIL/SPIR-V UB 契约；§4.5/D-205 当前不 bump、不触发 A-graphics 迁移；§9 全部裁决。记录方式：Codex 按 owner 本会话明确授权代理代录并执行，owner 事后 review。本批准不声称 G-G2-2 device 真跑、再分发法律审计或生产证书签名已完成 |
...
本 RFC 定义**图形=B 的设计面 + 混合分发判据 + 下游条款计划**；§4.4 落 strict-only 达标要求；§4.6 落 PR-D2 所需的三项 owner 规范性边界，但不自创任何寄存器编号、字节偏移、描述符编码、采样 opcode 或 UB 规则；codegen 实现留 PR-D2。
...
**为何需要 Full RFC（而非 Direct/Mini）**：本 RFC 引入**第二中间表示(SPIR-V)+ 外部转译依赖链(SPIRV-Cross/dxc/glslang)**,且触及 **签名/FFI ABI 二进制布局**(§4.6)、**DXIL 文本语义 UB 边界**、**纹理路径内存模型映射(06 §4.2)**——10 §3 / 硬规则 5 明列的 Full RFC / 禁区触发面;更触及**准永久公理 P-01(strict-only)的达标要求**(§4.4)。判档争议向上取严(硬规则 8);AI 不自判 Direct/Mini、不代签批准/合并(硬规则 1)。
...
> 本节落笔**混合分发架构与 B 路图形降级的设计面**；§4.4 为 **strict-only 达标要求**；§4.6 为 owner 授权代理落定的 PR-D2 规范性边界：外部 conformance / 未建模纹理显式拒绝 / 不建立独立后端 UB 契约。具体寄存器值、描述符编码与新 UB 规则仍不由本 RFC 发明。
...
- **当前 owner 裁决（PR-D2 前置）**：**不 bump D-205，不触发 A-graphics 迁移**。在 #90504 + #57928 均 merge、进入正式 release、该 release 可被 D-205 pin 覆盖、且 A 路通过与 B 路同口径的签名/validator/device 红绿前，图形分支维持 B。条件满足仅触发重新裁决，不自动迁移。
...
### 4.6 🔒 PR-D2 规范性边界（owner 授权代理裁决）

> owner/qwasg 于 2026-06-25 当前 Codex 会话授权代理完成以下 Full RFC 裁决。裁决的作用是给 PR-D2 划定可实现边界；它不声称人工设备测试、法律审计或生产签名已经完成。
...
落 `spec/dxil_backend.md`(承 RFC-0003)。**本 RFC 不创建 `### RXS-####` 裸条款头**——下表为条款重构/新增的**计划表**,条款体随 owner 批准本 RFC 后的实现 PR 同落(条款 PR 先于实现 PR,硬规则 7;trace 维持全锚定)。**区间裁决已锁定**(见 §9 Q-Range-B)。
...
- **🔒 P-01 边界/例外的规范性裁断**(§4.4):owner 已裁 P-01 不开例外——§4.4 仅定义 **strict-only 达标要求** 及其细化边界(「用户可观察」精确边界 / 未用输入 = 显式诊断 / 运行期等价验证形态),**不**写任何“例外豁免”。
- **绑定布局推导**(G2.3,P-11)/ **UC-04 渲染器**(G2.4)/ **PSO·资源状态·barrier 运行时面**:不在本 codegen RFC。
- **D-205 pin bump / A-graphics 迁移触发**：本次 owner 裁为“不 bump / 不迁移”；未来满足 §4.5 全部条件后另行裁决。
- **语言面扩展**：着色阶段类型面属 G2.1(RFC-0002);本 RFC 是 codegen 面,不新增语言构造。

## 9. 关键裁决（owner 授权代理代录；随本增补合入生效）
...
- **Provenance**：初版 `Assisted-by: kiro:claude-opus-4-8`；PR-D2 owner 裁决增补 `Assisted-by: codex:gpt-5`。owner/qwasg 于 2026-06-25 当前 Codex 会话明确授权代理完成技术裁决、落笔和机械合并，并将事后 review。该授权记录不替代 G-G2-2 device run URL、再分发法律签署或生产证书/CI secret 签名。
...
| Owner addendum v1.0 | 2026-06-25 | PR #104 合入后，owner/qwasg 在当前 Codex 会话明确授权代理完成 PR-D2 前置人工裁决：Q-ABI-B=packing/寄存器布局降级为外部 conformance；Q-Texture-B=仅 opaque handle 形态可过、访问语义未建模即 6xxx；Q-UB-B=不建立独立 DXIL/SPIR-V UB；Q-D205-B=当前不 bump、不迁移。同步 §4.5/§4.6/§8/§9/front matter/provenance。授权不代表 device、法律或生产签名已完成 | Full RFC（Owner-authorized proxy addendum） |
