...
本 RFC 只定义**图形=B 的设计面 + 混合分发判据 + 下游条款计划**;**§4.4 strict-only 达标要求、§4.6 🔒 禁区边界声明、codegen 实现**均按各自边界收口——前两者只落定稿文本与边界声明,**不**落例外豁免或禁区语义本体,后者留实现 PR(硬规则 7)。
...
> **本子节只落边界声明,不落禁区语义本体（硬规则 5 / 06 §4.2）。**

- **(a) 签名/内建变量·FFI ABI 二进制布局**：本 RFC 对签名元素**只**承诺源码层“是否存在/何种语义名/何种系统值/何种插值契约”;**不**承诺寄存器编号、顺序、component mask、packing、字节偏移、根参数/常量缓冲二进制布局。此类值由 dxc/D3D12/后端 conformance 决定,不进入语言 stable surface、不得被 spec/golden/用户代码当作语言保证。
- **(b) 纹理路径内存模型映射**：在独立 Full RFC 前,本 RFC **不**定义纹理/采样器在 SPIR-V/DXIL 的描述符编码、采样 opcode 映射、缓存一致性、导数/LOD 细节、越界采样后果或任何 memory-order 语义。凡某构造的正确性需要依赖这些未建模映射,编译器必须显式拒绝,不得猜测性 lower 后把责任推给下游工具链。
- **(c) DXIL/SPIR-V 文本语义 UB 边界**：Rurix **不**为中间表示(SPIR-V)或目标表示(DXIL)建立独立于源码语义的 UB/poison/undef 契约。对源码层已定义的程序,编译器不得借后端未定义/未指定行为空间实施静默语义漂移;凡 lowering 需要依赖未建模 UB 差异方能成立,必须显式编译错误而非接受产物。
...
- **🔒 签名/FFI ABI 二进制布局 / 纹理路径内存模型映射(06 §4.2)/ DXIL·SPIR-V UB 边界**(硬规则 5 禁区):§4.6 **只落边界声明**,不落禁区语义本体或实现细节。
