...
>
> **RFC 台账校准（2026-08-24，G23.1 治理波）**：**RFC-0040**（[`0040-physics-platform-deepening.md`](0040-physics-platform-deepening.md)，G23 物理平台深化：Jolt 5.6 采纳臂重判 + 神经变形重判 + 研究轨/物理 P3+ 处置程序）经 D-409 对抗性评审（评审 `milestones/g23/design/rfc0040_adversarial_review.md`）后 **Agent Approved**——编号按 G23.1 实测 `namespaces.RFC next_free=40` 领取（ledger 校准：on_tree_max 39→40、next_free 40→41）。此后自由号以 ledger 实测为准（当前 RFC-0041）。
>
> **RFC 台账校准（2026-08-24，G24.1 治理波）**：**RFC-0041**（[`0041-presentation-tail-gate-cleanup.md`](0041-presentation-tail-gate-cleanup.md)，G24 呈现与尾门清理：三行机器取证重判 + SAFE-GPU 处置 + 历史 RD 清册程序）经 D-409 对抗性评审（评审 `milestones/g24/design/rfc0041_adversarial_review.md`）后 **Agent Approved**——编号按 G24.1 实测 `namespaces.RFC next_free=41` 领取（ledger 校准：on_tree_max 40→41、next_free 41→42）。此后自由号以 ledger 实测为准（当前 RFC-0042）。
>
> **RFC 台账校准（2026-08-24，G25.1 治理波）**：**RFC-0042**（[`0042-campaign-final-review.md`](0042-campaign-final-review.md)，G25 战役终审程序：画质终态维持核验 + fps 终判两态 + 全链零降级 + 承接锚归档）经 D-409 对抗性评审（评审 `milestones/g25/design/rfc0042_adversarial_review.md`）后 **Agent Approved**——编号按 G25.1 实测 `namespaces.RFC next_free=42` 领取（ledger 校准：on_tree_max 41→42、next_free 42→43）。此后自由号以 ledger 实测为准（当前 RFC-0043）。
>
> **RFC 台账校准（2026-08-25，G26.1 治理波）**：**RFC-0043**（[`0043-framegen-device-kernel-realization.md`](0043-framegen-device-kernel-realization.md)，G26 时域/帧生成 device 化：FG/MFG device kernel 兑现语义 + RD-045 backfill 重判程序 + G17-MD-F1 重判窗程序）经 D-409 对抗性评审（评审 `milestones/g26/design/rfc0043_adversarial_review.md`，11 findings〔0 blocker/6 major/5 minor〕全部 disposition，v0.2 修法批）后 **Agent Approved**——编号按 G26.1 实测 `namespaces.RFC next_free=43` 领取（ledger 校准：on_tree_max 42→43、next_free 43→44）。此后自由号以 ledger 实测为准（当前 RFC-0044）。
>
> **RFC 台账校准（2026-08-25，G27.1 治理波）**：**RFC-0044**（[`0044-geometry-device-realization.md`](0044-geometry-device-realization.md)，G27 几何 device 化：HZB device kernel 兑现语义 + M61 重判程序 + cluster P4 差距重判程序 + M98-l4 重判窗程序）经 D-409 对抗性评审（评审 `milestones/g27/design/rfc0044_adversarial_review.md`，11 findings〔0 blocker/5 major/6 minor〕全部 disposition，v0.2 修法批）后 **Agent Approved**——编号按 G27.1 实测 `namespaces.RFC next_free=44` 领取（ledger 校准：on_tree_max 43→44、next_free 44→45）。此后自由号以 ledger 实测为准（当前 RFC-0045）。
>
