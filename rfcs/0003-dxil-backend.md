...
| 状态 | **Draft**。**owner 人工批准（FCP-lite）前不得推进下游 spec/实现 PR（硬规则 1，AI 不代签）**；§9 未决（特别 Q-D131 生成路径）由 owner 裁决，AI 不替 owner 选 |
...
| Owner 批准 | 〈待 owner — YYYY-MM-DD〉。批准前状态维持 Draft；批准范围须显式覆盖 §9 Q-D131 生成路径裁决与 §4.6 🔒 禁区边界处置。记录方式：owner 裁决后由 AI 代录（代录非代签，硬规则 1） |

> **批准记录（待 owner 回填）**：本 RFC §4.6 标 🔒 的 DXIL 文本语义 UB 边界 / 纹理路径内存模型映射 / FFI ABI 面属仅 owner 经 Full RFC 落笔的禁区（06 §4.2，AGENTS 硬规则 5）。本草案对上述禁区**仅作边界声明 + 占位「〈待 owner 后续 Full RFC〉」，不落笔**。§9 Q-D131（生成路径 = LLVM DirectX 后端 vs SPIR-V→DXIL 转译）为 D-131 的重评估载体——本 RFC **陈述两路结构性取舍但不替 owner 选**；最终路径由 owner 在批准时按当时后端成熟度裁决（13 §D-131：「G2 启动时按当时后端成熟度评估，所有者批准」）。
...
## 9. 未决问题 / 关键裁决（留 owner，AI 不代决）

> 以下 Q 由 owner 裁决；AI 仅陈述取舍输入，**不替 owner 选**（硬规则 1）。owner 裁决后由 AI 代录（代录非代签）。技术细节随 owner 批准后的 spec 脚手架/实现 PR 落条款体（trace 维持 156/156）。

- **Q-D131 生成路径（核心，= 13 §D-131 重评估）**：DXIL 生成路径 = **(A) LLVM DirectX 后端直接 emit DXIL** vs **(B) SPIR-V→DXIL 转译**。两路结构性取舍见 §7 A/B：
  - **A（LLVM DirectX 后端）**：与既有 NVPTX 后端同构、复用 LLVM codegen 框架与 MIR 前沿、无额外中间 IR、契合 D-205 LLVM 单栈;**风险 = LLVM DirectX target 当时成熟度**(DXIL 合规性/shader model 覆盖/validator 兼容),须 owner 核实裁决时实况。
  - **B（SPIR-V→DXIL 转译）**：绕开 LLVM DirectX target 成熟度风险、SPIR-V 工具相对独立;**代价 = 引入第二中间表示 + 外部转译依赖**(供应链/版本/合规长尾)、转译层语义保真与 strict-only/确定性需额外验证、转译路径内存模型保真涉禁区(§4.6)、与 D-205 LLVM 单栈/D-002 Windows-first 契合度需评估。
  - **裁决输入**：13 §D-131 规定「按当时后端成熟度评估」——owner 须以裁决时的 LLVM DirectX target 实况 / SPIR-V→DXIL 转译器合规性为准;本 RFC 不冻结某时点成熟度判断、不替 owner 选。**裁决结果回填本 Q + 13 §D-131（经独立勘误 PR，见配套 D-131 un-defer 勘误）**。
- **Q-Golden DXIL golden + bless 形态**：DXIL golden 的粒度/形态——DXIL 文本反汇编 vs 二进制容器 vs 规范化中间形态;是否经 dxc validator 验证;bless 审批流与既有 PTX/MIR golden 体系的整合方式（§4.4）。涉及 DXIL 容器格式与工具链依赖,留 owner。
- **Q-Builtin 内建变量 → DXIL 寄存器/语义槽映射边界**：RXS-0154 的 `#[builtin]` 内建变量降级到 DXIL 系统值语义（SV_*）的映射边界（§4.2）。**注**:此触及 DXIL 签名 ABI 的部分与 §4.6(c) FFI ABI 禁区相邻——owner 须厘清哪些属可在本 RFC 体系下定义的类型面映射、哪些属禁区 ABI 二进制布局(后者留禁区 Full RFC)。AI 不在禁区落笔。
- **Q-Range RXS 区间大小与拆分**：§5 计划表列 RXS-0157~0160 为**拟议占位**;实际区间/拆分粒度待定——路径选择(Q-D131)可能影响降级面条款拆分(如 B 路径的转译层是否需独立条款)。owner 与 Q-D131 一并裁定区间,本 RFC 不锁定、不预造。
- **Q-Gate feature gate 命名**：cargo feature 命名（候选 `dxil-backend`，§6）。
- **Q-CLI target 选择面**：`rx build --target dxil` 的精确 CLI/manifest 形态与能力探测面（§3/§4.1 拟议形态）。
...
- **Provenance**：`Assisted-by: claude-code:claude-opus-4-8`。本草案由 AI 起草；§4.6 🔒 禁区维持占位不落笔，§9 未决（特别 Q-D131）留 owner 裁决。**owner 经 FCP-lite 批准本 RFC + 裁决 Q-D131 路径前，AI 不推进下游 spec/实现 PR、不自启、不代签**（硬规则 1/7）。FCP-lite 额外评审/等待窗若适用,按 10 §2.2/§5 独立完成,本记录不虚构尚不存在的评审。
...
| Owner approval | 〈待 owner — YYYY-MM-DD〉 | 〈owner 人工批准全文 + 裁决 §9 Q-D131 生成路径（A/B）+ §4.6 🔒 禁区边界处置 + Q-Golden/Q-Builtin/Q-Range/Q-Gate/Q-CLI，并授权记录批准;AI 代录不代签。状态翻 Owner Approved。D-131 路径裁决结果经独立勘误 PR 回填 13 §D-131〉 | Full RFC（〈待 owner〉） |
