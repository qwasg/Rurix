...
| 2026-06-14 | tests/ui/shared/ 新增 2 条 + tests/ui/atomics/ 新增 3 条 snapshot(RX3009 ×2 / RX3010 ×3) | M5.2 黄金路径 5 的 shared+barrier / scoped atomics 子集(M5_PLAN §2 任务 2/4,spec/device.md RXS-0079/0080;UI 通道阶段化接入 views 不相交之后的 shared+barrier 一致性数据流 device 借用扩展 + typeck 层 scoped atomics scope 类型契约,既有 61 条 snapshot 零变化)。RX3009 shared 写后未同步跨 lane 读 / barrier 过晚;RX3010 shared 地址空间宽 scope 不相容(System/Gpu)/ Atomic brand 越权;`m5.counter.ui_golden_path5_snapshots` 现 11 条 ≥10 达成 G-M5-3。scoped atomics 的 PTX atom.{order}.{scope} 映射为 D-406 禁区(人工落笔),本批仅 scope 类型契约 snapshot | pending-human-review |
