...
| 2026-06-27 | tests/dxil/graphics/ 图形=B golden(gfx_vs_min:`.dxil-disasm`) owner bless | owner 白栀于本工作会话监督确认 G-G2-2 golden bless;AI 代录并执行机械落档。pin 环境:`RURIX_DXC_DIR=H:\dxc-round7\extracted\bin\x64`(含 `dxc.exe`/`dxv.exe`/`dxil.dll`,DXC 1.9.2602.24)+ `RURIX_SPIRV_CROSS=C:\ti-localappdata\ti-build-cache\vulkan-1.3.296.0\Bin\spirv-cross.exe`。命令:`RURIX_BLESS=1 cargo test -p rurixc --features dxil-backend --test dxil_golden dxil_b_disasm_golden_matches_when_toolchain_present -- --exact --nocapture` PASS;入 golden 前 `dxv.exe` validator 接受,版本噪声行规范化。当前语料仍是平凡 passthrough,RD-013/RD-017 未关闭,输出语义退化为 `TEXCOORD` 为已登记缺口下的 approved baseline,不声称 varying 用户语义保真已兑现。配套 device/run 见 PR smoke run [28284960733](https://github.com/qwasg/Rurix/actions/runs/28284960733) 步骤 46 `DXIL_DEVICE: ok adapter="NVIDIA GeForce RTX 4070 Ti" pixel=64,127,255,255 draw=ok` | qwasg/白栀(owner 会话授权,agent 代笔) |
