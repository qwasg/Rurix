...
| 2026-07-12 | tests/stable/stable_api.snapshot 重 bless（spec_clauses 180→184，加性追加） | MR-0006/MR-0007（agent 完全自主批准 2026-07-12）条款体 RXS-0181~0184 落 spec/dxil_backend.md（DXIL compute 整型能力包:整型 raw buffer 视图元素 u32/i32 / 整型位运算与移位量取模 / 位扫描·位计数 intrinsic 集 find_lsb·find_msb·popcount + DeviceMathFn f32 首期 sqrt/rsqrt/sin/cos DXIL 降级）。**加性变更**（同一 edition 内只增不破坏,RXS-0180 L2）:spec_clauses 集合 +4（RXS-0181/0182/0183/0184）,error_codes/editions/rx_cli_subcommands 三段 0 漂移（零新 RX 码——RX6006/RX6007/RX2001/RX2004/RX2006 全复用）。命令:`RURIX_BLESS=1 py -3 ci/stable_snapshot.py` 写快照后 `py -3 ci/stable_snapshot.py --check` PASS（spec_clauses=184,error_codes=88）。🔒 快照仅锚定 stable 面存在性+含义,不冻结二进制 ABI（RXS-0180 L3）。 | qwasg（agent 完全自主签署） |
