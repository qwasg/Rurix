...
- **交付物**：views 算子集 + 不相交证明（borrow check 扩展）；`shared let` + barrier 一致性检查；scoped atomics + PTX 映射层（spec 条款先行，D-406 禁区由人工落笔）；libdevice 链接；gpu 库并行基元（reduce/scan/transpose/tiled GEMM 自研 kernel）。
...
开源执行（D-003 既定，时点与形式所有者确认）；G1 启动优先级；是否引入协作者。
...
- 语言 1.0：spec 全量条款化 + conformance 覆盖全部 stable 特性 + 首个 edition 机制就绪。
- 生态成功判据（[01](01_VISION_AND_MISSION.md) §6 第二层）：≥3 个非作者维护的真实项目。
- registry 决策点（D-312）：社区规模驱动。

## 6. 5 年愿景

- 至少一个生产级渲染器/仿真系统以 Rurix 为主语言（使命判据）。
