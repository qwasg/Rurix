...
//!
//! 条款:spec/release.md RXS-0135 ~ RXS-0139——
...
//! - [`gate`] Release 层 hard-block 发布门(RXS-0139)
//!
...
/// 发布编排报告:bundle / SBOM 双视图 / 签名清单 / 白名单审计 / 发布门决策 /
/// 验签通过产物集(= `m8.counter.release_artifacts_signed` 的 `signed_artifacts`)。
#[derive(Debug, Clone)]
...
/// 发布编排(RXS-0135~0139 汇流):由 bundle + 签名清单 + CI 子门事实算出 SBOM、
/// 白名单审计、发布门决策与验签通过产物集。纯函数、确定性。`faults` 仅供发布门
/// 真实红绿自检注入子门红场景(正常路径传 [`Faults::none`])。
pub fn run_release(
...
mod tests {
    ...
    //@ spec: RXS-0139
    // 端到端编排:全门绿 → 放行上传 + signed_artifacts ≥1;注入未签名 / 缺 SBOM → 阻断。
    #[test]
