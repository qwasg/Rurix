...
//! body 销毁后 `JPC_Shape_Release`(Jolt `Body` 不持有 shape 引用,实测
//! `Body::SetShapeInternal`,登记 VENDOR.md §4)。销毁序见 `Inner::drop`。
//!
//! 相位门(§4.A4/§4.C3):变更路径 = `&mut self`,只读查询 = `&self`;
//! Jolt job 线程只活在 `JPC_PhysicsSystem_Update` 调用内;contact 回调在该窗口内
...
/// Jolt object layer 上限:16 位位宽,0xFFFF = `cObjectLayerInvalid` 保留(VENDOR.md §2)。
...
unsafe extern "C" fn bp_get_num_layers(_self: *const c_void) -> u32 {
    // 首版单 broadphase layer(VENDOR.md §3 收窄登记)
    1
}
...
unsafe extern "C" fn ray_body_should_collide_locked(
    ...
    let id = unsafe { JPC_Body_GetID(body) };
...
unsafe extern "C" fn on_contact_added(
    ...
    let (sink, a, b, m) = unsafe {
        (
            ...
            JPC_Body_GetID(body1),
            JPC_Body_GetID(body2),
            &*manifold,
        )
    };
...
/// (JoltC 回调不含求解后冲量,VENDOR.md §3 缺口收窄登记)。
fn manifold_event(
    a: JpcBodyId,
    b: JpcBodyId,
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            if !self.listener.is_null() {
                JPC_ContactListener_delete(self.listener);
            }
            if !self.ps.is_null() {
                JPC_PhysicsSystem_delete(self.ps);
            }
            if !self.olp.is_null() {
                JPC_ObjectLayerPairFilter_delete(self.olp);
            }
            if !self.ovb.is_null() {
                JPC_ObjectVsBroadPhaseLayerFilter_delete(self.ovb);
            }
            if !self.bp.is_null() {
                JPC_BroadPhaseLayerInterface_delete(self.bp);
            }
            if !self.job.is_null() {
                JPC_JobSystemThreadPool_delete(self.job);
            }
            if !self.temp.is_null() {
                JPC_TempAllocatorImpl_delete(self.temp);
            }
        }
    }
}
...
impl Inner {
    pub(crate) fn create(desc: &SysWorldDesc) -> Result<Inner, SysError> {
        ...
        // 堆地址(稳定,生命周期 ≥ 注册窗口)。
        unsafe {
            g.temp = JPC_TempAllocatorImpl_new(TEMP_ALLOCATOR_BYTES);
            if g.temp.is_null() {
                return Err(err(
                    SysErrorCode::BackendUnavailable,
                    "JPC_TempAllocatorImpl_new 返回 null",
                ));
            }
            ...
            g.job = JPC_JobSystemThreadPool_new3(
            ...
            if g.job.is_null() {
                return Err(err(
                    SysErrorCode::BackendUnavailable,
                    "JPC_JobSystemThreadPool_new3 返回 null",
                ));
            }
            g.bp = JPC_BroadPhaseLayerInterface_new(
                std::ptr::null(),
            ...
            if g.bp.is_null() {
                return Err(err(
                    SysErrorCode::BackendUnavailable,
                    "JPC_BroadPhaseLayerInterface_new 返回 null",
                ));
            }
            g.ovb = JPC_ObjectVsBroadPhaseLayerFilter_new(
                std::ptr::null(),
            ...
            if g.ovb.is_null() {
                return Err(err(
                    SysErrorCode::BackendUnavailable,
                    "JPC_ObjectVsBroadPhaseLayerFilter_new 返回 null",
                ));
            }
            g.olp = JPC_ObjectLayerPairFilter_new(
                std::ptr::null(),
            ...
            if g.olp.is_null() {
                return Err(err(
                    SysErrorCode::BackendUnavailable,
                    "JPC_ObjectLayerPairFilter_new 返回 null",
                ));
            }
            g.ps = JPC_PhysicsSystem_new();
            if g.ps.is_null() {
                return Err(err(
                    SysErrorCode::BackendUnavailable,
                    "JPC_PhysicsSystem_new 返回 null",
                ));
            }
            ...
            g.listener = JPC_ContactListener_new(
            ...
            if g.listener.is_null() {
                return Err(err(
                    SysErrorCode::BackendUnavailable,
                    "JPC_ContactListener_new 返回 null",
                ));
            }
            JPC_PhysicsSystem_SetContactListener(g.ps, g.listener);

            let bi = JPC_PhysicsSystem_GetBodyInterface(g.ps);
            let npq = JPC_PhysicsSystem_GetNarrowPhaseQuery(g.ps);
            let bli = JPC_PhysicsSystem_GetBodyLockInterface(g.ps);
    ...
    fn validate_token(&self, token: u64) -> Result<JpcBodyId, SysError> {
        if token > u32::MAX as u64 || token == JPC_BODY_ID_INVALID as u64 {
            return Err(err(
    ...
    fn body_transform_of(&self, id: JpcBodyId) -> SysTransform {
        ...
        unsafe {
            JPC_BodyInterface_GetPositionAndRotation(self.bi, id, &mut pos, &mut rot);
        }
        SysTransform {
    ...
    pub(crate) fn step(&mut self, dt: f32) -> Result<crate::SysStepStats, SysError> {
        ...
        // 睡眠统计(JoltC 无 activation listener,轮询差分 — VENDOR.md §3 收窄登记)
        ...
        for &id in &dyn_ids {
            // SAFETY: id 在册;body 存活;step 相位 &mut 独占。
            pre_active.push(unsafe { JPC_BodyInterface_IsActive(self.bi, id) });
        }
        ...
        let update_err = unsafe {
            JPC_PhysicsSystem_Update(self.ps, dt, 1, self.temp, self.job.cast::<JpcJobSystem>())
        };
        ...
        for (&id, rec) in &self.bodies {
            // SAFETY: 同 pre 轮询。
            let active = unsafe { JPC_BodyInterface_IsActive(self.bi, id) };
            if active {
                active_bodies += 1;
            }
    ...
    pub(crate) fn add_bodies_batch(&mut self, descs: &[SysBodyDesc]) -> Result<Vec<u64>, SysError> {
        ...
        for s in &settings {
            ...
            let body = unsafe { JPC_BodyInterface_CreateBody(self.bi, s) };
            if body.is_null() {
                // SAFETY: ids 内体已 Create 未 Add,DestroyBody 合法;shapes 未释放过。
                unsafe {
                    for &id in &ids {
                        JPC_BodyInterface_DestroyBody(self.bi, id);
                    }
                    for s in &shapes {
                        JPC_Shape_Release(*s);
                    }
            ...
            ids.push(unsafe { JPC_Body_GetID(body) });
        }
        ...
        unsafe {
            let state = JPC_BodyInterface_AddBodiesPrepare(self.bi, ids.as_mut_ptr(), n);
    ...
    pub(crate) fn remove_bodies_batch(&mut self, tokens: &[u64]) -> Result<(), SysError> {
        ...
        // DestroyBodies 在 JoltC impl 被上游注释,WIP 缺口处置 (c) = Rust 循环,VENDOR.md
        // §3);之后释放各 body 持有的 shape 引用并出册(token 失效)。
        unsafe {
            JPC_BodyInterface_RemoveBodies(self.bi, ids.as_mut_ptr(), n);
            for &id in &ids {
                JPC_BodyInterface_DestroyBody(self.bi, id);
            }
        }
        for id in ids {
            if let Some(rec) = self.bodies.remove(&id) {
                ...
                unsafe { JPC_Shape_Release(rec.shape) };
            }
        }
        Ok(())
    }
    ...
    pub(crate) fn active_transforms(&self) -> Vec<(u64, SysTransform)> {
        ...
        for (&id, rec) in &self.bodies {
            ...
            if unsafe { JPC_BodyInterface_IsActive(self.bi, id) } {
    ...
    pub(crate) fn cast_ray(&self, ray: &SysRay) -> Vec<SysHit> {
        ...
        if !olf.is_null() && !bf.is_null() {
            ...
            // 实现契约「全命中返回」(零 C++ 补丁,VENDOR.md §3 计划外缺口登记)
            loop {
                ...
                let hit = unsafe { JPC_NarrowPhaseQuery_CastRay(self.npq, &mut args) };
                if !hit {
                    break;
                }
        ...
        unsafe {
            ...
            if !olf.is_null() {
                JPC_ObjectLayerFilter_delete(olf);
            }
        }
        hits
    }
    ...
    fn surface_normal(
        ...
        unsafe {
            let lock = JPC_BodyLockRead_new(self.bli, id);
            if lock.is_null() {
                return [0.0; 3];
            }
            let out = if JPC_BodyLockRead_Succeeded(lock) {
                let body = JPC_BodyLockRead_GetBody(lock);
                if body.is_null() {
                    [0.0; 3]
                } else {
                    let n = JPC_Body_GetWorldSpaceSurfaceNormal(
                        body,
                        sub_shape,
    ...
    pub(crate) fn cast_shape(
        ...
        let com = unsafe { JPC_Shape_GetCenterOfMass(query_shape) };
        ...
        unsafe {
            ...
            let olf = JPC_ObjectLayerFilter_new(
                (&mask as *const u64).cast(),
                JpcObjectLayerFilterFns {
                    should_collide: Some(ol_should_collide),
                },
            ...
            if !collector.is_null() && !olf.is_null() {
                ...
                JPC_ShapeCastSettings_default(&mut settings);
                ...
                JPC_NarrowPhaseQuery_CastShape(self.npq, &mut args);
            }
            if !olf.is_null() {
                JPC_ObjectLayerFilter_delete(olf);
            }
    ...
    pub(crate) fn overlap_shape(
        ...
        // SAFETY: 同 cast_shape。
        unsafe {
            let collector = JPC_CollideShapeCollector_new(
            ...
            if !collector.is_null() && !olf.is_null() {
                ...
                JPC_CollideShapeSettings_default(&mut settings);
                ...
                JPC_NarrowPhaseQuery_CollideShape(self.npq, &mut args);
            }
            if !olf.is_null() {
                JPC_ObjectLayerFilter_delete(olf);
            }
...
fn create_shape(d: &SysBodyDesc) -> Result<*mut JpcShape, SysError> {
    ...
    if d.kind == SysBodyKind::Dynamic && d.mass > 0.0 {
        ...
        let volume = unsafe { JPC_Shape_GetVolume(first) };
        // SAFETY: 同上,仅此一次释放。
        unsafe { JPC_Shape_Release(first) };
...
fn take_create_error(err_str: *mut JpcString, what: &str) -> SysError {
    ...
    // SAFETY: err_str 为 Create 失败路径返回的有效 JPC_String;读完即 delete。
    let msg = unsafe {
        let c = JPC_String_c_str(err_str);
        let s = if c.is_null() {
            String::new()
        } else {
            CStr::from_ptr(c).to_string_lossy().into_owned()
        };
        JPC_String_delete(err_str);
        s
    };
    err(
        SysErrorCode::InvalidDesc,
    ...
    match params {
        SysShapeParams::Sphere { radius } => {
            ...
            unsafe {
                ...
                JPC_SphereShapeSettings_default(&mut s);
                s.radius = *radius;
                s.density = density;
                if !JPC_SphereShapeSettings_Create(&s, &mut shape, &mut err_str) {
        ...
        SysShapeParams::Box { half_extents } => {
            ...
            unsafe {
                ...
                JPC_BoxShapeSettings_default(&mut s);
                s.half_extent = JpcVec3::new(half_extents[0], half_extents[1], half_extents[2]);
                s.density = density;
                if !JPC_BoxShapeSettings_Create(&s, &mut shape, &mut err_str) {
        ...
        SysShapeParams::Capsule {
            ...
            unsafe {
                ...
                JPC_CapsuleShapeSettings_default(&mut s);
                s.radius = *radius;
                s.half_height_of_cylinder = *half_height;
                s.density = density;
                if !JPC_CapsuleShapeSettings_Create(&s, &mut shape, &mut err_str) {
        ...
        SysShapeParams::ConvexHull { points } => {
            ...
            unsafe {
                ...
                JPC_ConvexHullShapeSettings_default(&mut s);
                s.points = pts.as_ptr();
                s.points_len = pts.len();
                s.density = density;
                if !JPC_ConvexHullShapeSettings_Create(&s, &mut shape, &mut err_str) {
        ...
        SysShapeParams::StaticMesh {
            ...
            unsafe {
                ...
                JPC_MeshShapeSettings_default(&mut s);
                ...
                if !JPC_MeshShapeSettings_Create(&s, &mut shape, &mut err_str) {
...
fn make_body_settings(d: &SysBodyDesc, shape: *const JpcShape) -> JpcBodyCreationSettings {
    ...
    let mut s = unsafe {
        ...
        JPC_BodyCreationSettings_default(&mut s);
        s
    };
    ...
    s.motion_type = match d.kind {
        SysBodyKind::Static => JPC_MOTION_TYPE_STATIC,
        SysBodyKind::Kinematic => JPC_MOTION_TYPE_KINEMATIC,
        SysBodyKind::Dynamic => JPC_MOTION_TYPE_DYNAMIC,
    };
    s.allowed_dofs = JPC_ALLOWED_DOFS_ALL;
    ...
    s.override_mass_properties = JPC_OVERRIDE_MASS_PROPS_CALC_MASS_INERTIA;
    s.shape = shape;
    s
}
