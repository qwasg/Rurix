...
///
/// G8.2 M31(RXS-0304~0307):提取层自本文件搬至 [`crate::iface_extract`](纯 AST
/// 只读、零行为漂移;`pub(super)` → `pub(crate)`),供本模块(device MIR 附着)与
/// `--emit=reflection`(reflection v1)两侧复用同一提取律。下方 `dxil_io::` 调用点
/// 经别名原位维持。
#[cfg(any(feature = "dxil-backend", feature = "vulkan-backend"))]
use crate::iface_extract as dxil_io;
