//! G8.3 M04 page_format_abi **device 解码 harness**（RXS-0342；门
//! `g8.p0.m04.page_format_abi`）。
//!
//! 用法：
//! ```text
//! vk_geom_page_decode --spv <geom_page_decode.spv> --rxpm <page.rxpm>
//! ```
//!
//! 上传 RXPM → compute 结构性展开 → 回读展开流 → stdout JSON：
//! `expanded_digest`（SHA-256 hex）、`expanded_u32_count`、`validation_errors`、
//! `device_state`。digest 比对在 smoke 层与 CPU 对照。
//!
...
fn sha256(data: &[u8]) -> [u8; 32] {
    // 轻量：复用与 smoke 相同的手写路径不可用（rt 不依赖 pkg）——用简单
    // 内联不可行。改为输出原始展开字节 hex，由 smoke 算 SHA-256。
    // 为 JSON 自包含，这里用与 rurix_pkg 同算法的最小实现？
    // 裁决：输出 expanded_bytes_hex，smoke 侧 digest；同时用 std 不可用。
    // 本 harness 用 FNV 不可。直接嵌入极简 SHA-256（公有域实现，~80 行）。
    sha256_impl(data)
}
...
fn main() {
    ...
    let mut rxpm = std::fs::read(&rxpm_path).unwrap_or_else(|e| fail(&format!("读 rxpm: {e}")));
    if rxpm.len() < 48 {
        fail("RXPM 过短");
    }
    if &rxpm[0..4] != b"RXPM" {
    ...
    let expect = arg_value_str(&args, "--expect-u32-count")
        ...
        .unwrap_or_else(|| fail("--expect-u32-count <n> 必需（与 CPU expand_u32_count 对齐）"));
    // 输出缓冲：期望字数 + 余量（kernel 边界守卫用 out_cap）。
    let out_cap = (expect + 64) as u32;
    let out = vec![0u8; out_cap as usize * 4];
    let page_bytes_logical = std::fs::metadata(&rxpm_path)
        .map(|m| m.len() as u32)
        .unwrap_or(rxpm.len() as u32);

    let mut buffers = vec![rxpm, out];
    let mut pc = Vec::new();
    ...
    match rurix_rt::vk::run_compute(&spv, &entry, &mut buffers, &pc, [1, 1, 1]) {
        ...
        Err(e) if is_no_device(&e) => {
            eprintln!("GPD: SKIP 无 Vulkan 设备({})", e.trim());
            println!(
        ...
        Err(e) if e.contains("validation") || e.contains("VK_LAYER") => {
            eprintln!("GPD: FAIL validation: {e}");
            std::process::exit(2);
        }
...
// ── 最小 SHA-256（公有域；与 rurix_pkg 逐位兼容）──────────────────────────
