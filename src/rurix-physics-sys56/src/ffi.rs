...
//! 签名逐字段对齐 vendored 5.6 线 `JoltC/Functions.h` / `Enums.h`(pin 见 ../VENDOR56.md §1;
//! 全部导出符号 `JPC56_` 前缀,与 5.3 基线 `JPC_` 同进程并存零冲突)。
//!
//! 布局可信链:C 结构 ↔ JPH56 C++ 结构由 JoltCImpl 的 `LAYOUT_COMPATIBLE` 静态断言在
//! vendor 构建期强制;本模块 Rust 镜像 ↔ JoltC C 结构由 `ffi_layout_anchors`(本文件
//! 尾部)锚定——全部数值 = 布局真值测量(x86_64-pc-windows-msvc / 单精度 /
//! OBJECT_LAYER_BITS=16,测量程序 = `tools/layout_dump56.cpp` 对 vendored 5.6 头
//! `offsetof` 实测,2026-08-13;换画像/升 pin 必须重测,U32 `ffi_layout_anchors` 模式)。
//! 5.6 delta(Jolt 5.3→5.6 布局漂移全量,详见 VENDOR56.md §3):
//! - `JpcShapeCastSettings` 插入 `extra_convex_radius`(@32,尾部四字段 32~35→36~39,size 48 不变);
//! - `JpcCollideShapeSettings` 追加 `internal_edge_removal_vertex_tolerance_sq`(@40,占 5.3 尾垫,size 48 不变);
//! - `CollisionEstimationResult`(新摩擦模型聚合面)与 `BodyManager_DrawSettings` 重排,
//!   本 crate Rust 侧不镜像(safe 层不消费;C 侧由 LAYOUT_COMPATIBLE 锚定)。
//!
...
pub type JpcBodyId = u32; // JPC56_BodyID
pub type JpcSubShapeId = u32; // JPC56_SubShapeID
pub type JpcObjectLayer = u16; // JPC56_ObjectLayer(OBJECT_LAYER_BITS=16)
pub type JpcBroadPhaseLayer = u8; // JPC56_BroadPhaseLayer

pub const JPC56_BODY_ID_INVALID: JpcBodyId = u32::MAX; // JPH::BodyID::cInvalidBodyID

// JPC56_MotionType(u8)
pub const JPC56_MOTION_TYPE_STATIC: u8 = 0;
pub const JPC56_MOTION_TYPE_KINEMATIC: u8 = 1;
pub const JPC56_MOTION_TYPE_DYNAMIC: u8 = 2;
// JPC56_MotionQuality(u8)
pub const JPC56_MOTION_QUALITY_DISCRETE: u8 = 0;
pub const JPC56_MOTION_QUALITY_LINEAR_CAST: u8 = 1;
// JPC56_Activation(u32)
pub const JPC56_ACTIVATION_ACTIVATE: u32 = 0;
pub const JPC56_ACTIVATION_DONT_ACTIVATE: u32 = 1;
// JPC56_ConstraintSpace(u32)
pub const JPC56_CONSTRAINT_SPACE_LOCAL_TO_BODY_COM: u32 = 0;
pub const JPC56_CONSTRAINT_SPACE_WORLD_SPACE: u32 = 1;
// JPC56_MotorState(u32)
pub const JPC56_MOTOR_STATE_OFF: u32 = 0;
pub const JPC56_MOTOR_STATE_VELOCITY: u32 = 1;
pub const JPC56_MOTOR_STATE_POSITION: u32 = 2;
// JPC56_OverrideMassProperties(u8)
pub const JPC56_OVERRIDE_MASS_PROPS_CALC_MASS_INERTIA: u8 = 0;
// JPC56_PhysicsUpdateError(u32 位掩码)
pub const JPC56_PHYSICS_UPDATE_ERROR_NONE: u32 = 0;
// JPC56_AllowedDOFs(u8)
pub const JPC56_ALLOWED_DOFS_ALL: u8 = 0b0011_1111;
// Enums.h 常量
pub const JPC56_MAX_PHYSICS_JOBS: u32 = 2048;
pub const JPC56_MAX_PHYSICS_BARRIERS: u32 = 8;

// ---------------------------------------------------------------------------
// 几何基础类型(Functions.h;x86_64 单精度 → JPC56_VECTOR_ALIGNMENT = 16)
// ---------------------------------------------------------------------------
...
/// JPC56_Float3(12 字节无对齐垫,与 `[f32; 3]` 同布局)
#[repr(C)]
#[derive(Debug, Clone, Copy)]
...
/// JPC56_Mat44 = JPC56_RMat44(单精度):3 个 Vec4 旋转列 + Vec3 平移列
#[repr(C, align(16))]
#[derive(Debug, Clone, Copy)]
pub struct JpcMat44 {
    pub col: [JpcVec4; 3],
...
pub struct JpcRRayCast {
    pub origin: JpcVec3, // JPC56_RVec3 = JPC56_Vec3(单精度)
...
pub struct JpcCastRayArgs {
    ...
    pub shape_filter: *const c_void, // JPC56_ShapeFilter(本切片恒 null)
}
...
pub struct JpcShapeCastSettings {
    ...
    /// Jolt 5.6 新增 `mExtraConvexRadius`(query shape 额外凸半径膨胀;
    /// `JPC56_ShapeCastSettings_default` 填上游默认 0.0)。
    pub extra_convex_radius: f32,
...
pub struct JpcCollideShapeSettings {
    ...
    pub _pad0: [u8; 3],
    /// Jolt 5.6 新增 `mInternalEdgeRemovalVertexToleranceSq`(内部边去除算法
    /// 顶点容差平方;`JPC56_CollideShapeSettings_default` 填上游默认
    /// `cDefaultInternalEdgeRemovalVertexToleranceSq`)。
    pub internal_edge_removal_vertex_tolerance_sq: f32,
}
...
pub struct JpcContactManifold {
    pub base_offset: JpcVec3, // JPC56_RVec3
...
pub struct JpcBodyCreationSettings {
    pub position: JpcVec3, // JPC56_RVec3
...
// ---------------------------------------------------------------------------
// 回调函数表(Functions.h:348-471, 588-691;按值传入 JPC56_Xxx_new)
// ---------------------------------------------------------------------------
...
pub type JpcOnContactValidateFn = unsafe extern "C" fn(
    ...
    *const JpcBody,
    *const JpcBody,
    JpcVec3, // JPC56_RVec3 按值
    *const JpcCollideShapeResult,
) -> u32; // JPC56_ValidateResult
...
/// JPC56_ConstraintSettings(32B,align 8;layout_dump 2026-08-06)。
#[repr(C)]
#[derive(Clone, Copy)]
...
/// JPC56_HingeConstraintSettings(208B,align 16;layout_dump 2026-08-06)。
#[repr(C, align(16))]
#[derive(Clone, Copy)]
...
unsafe extern "C" {
    ...
    pub fn JPC56_RegisterDefaultAllocator();
    pub fn JPC56_FactoryInit();
    pub fn JPC56_RegisterTypes();

    // TempAllocatorImpl / JobSystem
    pub fn JPC56_TempAllocatorImpl_new(size: u32) -> *mut JpcTempAllocatorImpl;
    pub fn JPC56_TempAllocatorImpl_delete(object: *mut JpcTempAllocatorImpl);
    pub fn JPC56_JobSystemThreadPool_new3(
        max_jobs: u32,
        max_barriers: u32,
        num_threads: i32,
    ) -> *mut JpcJobSystemThreadPool;
    pub fn JPC56_JobSystemThreadPool_delete(object: *mut JpcJobSystemThreadPool);

    // 层接口 / 过滤器(构造签名统一:self 原样回传回调首参)
    pub fn JPC56_BroadPhaseLayerInterface_new(
        self_: *const c_void,
        fns: JpcBroadPhaseLayerInterfaceFns,
    ) -> *mut JpcBroadPhaseLayerInterface;
    pub fn JPC56_BroadPhaseLayerInterface_delete(object: *mut JpcBroadPhaseLayerInterface);
    pub fn JPC56_ObjectVsBroadPhaseLayerFilter_new(
        self_: *const c_void,
    ...
    pub fn JPC56_ObjectLayerPairFilter_delete(object: *mut JpcObjectLayerPairFilter);
    pub fn JPC56_ObjectLayerFilter_new(
        self_: *const c_void,
        fns: JpcObjectLayerFilterFns,
    ) -> *mut JpcObjectLayerFilter;
    pub fn JPC56_ObjectLayerFilter_delete(object: *mut JpcObjectLayerFilter);
    pub fn JPC56_BodyFilter_new(self_: *const c_void, fns: JpcBodyFilterFns) -> *mut JpcBodyFilter;
    pub fn JPC56_BodyFilter_delete(object: *mut JpcBodyFilter);

    // 接触监听器 / cast 收集器
    pub fn JPC56_ContactListener_new(
        self_: *mut c_void,
        fns: JpcContactListenerFns,
    ) -> *mut JpcContactListener;
    pub fn JPC56_ContactListener_delete(object: *mut JpcContactListener);
    pub fn JPC56_CastShapeCollector_new(
        self_: *mut c_void,
        fns: JpcCastShapeCollectorFns,
    ) -> *mut JpcCastShapeCollector;
    pub fn JPC56_CastShapeCollector_delete(object: *mut JpcCastShapeCollector);
    pub fn JPC56_CollideShapeCollector_new(
        self_: *mut c_void,
        fns: JpcCollideShapeCollectorFns,
    ) -> *mut JpcCollideShapeCollector;
    pub fn JPC56_CollideShapeCollector_delete(object: *mut JpcCollideShapeCollector);

    // PhysicsSystem
    pub fn JPC56_PhysicsSystem_new() -> *mut JpcPhysicsSystem;
    pub fn JPC56_PhysicsSystem_delete(object: *mut JpcPhysicsSystem);
    pub fn JPC56_PhysicsSystem_Init(
    ...
    pub fn JPC56_PhysicsSystem_Update(
        self_: *mut JpcPhysicsSystem,
        delta_time: f32,
        collision_steps: i32,
        temp_allocator: *mut JpcTempAllocatorImpl,
        job_system: *mut JpcJobSystem,
    ) -> u32; // JPC56_PhysicsUpdateError
    pub fn JPC56_PhysicsSystem_SetGravity(self_: *mut JpcPhysicsSystem, gravity: JpcVec3);
    pub fn JPC56_PhysicsSystem_GetBodyInterface(
        self_: *mut JpcPhysicsSystem,
    ) -> *mut JpcBodyInterface;
    pub fn JPC56_PhysicsSystem_GetBodyLockInterface(
        self_: *mut JpcPhysicsSystem,
    ) -> *const JpcBodyLockInterface;
    pub fn JPC56_PhysicsSystem_GetNarrowPhaseQuery(
        self_: *const JpcPhysicsSystem,
    ) -> *const JpcNarrowPhaseQuery;
    pub fn JPC56_PhysicsSystem_SetContactListener(
        self_: *mut JpcPhysicsSystem,
        listener: *mut JpcContactListener,
    );

    // Body / BodyInterface
    pub fn JPC56_Body_GetID(self_: *const JpcBody) -> JpcBodyId;
    pub fn JPC56_Body_GetWorldSpaceSurfaceNormal(
        self_: *const JpcBody,
        sub_shape_id: JpcSubShapeId,
        position: JpcVec3, // JPC56_RVec3 按值
    ) -> JpcVec3;
    pub fn JPC56_BodyInterface_CreateBody(
    ...
    pub fn JPC56_BodyInterface_AddBodiesFinalize(
        self_: *mut JpcBodyInterface,
        bodies: *mut JpcBodyId,
        number: i32,
        add_state: *mut c_void,
        activation_mode: u32, // JPC56_Activation
    );
    pub fn JPC56_BodyInterface_AddBodiesAbort(
        self_: *mut JpcBodyInterface,
        bodies: *mut JpcBodyId,
        number: i32,
        add_state: *mut c_void,
    );
    pub fn JPC56_BodyInterface_RemoveBodies(
        self_: *mut JpcBodyInterface,
        bodies: *mut JpcBodyId,
        number: i32,
    );
    ...
    pub fn JPC56_BodyInterface_SetPositionAndRotation(
        self_: *mut JpcBodyInterface,
        body_id: JpcBodyId,
        position: JpcVec3,
        rotation: JpcQuat,
    ...
    pub fn JPC56_BodyInterface_SetPositionRotationAndVelocity(
        self_: *mut JpcBodyInterface,
        body_id: JpcBodyId,
        position: JpcVec3,
        rotation: JpcQuat,
    ...
    pub fn JPC56_BodyInterface_GetLinearVelocity(
        self_: *const JpcBodyInterface,
        body_id: JpcBodyId,
    ) -> JpcVec3;
    pub fn JPC56_BodyInterface_SetLinearVelocity(
        self_: *mut JpcBodyInterface,
        body_id: JpcBodyId,
        linear_velocity: JpcVec3,
    );
    pub fn JPC56_BodyInterface_GetAngularVelocity(
        self_: *const JpcBodyInterface,
        body_id: JpcBodyId,
    ) -> JpcVec3;
    pub fn JPC56_BodyInterface_SetAngularVelocity(
        self_: *mut JpcBodyInterface,
        body_id: JpcBodyId,
        angular_velocity: JpcVec3,
    );
    pub fn JPC56_BodyInterface_MoveKinematic(
        self_: *mut JpcBodyInterface,
        body_id: JpcBodyId,
    ...
    pub fn JPC56_BodyInterface_AddImpulse(
        self_: *mut JpcBodyInterface,
        body_id: JpcBodyId,
        impulse: JpcVec3,
    );
    /// 世界系点施力(M70 载具悬挂/驱动/侧向力面;`JPC56_RVec3` 在
    /// `JPC56_DOUBLE_PRECISION` 未定义档 typedef 为 `JPC56_Vec3`,build.rs 固定
    /// `DOUBLE_PRECISION=OFF`,故 point 形参用 `JpcVec3` 布局等价。
    pub fn JPC56_BodyInterface_AddForceAtPoint(
        self_: *mut JpcBodyInterface,
        body_id: JpcBodyId,
        force: JpcVec3,
        point: JpcVec3,
    ...
    pub fn JPC56_BodyLockRead_new(
        interface: *const JpcBodyLockInterface,
        body_id: JpcBodyId,
    ) -> *mut JpcBodyLockRead;
    pub fn JPC56_BodyLockRead_delete(self_: *mut JpcBodyLockRead);
    pub fn JPC56_BodyLockRead_Succeeded(self_: *mut JpcBodyLockRead) -> bool;
    pub fn JPC56_BodyLockRead_GetBody(self_: *mut JpcBodyLockRead) -> *const JpcBody;

    // BodyLockWrite(约束 Create 取 Body*)
    pub fn JPC56_BodyLockWrite_new(
        interface: *const JpcBodyLockInterface,
        body_id: JpcBodyId,
    ) -> *mut JpcBodyLockWrite;
    pub fn JPC56_BodyLockWrite_delete(self_: *mut JpcBodyLockWrite);
    pub fn JPC56_BodyLockWrite_Succeeded(self_: *mut JpcBodyLockWrite) -> bool;
    pub fn JPC56_BodyLockWrite_GetBody(self_: *mut JpcBodyLockWrite) -> *mut JpcBody;

    // BodyLockMultiWrite(两体约束 Create 必须成对加锁,避免双 BodyLockWrite 未定义序)
    pub fn JPC56_BodyLockMultiWrite_new(
        interface: *const JpcBodyLockInterface,
        body_ids: *const JpcBodyId,
        number: i32,
    ) -> *mut JpcBodyLockMultiWrite;
    pub fn JPC56_BodyLockMultiWrite_delete(self_: *mut JpcBodyLockMultiWrite);
    pub fn JPC56_BodyLockMultiWrite_GetBody(
        self_: *mut JpcBodyLockMultiWrite,
        body_index: i32,
    ) -> *mut JpcBody;

    // Constraint / Hinge(M66 capture journal;消费既有导出,零 vendor 补丁)
    pub fn JPC56_HingeConstraintSettings_default(settings: *mut JpcHingeConstraintSettings);
    pub fn JPC56_HingeConstraintSettings_Create(
        self_: *const JpcHingeConstraintSettings,
        body1: *mut JpcBody,
        body2: *mut JpcBody,
    ) -> *mut JpcHingeConstraint;
    pub fn JPC56_PhysicsSystem_AddConstraint(
        self_: *mut JpcPhysicsSystem,
        constraint: *mut JpcConstraint,
    );
    pub fn JPC56_PhysicsSystem_RemoveConstraint(
        self_: *mut JpcPhysicsSystem,
        constraint: *mut JpcConstraint,
    );
    pub fn JPC56_Constraint_AddRef(self_: *const JpcConstraint);
    pub fn JPC56_Constraint_Release(self_: *const JpcConstraint);
    pub fn JPC56_Constraint_GetEnabled(self_: *const JpcConstraint) -> bool;
    pub fn JPC56_HingeConstraint_SetMotorState(self_: *mut JpcHingeConstraint, state: u32);
    pub fn JPC56_HingeConstraint_GetMotorState(self_: *const JpcHingeConstraint) -> u32;
    pub fn JPC56_HingeConstraint_SetTargetAngularVelocity(
        self_: *mut JpcHingeConstraint,
        angular_velocity: f32,
    );

    // 形状(default 填充 + Create;Create 成功 → shape 引用计数 1,调用方持有)
    pub fn JPC56_SphereShapeSettings_default(object: *mut JpcSphereShapeSettings);
    pub fn JPC56_SphereShapeSettings_Create(
        self_: *const JpcSphereShapeSettings,
        out_shape: *mut *mut JpcShape,
        out_error: *mut *mut JpcString,
    ) -> bool;
    pub fn JPC56_BoxShapeSettings_default(object: *mut JpcBoxShapeSettings);
    pub fn JPC56_BoxShapeSettings_Create(
        self_: *const JpcBoxShapeSettings,
        out_shape: *mut *mut JpcShape,
        out_error: *mut *mut JpcString,
    ) -> bool;
    pub fn JPC56_CapsuleShapeSettings_default(object: *mut JpcCapsuleShapeSettings);
    pub fn JPC56_CapsuleShapeSettings_Create(
        self_: *const JpcCapsuleShapeSettings,
        out_shape: *mut *mut JpcShape,
        out_error: *mut *mut JpcString,
    ) -> bool;
    pub fn JPC56_ConvexHullShapeSettings_default(object: *mut JpcConvexHullShapeSettings);
    pub fn JPC56_ConvexHullShapeSettings_Create(
        self_: *const JpcConvexHullShapeSettings,
        out_shape: *mut *mut JpcShape,
        out_error: *mut *mut JpcString,
    ) -> bool;
    pub fn JPC56_MeshShapeSettings_default(object: *mut JpcMeshShapeSettings);
    pub fn JPC56_MeshShapeSettings_Create(
        self_: *const JpcMeshShapeSettings,
        out_shape: *mut *mut JpcShape,
        out_error: *mut *mut JpcString,
    ) -> bool;
    pub fn JPC56_Shape_Release(self_: *const JpcShape);
    pub fn JPC56_Shape_GetCenterOfMass(self_: *const JpcShape) -> JpcVec3;
    pub fn JPC56_Shape_GetVolume(self_: *const JpcShape) -> f32;

    // NarrowPhaseQuery(step 外只读路径,多线程并发安全 — Jolt 上游文档口径)
    pub fn JPC56_NarrowPhaseQuery_CastRay(
        self_: *const JpcNarrowPhaseQuery,
        args: *mut JpcCastRayArgs,
    ) -> bool;
    pub fn JPC56_NarrowPhaseQuery_CastShape(
        self_: *const JpcNarrowPhaseQuery,
        args: *mut JpcCastShapeArgs,
    );
    pub fn JPC56_NarrowPhaseQuery_CollideShape(
        self_: *const JpcNarrowPhaseQuery,
        args: *mut JpcCollideShapeArgs,
    );

    // 设置默认值填充 / 体创建默认值
    pub fn JPC56_ShapeCastSettings_default(object: *mut JpcShapeCastSettings);
    pub fn JPC56_CollideShapeSettings_default(object: *mut JpcCollideShapeSettings);
    pub fn JPC56_BodyCreationSettings_default(settings: *mut JpcBodyCreationSettings);

    // 错误串(Create 失败路径;读完即 delete)
    pub fn JPC56_String_c_str(self_: *mut JpcString) -> *const std::ffi::c_char;
    pub fn JPC56_String_delete(self_: *mut JpcString);
}
...
mod ffi_layout_anchors {
    ...
    const _: () = {
        ...
        // Jolt 5.6 delta(tools/layout_dump56.cpp 实测):ExtraConvexRadius@32 插入,
        // 尾部四字段 32~35 → 36~39(5.3 锚字面保持于 5.3 线 ffi.rs,本线不动)。
        assert!(offset_of!(JpcShapeCastSettings, extra_convex_radius) == 32);
        assert!(offset_of!(JpcShapeCastSettings, back_face_mode_triangles) == 36);
        assert!(offset_of!(JpcShapeCastSettings, back_face_mode_convex) == 37);
        assert!(offset_of!(JpcShapeCastSettings, use_shrunken_shape_and_convex_radius) == 38);
        assert!(offset_of!(JpcShapeCastSettings, return_deepest_point) == 39);
        assert!(
