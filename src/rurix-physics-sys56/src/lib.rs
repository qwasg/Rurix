//! rurix-physics-sys56 — JoltC(Jolt 5.6 评估臂)FFI 边界(G9.6 M125,RXS-0377;
//! RFC-0024 §4.E1 + RFC-0021 §4.A4 七步程序第②步)。
//!
//! 本 crate = Jolt 5.3→5.6 升级 A/B 的 5.6 评估臂 FFI 面:与 5.3 基线
//! `rurix-physics-sys` **并存不覆盖**——独立 vendor 线(`vendor/JoltC` =
//! JoltC@2982004 + JoltPhysics v5.6.0 + 5.6 适配补丁集,VENDOR56.md §1/§3),
//! 导出符号全量 `JPC56_` 前缀(`namespace JPH56`),与 5.3 线同进程零符号冲突;
//! 5.3 基线默认后端面 0-byte 不动,本臂**评估用途不升格生产默认**(两臂诚实登记)。
//! 物理 unsafe 集中地之二(`unsafe_code = "allow"` 块级豁免;U33~U53 审计模式
//! 同构镜像 + 5.6 delta 登记于 `unsafe-audit/rurix-physics-sys.md` §M125 追加段,
//! 每块 `// SAFETY:`)。
//!
//! 边界契约(RFC-0017 §4.C3 ABI 纪律 + §4.0-1 crate 布局,与 5.3 线字面同构):
