...
You can use `JPC56_{type}_AddRef` and `JPC56_{type}_Release`, where `{type}` is the type that directly inherits from `RefTarget<{type}>`, to add or release references. For example, for `FixedConstraint`, use `JPC56_Constraint_AddRef` to add a new reference.
