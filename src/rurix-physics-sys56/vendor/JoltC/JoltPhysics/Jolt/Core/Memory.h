...
// See: https://github.com/godotengine/godot/issues/105455#issuecomment-2824311547
	...
	#define JPH_INTERNAL_DEFAULT_ALLOCATE(size) JPH56::AlignedAllocate(size, 16)
	#define JPH_INTERNAL_DEFAULT_FREE(pointer) JPH56::AlignedFree(pointer)
#else
	#define JPH_INTERNAL_DEFAULT_ALLOCATE(size) JPH56::Allocate(size)
	#define JPH_INTERNAL_DEFAULT_FREE(pointer) JPH56::Free(pointer)
#endif
...
#define JPH_OVERRIDE_NEW_DELETE \
	...
	JPH_INLINE void operator delete[] (void *inPointer) noexcept								{ JPH_INTERNAL_DEFAULT_FREE(inPointer); } \
	JPH_INLINE void operator delete[] (void *inPointer, [[maybe_unused]] size_t inSize) noexcept{ JPH_INTERNAL_DEFAULT_FREE(inPointer); } \
	JPH_INLINE void *operator new (size_t inCount, std::align_val_t inAlignment)				{ return JPH56::AlignedAllocate(inCount, static_cast<size_t>(inAlignment)); } \
	JPH_INLINE void operator delete (void *inPointer, [[maybe_unused]] std::align_val_t inAlignment) noexcept { JPH56::AlignedFree(inPointer); } \
	JPH_INLINE void operator delete (void *inPointer, [[maybe_unused]] size_t inSize, [[maybe_unused]] std::align_val_t inAlignment) noexcept { JPH56::AlignedFree(inPointer); } \
	JPH_INLINE void *operator new[] (size_t inCount, std::align_val_t inAlignment)				{ return JPH56::AlignedAllocate(inCount, static_cast<size_t>(inAlignment)); } \
	JPH_INLINE void operator delete[] (void *inPointer, [[maybe_unused]] std::align_val_t inAlignment) noexcept	{ JPH56::AlignedFree(inPointer); } \
	JPH_INLINE void operator delete[] (void *inPointer, [[maybe_unused]] size_t inSize, [[maybe_unused]] std::align_val_t inAlignment) noexcept { JPH56::AlignedFree(inPointer); } \
