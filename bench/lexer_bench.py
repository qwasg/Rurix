"""lexer 吞吐基准 harness(M1 契约 D-M1-6;M1_PLAN §2 任务 4)。

用法:
  py -3 bench/lexer_bench.py --smoke                 # 小语料 + 5 次迭代,正确性哨兵(Nightly 冒烟)
  py -3 bench/lexer_bench.py --emit evidence/frontend_lexer_x.json  # 完整协议采样

协议:BENCH_PROTOCOL.md §3 的统计形态(warmup ≥10 + 稳态 CV<5% → 50×3 →
trial 内中位数 → 跨 trial trimmed mean;统计函数复用 bench/stats.py);
计时为 lex_bench 进程内 std::time::Instant(逐迭代 ns)。
...
BIN = ROOT / "target" / "release" / ("lex_bench.exe" if os.name == "nt" else "lex_bench")

# 协议参数(BENCH_PROTOCOL §3 同源;L2 清理为 GPU 专属条款,CPU 路径不适用)
WARMUP_MIN = 10
WARMUP_MAX = 50
STEADY_WINDOW = 5
STEADY_CV = 0.05
...
def build_corpus(target_bytes: int) -> tuple[str, str]:
    ...
    corpus = "\n".join([unit] * repeats)
    size_mib = len(corpus.encode("utf-8")) / (1024 * 1024)
    return corpus, f"{size_mib:.2f} MiB corpus ({len(files)} files x {repeats})"


def ensure_binary() -> None:
    print("[lexer_bench] cargo build --release --bin lex_bench ...")
    r = guarded_run(["cargo", "build", "--release", "--bin", "lex_bench"],
                    cwd=ROOT, timeout=CARGO_BUILD_TIMEOUT, capture=False,
                    label="cargo build lex_bench")
    if r.returncode != 0:
        raise RuntimeError(f"cargo build lex_bench 退出码 {r.returncode}"
...
def run_binary(corpus_path: Path, iters: int) -> tuple[int, list[int]]:
    """执行 lex_bench,返回 (语料字节数, 逐迭代 ns 列表)。"""
    proc = guarded_run(
        [str(BIN), str(corpus_path), str(iters)],
        timeout=EXE_RUN_TIMEOUT, quarantine_exe=BIN, label="lex_bench",
    )
    if proc.returncode != 0:
        raise RuntimeError(f"lex_bench 退出码 {proc.returncode}: {proc.stderr.strip()}")
    lines = [json.loads(line) for line in proc.stdout.splitlines() if line.strip()]
    header, samples = lines[0], lines[1:]
    if header.get("tokens", 0) <= 0:
        raise RuntimeError("lex_bench 哨兵行异常(tokens<=0)")
    return header["bytes"], [s["ns"] for s in samples]
...
def mbps(ns: int, nbytes: int) -> float:
    return nbytes / (ns / 1e9) / 1e6
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if smoke:
            nbytes, ns_list = run_binary(corpus_path, SMOKE_ITERS)
            best = max(mbps(ns, nbytes) for ns in ns_list)
            print(f"[lexer_bench] smoke PASS ({problem_size}, best {best:.1f} MB/s, "
                  f"{SMOKE_ITERS} iters, correctness sentinel ok)")
            return 0

        total = WARMUP_MAX + TRIALS * TIMED_ITERATIONS
        nbytes, ns_list = run_binary(corpus_path, total)
    ...
    for t in range(TRIALS):
        ...
        metrics = sorted(mbps(ns, nbytes) for ns in chunk)
    ...
    doc = {
        ...
        "notes": "in-process Instant timing; corpus = conformance/syntax concatenated; "
                 "zero-diagnostic sentinel enforced by lex_bench (exit 2 otherwise)",
    }
    if emit:
        emit.parent.mkdir(parents=True, exist_ok=True)
        emit.write_text(json.dumps(doc, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        print(f"[lexer_bench] evidence written: {emit.relative_to(ROOT)} "
              f"({doc['results']['trimmed_mean']} MB/s, level={doc['evidence_level']})")
    else:
