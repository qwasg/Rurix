"""Rurix tiled GEMM 三次进程级独立运行 + m5_budget.json 回填(契约 G-M5-1,M5.4)。
...
回填 [milestones/m5/m5_budget.json](../milestones/m5/m5_budget.json):
  - entries[] 追加/更新 numerator `m5.bench.gemm_tile.throughput_gflops`
    (measured_local + evidence_file + threshold = 实测 × 0.95);
  - ratio_assertions `m5.ratio.gemm_tile_vs_cuda` 翻 estimated→measured_local
    + measured_value = numerator / denominator(CUDA C++ 对照锚点);
  - revision_log 追加。

denominator 须已 measured_local(M5.4 先跑 CUDA 对照三次运行回填
`m5.bench.gemm_tile_cuda.throughput_gflops`)。

回填后核验:`py -3 ci/budget_eval.py --strict`(比值 ≥0.90 通过,契约 G-M5-1)。

用法(锁频后):py -3 bench/rurix_gemm_tile_triple.py
"""
...
PREFIX = "rurix_gemm_tile"
NUMERATOR_ID = "m5.bench.gemm_tile.throughput_gflops"
DENOMINATOR_ID = "m5.bench.gemm_tile_cuda.throughput_gflops"
RATIO_ID = "m5.ratio.gemm_tile_vs_cuda"
BENCH_SCRIPT = "bench/gemm_tile_bench.py"
...
def collect_runs() -> str:
    ...
    for seq in range(1, RUNS + 1):
        print(f"[rurix_gemm_tile_triple] === 第 {seq}/{RUNS} 次进程级独立运行 ===")
...
def aggregate(date: str) -> tuple[Path, dict]:
    ...
    print(f"[rurix_gemm_tile_triple] aggregate: {out.name} = {agg_value:.4f} {agg['results']['unit']}")
    return out, agg
...
def backfill_budget(agg_path: Path, agg: dict) -> None:
    ...
    budget["revision_log"].append({
        ...
        "change": "M5.4 回填:Rurix tiled GEMM measured_local(三次进程级独立运行 trimmed mean "
                  f"= {value:.4f} GFLOPS)→ numerator entry + ratio vs CUDA C++ 对照 "
                  f"({den:.4f} GFLOPS)= {ratio:.4f}(契约 G-M5-1,阈值 0.90)",
    })
    budget_path.write_text(json.dumps(budget, ensure_ascii=False, indent=2) + "\n",
                           encoding="utf-8")
    print(f"[rurix_gemm_tile_triple] budget backfilled: ratio = {ratio:.4f} "
          f"(numerator {value:.4f} / denominator {den:.4f}; 阈值 0.90 "
          f"{'PASS' if ratio >= 0.90 else 'FAIL'})")
    print("[rurix_gemm_tile_triple] 下一步核验:py -3 ci/budget_eval.py --strict")
