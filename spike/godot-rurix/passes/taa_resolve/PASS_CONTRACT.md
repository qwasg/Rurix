...
## 13. 路线 B(Route B rd_native)close-out —— 非 scaffold 真替代(resolve dispatch)

> 本节为 close-out 追加段;§1–§12 的调查/契约/marker/shim-path close-out 保持不变(pass_id=taa_resolve、`RXGD_PASS_TAA_RESOLVE`、`RXGD_CAP_TAA_RESOLVE_REAL_PASS`、§5/§11 known gaps 等契约字面不动)。路线 B rd_native 是与 §1–§12 记述的 shim/rxgd 路径**并列且独立**的另一条路径,不改任何 shim-path 字段(`godot_runtime_taa_resolve_path_enabled` 仍 false;shim 的 `RXGD_CAP_TAA_RESOLVE_REAL_PASS` bit 6 与之无关)。

**真替代事实 + history 维护保留**:与 patch 0019 shim writeback **scaffold** 本质不同——路线 B rd_native 让 Rurix taa_resolve kernel 作为主 `RenderingDevice` 帧内 compute dispatch 运行,**替代 `TAA::process` 里 `resolve()` 那一次 compute dispatch**,成功 record 时真正跳过 native resolve dispatch(`if (!rurix_recorded_taa_resolve_rd_native) resolve(...)` 守卫);而其后三次 native `copy_effects->copy_to_rect`(temp→internal / internal→history / velocity→prev_velocity)**照常运行**,故 TAA 时域反馈环完整保留。**设计取舍如实**:采「让 native copy 段照跑」而非用 RD copy API(后者需 CAN_COPY bits,这些纹理只有 SAMPLING+STORAGE)。

**注入点**:在 `servers/rendering/renderer_rd/effects/taa.cpp` 的 `TAA::process` 内(非 shim 的 `render_forward_clustered` `taa->process` 调用点),仅替代 `!just_allocated` 分支的 `resolve()`。两路互斥独立:shim gate 若 record 会整体短路 process;shim 禁用时 process 运行、rd_native gate(backend==2)在内部替代 resolve。

**patch**:单片 `0042-rurix-accel-taa-resolve-rd-native-inframe-replacement.patch`(叠 `0001-0029+0040+0041`)。新增默认 false 虚函数 `D3D12Hooks::try_record_taa_resolve_rd_native(RID color,RID depth,RID velocity,RID prev_velocity,RID history,RID temp,Size2i,f32 disocclusion,f32 variance)`;三态设置 `.../passes/taa_resolve/backend` + `.../taa_resolve/rd_container_path`(独立于既有 shim 四设置);module 惰性建管线 + usage-bits 前置(5 SRV 输入 `SAMPLING_BIT` / temp 输出 `STORAGE_BIT`)+ `UniformSetCacheRD`(六资源 t0..t4 SRV / u0 UAV binding 5)+ 28B b0([i64 resolution_width, i64 resolution_height, f32 disocclusion_threshold, f32 variance_dynamic, f32 reserved0])+ 自开自闭 `compute_list`(resolve() 本是独立 list);taa.cpp 加 `core/config/project_settings.h`+`drivers/d3d12/d3d12_hooks.h`;全链 fail-closed 回落 native。

**temporal DoD(GRX_PLAN 硬约束)**:TAA 是时域累积,单帧截图不可替代证据。enablement 用 `use_taa=true` + 定种子 orbit 运动场景,连续捕 8 帧(frame 16..23,`CAPTURE_START=16`/`CAPTURE_COUNT=8`/floor=8)。

**Enablement strict MEASURED success**:`ci/grx_rb_taa_resolve_rd_native_enablement_smoke.py` 在 `0001-0029+0040+0041+0042` 共享 Route B scratch Godot(Windows D3D12 Forward+,RTX 4070 Ti)记 strict MEASURED success(`rd_native_enablement_success_evidence.json`,`real_gpu_pass=true`/`real_replacement=true`/`native_resolve_skipped=true`):candidate 腿印一次性 `RXGD_RD_NATIVE_TAA_RESOLVE active`(rd_native 接管 resolve、native resolve 按构造跳过、native history copy 保留),连续 8 帧序列**逐帧**在 LDR parity 阈值内(max<=2/mean<=0.25),最差帧 `max_abs=1`、最差 `mean_abs≈0.0305`;fail_closed 红腿实测 RD 拒容器 → latch → 回落 native,与 reference 序列**逐帧逐字节一致**;reference/fail_closed 序列携带真实运动且逐帧字节一致(整数驱动确定性)。容器 sha256 `42af7d19…` 与 S1 container smoke(69/69 结构断言)逐字节一致;**无 S2 probe → `s2_probe_proven=false`**。溯源 `0001-0029+0040+0041+0042`(32 patch)。

**default disabled 不变 / 零性能宣称**:backend 默认 0;无 FPS、p95、GPU timestamp、时序稳定性或任何加速宣称;native Godot TAA resolve 仍作 fail-closed backstop。§5/§11 known gaps 全部保留(单 resolve 子集 + Rurix/native kernel 差异经时域累积可漂移已入 remaining rounds)。
