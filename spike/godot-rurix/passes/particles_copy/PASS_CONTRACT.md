...
**子集边界(如实标注,含实测根因修订)**:仅覆盖 3D `COPY_MODE_FILL_INSTANCES` 的 `ALIGN_DISABLED`/`ALIGN_Z_BILLBOARD` **no-userdata** 子集,gate `!do_sort && copy_mode_2d==0 && userdata_count==0`;VIEW_DEPTH 排序、2D、lifetime reindex、trail、以及**任何带 userdata 的 process material** 走 native。**userdata 门是 enablement 真机实测出的 STRIDE 边界(0043 原地修订,号不变,§4 规则2)**:kernel 按 no-userdata 112B stride 编译,而 `userdata_count>0` 使 ParticleData stride 变为 112+16u——标准 `ParticleProcessMaterial` **恒用 USERDATA1** 累积速度(stride 128),首轮 enablement(该材质场景)实测 candidate 画面为精确子集稀疏图(max_abs=191,仅 112*i 恰 128 对齐的每第 8 实例读到完整粒子);诊断链=local-RD probe 两连:①合成 112B 数据 kernel+RAW 绑定 0/1280 逐字节无差(kernel 无罪)②128B 数据 + CPU 错读模型 0/1280 逐字节复现 GPU 输出(stride 定罪)。修订后带 userdata 系统 fail-closed 走 native。
...
**Enablement strict MEASURED success**:`ci/grx_rb_particles_copy_rd_native_enablement_smoke.py` 在 `0001-0029+0040-0045` scratch Godot(Windows D3D12 Forward+,RTX 4070 Ti)记 strict MEASURED success(`rd_native_enablement_success_evidence.json`,`real_gpu_pass=true`/`real_replacement=true`):
- **candidate 腿**(backend=2 + staged 容器,no-userdata 自定义 particles shader 场景/4096 粒子/凍结 sim/ACTIVE 3-of-4 模式)印一次性 `RXGD_RD_NATIVE_PARTICLES_COPY active`(rd_native 记录到已开 compute list、native fill-instances dispatch 按构造跳过——真省一次 dispatch),captured 帧与 native reference **逐字节一致**(LDR `max_abs=0`/`mean_abs=0`,3511 亮粒子像素,非空画面);
- **fail_closed 红腿**(backend=2 + 垃圾容器)实测 RD 拒容器 → latch → 回落 native,画面与 reference 逐字节一致;三腿各连捕 3 帧全稳定;
- 首轮诚实弯路:`ParticleProcessMaterial` 场景暴露 userdata stride 边界(max_abs=191 稀疏图)→ 两连 local-RD probe 定位根因 → 0043 原地修订 + in-subset 场景后复跑 strict。修订版 0043 sha256 `fcb74373…bef0d1ca`。
