...
## 13. 路线 B(Route B rd_native)close-out —— 非 scaffold 真替代(SMART blur slice)

> 本节为 close-out 追加段;§1–§12 的调查/契约/marker/shim-path close-out 保持不变(pass_id=ssao_blur、`RXGD_PASS_SSAO_BLUR`、`RXGD_CAP_SSAO_BLUR_REAL_PASS`、§5/§11 known gaps 等契约字面不动)。路线 B rd_native 是与 §1–§12 记述的 shim/rxgd 路径**并列且独立**的另一条路径,不改任何 shim-path 字段(`godot_runtime_ssao_blur_path_enabled` 仍 false;shim 的 `RXGD_CAP_SSAO_BLUR_REAL_PASS` bit 5 与之无关)。

**真替代事实**:与 patch 0016 shim writeback **scaffold**(印 marker 后仍让 native SMART blur 每帧重跑,画面不可能改变)本质不同——路线 B rd_native 让 Rurix ssao_blur kernel 作为主 `RenderingDevice` 帧内 compute dispatch 运行,成功 record 时**真正跳过** native Godot SMART blur 对该 slice 的 dispatch(调用点 `if (!rurix_recorded_ssao_blur && !rurix_recorded_ssao_blur_rd_native)` 守卫,rd_native 无 scaffold 清空动作)。

**共享 compute list**:与 tonemap/taa_resolve 不同,SSAO 的 compute list 由 `generate_ssao` 在整条 gather/blur/interleave 链外**只开一次**,故 rd_native 记到调用方已开的 `compute_list`(hook 首参 `int64_t p_compute_list`),**不** begin/end 自己的 list;循环每 pass 的 `compute_list_add_barrier` 负责 write→read 冒险。

**patch**:单片 `0041-rurix-accel-ssao-blur-rd-native-inframe-replacement.patch`(叠 `0001-0029+0040` culling 尾栈 + tonemap rd_native)。新增默认 false 虚函数 `D3D12Hooks::try_record_ssao_blur_rd_native(int64_t,RID,RID,Size2i,f32,f32,f32)`;三态设置 `.../passes/ssao_blur/backend`(0 disabled/1 shim/2 rd_native,默认 0)+ `.../ssao_blur/rd_container_path`(默认空)——**独立于既有 shim 四设置**;module 惰性建管线 + usage-bits 前置(src `SAMPLING_BIT`/dst `STORAGE_BIT`,deinterleaved AO 纹理 R8G8_UNORM 双 bit 皆备)+ `UniformSetCacheRD`(t0 SRV/u0 UAV)+ 28B b0([i64 slice_width, i64 slice_height, f32 edge_sharpness, f32 half_screen_pixel_size_x, f32 half_screen_pixel_size_y])+ 记到共享 list 的 `compute_list` dispatch `ceil(slice/8)`;全链 fail-closed 回落 native。

**MODE_SMART 子集边界(如实标注)**:kernel 仅覆盖 `MODE_SMART` 单 slice edge-aware blur;enablement 场景锁 `ssao/blur_passes=1` 且 quality>VERY_LOW 使每 pass 皆 SMART。`MODE_WIDE`/very-low blur、4-slice deinterleave/interleave、SSIL blur **不在** rd_native 子集内,走 native。Rurix kernel 以纯 SRV Load 读 AO slice,native SMART blur 以 mirror/linear `SAMPLER_WITH_TEXTURE` 读。

**Enablement strict MEASURED success**:`ci/grx_rb_ssao_blur_rd_native_enablement_smoke.py` 在 `0001-0029+0040+0041+0042` 共享 Route B scratch Godot(Windows D3D12 Forward+,RTX 4070 Ti;0042 于本 pass 为 additive/默认禁用)记 strict MEASURED success(`rd_native_enablement_success_evidence.json`,`real_gpu_pass=true`/`real_replacement=true`/`native_smart_blur_skipped=true`):candidate 腿(backend=2 + S1 已核容器)印一次性 `RXGD_RD_NATIVE_SSAO_BLUR active`,captured 帧与 native reference **逐字节一致**(LDR `max_abs=0`/`mean_abs=0`);fail_closed 红腿(backend=2 + 垃圾容器)实测 RD 拒容器 → latch → 回落 native,画面与 reference 逐字节一致;三腿各连捕 3 帧全稳定。所用容器 sha256 `6887b3f6…` 与 S1 container smoke(49/49 结构断言)逐字节一致;**无 S2 ~1-ULP probe(仅 tonemap 有)→ `s2_probe_proven=false`**,provenance 为结构级锚。SSAO 经 lighting 间接调制,故此帧一致性证明 SMART blur 真替代未在 LDR 容差内扰动可见结果(非直接 AO 纹理表征,已入 known gaps)。溯源 `0001-0029+0040+0041+0042`(`tracked_patch_stack_only=true`,32 patch)。

**default disabled 不变 / 零性能宣称**:backend 默认 0,rd_native 从不默认启用;无 FPS、p95、GPU timestamp 或任何加速宣称;native Godot SSAO blur 仍作 fail-closed backstop。§5/§11 known gaps 全部保留。
