...
## 12. Close-out(GRX-010/013 stage-A5 equivalent)

> Close-out addendum。§1-§11(侦察 / 契约 / marker)保持不变(pass_id = fused_post_chain、`RXGD_PASS_FUSED_POST_CHAIN`、`RXGD_CAP_FUSED_POST_CHAIN_REAL_PASS`、LINEAR+sRGB+auto-exposure 子集等);§5 known gaps 与 §11 残留项保留。本节只记录已落地内容。§11 的 S4-S9(bridge gate、patches 0036-0038、dispatch smoke、enablement、close-out)现已 **DELIVERED**。

GRX-019 fused_post_chain 已 close-out(复用 GRX-009/010/013 模板)。S4-S9 链已落地:

- **Bridge gate(S4)** — `src/rurix-godot/src/lib.rs` 的 fail-closed `FusedPostChainGate`:runtime binding preflight(恰好 5 texture 资源、64-byte b0、source dims 与 texture extents 非零;**注意实现版 preflight 不校验 int64**)-> dispatch eligibility(`RXGD_CAP_FUSED_POST_CHAIN_REAL_PASS (1<<13)` opt-in、`RXGD_CAP_SHADER_INT64`、非空 device/queue、非空 native handle、package layout/digest 匹配)-> per-slot binding kinds(`[texture2d x3, rwtexture2d x2]`)-> math parity -> real dispatch 仅在 `d3d12-recording-shim` feature 下。任一失败打印每 session 一次的 `RXGD_FUSED_POST_CHAIN_REAL_PASS_BLOCKED` 诊断。因 int64 不在 preflight,forced capability downgrade 在**下一级 dispatch_eligibility** fail-closed(`dispatch_eligibility_failed`/`unsupported_device`,非纹理 pass 的 `runtime_binding_preflight_failed`),enablement smoke forced 腿实测印证。

- **Patches 0036-0038** — `0036`(per-pass `enabled` 设置默认 false + `try_record_fused_post_chain()` 模块 gate + `_render_buffers_post_process_and_tonemap` 的 **fusion-first** call-site opt-in gate,置于逐成员单 pass tonemap gate 之前,接线两级回退 fusion -> member -> native),`0037`(runtime native texture-handle binding via `get_driver_resource(DRIVER_RESOURCE_TEXTURE, ...)`;fallback marker `RurixAccel: fused_post_chain native resource handle mapping fallback rc=`),`0038`(recording-smoke + real-pass opt-in 三设置 + `RXGD_CAP_FUSED_POST_CHAIN_REAL_PASS` 武装 + forced downgrade + writeback scaffold marker `RXGD_GODOT_RUNTIME_FUSED_POST_CHAIN_REAL_PASS_WRITEBACK`)。0036-0038 叠在 0026 tip 上;0027-0035 为 GRX-015/016/018 预留孔(合法单调孔,PATCH_ALLOCATION.md §4 rule 2)。`0001..0026 + 0036..0038` scratch Godot 编译干净(console exe 产出、`tracked_patch_stack_only=true`)。

- **Scaffold binding 诚实声明** — fused luminance 段的 distinct ≤8x8 reduce 级与双缓冲 1×1 prev/dst luminance 目标**未被公开 Luminance API 在 tonemap call site 暴露**,故 patch 0037 把唯一可取的公开 1×1 current luminance buffer 别名给 lum_source 与 prev_luminance(只读),把 LDR tonemap 目标别名给 dst_color 与 dst_luminance(native tonemapper 完全覆写)。**这满足了 §3.4/§10 声明的「自队列融合 dispatch 不能替代当帧真实 tonemap 输出」约束——真替代仍需 draw_graph 路线 B / Luminance-API 扩展**。strict success 只断言「一次真实 D3D12 fused dispatch 被录制且 native-continuation 视觉门逐字节稳定」,**不**断言 fused 输出正确或净收益。

- **Standalone dispatch smoke(S6)** — `real_d3d12_dispatch_smoke.json`(`real_d3d12_dispatch_recorded=true`、`cpu_reference_match=true`),`ci/grx019_fused_post_chain_d3d12_dispatch_smoke.py` 在真实 D3D12 设备上以 5 纹理 + 64-byte b0 跑通一次最小 dispatch 并与 CPU 参考匹配。

- **Real-pass enablement(S8)** — `ci/grx019_fused_post_chain_real_pass_enablement_smoke.py` 在 `0001..0026 + 0036..0038` scratch Godot(Windows D3D12 Forward+)上取得 **strict measured success**(`real_pass_enablement_success_evidence.json`,`status=success`、`strict_success=true`、`real_gpu_pass=true`、`real_d3d12_dispatch_recorded=true`)。三腿 pass enable matrix:reference(零 marker)/ candidate(`RXGD_GODOT_RUNTIME_FUSED_POST_CHAIN_REAL_PASS recorded=1` + writeback + record)/ forced(`dispatch_eligibility_failed`/`unsupported_device`)。场景 = auto-exposure(`CameraAttributesPractical.auto_exposure_enabled=true`)+ FILMIC tonemapper,`fixed_fps` 确定性;LDR visual gate `max_abs=0`/`mean_abs=0`(scaffold + native continuation 逐帧重渲染,fused dispatch 只写被覆写的 LDR 目标、从不写活 luminance buffer);telemetry `measured_local` 过 GRX-008 校验。

- **Gate module + probe(S9)** — `ci/grx_gates/grx019_fused_post_chain.py` 五级(contract / patch applyability / dispatch smoke / enablement / decision)全绿;probe `GRX_GATE_SEQUENCE` 顺序 grx011..grx019,grx019 gate 独立 `all_ready`。**注意**:序列中 grx015(gpu_culling)未做,fail-closed 走链在 grx015 停,故 probe `next_action` 仍停在 grx014 的 `start_grx015_gpu_culling_pass_contract`——grx019 gate ready 但不推进 next_action,如实反映(GRX-015/016/018 未 author)。

- **Owner decision(S9)** — `real_pass_default_enable_decision.json`(`keep_default_disabled`)+ `.md`。`default_enable_state` 保持 `disabled`,`implemented=true`/`real_gpu_pass=true`/`real_d3d12_dispatch_recorded=true` 只记录 opt-in real-pass arm 的 MEASURED 成功;shipping feature-off bridge 仍以 `real_dispatch_path_not_linked` fail-closed;patch 0038 writeback 为 SCAFFOLD;**无** fused-输出正确性 / 视觉优越 / GPU-timestamp / FPS / 性能提升宣称。

## 13. 路线 B(Route B rd_native)—— fusion-first 级联(诚实 measured-blocked)

> 本节为追加段;§1–§12 的调查/契约/marker/shim-path 记述保持不变。路线 B rd_native 与 shim/rxgd 路径**并列且独立**,不改任何 shim-path 字段。

**级联设计(两级回退)**:fused rd_native 是两级回退的 fusion-first 腿:**fused rd_native → 单 pass tonemap rd_native(patch 0040,已在栈)→ native**。本片**自包含于 culling 尾**:fused shim scaffold(0036-0038)在互斥的 fused 尾,不在本栈,故 0045 自带 bridge-independent backend selector 并级联进 0040 的 tonemap rd_native(而非 fused shim gate)。调用点在 `_render_buffers_post_process_and_tonemap` 的 can_use_storage tonemap 块:fused gate 跑在 0040 tonemap rd_native gate **之前**,成功则跳过 tonemap rd_native + shim + native tonemapper;三处守卫全部前置 `!rurix_recorded_fused_post_chain_rd_native`。

**patch**:`0045-rurix-accel-fused-post-chain-rd-native-inframe-replacement.patch`(sha256 `418406b2…6a3f3f2b9`,叠 `0001-0029+0040..0044`)。新增默认 false 虚函数 `try_record_fused_post_chain_rd_native(RID src_color,RID lum_source,RID prev_luminance,RID dst_color,RID dst_luminance,Size2i,Size2i,f32,f32,f32)`;自带 `RD_NATIVE_FUSED_POST_CHAIN_*` 64B/5-resource 常量(与 fused 尾 `FUSED_POST_CHAIN_RESOURCE_MAPPING_*` **区分命名,两尾永不撞号**);五纹理(t0..t2 SRV / u0 u1 UAV)+ usage-bits 前置校验 + 自开 compute list(tonemap 块无活动 list)+ 64B b0 + dispatch ceil(dest/8)。

**诚实 measured-blocked 边界(核心)**:公有 Luminance API 在 tonemap 调用点**不暴露**独立的双缓冲 luminance-final 目标(lum_source/prev_luminance/dst_luminance),调用点只能拿到单个 current luminance buffer 与 LDR dest(镜像 0037 scaffold 绑定)。module 内设**别名守卫**:当 `lum_source==prev_luminance || dst_color==dst_luminance` 时**直接 fail-closed**——既避免绑定退化/别名的 luminance UAV(会破坏反馈环或在格式不匹配时触发帧内 `uniform_set_create` assert),又让调用点级联到 0040 tonemap rd_native 单 pass。故在 culling 尾上 fused gate 今日**恒 fail-closed 级联**,`RXGD_RD_NATIVE_FUSED_POST_CHAIN active` marker 仅在提供独立 luminance 目标后才出现。**这是诚实的 measured-blocked 边界,非失败**:结构路径完整,待 Luminance-API 扩展落地即自动生效。dst_luminance 双缓冲 SWAP 语义(0038/luminance 先例)属该扩展范围。

**Enablement:measured_blocked_by_design 真机 CONFIRMED(含级联实测)**:`ci/grx_rb_fused_post_chain_rd_native_enablement_smoke.py`(5 腿矩阵)在 `0001-0029+0040-0045` scratch Godot(Windows D3D12 Forward+,RTX 4070 Ti)记 `measured_prerequisite_blocked`,`first_missing_prerequisite=fused_luminance_double_buffer_api_unexposed`,`cascade_confirmed=true`:
- **别名守卫真到达**(AE 三腿):auto exposure 使 current luminance buffer 真实存在(LuminanceReduce shader 加载、AE reference 与非 AE reference 画面实测不同),fused gate 走到别名守卫并 fail-closed——fused active marker 五腿全缺席,candidate 与 fail_closed(垃圾容器)帧均与 native reference **逐字节一致**(zero image impact;垃圾容器因守卫先于容器加载而从未被读,RD 拒容器错误按预期缺席,仅记录不强制);
- **两级回退级联实测打通**(cascade 腿):AE OFF → fused gate 在 invalid-lum-RID 关口 fail-closed → tonemap backend=2(已证 0040 容器)接管,`RXGD_RD_NATIVE_TONEMAP active` **仅**现于该腿,帧与非 AE native reference **逐字节一致**(LDR `max_abs=0`)——fused rd_native → tonemap rd_native → native 两级回退按设计解析;
- 五腿各连捕 3 帧全稳定。**不宣称 real_gpu_pass**;fused 真替代待 Luminance-API 扩展(独立双缓冲目标 + SWAP)后自动生效。

**遗留(归下批)**:Luminance-API 扩展暴露独立 luminance 双缓冲目标(含 SWAP)是 fused 真替代的前置;luminance 多 kernel 末级同属下批。

**Fail-closed / 零性能宣称**:默认 backend=0 时 fused gate 从不 engage,tonemap rd_native / native 链渲染;`default_enable_state` 保持 `disabled`;无任何性能宣称;不改 `external/godot-master`。

## 13.1 Route B rd_native 原地修订(Wave 6 — Design 1 shadow-recompute)

> 本节为 §13 的**原地修订追加段**(patch 0045 号不变,§4 rule 2)。§13 中「恒
> fail-closed 级联 / `measured_blocked_by_design`」的框架**已被本修订取代**;§13
> 的历史记述保留供追溯,但当前行为以本节为准。

**解锁前置(patch 0048)**:新增两个纯只读 Luminance getter —
`get_previous_luminance_buffer`(返回 `reduce[size-1]`,SWAP 后即上帧 final)与
`get_luminance_reduce_penultimate_buffer`(返回 `reduce[size-2]`,安全读窗)。零行为
改变:不碰 `luminance_reduction`/SWAP/`configure`,无新 dispatch/资源。这补上了 §13
所说「公有 Luminance API 不暴露独立双缓冲目标」的缺口的**读侧**。

**Design 1 修订(patch 0045)**:调用点改用**三个不同 RID**——`lum_source = current`、
`prev_luminance = previous`、`dst_luminance` 参数传 `penultimate`(仅为占位有效句柄)。
module 侧**新增自持 1×1 R32F STORAGE scratch RID**(懒创建,先例 = 0028
`rurix_gpu_culling_visibility_buffers`),把 fused kernel 的 dst_luminance UAV(u1)
**改绑 scratch**,而非任何真 luminance buffer:写 current 会污染 auto-exposure 反馈环,
写 penultimate 会污染下帧 reduction 源,故影子写入落 scratch 且**永不被读**。别名守卫
收窄为仅 `lum_source==prev_luminance`(无 auto-exposure 时退化 → fail-closed 级联)。
fused gate 另 AND patch 0040 的共享 `rurix_tonemap_linear_subset_compatible` 谓词
(LINEAR + convert_to_srgb + 无 glow/BCS/CC/FXAA/debanding/multiview;auto-exposure
在此允许——fused kernel 自身折入 luminance 段)。

**诚实边界(契约必写,manifest `rd_native.honest_boundary` 同步)**:这是
**shadow-recompute**——`engaged / shadow-luminance-write / dispatch-savings-not-claimed`。
native `luminance_reduction` 金字塔**仍每帧全跑**,fused 的 luminance-final 重算落
scratch 丢弃,故**净 dispatch 节省 = 0,非真融合、无结构性节省**;唯一真替代是 LINEAR +
