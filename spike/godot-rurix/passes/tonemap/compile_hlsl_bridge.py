#!/usr/bin/env python3
"""GRX-010: compile the math-equivalent HLSL tonemap kernel.

Compiles ``artifacts/hlsl_bridge/tonemap_apply.hlsl`` via DXC (cs_6_0),
...
through ``cargo run --example emit_grx010_tonemap_rts0``
...
paths (``tonemap.dxil``, ``tonemap.rts0.bin``,
``tonemap_descriptor_layout.json``) under the owner-approved
...
Tool discovery follows the GRX-009 template (``RURIX_DXC_DIR`` /
...
BRIDGE_DXIL_PATH = BRIDGE_DIR / "tonemap_apply.dxil"
CANONICAL_DXIL_PATH = ARTIFACTS_DIR / "tonemap.dxil"
CANONICAL_RTS0_PATH = ARTIFACTS_DIR / "tonemap.rts0.bin"
CANONICAL_DESCRIPTOR_PATH = ARTIFACTS_DIR / "tonemap_descriptor_layout.json"
...
# shared with the GRX-009 canonical layout). The i64 dims are carried as
# uint2 (low, high dword) in the HLSL cbuffer; the runtime must write 0 to
# the high dwords.
ROOT_CONSTANT_LAYOUT = [
    ...
    {"name": "exposure", "type": "f32", "order": 2, "root_parameter_index": 0, "dword_offset": 4, "dword_size": 1},
    {"name": "white", "type": "f32", "order": 3, "root_parameter_index": 0, "dword_offset": 5, "dword_size": 1},
    {"name": "luminance_multiplier", "type": "f32", "order": 4, "root_parameter_index": 0, "dword_offset": 6, "dword_size": 1},
]
...
def descriptor_layout_doc() -> dict[str, object]:
    return {
        ...
        "grx010_mapping": {
            ...
            "dst_shape": "dst extent == src extent (1:1 full-resolution tonemap)",
        },
    }
...
def finish(evidence: dict[str, object], status: str, blocker_category: str | None, blocker_summary: str | None, exit_code: int) -> int:
    ...
    print(f"[grx010-hlsl-bridge] status={status} blocker={blocker_category} evidence={EVIDENCE_PATH}")
    return exit_code
...
def main() -> int:
    ...
    cargo_argv = [
        ...
        "--example", "emit_grx010_tonemap_rts0",
        "--",
    ...
    evidence["commands"].append({
        "label": "emit_grx010_tonemap_rts0",
        "argv": cargo_argv,
    ...
    if cargo_completed.returncode != 0 or not CANONICAL_RTS0_PATH.is_file():
        return finish(evidence, "compile_failed", "rts0_emit_failed",
                      "emit_grx010_tonemap_rts0 failed; see emit_rts0_stderr.txt", 1)
    ...
    evidence["notes"] = [
        "Runtime remains fallback_only: the real dispatch path is linked only under the d3d12-recording-shim feature and armed only by the opt-in RXGD_CAP_TONEMAP_REAL_PASS flag; the shipping feature-off bridge fails closed with real_dispatch_path_not_linked.",
        ...
        "This canonical package uses the owner-approved hlsl_bridge_workaround provenance exception (GRX-009 texture_artifact_provenance_policy.json, which applies to every texture compute pass): the artifact is runtime-mappable but NOT rurix_owned; a rurixc-owned tonemap compile still requires a patched llc that supports texture intrinsics plus float4 texture element support in the Rurix lang items.",
        "The kernel math mirrors the tonemap.glsl LINEAR + convert_to_srgb subset only; see known_gaps.",
    ]
