...
> **勘误（2026-08-24，只追加不改机制）**：上行「M0–M8」为 MVP 期快照——本契约制自 MVP 后已同构延用至全部执行期里程碑（G1~G17、V1/MS1/MB1/EI1/EA1，见 [11](11_ROADMAP.md) §8 实况表），契约落位 `milestones/<里程碑>/` 且四要素机制不变；G 系列自 G8 起增加 front matter 双状态机（`status` + `implementation_status`）与治理波互锁先例。机制本体 0-byte。
...
> **勘误（2026-08-24，只追加不改机制）**：上表「多后端」行触发条件已于 2026-07-15 满足并消费——G2 已完成且红线 3 经 owner 主动裁决解除（D-008，[13](13_DECISION_LOG.md) §8 v2.1），SG-003 → triggered（RFC-0011），承接里程碑 MB1 已收口（`mb1-closed`）。该行按只追加纪律不回写；gating 终态事实源恒为 `registry/spike_gating.json`，本表只作规划期快照。其余行触发条件截至本日未满足，维持 gating。
...
| v1.2 | 2026-08-24 | §1 追加勘误注（契约制「M0–M8」快照已同构延用至全部执行期里程碑 G1~G17，机制本体 0-byte）+ §7 gating 表后追加勘误注（「多后端」行触发条件已于 2026-07-15 满足并消费：D-008 解除 + SG-003 triggered(RFC-0011) + MB1 收口；终态事实源 = `registry/spike_gating.json`，既有行不回写）。规划文档勘误（00 §6.3 追加式修订，独立勘误 commit，check_planning_docs advisory 不阻断） |
