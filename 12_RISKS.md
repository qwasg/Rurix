...
> **状态复核勘误（2026-08-24，只追加不回写；依头注「状态变化以追加方式记录」）**：截至 G16 收口（`g16-closed`）/ G17 进行中的存续风险复核——**R-109**（RD-027 生产档 PT 毒径）维持 open：`registry/deferred.json` RD-027 status=open，切片隔离口径未变，未获上游归因；**R-111**（G-MB1-6 AMD 真卡尾门）维持 open：仍缺 AMD 硬件，不伪造 device 绿；**R-606**（nightly 无 timeout 僵尸锁 runner）根治项维持未落地登记。**执行期新增存续项登记（事实源在 `registry/` 与各契约，本行只作风险面索引）**：① **RD-045 间歇性 digest 漂移**（G14.5a 登记的生产化缺陷修复项，M165 同型；G15 全期/G16/G17.2 复测窗监控面均零检出，维持 open 长窗承接）；② **G15-MD-F1 性能单格未达标**（bistro-interior/t100/dlss_sr ratio < ×1.00，G15 四轮 0.833~0.857 → G17.0 baseline 0.9810 → G17.2 复测窗 [0.981, 0.8157, 0.7966, 0.8086]——UE 参照臂缓存暖态跨会话双向波动 + NGX in-stream 物理地板两因素叠加；已由 G17 里程碑法定承接，终判两态〔18/18 或维持未达标登记〕均为合法收口）；③ **G15 商用收口 0/18 画质面历史定盘**（M-c 历史登记未改写；G16plus M-g 以程序产阈 18/18 收口，两口径并存互不冒充）。§1~§7 既有风险行与编号 0-byte。
...
| v1.2 | 2026-08-24 | §7 后追加「状态复核勘误」块（只追加不回写）：R-109/R-111/R-606 存续复核 + 执行期三项存续登记（RD-045 漂移长窗 / G15-MD-F1 性能单格未达标已由 G17 承接 / G15 商用 0/18 历史定盘与 G16plus 程序产阈两口径并存）。§1–§7 既有风险行与编号 0-byte；风险编号不新增（存续项事实源在 `registry/deferred.json` 与各契约，不重复登记为 R-xxx）。规划文档勘误（00 §6.3 追加式修订，独立勘误 commit，check_planning_docs advisory 不阻断）。 |
